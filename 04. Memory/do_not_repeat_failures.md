...
- `HYP-M15-TRENDPB-XAUUSD-M15-001` is terminal market KILL (2026-08-16) for
  ...
  MetaQuotes-Demo only. Train `20260816_132913` HQ 99% / Bars 141366 /
  SO 0 / N=721 / PF 0.66 / net −790 / DD 8.02% / WR ~33%. 2018 PF 0.67
  n=625; 2019 PF 0.59 n=96; 2020–2023 n=0 after DD lock. STOP same-ID
  rescue, EMA/k/TP revision, session/year salvage, holdout read, or nới
  HQ/DD. Radius is this M15 pullback envelope, not every breakout and
  not the goal.

- `HYP-SWEEPFADE-XAUUSD-H1-001` is terminal market KILL (2026-08-16) for
  closed-bar PDH/PDL sweep-reclaim fade on XAUUSD H1 MetaQuotes-Demo only.
  Train `20260816_130548` HQ 99% / N=1150 / PF 1.01 / net +85 / DD 4.70% /
  exp +0.07. Six years PF 0.78–1.21 around 1; none ≥1.30. Exit mix
  SL-like ~48% / TP-like ~29% / DAILY_FLAT ~17% / FRIDAY_FLAT ~5% /
  TIME_STOP ~0 — no single exit defect. STOP same-ID rescue, H1-002
  without a new defect, session/year/hour salvage, holdout read, or
  nới HQ/DD. Radius is this PDH/PDL fade envelope, not every sweep
  forever and not the goal.

- `HYP-GBB-S2-XAUUSD-H1-002` is terminal market KILL (2026-08-16) for
  closed-bar GBB S2 trend-pullback on XAUUSD H1 MetaQuotes-Demo only.
  Train `20260816_124307` HQ 99% / N=582 / PF 0.81 / net −787 / DD 8.03%.
  2018–2020 all PF<1; 2021–2023 n=0 from the 8% DD lock (do not relax).
  Exit mix SL ~38% / DAILY_FLAT ~37% / TP ~15% / FRIDAY_FLAT ~8% / TIME ~2%
  — no single exit defect. STOP same-ID rescue, S2 retune, 1.5R change,
  session/year salvage, holdout read, or nới DD. `HYP-GBB-S2-XAUUSD-H1-001`
  remains INFRA_BLOCKED FivePercent (not this kill). Radius is this S2
  envelope, not every GBB forever and not the goal.
...
## Treasury TIC announcement guard (2026-08-13)

- Do not divide signed net TIC purchases by the prior signed value and call the
  result the BIS month-on-month percentage. Negative, sign-changing and zero
  priors make that transform undefined or sign-reversing, and BIS gives no
  algebraic repair.
- Do not substitute the Fed 2014 trailing-absolute normalization: it is a
  bilateral contemporaneous equity-flow object, not the aggregate first-vintage
  announcement field.
- Do not pool legacy Form S and expanded SLT. Fed 2025 recommends different
  estimated/measured series on the two sides of January 2023 and documents
  changed collection/sign conventions.
- Do not invent a 16:00 closed-bar entry, 4h/20h/24h hold, Friday exception,
  threshold or surprise proxy. No primary modern study binds that rule.
- Terminal verdict:
  `KILL_TIC_ANNOUNCEMENT_MECHANISM_NO_MODERN_PREREGISTRABLE_TRANSFORM`.
  Receipt:
  `04. Memory/research/20260813_TREASURY_TIC_MECHANISM_TERMINAL_RECONCILIATION.md`.

## Candidate registry recovery guard (2026-08-13)

- Do not replace the live registry with the 877-row runtime snapshot. It is the
  latest intact local recovery artifact, not the missing 978-row canonical
  bytes.
- Do not reconstruct rows 878-978 from prose receipts or auto-rebind lineage
  overrides to either 469 or 877 rows. Keep economic authorization fail-closed.
- Receipt:
  `04. Memory/research/20260813_CANDIDATE_REGISTRY_RECOVERY_INVENTORY.md`.
