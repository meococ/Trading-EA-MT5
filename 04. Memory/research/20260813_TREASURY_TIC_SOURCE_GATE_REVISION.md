...
## 2026-08-13 terminal supersession

A second primary-source pass resolved that the missing transform/horizon is not
merely unfinished research. No primary algebra handles the signed/zero prior
net-flow denominator; modern Fed guidance requires an estimated pre-2023 series
and measured expanded-SLT series rather than a first-vintage announcement
bridge; and no 16:00 closed-bar/Friday-flat horizon is evidence-backed.

This receipt's `REOPEN_SOURCE_ONLY_BUT_HOLD_MECHANISM` verdict is superseded by
`KILL_TIC_ANNOUNCEMENT_MECHANISM_NO_MODERN_PREREGISTRABLE_TRANSFORM` in:
