#!/usr/bin/env python3
"""V8 offline probe: lagged US-UK 10Y bond yield differential -> GBPUSD.

Frozen contract:
  03. EA Developer/EA_SonicR/research/preflight/v8_exogenous/
  20260713_V8_USUK_10Y_DIFF_GBPUSD_PROBE_CONTRACT_V1.md
...
PANEL = RESEARCH / "preflight" / "v8_exogenous" / "panels" / "us_uk_bond_yield_diff_d1_v1.csv"
...
MOM_LOOKBACK = 20
MAX_GAP_DAYS = 3
RESULT_NAME = "20260713_V8_USUK_10Y_DIFF_GBPUSD_PROBE_RESULT_V1.json"
TRADES_NAME = "20260713_V8_USUK_10Y_DIFF_GBPUSD_PROBE_TRADES_V1.csv"
PROBE_ID = "V8_USUK_10Y_DIFF_GBPUSD_V1"
READOUT_NAME = "20260713_V8_USUK_10Y_DIFF_GBPUSD_OFFLINE_PROBE_READOUT.md"
...
def load_diff_available() -> tuple[dict[date, float], dict]:
    ...
    with PANEL.open(encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            ...
            diff_s = (row.get("diff_us_uk_10y") or "").strip()
...
def main() -> int:
    ...
    try:
        ...
        result = {
            "schema": "v8_usuk_10y_diff_gbpusd_offline_probe.v1",
            "probe_id": PROBE_ID,
