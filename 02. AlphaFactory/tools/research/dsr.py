"""Deflated Sharpe Ratio (Bailey & Lopez de Prado 2014) — canonical, dependency-free.

Workspace conventions (research_doctrine.md, Multiple Testing):
- Per-trade SR = mean(net_R)/std(net_R).
- N = EVERY executed simulation across all stages (controls and failed arms
  included); cost tiers x1/x1.5/x2 are NOT separate trials.
- V[SR] across all evaluated arms; PSR uses skew + NON-excess kurtosis;
  radicand <= 0 => fail (returns 0.0), not skip. Floor: DSR >= 0.95.
Self-test: N=1000, V=1 -> E[max SR] ~ 3.2554.
"""
