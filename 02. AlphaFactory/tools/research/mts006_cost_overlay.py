...
"""Apply the frozen MTS006 commission, slippage and financing contract.
...
EXPOSURE_RE = re.compile(
    r"MTS006_FINANCE_EXPOSURE\s+epoch=(?P<epoch>\d+)\s+"
    r"day=(?P<day>\d{8})\s+reason=(?P<reason>[A-Za-z0-9_.-]+)\s+"
    r"fx_usd=(?P<fx>-?\d+(?:\.\d+)?)\s+"
    r"xau_usd=(?P<xau>-?\d+(?:\.\d+)?)"
)
ECON_RE = re.compile(
    r"MTS006_ECON_TELEMETRY\s+ticks=(?P<ticks>\d+)\s+"
...
DEAL_RE = re.compile(
    r"MTS006_DEAL_COST\s+epoch=(?P<epoch>\d+)\s+deal=(?P<deal>\d+)\s+"
    r"symbol=(?P<symbol>\S+)\s+class=(?P<class>FX|XAU|UNKNOWN)\s+"
...
def parse_exposures(text: str) -> list[ExposureEvent]:
    ...
    for match in EXPOSURE_RE.finditer(text):
        ...
        if key not in seen:
            ...
            rows.append(row)
    if not rows:
        raise CostOverlayError("no MTS006_FINANCE_EXPOSURE rows")
...
def parse_economic_telemetry(text: str) -> dict[str, float | int]:
    ...
    if len(rows) != 1:
        raise CostOverlayError(f"expected one distinct MTS006_ECON_TELEMETRY row, got {len(rows)}")
...
def parse_deals(text: str) -> list[DealCost]:
    ...
    if not rows:
        raise CostOverlayError("no MTS006_DEAL_COST rows")
    return rows
...
def validate_contract(contract: dict[str, object]) -> None:
    if contract.get("schema_version") != "mts006_cost_contract.v1" or contract.get("hypothesis_id") != HYPOTHESIS_ID:
    ...
    if not isinstance(commissions, dict) or set(commissions) != {"fx", "xau"}:
    ...
    if not isinstance(classes, dict) or set(classes) != {"fx", "xau"}:
...
def calculate(text: str, summary: dict[str, object], contract: dict[str, object], start: date, end: date) -> dict[str, object]:
    ...
    expected_by_class = {"fx": 0.0, "xau": 0.0}
    native_by_class = {"fx": 0.0, "xau": 0.0}
    spread_by_class = {"fx": 0.0, "xau": 0.0}
    count_by_class = {"fx": 0, "xau": 0}
    for deal in deals:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Apply frozen MTS006 cost overlay")
    ...
    payload: dict[str, object] = {
        "schema_version": "mts006_cost_overlay_result.v1",
