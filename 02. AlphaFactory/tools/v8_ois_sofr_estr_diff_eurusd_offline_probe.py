#!/usr/bin/env python3
"""V8 offline probe: lagged SOFR−€STR overnight RFR differential → EURUSD.

Frozen contract:
  03. EA Developer/EA_SonicR/research/preflight/v8_exogenous/
  20260713_V8_OIS_SOFR_ESTR_DIFF_EURUSD_PROBE_CONTRACT_V1.md

Authority: Owner self-research OIS/RFR panel freeze. Discovery-only.
...
PANEL = RESEARCH / "preflight" / "v8_exogenous" / "panels" / "us_eu_ois_rfr_diff_d1_v1.csv"
DIFF_FIELD = "diff_sofr_minus_estr"
...
MOM_LOOKBACK = 20
MAX_GAP_DAYS = 3
RESULT_NAME = "20260713_V8_OIS_SOFR_ESTR_DIFF_EURUSD_PROBE_RESULT_V1.json"
TRADES_NAME = "20260713_V8_OIS_SOFR_ESTR_DIFF_EURUSD_PROBE_TRADES_V1.csv"
PROBE_ID = "V8_OIS_SOFR_ESTR_DIFF_EURUSD_V1"
READOUT_NAME = "20260713_V8_OIS_SOFR_ESTR_DIFF_EURUSD_OFFLINE_PROBE_READOUT.md"
...
def load_diff_available() -> tuple[dict[date, float], dict]:
    ...
    with PANEL.open(encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            ...
            diff_s = (row.get(DIFF_FIELD) or "").strip()
...
def main() -> int:
    ...
    try:
        ...
        cand = simulate(rows, atrs, idx, cand_dir, "candidate_ois_rfr_diff")
        ...
        result = {
            "schema": "v8_ois_sofr_estr_diff_eurusd_offline_probe.v1",
            "probe_id": PROBE_ID,
