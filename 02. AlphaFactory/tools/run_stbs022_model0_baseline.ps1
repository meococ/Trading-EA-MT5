...
$hyp022NonRepaintAuditorSha256 = "366D70F0C6FAF02F85B4819E7305CD1BD271BA6A78B4789CF0DCDF2FB651E360"
$hyp022StaticNonRepaintManifestPath = Join-Path $repoRoot "03. EA Developer\EA_SupertrendBurstScalperTradeV9\HYP-STBS-XAUUSD-M15-022_NONREPAINT_MANIFEST.json"
$hyp022StaticNonRepaintManifestSha256 = "899E2C031DBC93FD99450990347C3FB1FB412E848964820AD4A0887FAAE3F6F1"
$hyp022StaticNonRepaintAuditPath = Join-Path $repoRoot "03. EA Developer\EA_SupertrendBurstScalperTradeV9\research\HYP-STBS-XAUUSD-M15-022_NONREPAINT_AUDIT.json"
$hyp022StaticNonRepaintAuditSha256 = "359E11DC5979E5D0B915A510F0148D3451D11994EDF46ADA1D18F3CF0C238509"
$hyp022SourceSha256 = "9B82946CF17A876B547E7227F7FA131183C2383D38BF639574001CAB03DF8D82"
$hyp022CopyTimeLine = 678
...
function Add-Model0EconomicLaunchAuthorityBlockers(
    ...
    foreach ($boundControl in @(
        ...
        [pscustomobject]@{ PathField = 'reviewed_nonrepaint_auditor_path'; ShaField = 'reviewed_nonrepaint_auditor_sha256'; Label = 'HYP022 non-repaint auditor' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_manifest_path'; ShaField = 'reviewed_static_nonrepaint_manifest_sha256'; Label = 'HYP022 static non-repaint manifest' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_audit_path'; ShaField = 'reviewed_static_nonrepaint_audit_sha256'; Label = 'HYP022 static non-repaint audit' }
    )) {
    ...
    foreach ($exactControl in @(
        [pscustomobject]@{ PathField = 'reviewed_nonrepaint_auditor_path'; ShaField = 'reviewed_nonrepaint_auditor_sha256'; ExpectedPath = $nonRepaintToolPath; ExpectedSha = $hyp022NonRepaintAuditorSha256; Label = 'non-repaint auditor' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_manifest_path'; ShaField = 'reviewed_static_nonrepaint_manifest_sha256'; ExpectedPath = $hyp022StaticNonRepaintManifestPath; ExpectedSha = $hyp022StaticNonRepaintManifestSha256; Label = 'static non-repaint manifest' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_audit_path'; ShaField = 'reviewed_static_nonrepaint_audit_sha256'; ExpectedPath = $hyp022StaticNonRepaintAuditPath; ExpectedSha = $hyp022StaticNonRepaintAuditSha256; Label = 'static non-repaint audit' }
    )) {
        ...
        if (-not [string]::Equals($resolvedPath, [System.IO.Path]::GetFullPath($exactControl.ExpectedPath), [System.StringComparison]::OrdinalIgnoreCase) -or
            ...
            $Blockers.Add("Model0 economic authority does not bind the exact HYP022 $($exactControl.Label) path/SHA256.")
        }
    }
...
function New-ExecutionReceipt($ReceiptPath, $Contract, $PacketResult, $Binding, $LaunchClaimRecord = $null, [string]$RunnerPath = $PSCommandPath) {
    ...
    if ([string]$PacketResult.Authority -ceq 'MODEL0_TRAIN_FALSIFICATION_ONLY' -and
        $Contract.HypothesisId -ceq 'HYP-STBS-XAUUSD-M15-022') {
        $validation = Get-ObjectProperty $Contract.LatestRow 'validation'
        foreach ($boundControl in @(
            [pscustomobject]@{ Label = 'hyp022_nonrepaint_auditor'; PathField = 'reviewed_nonrepaint_auditor_path'; ShaField = 'reviewed_nonrepaint_auditor_sha256' },
            [pscustomobject]@{ Label = 'hyp022_static_nonrepaint_manifest'; PathField = 'reviewed_static_nonrepaint_manifest_path'; ShaField = 'reviewed_static_nonrepaint_manifest_sha256' },
            [pscustomobject]@{ Label = 'hyp022_static_nonrepaint_audit'; PathField = 'reviewed_static_nonrepaint_audit_path'; ShaField = 'reviewed_static_nonrepaint_audit_sha256' }
        )) {
...
function New-Hyp022NonRepaintAuditManifest($RunManifestPath, $RunManifestSha256, $AnalysisDir) {
    if ($HypothesisId -cne 'HYP-STBS-XAUUSD-M15-022' -or
        $EaName -cne 'EA_SupertrendBurstScalperTradeV9') {
        throw "The HYP022 runner cannot adapt another hypothesis or EA."
    }
    if ((Get-Sha256IfExists $hyp022StaticNonRepaintManifestPath) -ine $hyp022StaticNonRepaintManifestSha256) {
        throw "HYP022 static non-repaint manifest hash mismatch."
    }
    $static = Get-Content -LiteralPath $hyp022StaticNonRepaintManifestPath -Raw | ConvertFrom-Json
    if ((Get-Sha256IfExists $hyp022StaticNonRepaintManifestPath) -ine $hyp022StaticNonRepaintManifestSha256) {
        throw "HYP022 static non-repaint manifest changed while being read."
    }
    if ([string]$static.hypothesis_id -cne $HypothesisId -or
        [string]$static.source_sha256 -cne $hyp022SourceSha256 -or
        $static.nondecision_provenance_copytime_authorized -ne $true) {
        throw "HYP022 static non-repaint provenance authority is invalid."
    }
    if ((Get-Sha256IfExists $RunManifestPath) -ine $RunManifestSha256) {
        throw "HYP022 original run manifest drifted before the audit adapter read."
    }
    $run = Get-Content -LiteralPath $RunManifestPath -Raw | ConvertFrom-Json
    if ((Get-Sha256IfExists $RunManifestPath) -ine $RunManifestSha256) {
        throw "HYP022 original run manifest changed while being read."
    }
    if ([string]$run.hypothesis_id -cne $HypothesisId -or
        [string]$run.ea_name -cne $EaName -or
        [string]$run.source_sha256 -cne $hyp022SourceSha256) {
        throw "HYP022 run manifest identity does not match the frozen static provenance authority."
    }
    if ($null -ne $run.PSObject.Properties['nondecision_provenance_copytime_authorized'] -or
        ...
        throw "Original HYP022 run manifest already contains unexpected provenance fields."
    }
    ...
    $run | Add-Member -MemberType NoteProperty -Name nondecision_provenance_authority_source -Value ([ordered]@{
        path = $hyp022StaticNonRepaintManifestPath
        sha256 = $hyp022StaticNonRepaintManifestSha256
    ...
    if ((Get-Sha256IfExists $RunManifestPath) -ine $RunManifestSha256) {
        throw "HYP022 original run manifest changed while the derivative was written."
    }
    return $out
}
...
if ($EaName -cne 'EA_SupertrendBurstScalperTradeV9' -or
    $HypothesisId -cne 'HYP-STBS-XAUUSD-M15-022' -or
    ...
    throw "The HYP022 runner accepts only the frozen XAUUSD M15 2005-2023 Model0 control baseline contract."
}
...
try {
    ...
    $nonRepaintRunManifestPath = New-Hyp022NonRepaintAuditManifest $runManifestPath $runManifestShaForAudit $analysisDir
    ...
    if ($nonRepaintAuditorShaForAudit -cne $hyp022NonRepaintAuditorSha256) {
        throw "HYP022 non-repaint auditor hash drifted before execution."
    }
    ...
    if ($auditedFiles.Count -ne 1 -or
        ...
        [string]$auditedFiles[0].sha256 -cne $hyp022SourceSha256 -or
        ...
        [int]$allowedGates[0].line -ne $hyp022CopyTimeLine -or
    ...
    if ((Get-Sha256IfExists $nonRepaintToolPath) -cne $nonRepaintAuditorShaForAudit -or
        ...
        throw "HYP022 non-repaint auditor, derivative manifest or audit artifact drifted during validation."
    }
    ...
    $evidence.nonrepaint_audited_source_sha256 = $hyp022SourceSha256
    $evidence.nonrepaint_allowed_gate = [ordered]@{
        path = $expectedSnapshotSourcePath
        line = $hyp022CopyTimeLine
