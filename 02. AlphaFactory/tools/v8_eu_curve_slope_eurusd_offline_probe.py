#!/usr/bin/env python3
"""V8 offline probe: lagged EU AAA curve slope (10Y−2Y) → EURUSD.

Frozen contract:
  03. EA Developer/EA_SonicR/research/preflight/v8_exogenous/
  20260713_V8_EU_CURVE_SLOPE_EURUSD_PROBE_CONTRACT_V1.md
"""
...
MOM_LOOKBACK = 20
MAX_GAP_DAYS = 3
RESULT_NAME = "20260713_V8_EU_CURVE_SLOPE_EURUSD_PROBE_RESULT_V1.json"
TRADES_NAME = "20260713_V8_EU_CURVE_SLOPE_EURUSD_PROBE_TRADES_V1.csv"
PROBE_ID = "V8_EU_CURVE_SLOPE_EURUSD_V1"
READOUT_NAME = "20260713_V8_EU_CURVE_SLOPE_EURUSD_OFFLINE_PROBE_READOUT.md"
...
def evaluate(cand: list[Trade], ctrl: list[Trade]) -> dict:
    ...
    verdict = "PROBE_SURVIVOR" if train_pass else "KILL_AT_OFFLINE_PROBE"
    return {
        "verdict": verdict,
        "kill_reasons": kills,
...
def write_readout(result: dict) -> Path:
    ...
    lines.extend(
        [
            ...
            "on kill. Do not retune z/tenors from this readout.",
            "",
        ]
    )
...
def main() -> int:
    ...
    try:
        avail, panel_meta = load_slope_available()
        zmap = build_z(avail)
        ...
        def cand_dir(d: date) -> int | None:
            ...
            for zd in zmap:
                if zd <= d and (best is None or zd > best):
                    best = zd
            if best is None or (d - best).days > MAX_GAP_DAYS:
                return None
            z = zmap[best]
            # Contract: steep (high z) -> long EURUSD
            if z >= Z_THRESH:
                return 1
            if z <= -Z_THRESH:
                return -1
        ...
        cand = simulate(rows, atrs, idx, cand_dir, "candidate_eu_curve_slope")
        ctrl = simulate(rows, atrs, idx, ctrl_dir, "control_mom")
        ...
        result = {
            "schema": "v8_eu_curve_slope_eurusd_offline_probe.v1",
            "probe_id": PROBE_ID,
            ...
            **ev,
            ...
            "authority_flags": {
                "registry_append_authorized": ev["verdict"] == "PROBE_SURVIVOR",
                "prereg_freeze_authorized": ev["verdict"] == "PROBE_SURVIVOR",
            ...
            json.dumps(
                {
                    ...
                    "ctrl_pf_a": result["train_control"]["pf_stress_a"],
                    "result": str(out),
                    "readout": str(readout),
                },
                indent=2,
        ...
        return 0 if result["verdict"] == "PROBE_SURVIVOR" else 2
    finally:
        mt5.shutdown()
