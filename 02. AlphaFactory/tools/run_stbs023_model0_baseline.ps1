...
$hyp023NonRepaintAuditorSha256 = "366D70F0C6FAF02F85B4819E7305CD1BD271BA6A78B4789CF0DCDF2FB651E360"
$hyp023StaticNonRepaintManifestPath = Join-Path $repoRoot "03. EA Developer\EA_SupertrendBurstScalperTradeV10\HYP-STBS-XAUUSD-M15-023_NONREPAINT_MANIFEST.json"
$hyp023StaticNonRepaintManifestSha256 = "2FD0CB06E4273671994A7D6B105701FC78AFAC37536E111D791519D99CF2086D"
$hyp023StaticNonRepaintAuditPath = Join-Path $repoRoot "03. EA Developer\EA_SupertrendBurstScalperTradeV10\research\HYP-STBS-XAUUSD-M15-023_NONREPAINT_AUDIT.json"
$hyp023StaticNonRepaintAuditSha256 = "7E281A579EB6DE5E90C5263BA6E5E064015A96231753C217DE592CA6AE641053"
$hyp023SourceSha256 = "4B481CE867DB8A9F9E02AB218FEA50C88FD37A48B8ECB92E2048418DB7F7769B"
$hyp023CopyTimeLine = 678
...
function Add-Model0EconomicLaunchAuthorityBlockers(
    ...
    foreach ($boundControl in @(
        ...
        [pscustomobject]@{ PathField = 'reviewed_nonrepaint_auditor_path'; ShaField = 'reviewed_nonrepaint_auditor_sha256'; Label = 'HYP023 non-repaint auditor' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_manifest_path'; ShaField = 'reviewed_static_nonrepaint_manifest_sha256'; Label = 'HYP023 static non-repaint manifest' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_audit_path'; ShaField = 'reviewed_static_nonrepaint_audit_sha256'; Label = 'HYP023 static non-repaint audit' }
    )) {
    ...
    foreach ($exactControl in @(
        [pscustomobject]@{ PathField = 'reviewed_nonrepaint_auditor_path'; ShaField = 'reviewed_nonrepaint_auditor_sha256'; ExpectedPath = $nonRepaintToolPath; ExpectedSha = $hyp023NonRepaintAuditorSha256; Label = 'non-repaint auditor' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_manifest_path'; ShaField = 'reviewed_static_nonrepaint_manifest_sha256'; ExpectedPath = $hyp023StaticNonRepaintManifestPath; ExpectedSha = $hyp023StaticNonRepaintManifestSha256; Label = 'static non-repaint manifest' },
        [pscustomobject]@{ PathField = 'reviewed_static_nonrepaint_audit_path'; ShaField = 'reviewed_static_nonrepaint_audit_sha256'; ExpectedPath = $hyp023StaticNonRepaintAuditPath; ExpectedSha = $hyp023StaticNonRepaintAuditSha256; Label = 'static non-repaint audit' }
    )) {
        ...
        if (-not [string]::Equals($resolvedPath, [System.IO.Path]::GetFullPath($exactControl.ExpectedPath), [System.StringComparison]::OrdinalIgnoreCase) -or
            ...
            $Blockers.Add("Model0 economic authority does not bind the exact HYP023 $($exactControl.Label) path/SHA256.")
        }
    }
    ...
    $packetAttemptStart = '03. EA Developer/EA_SupertrendBurstScalperTradeV10/research/evidence/HYP-STBS-XAUUSD-M15-023/STBS023-PACKET-BUILD-001/attempt_started.json'
    $packetAttemptTerminal = '03. EA Developer/EA_SupertrendBurstScalperTradeV10/research/evidence/HYP-STBS-XAUUSD-M15-023/STBS023-PACKET-BUILD-001/attempt_terminal.json'
    if (
        [string](Get-ObjectProperty $validation 'packet_build_attempt_id') -cne 'STBS023-PACKET-BUILD-001' -or
        ...
        $Blockers.Add("Model0 economic authority does not bind the exact completed HYP023 packet-build attempt metadata.")
    }
...
function Assert-Hyp023PacketBuildChain($Contract, $PacketResult, $LaunchClaimRecord) {
    if ($Contract.HypothesisId -cne 'HYP-STBS-XAUUSD-M15-023') {
        return
    }
    $validation = Get-ObjectProperty $Contract.LatestRow 'validation'
    $metrics = Get-ObjectProperty $Contract.LatestRow 'metrics'
    $expectedStart = '03. EA Developer/EA_SupertrendBurstScalperTradeV10/research/evidence/HYP-STBS-XAUUSD-M15-023/STBS023-PACKET-BUILD-001/attempt_started.json'
    $expectedTerminal = '03. EA Developer/EA_SupertrendBurstScalperTradeV10/research/evidence/HYP-STBS-XAUUSD-M15-023/STBS023-PACKET-BUILD-001/attempt_terminal.json'
    ...
    if (
        ...
        $gitignoreRelative -cne '.gitignore'
    ) {
        throw 'HYP023 packet-build authority metadata is not exact.'
    }
    ...
    if (
        ...
        throw 'HYP023 packet-build chain file hash mismatch.'
    }
    ...
    if (-not (Test-ExactObjectKeys $start @(
        ...
        throw 'HYP023 packet-build start/terminal schema is not exact.'
    }
    if (
        [string]$start.schema_version -cne 'alphafactory_packet_attempt_started.v1' -or
        [string]$start.hypothesis_id -cne 'HYP-STBS-XAUUSD-M15-023' -or
        [string]$start.attempt_id -cne 'STBS023-PACKET-BUILD-001' -or
        [string]$start.builder_path -cne '03. EA Developer/EA_SupertrendBurstScalperTradeV10/research/build_stbs023_task_packet.py' -or
        [string]$terminal.schema_version -cne 'alphafactory_packet_attempt_terminal.v1' -or
        [string]$terminal.hypothesis_id -cne 'HYP-STBS-XAUUSD-M15-023' -or
        [string]$terminal.attempt_id -cne 'STBS023-PACKET-BUILD-001' -or
        ...
        [string](Get-ObjectProperty $PacketResult.Packet 'packet_build_attempt_id') -cne 'STBS023-PACKET-BUILD-001' -or
        ...
        [string](Get-ObjectProperty $PacketResult.Packet 'gitignore_sha256') -cne $gitignoreSha
    ) {
        throw 'HYP023 packet-build start/terminal/packet semantic chain is invalid.'
    }
    if ($null -eq $LaunchClaimRecord -or -not (Test-Path -LiteralPath $LaunchClaimRecord.Path -PathType Leaf)) {
        throw 'HYP023 Model0 launch claim is absent before packet-build chain validation.'
    }
    ...
    if (-not ($startAt -le $terminalAt -and $terminalAt -le $authorityAt -and $authorityAt -le $launchAt)) {
        throw 'HYP023 packet-build/start/terminal/authority/launch chronology is invalid.'
    }
}
...
function New-ExecutionReceipt($ReceiptPath, $Contract, $PacketResult, $Binding, $LaunchClaimRecord = $null, [string]$RunnerPath = $PSCommandPath) {
    ...
    if ([string]$PacketResult.Authority -ceq 'MODEL0_TRAIN_FALSIFICATION_ONLY' -and
        $Contract.HypothesisId -ceq 'HYP-STBS-XAUUSD-M15-023') {
        $validation = Get-ObjectProperty $Contract.LatestRow 'validation'
        foreach ($packetControl in @(
            [pscustomobject]@{ Label = 'hyp023_packet_attempt_start'; PathField = 'packet_build_attempt_start_path'; ShaField = 'packet_build_attempt_start_sha256' },
            [pscustomobject]@{ Label = 'hyp023_packet_attempt_terminal'; PathField = 'packet_build_attempt_terminal_path'; ShaField = 'packet_build_attempt_terminal_sha256' },
            [pscustomobject]@{ Label = 'hyp023_gitignore'; PathField = 'gitignore_path'; ShaField = 'gitignore_sha256' }
        )) {
        ...
        foreach ($boundControl in @(
            [pscustomobject]@{ Label = 'hyp023_nonrepaint_auditor'; PathField = 'reviewed_nonrepaint_auditor_path'; ShaField = 'reviewed_nonrepaint_auditor_sha256' },
            [pscustomobject]@{ Label = 'hyp023_static_nonrepaint_manifest'; PathField = 'reviewed_static_nonrepaint_manifest_path'; ShaField = 'reviewed_static_nonrepaint_manifest_sha256' },
            [pscustomobject]@{ Label = 'hyp023_static_nonrepaint_audit'; PathField = 'reviewed_static_nonrepaint_audit_path'; ShaField = 'reviewed_static_nonrepaint_audit_sha256' }
        )) {
...
function New-Hyp023NonRepaintAuditManifest($RunManifestPath, $RunManifestSha256, $AnalysisDir) {
    if ($HypothesisId -cne 'HYP-STBS-XAUUSD-M15-023' -or
        $EaName -cne 'EA_SupertrendBurstScalperTradeV10') {
        throw "The HYP023 runner cannot adapt another hypothesis or EA."
    }
    if ((Get-Sha256IfExists $hyp023StaticNonRepaintManifestPath) -ine $hyp023StaticNonRepaintManifestSha256) {
        throw "HYP023 static non-repaint manifest hash mismatch."
    }
    $static = Get-Content -LiteralPath $hyp023StaticNonRepaintManifestPath -Raw | ConvertFrom-Json
    if ((Get-Sha256IfExists $hyp023StaticNonRepaintManifestPath) -ine $hyp023StaticNonRepaintManifestSha256) {
        throw "HYP023 static non-repaint manifest changed while being read."
    }
    if ([string]$static.hypothesis_id -cne $HypothesisId -or
        [string]$static.source_sha256 -cne $hyp023SourceSha256 -or
        $static.nondecision_provenance_copytime_authorized -ne $true) {
        throw "HYP023 static non-repaint provenance authority is invalid."
    }
    if ((Get-Sha256IfExists $RunManifestPath) -ine $RunManifestSha256) {
        throw "HYP023 original run manifest drifted before the audit adapter read."
    }
    $run = Get-Content -LiteralPath $RunManifestPath -Raw | ConvertFrom-Json
    if ((Get-Sha256IfExists $RunManifestPath) -ine $RunManifestSha256) {
        throw "HYP023 original run manifest changed while being read."
    }
    if ([string]$run.hypothesis_id -cne $HypothesisId -or
        [string]$run.ea_name -cne $EaName -or
        [string]$run.source_sha256 -cne $hyp023SourceSha256) {
        throw "HYP023 run manifest identity does not match the frozen static provenance authority."
    }
    if ($null -ne $run.PSObject.Properties['nondecision_provenance_copytime_authorized'] -or
        ...
        throw "Original HYP023 run manifest already contains unexpected provenance fields."
    }
    ...
    $run | Add-Member -MemberType NoteProperty -Name nondecision_provenance_authority_source -Value ([ordered]@{
        path = $hyp023StaticNonRepaintManifestPath
        sha256 = $hyp023StaticNonRepaintManifestSha256
    ...
    if ((Get-Sha256IfExists $RunManifestPath) -ine $RunManifestSha256) {
        throw "HYP023 original run manifest changed while the derivative was written."
    }
    return $out
}
...
if ($EaName -cne 'EA_SupertrendBurstScalperTradeV10' -or
    $HypothesisId -cne 'HYP-STBS-XAUUSD-M15-023' -or
    ...
    throw "The HYP023 runner accepts only the frozen XAUUSD M15 2005-2023 Model0 control baseline contract."
}
...
try {
    ...
    if ($null -ne $launchClaimRecord) {
        if ($contract.HypothesisId -ceq 'HYP-STBS-XAUUSD-M15-023') {
    ...
    $nonRepaintRunManifestPath = New-Hyp023NonRepaintAuditManifest $runManifestPath $runManifestShaForAudit $analysisDir
    ...
    if ($nonRepaintAuditorShaForAudit -cne $hyp023NonRepaintAuditorSha256) {
        throw "HYP023 non-repaint auditor hash drifted before execution."
    }
    ...
    if ($auditedFiles.Count -ne 1 -or
        ...
        [string]$auditedFiles[0].sha256 -cne $hyp023SourceSha256 -or
        ...
        [int]$allowedGates[0].line -ne $hyp023CopyTimeLine -or
    ...
    if ((Get-Sha256IfExists $nonRepaintToolPath) -cne $nonRepaintAuditorShaForAudit -or
        ...
        throw "HYP023 non-repaint auditor, derivative manifest or audit artifact drifted during validation."
    }
    ...
    $evidence.nonrepaint_audited_source_sha256 = $hyp023SourceSha256
    $evidence.nonrepaint_allowed_gate = [ordered]@{
        path = $expectedSnapshotSourcePath
        line = $hyp023CopyTimeLine
