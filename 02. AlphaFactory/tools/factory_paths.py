...
# A directory is the repo root when it holds both of these. AGENTS.md used to be
# a third marker; it was removed from the repo on 2026-08-31, so root detection
# must not depend on it.
_ROOT_MARKERS = ("02. AlphaFactory", "03. EA Developer")

# `alpha.local.ps1` is generated by tools/init_machine_paths.ps1 and uses four
# assignment shapes. All are matched as text; nothing is ever executed.
#   $MT5X = "literal"
#   $MT5X = $MT5Y
#   $MT5X = Join-Path $Base "tail"
#   $MT5X = $true / $false
_ASSIGN_LITERAL = re.compile(
    r'^\s*\$(?P<name>MT5[A-Za-z0-9_]*)\s*=\s*"(?P<value>[^"]*)"\s*$'
)
_ASSIGN_VAR = re.compile(
    r'^\s*\$(?P<name>MT5[A-Za-z0-9_]*)\s*=\s*\$(?P<ref>[A-Za-z][A-Za-z0-9_]*)\s*$'
)
_ASSIGN_JOIN = re.compile(
    r'^\s*\$(?P<name>MT5[A-Za-z0-9_]*)\s*=\s*Join-Path\s+'
    r'\$(?P<base>[A-Za-z][A-Za-z0-9_]*)\s+"(?P<tail>[^"]*)"\s*$',
    re.IGNORECASE,
)
_BOOL_RE = re.compile(
...
def load_alpha_local(repo_root: Path | None = None) -> dict[str, object]:
    ...
    if not local.is_file():
        raise FactoryPathError(
            ...
            r'Generate it with:  & ".\02. AlphaFactory\tools\init_machine_paths.ps1"'
        )

    # $PSScriptRoot inside alpha.local.ps1 is the directory holding that file.
    values: dict[str, object] = {"PSScriptRoot": str(local.parent)}
    for raw in local.read_text(encoding="utf-8-sig", errors="replace").splitlines():
        ...
        match = _ASSIGN_JOIN.match(line)
        if match:
            base = values.get(match.group("base"))
            if isinstance(base, str) and base:
                values[match.group("name")] = str(Path(base) / match.group("tail"))
            continue

        match = _ASSIGN_VAR.match(line)
        if match:
            ref = values.get(match.group("ref"))
            if ref is not None:
                values[match.group("name")] = ref
            continue

    values.pop("PSScriptRoot", None)
    return values
