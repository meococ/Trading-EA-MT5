...
_MQ_TERMINAL = Path(os.environ.get("APPDATA", "")) / "MetaQuotes" / "Terminal"
_MQ_TERMINAL_IDS = (
    sorted(
        d
        for d in _MQ_TERMINAL.glob("*")
        if d.is_dir()
        and len(d.name) == 32
        and all(c in "0123456789ABCDEF" for c in d.name.upper())
    )
    if _MQ_TERMINAL.is_dir()
    else []
)
DEFAULT_IMPORT_DIRS = [
    _MQ_TERMINAL / "Common" / "Files" / "PaperDeploy",
    _MQ_TERMINAL / "Common" / "Files",
    *[d / "MQL5" / "Files" for d in _MQ_TERMINAL_IDS],
]
CSV_ALIASES = {
