    ...
    if ([string]::IsNullOrWhiteSpace($costSourceExpectedHash)) {
        ...
        if ($actualCostSourceHash -ine $costSourceExpectedHash) {
            ...
            try {
                ...
                if ($provenanceStatus -cne 'VERIFIED') {
                    $blockers.Add("Cost source manifest provenance_status must be VERIFIED; got '$provenanceStatus'.")
                }
                $rootAuditStatus = [string](Get-ObjectProperty $costSourceManifest 'audit_status')
                if (-not [string]::IsNullOrWhiteSpace($rootAuditStatus) -and $rootAuditStatus -notin @('PASS', 'VERIFIED')) {
                    $blockers.Add("Cost source manifest audit_status '$rootAuditStatus' is incompatible with VERIFIED provenance.")
                }
                $rootVerdict = [string](Get-ObjectProperty $costSourceManifest 'verdict')
                if (-not [string]::IsNullOrWhiteSpace($rootVerdict) -and $rootVerdict -cne 'PASS') {
                    $blockers.Add("Cost source manifest verdict '$rootVerdict' is incompatible with VERIFIED provenance.")
                }
                ...
                if (-not (Test-ProvenanceObject $commissionNode)) {
                    ...
                    if ([string](Get-ObjectProperty $commissionNode 'verification_status') -cne 'VERIFIED') {
                        $blockers.Add("commission_provenance.verification_status must be VERIFIED.")
                    }
                ...
                if (-not (Test-ProvenanceObject $slippageNode)) {
                    ...
                    if ([string](Get-ObjectProperty $slippageNode 'verification_status') -cne 'VERIFIED') {
                        $blockers.Add("slippage_provenance.verification_status must be VERIFIED.")
                    }
                    ...
                    if (-not (Test-PositiveInteger $slippageSamples) -or [int64]$slippageSamples -lt 100 -or -not $slippageIndependent) {
                        $blockers.Add("slippage_provenance requires at least 100 independent-reference samples.")
                    }
                ...
                if (-not (Test-ProvenanceObject $methodologyNode)) {
                    ...
                    if ([string](Get-ObjectProperty $methodologyNode 'verification_status') -cne 'VERIFIED') {
                        $blockers.Add("direction_aware_methodology.verification_status must be VERIFIED.")
                    }
...
try {
    ...
    $backtestArgs = @(
        ...
        "-FixedDelayMs", [string]$FixedDelayMs,
    ...
    if (-not [string]::IsNullOrWhiteSpace($Spread)) { $backtestArgs += @("-Spread", $Spread) }
    $backtest = Invoke-RequiredStep "Backtest $EaName $Symbol $Period $From-$To Model=$Model" { & $alphaPs1 @backtestArgs } $steps
    ...
    [void](Invoke-RequiredStep "Build report-bound verified cost artifact $runId" { & python @costBuildArgs } $steps)
    ...
    Add-StateTransition "verified_cost_artifact_built" $costArtifact | Out-Null
    ...
    [void](Invoke-RequiredStep "Unified validation $runId stage=$($packetResult.ValidationStage)" { & python @validationArgs } $steps)
    ...
    [void](Invoke-RequiredStep "Refresh runs database" { & python $runsDb build } $steps)
