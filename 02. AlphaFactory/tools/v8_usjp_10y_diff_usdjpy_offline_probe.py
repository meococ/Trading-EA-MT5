#!/usr/bin/env python3
"""V8 offline probe: lagged US-JP 10Y bond yield differential -> USDJPY.

Frozen contract:
  03. EA Developer/EA_SonicR/research/preflight/v8_exogenous/
  20260713_V8_USJP_10Y_DIFF_USDJPY_PROBE_CONTRACT_V1.md

Authority: Owner self-research MoF JGB panel freeze. Discovery-only.
...
PANEL = RESEARCH / "preflight" / "v8_exogenous" / "panels" / "us_jp_bond_yield_diff_d1_v1.csv"
DIFF_FIELD = "diff_us_jp_10y"
...
MOM_LOOKBACK = 20
MAX_GAP_DAYS = 3
RESULT_NAME = "20260713_V8_USJP_10Y_DIFF_USDJPY_PROBE_RESULT_V1.json"
TRADES_NAME = "20260713_V8_USJP_10Y_DIFF_USDJPY_PROBE_TRADES_V1.csv"
PROBE_ID = "V8_USJP_10Y_DIFF_USDJPY_V1"
READOUT_NAME = "20260713_V8_USJP_10Y_DIFF_USDJPY_OFFLINE_PROBE_READOUT.md"
...
def main() -> int:
    ...
    try:
        ...
        def cand_dir(d: date) -> int | None:
            ...
            z = zmap[best]
            if z >= Z_THRESH:
                return 1  # long USDJPY (USD premium)
            if z <= -Z_THRESH:
                return -1
            return 0
        ...
        cand = simulate(rows, atrs, idx, cand_dir, "candidate_usjp_bond_diff")
        ...
        result = {
            "schema": "v8_usjp_10y_diff_usdjpy_offline_probe.v1",
            "probe_id": PROBE_ID,
