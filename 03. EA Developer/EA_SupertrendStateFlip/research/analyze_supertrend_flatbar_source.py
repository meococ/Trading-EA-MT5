#!/usr/bin/env python3
"""Outcome-blind native-H1 Supertrend 10/3 source analyzer with valid flat bars."""
...
HYPOTHESIS_ID = "HYP-ST-XAUUSD-H1-002"
ATTEMPT_ID = "ST002-SOURCE-ATTEMPT-001"
PREREG_SHA256 = "FF23C8FDD5BDCD4B25AC02A46D984D2B9815A099BD92FB47E4F565DF404668D8"
FORMULA_DEPENDENCY_SHA256 = "2B48F3AA01BB2B00EB66A5AE97346F810EF549CEC2626B0DC9F175EEC890211C"
...
EVENT_KEYS = {
    ...
    "prior_state", "state", "atr10", "final_upper", "final_lower",
    "supertrend", "source_close",
}
...
FORMULA_DEPENDENCY_PATH = Path(__file__).resolve().with_name("analyze_supertrend_source.py")


def load_formula_dependency() -> Any:
    if sha256_file(FORMULA_DEPENDENCY_PATH) != FORMULA_DEPENDENCY_SHA256:
        raise ValueError("frozen ST001 formula dependency SHA mismatch")
    spec = importlib.util.spec_from_file_location("st001_formula_dependency", FORMULA_DEPENDENCY_PATH)
    if not spec or not spec.loader:
        raise ValueError("unable to load frozen ST001 formula dependency")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


FORMULA = load_formula_dependency()
DOWN = FORMULA.DOWN
UP = FORMULA.UP
calculate_supertrend = FORMULA.calculate_supertrend
...
def validate_selected_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if data.empty or data.at[0, "time_utc"] != SOURCE_START:
        raise ValueError("source does not begin at manifest-declared first H1 timestamp")
    if (data["time_utc"] >= DESIGN_END).any():
        raise ValueError("reader materialized sealed 2023+ rows")
    ...
    if not valid.all():
        raise ValueError("all inception-through-design prices must be finite with high>=low and close inside range")
...
def analyze_frame(frame: pd.DataFrame) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for column in ("high", "low", "close"):
        data[column] = pd.to_numeric(data[column], errors="raise")
    indicator = calculate_supertrend(data)
    state = indicator["state"]
    usable = indicator["feature_valid"]
    design = ((data["time_utc"] >= DESIGN_START) & (data["time_utc"] < DESIGN_END)).to_numpy()
    prior = np.roll(state, 1)
    prior[0] = 0
    raw_long = design & usable & (prior == DOWN) & (state == UP)
    raw_short = design & usable & (prior == UP) & (state == DOWN)
    conflicts = raw_long & raw_short
    raw_long &= ~conflicts
    raw_short &= ~conflicts
    raw_mask = raw_long | raw_short
    exact_next = ((data["time_utc"].shift(-1) - data["time_utc"]) == pd.Timedelta(hours=1)).to_numpy()
    ...
    for index in np.flatnonzero(event_mask):
        ...
        events.append({
            ...
            "prior_state": "DOWN" if prior[index] == DOWN else "UP",
            "state": "UP" if state[index] == UP else "DOWN",
    ...
    usable_design_rows = int((usable & design).sum())
    feature_coverage = usable_design_rows / max(design_rows, 1)
    ...
    gates = {
        ...
        "each_year_cadence": all(
            MIN_YEAR_CADENCE <= row["cadence_per_week"] <= MAX_YEAR_CADENCE for row in yearly.values()
        ),
        "zero_direction_conflicts": int(conflicts.sum()) == 0,
    }
    passed = all(gates.values())
    report = {
        "schema_version": "supertrend10x3_flatbar_source_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "epistemic_scope": "OUTCOME_BLIND_SUPERTREND10X3_FLATBAR_VALID_STATE_FLIPS_AND_CADENCE_ONLY",
        "source_window": {"from": data.at[0, "time_utc"].isoformat(), "to_exclusive": DESIGN_END.isoformat()},
        "scored_window": {"from": DESIGN_START.isoformat(), "to_exclusive": DESIGN_END.isoformat()},
        "geometry_contract": "finite_high_gte_low_close_inside_range_flat_bars_valid_no_resets",
        "parameters": {
            "timeframe": "H1", "atr_period": 10, "factor": 3.0,
            "atr_smoothing": "sma_seeded_wilder_rma", "initial_state": "DOWN",
        },
        "funnel": {
            "source_rows": int(len(data)), "design_rows": design_rows,
            "feature_usable_design_rows": usable_design_rows, "raw_events": raw_count,
            "executable_events": count, "gap_rejected_events": raw_count - count,
            "direction_conflicts": int(conflicts.sum()), "long_events": longs, "short_events": shorts,
        },
        "metrics": {
            "elapsed_weeks": elapsed_weeks, "feature_coverage": feature_coverage,
            "raw_event_exact_next_coverage": next_coverage, "event_cadence_per_week": cadence,
        ...
        "verdict": (
            ...
            if passed else "PARK_SOURCE_FEASIBILITY_EXACT_SUPERTREND10X3_FLATBAR_VALID"
        ),
        "prohibitions": {
            "post_event_ohlc_read": False, "returns_computed": False,
            "trades_simulated": False, "economics_executed": False,
            "validation_opened": False, "holdout_opened": False,
            "mql5_build_authorized_by_attempt": passed, "economic_build_authorized": False,
            "native_supertrend_claim_authorized": False, "live_trading_authorized": False,
        },
    }
...
def validate_manifest(manifest_path: Path, data_path: Path) -> None:
    ...
    if len(matches) != 1 or matches[0].get("sha256") != DATA_SHA256 or matches[0].get("first_time_utc") != "2004-06-11T04:00:00Z":
        raise ValueError("manifest does not bind frozen H1 data/inception")
...
def validate_registry_authority(registry_path: Path) -> dict[str, str]:
    ...
    checks = {
        ...
        "parent": row.get("parent_candidate") == "HYP-ST-XAUUSD-H1-001",
        ...
        "formula_dependency": validation.get("formula_dependency_sha256") == FORMULA_DEPENDENCY_SHA256,
        "prehistory": validation.get("prehistory_source_access_authorized") is True,
        "flat_bars": validation.get("flat_bar_source_validity_authorized") is True,
...
def claim_attempt(output_dir: Path, authority: dict[str, str]) -> tuple[str, Path]:
    ...
    marker = {
        "schema_version": "supertrend_flatbar_attempt_started.v1",
        ...
        "formula_dependency_sha256": FORMULA_DEPENDENCY_SHA256,
...
def execute(root: Path) -> dict[str, Any]:
    prereg = root / "03. EA Developer/EA_SupertrendStateFlip/research/HYP-ST-XAUUSD-H1-002_FROZEN_PREREG.md"
    ...
    output_dir = root / "03. EA Developer/EA_SupertrendStateFlip/research/evidence/HYP-ST-XAUUSD-H1-002/ST002-SOURCE-ATTEMPT-001"
    if sha256_file(prereg) != PREREG_SHA256:
        raise ValueError("preregistration SHA mismatch")
    if sha256_file(FORMULA_DEPENDENCY_PATH) != FORMULA_DEPENDENCY_SHA256:
        raise ValueError("formula dependency SHA mismatch")
    ...
    report_path = output_dir / "st_002_source_report.json"
    ledger_path = output_dir / "st_002_event_ledger.jsonl"
    atomic_write(report_path, report_bytes)
    atomic_write(ledger_path, ledger_bytes)
    receipt = {
        "schema_version": "supertrend_flatbar_source_receipt.v1",
        "hypothesis_id": HYPOTHESIS_ID, "attempt_id": ATTEMPT_ID,
        "started_at_utc": started,
        "completed_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "bindings": {
            "preregistration": {"path": prereg.relative_to(root).as_posix(), "sha256": sha256_file(prereg)},
            "formula_dependency": {"path": FORMULA_DEPENDENCY_PATH.relative_to(root).as_posix(), "sha256": sha256_file(FORMULA_DEPENDENCY_PATH)},
        ...
        "outcome_blind_counters": {
            "post_event_ohlc_rows_read": 0, "returns_computed": 0, "trades_simulated": 0,
            "pnl_computed": 0, "profit_factor_computed": 0,
    ...
    terminal = {
        "schema_version": "supertrend_flatbar_attempt_terminal.v1",
        "hypothesis_id": HYPOTHESIS_ID, "attempt_id": ATTEMPT_ID,
        "completed_at_utc": receipt["completed_at_utc"], "status": "COMPLETE",
        "verdict": report["verdict"],
