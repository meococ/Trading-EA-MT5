//+------------------------------------------------------------------+
//| EA_SonicR_PVSRA.mq5                                              |
//| HYP-SONICR-XAU-H1-H4AT-001                                       |
//| XAUUSD H1 hour-8 H4 marubozu + body vs ATR. SL=H1. TP=1R.        |
//+------------------------------------------------------------------+
...
input string InpHypothesisId="HYP-SONICR-XAU-H1-H4AT-001";
input string InpVariantTag="XAU_H1_H4AT";

input group "--- Execution ---"
input long   InpMagic=16082776;
...
input int    InpDailyFlattenHour=21;
...
const ENUM_TIMEFRAMES SNR_SIGNAL_TF=PERIOD_H1;
const ENUM_TIMEFRAMES SNR_CONTEXT_TF=PERIOD_H1;
const ENUM_TIMEFRAMES SNR_TREND_TF=PERIOD_D1;
...
void LoadCfg()
  {
   ...
   g_cfg.daily_flatten_hour=InpDailyFlattenHour;
...
bool InputsSane()
  {
   return(_Period==SNR_SIGNAL_TF && SymbolAllowed() &&
          ...
          InpDailyFlattenHour>=0 && InpDailyFlattenHour<=24 &&
          InpRiskPercent>0.0 &&
...
bool SubmitMarket(const SnrSignalDecision &sig)
  {
   int pos_count=0;
   int pend_count=0;
   ulong owned_pending=0;
   ...
   double sl_est=sig.wave.structure_swing;
   if(sig.direction>0)
      sl_est=MathMin(sig.wave.structure_swing,entry)-InpSlBufferAtr*sig.atr;
   else
      sl_est=MathMax(sig.wave.structure_swing,entry)+InpSlBufferAtr*sig.atr;
   const double risk=(sig.direction>0 ? entry-sl_est : sl_est-entry);
   if(!SnrFinite(risk) || risk<=0.0)
     {
      g_tel.sr_blocks++;
      return(false);
     }
   const double tp=entry+(double)sig.direction*1.0*risk;

   SnrRiskPlan plan;
   if(!SnrPlanMarketLevels(_Symbol,sig.direction,entry,sig.wave.structure_swing,tp,
                           sig.atr,InpSlBufferAtr,g_cfg.sl_cap,InpMinSlSpreadMult,
                           InpRiskPercent,tick.ask,tick.bid,plan))
     {
      g_tel.volume_rejects++;
   ...
   if(!SnrSendMarket(g_trade,InpMagic,InpDeviationPoints,sig.direction,plan,
     ...
     {
      g_tel.entry_rejects++;
      PrintFormat("SNR001_MARKET_REJECT dir=%s vol=%.2f px=%.5f sl=%.5f tp=%.5f retcode=%u",
                  (sig.direction>0 ? "LONG" : "SHORT"),plan.volume,plan.entry,plan.sl,plan.tp,retcode);
      return(false);
     }
   ...
   g_initial_sl=plan.sl;
   g_initial_tp=plan.tp;
   PrintFormat("SNR001_MARKET decision=%I64d dir=%s vol=%.2f px=%.5f sl=%.5f tp=%.5f retcode=%u",
               (long)sig.decision_time,(sig.direction>0 ? "LONG" : "SHORT"),
               plan.volume,plan.entry,plan.sl,plan.tp,retcode);
   return(true);
  }
...
int OnInit()
  {
   ...
   if(!SnrDragonCreate(g_handles,_Symbol,SNR_CONTEXT_TF,InpDragonPeriod,InpATRPeriod) ||
      !SnrTrendCreate(g_handles,_Symbol,SNR_CONTEXT_TF,InpTrendPeriod) ||
      !SnrHigherTfTrendCreate(g_handles,_Symbol,SNR_TREND_TF,InpTrendPeriod,InpATRPeriod) ||
      !SnrIchiCreate(g_handles,_Symbol,SNR_SIGNAL_TF) ||
   ...
   g_last_bar_open=iTime(_Symbol,SNR_SIGNAL_TF,0);
...
void OnTick()
  {
   ...
   const datetime current_bar_open=iTime(_Symbol,SNR_SIGNAL_TF,0);
   ...
   if(!SnrBuildH1H4atSignal(_Symbol,SNR_SIGNAL_TF,g_handles,g_cfg,current_bar_open,sig))
     {
      g_tel.data_fails++;
      return;
     }
