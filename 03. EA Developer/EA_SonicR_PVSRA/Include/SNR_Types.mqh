...
void SnrHandlesReset(SnrHandles &h)
  {
   ...
   h.h1_dragon_high=INVALID_HANDLE;
   h.h1_dragon_mid=INVALID_HANDLE;
   h.h1_dragon_low=INVALID_HANDLE;
   h.h1_trend=INVALID_HANDLE;
   h.h1_atr=INVALID_HANDLE;
   h.cci=INVALID_HANDLE;
   h.bands=INVALID_HANDLE;
   h.kc_ema=INVALID_HANDLE;
   h.sqz_atr=INVALID_HANDLE;
   h.adx=INVALID_HANDLE;
   h.ichi=INVALID_HANDLE;
  }
...
void SnrHandlesRelease(SnrHandles &h)
  {
   ...
   if(h.h1_dragon_high!=INVALID_HANDLE)
      IndicatorRelease(h.h1_dragon_high);
   if(h.h1_dragon_mid!=INVALID_HANDLE)
      IndicatorRelease(h.h1_dragon_mid);
   if(h.h1_dragon_low!=INVALID_HANDLE)
      IndicatorRelease(h.h1_dragon_low);
   if(h.h1_trend!=INVALID_HANDLE)
      IndicatorRelease(h.h1_trend);
   if(h.h1_atr!=INVALID_HANDLE)
      IndicatorRelease(h.h1_atr);
   if(h.cci!=INVALID_HANDLE)
      IndicatorRelease(h.cci);
   if(h.bands!=INVALID_HANDLE)
      IndicatorRelease(h.bands);
   if(h.kc_ema!=INVALID_HANDLE)
      IndicatorRelease(h.kc_ema);
   if(h.sqz_atr!=INVALID_HANDLE)
      IndicatorRelease(h.sqz_atr);
   if(h.adx!=INVALID_HANDLE)
      IndicatorRelease(h.adx);
   if(h.ichi!=INVALID_HANDLE)
      IndicatorRelease(h.ichi);
   SnrHandlesReset(h);
  }
...
bool SnrCopyClosedBufferIdx(const int handle,const int buffer,const int count,double &buf[])
  {
   if(handle==INVALID_HANDLE || buffer<0 || count<=0)
      return(false);
   ArraySetAsSeries(buf,true);
   const int got=CopyBuffer(handle,buffer,1,count,buf);
   return(got>=count);
  }

bool SnrCopyClosedBuffer(const int handle,const int count,double &buf[])
  {
   return(SnrCopyClosedBufferIdx(handle,0,count,buf));
  }
