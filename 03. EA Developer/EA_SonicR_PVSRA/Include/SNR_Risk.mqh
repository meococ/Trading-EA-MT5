...
bool SnrPlanMarketLevels(const string symbol,const int direction,
                         const double entry,const double structural_sl,
                         const double tp_price,const double atr,
                         const double sl_buffer_atr,const double sl_cap,
                         const double min_sl_spread_mult,const double risk_percent,
                         const double ask,const double bid,SnrRiskPlan &plan)
  {
   ...
   const double point=SymbolInfoDouble(symbol,SYMBOL_POINT);
   const double tick_size=SymbolInfoDouble(symbol,SYMBOL_TRADE_TICK_SIZE);
   if(!SnrFinite(entry) || !SnrFinite(structural_sl) || !SnrFinite(tp_price) ||
      !SnrFinite(atr) || !SnrFinite(ask) || !SnrFinite(bid) || atr<=0.0 ||
      point<=0.0 || tick_size<=0.0 || direction==SNR_DIR_NONE || ask<=bid)
      return(false);
   if((direction>0 && MathAbs(entry-ask)>10.0*point) ||
      (direction<0 && MathAbs(entry-bid)>10.0*point))
      return(false);

   double sl_raw=structural_sl;
   if(direction>0)
      sl_raw=MathMin(structural_sl,entry)-sl_buffer_atr*atr;
   else
      sl_raw=MathMax(structural_sl,entry)+sl_buffer_atr*atr;
