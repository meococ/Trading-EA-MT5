//+------------------------------------------------------------------+
//| EA_H1PinPDLevel.mq5 — H1 pin at prior D1 HL fade RR=3            |
//| Symbol: USDJPY | Period: H1 | Magic: 880993                       |
//| Hypothesis: HYP-H1-PIN-PDLEVEL-001                                 |
//| Closed-bar[1] only. Mon–Thu; weekend flat; risk 0.5%; RR=3.0.     |
//| Not PDH M15 break/retest; not H1SwingFailure densify.             |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_H1PinPDLevel"
...
input group "=== Pin / PD level ==="
input double   InpMinWickFrac   = 0.60;   // wick / range
input double   InpMaxOppWick    = 0.25;   // opposite wick max frac
input double   InpTouchBufATR   = 0.05;   // touch buffer vs PD HL
...
int OnInit()
{
   if(Period() != PERIOD_H1)
   {
      Print("[PINPD] FAIL — chart must be H1");
      return INIT_FAILED;
   }
   ...
   Print("[PINPD] HYP-H1-PIN-PDLEVEL-001 RR=", InpTP_Ratio);
...
// Prior completed D1 bar high/low (shift 1 on D1).
bool PriorDayHL(double &pdh, double &pdl)
{
   pdh = iHigh(_Symbol, PERIOD_D1, 1);
   pdl = iLow(_Symbol, PERIOD_D1, 1);
   return (pdh > pdl && pdh > 0.0 && pdl > 0.0);
}

// Closed H1 pin touching PDH/PDL → fade the wick.
bool DetectPinFade(bool &isBuy, double &slExtreme)
{
   isBuy = false;
   slExtreme = 0.0;
   ...
   if(!PriorDayHL(pdh, pdl))
      return false;
   double touchBuf = atr[0] * InpTouchBufATR;

   // Bearish pin at PDH → short
   if(upperWick / range >= InpMinWickFrac &&
      lowerWick / range <= InpMaxOppWick &&
      h1 >= pdh - touchBuf && h1 <= pdh + touchBuf)
   {
      isBuy = false;
      slExtreme = h1;
      return true;
   }
   // Bullish pin at PDL → long
   if(lowerWick / range >= InpMinWickFrac &&
      upperWick / range <= InpMaxOppWick &&
      l1 <= pdl + touchBuf && l1 >= pdl - touchBuf)
   {
      isBuy = true;
      slExtreme = l1;
      return true;
...
void OnTick()
{
   ...
   if(!DetectPinFade(isBuy, slExtreme))
      return;
   ...
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, "PINPD|fade", res))
      return;
