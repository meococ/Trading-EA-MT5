//+------------------------------------------------------------------+
//| EA_M15IBOverlapBreak.mq5 — London IB break in L/NY overlap RR=2.5|
//| Symbol: USDJPY | Period: M15 | Magic: 880993                      |
//| Hypothesis: HYP-M15-IB-OVERLAP-BREAK-001                          |
//| Closed-bar[1] only. Mon–Thu; weekend flat; risk 0.5%.             |
//| Not LondonORB (Asian range); not NYOpenDrive; not LNY DualWin PB. |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15IBOverlapBreak"
...
input group "=== General ==="
input ulong    InpMagic         = 880993;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== Initial Balance ==="
input int      InpIBStartHour   = 7;      // London IB start (server)
input int      InpIBEndHour     = 8;      // exclusive end → first hour
input int      InpOverlapStart  = 13;     // London–NY overlap start
input int      InpOverlapEnd    = 16;     // exclusive
input double   InpMinBodyATR    = 0.35;   // break bar body vs ATR
input double   InpMinIBATR      = 0.40;   // IB width vs ATR (quality)
...
int OnInit()
{
   ...
   {
      Print("[IBOV] FAIL — chart must be M15");
      return INIT_FAILED;
   }
   ...
   Print("[IBOV] HYP-M15-IB-OVERLAP-BREAK-001 RR=", InpTP_Ratio);
...
void UpdateIB(const datetime bar1Time, const MqlDateTime &dt, const int dayKey)
{
   ...
   // Lock IB once first bar of EndHour has closed (bar[1] in EndHour means IB hour finished)
...
void OnTick()
{
   ...
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, "IBOV|break", res))
      return;
