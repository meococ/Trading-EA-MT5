//+------------------------------------------------------------------+
//| EA_EffPersist_USDJPY_M15_V7.mq5                                 |
//| HYP-EPC-USDJPY-M15-001: efficiency persistence continuation     |
//+------------------------------------------------------------------+
#property strict
...
input string InpHypothesisId="HYP-EPC-USDJPY-M15-001";
...
input long InpMagic=5605301;

const string EA_NAME="EA_EffPersist_USDJPY_M15_V7";
const string EXPECTED_HYPOTHESIS="HYP-EPC-USDJPY-M15-001";
...
void ResetEfficiency(const string reason)
  {
   if(g_state==HIGH_EFFICIENCY&&InpEnableTelemetry)PrintFormat("EPC001_STATE from=1 to=0 reason=%s trigger_time=%I64d age=%d",reason,(long)g_trigger_time,g_persist_age);
...
void FillSignal(EntrySignal &s,const MqlRates &bar,const datetime availability,const int direction,const int age,const double er,const double atr,const double hi,const double lo,const bool control)
  {
   ...
   g_signals++;if(direction>0)g_long_signals++;else g_short_signals++;if(InpEnableTelemetry)PrintFormat("EPC001_SIGNAL decision=%I64d availability=%I64d direction=%s age=%d control=%s er=%.6f trigger_er=%.6f reversal_atr=%.6f window_high=%.5f window_low=%.5f",(long)bar.time,(long)availability,(direction>0?"LONG":"SHORT"),age,(control?"true":"false"),er,g_trigger_er,s.reversal_atr,hi,lo);
  }
...
bool BuildSignal(const datetime availability,EntrySignal &signal)
  {
   ...
   if(g_state==NEUTRAL)
     {
      ...
      if(InpEnableTelemetry)PrintFormat("EPC001_HIGH_EFF time=%I64d direction=%s er=%.6f close=%.5f atr=%.6f window_high=%.5f window_low=%.5f",(long)bar.time,(direction>0?"LONG":"SHORT"),er,bar.close,atr,hi,lo);
   ...
   if(er<InpERPersistThreshold||!same_direction||!not_reversed)return(false);g_persistence++;if(InpEnableTelemetry)PrintFormat("EPC001_PERSIST time=%I64d direction=%s age=%d er=%.6f reverse_atr=%.6f",(long)bar.time,(g_eff_direction>0?"LONG":"SHORT"),g_persist_age,er,reverse/atr);
...
bool CloseOwned(const string reason){ulong t=0;if(!OwnedPosition(t))return(true);g_close_attempts++;g_pending_exit_reason=reason;if(!g_trade.PositionClose(t,InpDeviationPoints)){g_close_rejects++;return(false);}uint c=g_trade.ResultRetcode();if(c!=TRADE_RETCODE_DONE&&c!=TRADE_RETCODE_DONE_PARTIAL){g_close_rejects++;return(false);}g_closes++;PrintFormat("EPC001_CLOSE_REQUEST reason=%s ticket=%I64u",reason,t);return(true);}
