...
MODULE_PATH = Path(__file__).parents[1] / "build_lvor_003_source.py"
SPEC = importlib.util.spec_from_file_location("build_lvor_003_source", MODULE_PATH)
    ...
    return canonical(
        {
            ...
            "reason": {
                "message": "forbidden outcome field: FOLLOWING_M5_INCOMPLETE",
                "type": "ContractError",
            },
...
def synthetic_source_report(verdict: str = "SOURCE_FAIL_NO_ECONOMICS_AUTHORITY") -> dict[str, object]:
    return {
        "schema_version": "lvor_003_source_feasibility_report.v1",
        "hypothesis_id": sut.HYPOTHESIS_ID,
        "ea_name": sut.EA_NAME,
        "feature_family": sut.FAMILY,
...
def test_complete_source_report_with_every_diagnostic_reason_is_outcome_blind() -> None:
    source = MODULE_PATH.read_text(encoding="utf-8")
    assert 'reasons["CONFIRM_M5_MISSING"] += 1' in source
    assert 'reasons["FOLLOWING_M5_INCOMPLETE"] += 1' not in source
    ...
    report.update(
        {
            ...
            "source_contract": {
                "design_start": "2016-01-04",
                "design_end": "2020-12-31",
                "elapsed_calendar_weeks": sut.ELAPSED_CALENDAR_WEEKS,
                ...
                "h1_price_side": "BID",
                "manifest_dates": sut.EXPECTED_MANIFEST_DATES,
                "business_decision_dates": sut.EXPECTED_BUSINESS_DECISION_DATES,
                "m1_design_rows": sut.EXPECTED_M1_DESIGN_ROWS,
                "h1_design_rows": sut.EXPECTED_H1_DESIGN_ROWS,
                "m15_m5_quality": {
                    "m15_observed": 1, "m15_complete": 1, "m15_incomplete": 0,
                    "m5_observed": 3, "m5_complete": 3, "m5_incomplete": 0,
                },
            },
            "arm_counts": {"PRIMARY": 0, "PRICE_ONLY": 0, "SHIFTED_ACTIVITY": 0},
            "formation_funnel": {
                "scheduled_after_activity_warmup": 1,
                "formed_m15_plus_confirm_m5": 1,
                "complete_ratio": 1.0,
                "ineligibility_reasons": {
                    "CONFIRM_M5_MISSING": 1,
                    "H1_ATR20_UNAVAILABLE": 1,
                    "PRIOR20_ACTIVITY_UNAVAILABLE": 1,
                    "SHIFTED_ACTIVITY_UNAVAILABLE": 1,
                },
            },
            "horizon_funnel": {
                "primary": 0, "source_executable": 0, "delayed_over_60m": 0,
                "unavailable": 0, "right_censored": 0,
            },
    ...
    report["formation_funnel"]["ineligibility_reasons"] = {
        "FOLLOWING_M5_INCOMPLETE": 1
    }
    with pytest.raises(sut.ContractError, match="forbidden outcome field"):
        sut.assert_outcome_blind(report)
...
def prepare_execute_probe_mocks(
    ...
    *,
    ...
    tests = Path(__file__).read_bytes()
    receipt = review_receipt_payload(builder, tests)
    plan = (MODULE_PATH.parent / "HYP-LVOR-EURUSD-M15-003_SOURCE_FEASIBILITY_PLAN.md").read_bytes()
    ...
    monkeypatch.setattr(sut, "EVIDENCE_ROOT_REL", "evidence/LVOR003-SOURCE-ATTEMPT-001")
...
def test_receipt_precedes_design_and_attempt_is_durable_one_use(
    ...
    assert {path.name for path in root.iterdir()} == {
        ...
        "lvor_003_source_report.json",
        "lvor_003_source_ledger.jsonl",
...
def test_plan_hash_binding_matches_exact_bytes() -> None:
    plan = MODULE_PATH.parent / "HYP-LVOR-EURUSD-M15-003_SOURCE_FEASIBILITY_PLAN.md"
...
def test_sentinel_is_exactly_disarmed_and_outcomes_are_rejected() -> None:
    ...
    for key in (
        "return", "pnl", "profit_factor", "post_entry_open", "trade_count",
        "win", "loss", "entry", "exit", "entry_price", "exit_price",
    ):
        with pytest.raises(sut.ContractError, match="outcome|forbidden"):
            sut.assert_outcome_blind({key: 1})


def test_only_frozen_entry_exit_timestamp_keys_are_legal_in_source_report() -> None:
    report = {
        "signal_ledgers": {
            "PRIMARY": [
                {
                    "signal_id": "LVOR003-PRIMARY-0000000000000000",
                    "horizon": {
                        "entry_observed_m1_utc": utc(2020, 1, 6, 6, 21),
                        "entry_delay_minutes": 1.0,
                        "exit_availability_utc": utc(2020, 1, 6, 6, 50),
...
@pytest.mark.parametrize(
    "key",
    (
        "entry_time", "exit_time", "entry_order_id", "exit_reason",
        "entry_price", "exit_price", "entry_fill", "exit_fill",
    ),
)
@pytest.mark.parametrize("placement", ("direct", "nested"))
def test_every_other_entry_exit_key_is_rejected_at_any_depth(
    key: str,
    placement: str,
) -> None:
    payload: object
    if placement == "direct":
        ...
        payload = {
            "signal_ledgers": {
                "PRIMARY": [{"metadata": {key: "sealed"}}],
            }
        }

    with pytest.raises(sut.ContractError, match="outcome|forbidden"):
        sut.assert_outcome_blind(payload)
...
def test_plan_source_paths_and_forbidden_permissions_are_frozen() -> None:
    assert sut.PLAN_REL.endswith("HYP-LVOR-EURUSD-M15-003_SOURCE_FEASIBILITY_PLAN.md")
    assert sut.BUILDER_REL.endswith("build_lvor_003_source.py")
    assert sut.TEST_REL.endswith("test_build_lvor_003_source.py")
