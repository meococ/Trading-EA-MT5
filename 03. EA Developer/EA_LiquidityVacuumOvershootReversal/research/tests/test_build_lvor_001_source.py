...
MODULE_PATH = Path(__file__).parents[1] / "build_lvor_001_source.py"
SPEC = importlib.util.spec_from_file_location("build_lvor_001_source", MODULE_PATH)
...
def prepare_execute_probe_mocks(
    ...
    tests = Path(__file__).read_bytes()
    receipt = review_receipt_payload(builder, tests)
    plan = (MODULE_PATH.parent / "HYP-LVOR-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN.md").read_bytes()
    row = {"validation": {"independent_review_receipt_sha256": sha(receipt)}}
    payloads = {
    ...
    monkeypatch.setattr(sut, "EVIDENCE_ROOT_REL", "evidence/LVOR001-SOURCE-ATTEMPT-001")
...
def test_plan_hash_binding_matches_exact_bytes() -> None:
    plan = MODULE_PATH.parent / "HYP-LVOR-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN.md"
...
def test_plan_source_paths_and_forbidden_permissions_are_frozen() -> None:
    assert sut.PLAN_REL.endswith("HYP-LVOR-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN.md")
    assert sut.BUILDER_REL.endswith("build_lvor_001_source.py")
    assert sut.TEST_REL.endswith("test_build_lvor_001_source.py")
