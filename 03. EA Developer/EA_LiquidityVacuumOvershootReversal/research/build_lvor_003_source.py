#!/usr/bin/env python3
"""Inert outcome-blind source probe for HYP-LVOR-EURUSD-M15-003.
...
HYPOTHESIS_ID = "HYP-LVOR-EURUSD-M15-003"
PARENT_HYPOTHESIS_ID = "HYP-LVOR-EURUSD-M15-002"
EA_NAME = "EA_LiquidityVacuumOvershootReversal"
FAMILY = "liquidity-vacuum-overshoot-reversal"
ATTEMPT_ID = "LVOR003-SOURCE-ATTEMPT-001"
PLAN_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/"
    "HYP-LVOR-EURUSD-M15-003_SOURCE_FEASIBILITY_PLAN.md"
)
PLAN_SHA256 = "CDDA9DF55FAEC3033DBCD93FB5CEBCD12873CB7878F492ADBCE81E057B2119D5"
BUILDER_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/"
    "build_lvor_003_source.py"
)
TEST_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/tests/"
    "test_build_lvor_003_source.py"
)
EVIDENCE_ROOT_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/evidence/"
    "HYP-LVOR-EURUSD-M15-003_SOURCE_FEASIBILITY_ATTEMPTS/"
    f"{ATTEMPT_ID}"
)
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/"
    "HYP-LVOR-EURUSD-M15-003_SOURCE_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
REVIEW_RECEIPT_SCHEMA = "lvor_003_source_implementation_review_receipt.v1"
PROBE_STATUS = "FROZEN_SOURCE_FEASIBILITY_AUTHORIZED_PRE_RUN"
PARENT_TERMINAL_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/evidence/"
    "HYP-LVOR-EURUSD-M15-002_SOURCE_FEASIBILITY_ATTEMPTS/"
    "LVOR002-SOURCE-ATTEMPT-001/attempt_terminal.json"
)
PARENT_TERMINAL_SHA256 = "283EAF1BD92F79247CA5111DD447C44FCB6656C5FD1032E8D19D25A01498ACDA"
PARENT_ATTEMPT_STARTED_SHA256 = "5ABE90BFBF5777004F9058F74509E3BD212919E9DFE9C5F70A801728BC341F2B"
PARENT_REVIEWED_REGISTRY_ROW_SHA256 = "56A6BC459C04BAE598632113AC7AD9F10BD7A0A97CF7DB71B53FE4CB241B7D63"
...
def build_arm_ledgers(
    ...
    for arm in arms:
        ...
        for signal in sorted(signals_by_arm[arm], key=lambda row: _as_utc(row["time_utc"])):
            ...
            signal_id = f"LVOR003-{arm}-{sha256_bytes(identity)[:16]}"
...
def assert_outcome_blind(value: object) -> None:
    ...
    allowed_entry_exit_source_keys = {
        "entry_observed_m1_utc", "entry_delay_minutes", "exit_availability_utc",
    }
    ...
    def visit(item: object) -> None:
        if isinstance(item, Mapping):
            for key, child in item.items():
                ...
                has_entry_or_exit = "entry" in lowered_key or "exit" in lowered_key
                if (
                    has_entry_or_exit
                    and lowered_key not in allowed_entry_exit_source_keys
                ) or any(token in lowered_key for token in forbidden):
...
def validate_parent_terminal(payload: bytes) -> dict[str, object]:
    """Require the exact engineering-invalid HYP002 terminal and nothing else."""

    if sha256_bytes(payload) != PARENT_TERMINAL_SHA256:
        raise ContractError("parent terminal SHA binding mismatch")
    terminal = parse_canonical_object(payload, label="HYP002 parent terminal")
    expected = {
        "artifact_hashes": {
...
def _reserve_attempt(workspace: Path, reviewed_row_sha256: str) -> Path:
    ...
    try:
        _write_new_canonical(
            ...
            {
                "schema_version": "lvor_003_attempt_started.v1",
                "hypothesis_id": HYPOTHESIS_ID,
...
def _flatten_ledgers(
    ...
    for arm in arms:
        ...
        for row in rows:
            ...
            flattened.append(
                {
                    "schema_version": "lvor_003_source_ledger_row.v1",
                    "hypothesis_id": HYPOTHESIS_ID,
                    "attempt_id": ATTEMPT_ID,
                    "reviewed_registry_row_sha256": reviewed_row_sha256,
...
def _persist_success(
    ...
    _write_new_canonical(root / "lvor_003_source_report.json", enriched)
    flattened = _flatten_ledgers(
        enriched,
        reviewed_row_sha256=reviewed_row_sha256,
        attempt_started_sha256=started_sha,
    )
    _write_new_jsonl(root / "lvor_003_source_ledger.jsonl", flattened)
    ...
    receipt = {
        "schema_version": "lvor_003_source_feasibility_receipt.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "reviewed_registry_row_sha256": reviewed_row_sha256,
    ...
    terminal = {
        "schema_version": "lvor_003_attempt_terminal.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "reviewed_registry_row_sha256": reviewed_row_sha256,
