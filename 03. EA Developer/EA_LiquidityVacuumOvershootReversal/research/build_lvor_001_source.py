#!/usr/bin/env python3
"""Inert outcome-blind source probe for HYP-LVOR-EURUSD-M15-001.
...
HYPOTHESIS_ID = "HYP-LVOR-EURUSD-M15-001"
...
PLAN_REL = (
    ...
    "HYP-LVOR-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN.md"
)
PLAN_SHA256 = "EB621243CF5368AE702ABCA3660E641F39AD885177E987DA067DC910DEB7158A"
BUILDER_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/"
    "build_lvor_001_source.py"
)
TEST_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/tests/"
    "test_build_lvor_001_source.py"
)
EVIDENCE_ROOT_REL = (
...
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_LiquidityVacuumOvershootReversal/research/"
    "HYP-LVOR-EURUSD-M15-001_SOURCE_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
REVIEW_RECEIPT_SCHEMA = "lvor_001_source_implementation_review_receipt.v1"
...
def build_arm_ledgers(
    ...
    for arm in arms:
        ...
        for signal in sorted(signals_by_arm[arm], key=lambda row: _as_utc(row["time_utc"])):
            ...
            signal_id = f"LVOR001-{arm}-{sha256_bytes(identity)[:16]}"
...
def validate_review_receipt(
    ...
    *,
    ...
    """Bind real canonical review bytes to source, tests and immutable V1 plan."""
...
def _reserve_attempt(workspace: Path, reviewed_row_sha256: str) -> Path:
    ...
    try:
        _write_new_canonical(
            ...
            {
                "schema_version": "lvor_001_attempt_started.v1",
                "hypothesis_id": HYPOTHESIS_ID,
...
def _existing_artifact_hashes(root: Path) -> dict[str, str]:
    names = (
        ...
        "lvor_001_source_report.json",
        "lvor_001_source_ledger.jsonl",
...
def _flatten_ledgers(
    ...
    for arm in arms:
        ...
        for row in rows:
            ...
            flattened.append(
                {
                    "schema_version": "lvor_001_source_ledger_row.v1",
                    "hypothesis_id": HYPOTHESIS_ID,
                    "attempt_id": ATTEMPT_ID,
                    "reviewed_registry_row_sha256": reviewed_row_sha256,
...
def _persist_success(
    ...
    _write_new_canonical(root / "lvor_001_source_report.json", enriched)
    flattened = _flatten_ledgers(
        enriched,
        reviewed_row_sha256=reviewed_row_sha256,
        attempt_started_sha256=started_sha,
    )
    _write_new_jsonl(root / "lvor_001_source_ledger.jsonl", flattened)
    ...
    receipt = {
        "schema_version": "lvor_001_source_feasibility_receipt.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "reviewed_registry_row_sha256": reviewed_row_sha256,
...
def _scan_source(
    ...
    report = {
        "schema_version": "lvor_001_source_feasibility_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "ea_name": EA_NAME,
        ...
        "formation_funnel": {
            ...
            "complete_m15_plus_following_m5": formation_complete,
