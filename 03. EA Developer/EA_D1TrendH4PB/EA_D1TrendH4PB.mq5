//+------------------------------------------------------------------+
//| EA_D1TrendH4PB.mq5 — D1 EMA50 trend + H4 EMA20 PB reclaim (RR=3) |
//| Symbol: USDJPY | Period: H4 | Magic: 880991                       |
//| Hypothesis: HYP-D1-TREND-H4-PB-001                                  |
//| Closed-bar[1] only. Mon–Thu; weekend flat; risk 0.5%; RR=3.0.     |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_D1TrendH4PB"
...
int OnInit()
{
   if(Period() != PERIOD_H4)
   {
      Print("[D1H4] FAIL — chart must be H4");
      return INIT_FAILED;
   }
   g_trade.SetExpertMagicNumber((long)InpMagic);
   g_trade.SetDeviationInPoints(InpDeviation);
   g_hATRH4 = iATR(_Symbol, PERIOD_H4, InpATRPeriod);
   g_hEMAD1 = iMA(_Symbol, PERIOD_D1, InpD1EMA, 0, MODE_EMA, PRICE_CLOSE);
   g_hEMAH4 = iMA(_Symbol, PERIOD_H4, InpH4EMA, 0, MODE_EMA, PRICE_CLOSE);
   if(g_hATRH4 == INVALID_HANDLE || g_hEMAD1 == INVALID_HANDLE || g_hEMAH4 == INVALID_HANDLE)
      return INIT_FAILED;
   g_dayStartBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   Print("[D1H4] HYP-D1-TREND-H4-PB-001 RR=", InpTP_Ratio);
   return INIT_SUCCEEDED;
}

void OnDeinit(const int reason)
{
   if(g_hATRH4 != INVALID_HANDLE) IndicatorRelease(g_hATRH4);
...
bool FindPullbackReclaim(const int bias, double &pbExtreme)
{
   ...
   if(CopyBuffer(g_hEMAH4, 0, 1, need, ema) < need)
      return false;
...
void OnTick()
{
   ...
   if(CopyBuffer(g_hATRH4, 0, 1, 1, atr) < 1 || atr[0] <= 0.0)
      return;
   ...
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, "D1H4|pb", res))
      return;
