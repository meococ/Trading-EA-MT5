...
#property strict
#property description "VRAS EURUSD M5 path-confirmed TREND hypothesis; diagnostic research only"
#property description "Closed-bar one-step continuation, Session VWAP/SD, ADX hysteresis and confirmed AVWAP"
...
input string                InpHypothesisId="HYP-VRAS-EURUSD-M5-004";
input string                InpVariantTag="CONTROL_IMMEDIATE_TREND";
input bool                  InpUsePathConfirmation=false;
input long                  InpMagic=5600744;
...
const string EA_NAME="EA_VRAS_PathConfirmedTrend";
...
const string PREREG_SHA256="8548E9FA1ECC75D49A152589833F990D577BE58270D06D863AAB951CB2D5955F";
const string PROBE_PLAN_SHA256="F69A021862DB6BFFC7F561C73C9EBDA736584EAF59E996E634F92D6E084D73D6";
...
DecisionState g_pending_state;
long g_path_candidates_armed=0;
long g_path_confirmations_passed=0;
long g_path_confirmations_rejected=0;
long g_path_confirmations_expired=0;
...
bool EntryGuardsAllow(const datetime decision_server,string &reject_status)
  {
   reject_status="ENTRY_GUARD_REJECT";
   ResetRiskDayIfNeeded(decision_server);
   if(!InpResearchAutoMode || !MQLInfoInteger(MQL_TESTER))
     {
      reject_status="RESEARCH_MODE_REJECT";
      return false;
     }
   ...
   if(!SessionAllows(decision_utc))
     {
      ...
      reject_status="SESSION_REJECT";
      return false;
     }
   if(g_trades_today>=InpMaxTradesPerDay)
     {
      reject_status="TRADE_LIMIT_REJECT";
      return false;
     }
   if(g_consecutive_losses>=3)
     {
      reject_status="LOSS_STREAK_REJECT";
      return false;
     }
   if(DailyLossHit())
     {
      reject_status="DAILY_LOSS_REJECT";
      return false;
     }
   if(AccountDrawdownHit())
     {
      reject_status="ACCOUNT_DD_REJECT";
      return false;
     }
   if(AnySymbolExposure())
     {
      reject_status="EXPOSURE_REJECT";
      return false;
     }
   if(NewsBlocked(decision_server))
     {
      g_news_rejections++;
      reject_status="NEWS_REJECT";
      return false;
     }
...
void ClearPendingTrend()
  {
   ZeroMemory(g_pending_state);
   g_pending_active=false;
  }

void ArmPendingTrend(const DecisionState &state)
  {
   if(state.signal!=SIGNAL_TREND_LONG && state.signal!=SIGNAL_TREND_SHORT)
      return;
   g_pending_state=state;
   g_pending_active=true;
   g_path_candidates_armed++;
   WriteDecisionTelemetry(state,0.0,state.planned_stop,0.0,0.0,
                          "PATH_CANDIDATE_ARMED");
  }

bool RefreshConfirmationState(MqlRates &bars[],const datetime current_bar,
                              const DecisionState &pending,
                              DecisionState &state,string &reject_reason)
  {
   ZeroMemory(state);
   reject_reason="PATH_DATA_INVALID";
   state.signal=pending.signal;
   state.direction=pending.direction;
   state.decision_server=current_bar;
   state.decision_utc=ServerToUtc(current_bar);
   state.anchor_utc=SessionAnchorUtc(state.decision_utc);
   state.open1=bars[0].open;
   state.high1=bars[0].high;
   state.low1=bars[0].low;
   ...
   state.open2=bars[1].open;
   state.close2=bars[1].close;
   state.planned_stop=pending.planned_stop;
   state.avwap_anchor_time=pending.avwap_anchor_time;
   state.avwap_confirmed_time=pending.avwap_confirmed_time;
   state.event_code=(pending.direction>0 ? "TREND_LONG_CONFIRM" : "TREND_SHORT_CONFIRM");

   if(g_regime!=REGIME_TREND)
     {
      reject_reason="PATH_REGIME_REJECT";
      return false;
     }
   if(!ReadIndicator(g_adx_handle,0,state.adx) ||
      !ReadIndicator(g_atr_handle,0,state.atr) ||
      !ReadIndicator(g_rsi_handle,0,state.rsi) || state.atr<=0.0)
      return false;

   WeightedStats tick_stats,equal_stats;
   double tick_close=0.0,equal_close=0.0;
   if(!ComputeSessionStats(PERIOD_M5,current_bar,state.anchor_utc,
                           VRAS_WEIGHT_TICK,tick_stats,tick_close) ||
      !ComputeSessionStats(PERIOD_M5,current_bar,state.anchor_utc,
                           VRAS_WEIGHT_EQUAL,equal_stats,equal_close))
      return false;
   WeightedStats primary=(InpVolumeMode==VRAS_WEIGHT_TICK ? tick_stats : equal_stats);
   WeightedStats shadow=(InpVolumeMode==VRAS_WEIGHT_TICK ? equal_stats : tick_stats);
   ...
   state.shadow_vwap=shadow.mean;
   state.shadow_sd=shadow.sd;
   if(primary.samples<InpWarmupBars || state.session_sd<InpSdFloorAtr*state.atr)
     {
      reject_reason="PATH_SESSION_STATE_REJECT";
      return false;
     }

   if(pending.avwap_anchor_time<=0 ||
      !ComputeAnchoredVwap(pending.avwap_anchor_time,current_bar,InpVolumeMode,
                           state.anchored_vwap))
     {
      reject_reason="PATH_AVWAP_DATA_REJECT";
      return false;
     }
   ...
   double m15_close=0.0;
   if(InpUseM15Bias &&
      !ComputeSessionStats(PERIOD_M15,current_bar,state.anchor_utc,
                           InpVolumeMode,m15_stats,m15_close))
     {
      reject_reason="PATH_M15_DATA_REJECT";
      return false;
     }
   state.m15_close=m15_close;
   state.m15_vwap=InpUseM15Bias ? m15_stats.mean : 0.0;
   ...
   if(pending.direction>0)
     {
      if(bars[0].close<=pending.high1)
        {
         reject_reason="PATH_EXTREME_BREAK_REJECT";
         return false;
        }
      if(bars[0].close<=state.session_vwap ||
         bars[0].close<=state.anchored_vwap)
        {
         reject_reason="PATH_MEAN_STACK_REJECT";
         return false;
        }
      if(InpUseM15Bias && state.m15_close<=state.m15_vwap)
        {
         reject_reason="PATH_M15_REJECT";
         return false;
        }
     }
   else
     {
      if(bars[0].close>=pending.low1)
        {
         reject_reason="PATH_EXTREME_BREAK_REJECT";
         return false;
        }
   ...
   reject_reason="PATH_CONFIRM_ACCEPTED";
   return true;
  }

bool ResolvePendingTrend(MqlRates &bars[],const datetime current_bar,
                         DecisionState &confirmed)
  {
   if(!g_pending_active)
      return false;
   DecisionState pending=g_pending_state;
   ClearPendingTrend();
   if(current_bar!=pending.decision_server+PeriodSeconds(PERIOD_M5))
     {
      // Use the copied state after fail-closed clearing; a later bar may not revive it.
      confirmed=pending;
      confirmed.decision_server=current_bar;
      confirmed.decision_utc=ServerToUtc(current_bar);
      confirmed.event_code="PATH_CONFIRM_EXPIRED";
      g_path_confirmations_expired++;
      WriteDecisionTelemetry(confirmed,0.0,pending.planned_stop,0.0,0.0,
                             "PATH_CONFIRM_REJECT");
      return false;
     }
   string reject_reason="";
   if(!RefreshConfirmationState(bars,current_bar,pending,confirmed,reject_reason))
     {
      confirmed.event_code=reject_reason;
      g_path_confirmations_rejected++;
      WriteDecisionTelemetry(confirmed,0.0,pending.planned_stop,0.0,0.0,
                             "PATH_CONFIRM_REJECT");
      return false;
     }
   g_path_confirmations_passed++;
   WriteDecisionTelemetry(confirmed,0.0,confirmed.planned_stop,0.0,0.0,
...
bool TryOpenTrade(const DecisionState &state)
  {
   g_entries_attempted++;
   string guard_status="";
   if(!EntryGuardsAllow(state.decision_server,guard_status))
     {
...
bool WriteRunMeta()
  {
   ...
   string payload=StringFormat(
      ...
      "\"path_confirmation_enabled\":%s,"
      "\"report_sha256\":\"%s\",\"gap_report_sha256\":\"%s\","
      "\"prereg_sha256\":\"%s\",\"probe_plan_sha256\":\"%s\","
      ...
      "\"trend_short_entries\":%I64d,\"path_candidates_armed\":%I64d,"
      "\"path_confirmations_passed\":%I64d,"
      "\"path_confirmations_rejected\":%I64d,"
      "\"path_confirmations_expired\":%I64d}}",
      g_run_id,EA_NAME,_Symbol,TELEMETRY_PROFILE,InpHypothesisId,InpVariantTag,
      (int)InpVolumeMode,(int)InpAnchorMode,InpMagic,
      InpUsePathConfirmation ? "true" : "false",
      REPORT_SHA256,GAP_REPORT_SHA256,PREREG_SHA256,PROBE_PLAN_SHA256,
      ...
      g_trend_short_entries,g_path_candidates_armed,
      g_path_confirmations_passed,g_path_confirmations_rejected,
      g_path_confirmations_expired);
...
bool ValidateInputs()
  {
   if(_Symbol!="EURUSD" || _Period!=PERIOD_M5)
      return false;
   if(InpHypothesisId!="HYP-VRAS-EURUSD-M5-004" || InpMagic!=5600744)
      return false;
   if(InpUsePathConfirmation)
     {
      if(InpVariantTag!="CHALLENGER_PATH_CONFIRM")
         return false;
     }
   else
     {
      if(InpVariantTag!="CONTROL_IMMEDIATE_TREND")
         return false;
     }
...
int OnInit()
  {
   ...
   if(!OpenTelemetry())
      return INIT_FAILED;
   PrintFormat("VRAS init hyp=%s variant=%s path_confirm=%s volume=%d anchor=%d auto=%s closed_bar=true promotion=false",
               InpHypothesisId,InpVariantTag,
               InpUsePathConfirmation ? "true" : "false",
               (int)InpVolumeMode,(int)InpAnchorMode,
...
void OnTick()
  {
   ...
   bool confirmed_opened=false;
   if(InpUsePathConfirmation && g_pending_active)
     {
      DecisionState confirmed;
      if(ResolvePendingTrend(bars,current_bar,confirmed))
         confirmed_opened=TryOpenTrade(confirmed);
     }
   if(has_signal)
     {
      if(state.signal==SIGNAL_RANGE_LONG || state.signal==SIGNAL_RANGE_SHORT)
        {
         if(!confirmed_opened)
            TryOpenTrade(state);
        }
      else if(state.signal==SIGNAL_TREND_LONG || state.signal==SIGNAL_TREND_SHORT)
        {
         if(!InpUsePathConfirmation)
            TryOpenTrade(state);
         else if(!confirmed_opened && !AnySymbolExposure())
            ArmPendingTrend(state);
         else
            WriteDecisionTelemetry(state,0.0,state.planned_stop,0.0,0.0,
                                   "PATH_ARM_EXPOSURE_REJECT");
        }
     }
