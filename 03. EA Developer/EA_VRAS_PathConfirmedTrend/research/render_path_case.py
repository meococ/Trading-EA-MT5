#!/usr/bin/env python3
"""Render one HYP-004 accepted or rejected path case with active indicators.
...
DEFAULT_RUN = ROOT / "02. AlphaFactory" / "runs" / "EA_VRAS_PathConfirmedTrend" / "20260722_155635"
DEFAULT_BARS = ROOT / "02. AlphaFactory" / "data" / "fivepercent" / "EURUSD" / "EURUSD_M1_2015_now.parquet"
DEFAULT_CASES = RESEARCH / "HYP-VRAS-EURUSD-M5-004_CHART_CASES.csv"
DEFAULT_OUTPUT = RESEARCH / "evidence" / "HYP-VRAS-EURUSD-M5-004_CHARTS"
...
def main() -> None:
    ...
    parser.add_argument("--case-id", default="VRAS-004-ACCEPT-P194")
    ...
    args.bars = args.bars.resolve()
    telemetry_path = next((args.run_dir / "logs").glob("*DecisionTelemetry*.csv"))
    ...
    title_kind = "ACCEPTED TRADE" if is_trade else "REJECTED PATH"
    ...
    manifest = {
        ...
        "source_m1": str(args.bars.relative_to(ROOT)).replace("\\", "/"),
        "source_m1_sha256": sha256(args.bars),
        "telemetry": str(telemetry_path.relative_to(ROOT)).replace("\\", "/"),
