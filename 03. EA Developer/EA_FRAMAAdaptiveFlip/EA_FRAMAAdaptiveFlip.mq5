//+------------------------------------------------------------------+
//| EA_FRAMAAdaptiveFlip.mq5                                       |
//| HYP-FRAMA-XAUUSD-M15-001: native FRAMA16 price crossover       |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Untuned XAUUSD M15 native FRAMA16 adaptive price crossover"

input group "--- Frozen research authority ---"
input bool   InpResearchAutoMode=false;
input bool   InpEnableTelemetry=false;
input string InpHypothesisId="HYP-FRAMA-XAUUSD-M15-002";
input string InpVariantTag="FRAMA16_PRICE_CROSS_ADAPTIVE_FLIP_DEFERRED_WARMUP";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5604102;
...
const string EA_NAME="EA_FRAMAAdaptiveFlip";
const string EXPECTED_HYPOTHESIS="HYP-FRAMA-XAUUSD-M15-002";
const string EXPECTED_VARIANT="FRAMA16_PRICE_CROSS_ADAPTIVE_FLIP_DEFERRED_WARMUP";
const string EXPECTED_SYMBOL="XAUUSD";
const int    ATR_PERIOD=14;
const int    FRAMA_PERIOD=16;
...
int g_frama_handle=INVALID_HANDLE;
bool g_indicators_ready=false;

// Legacy KVO state remains compile-isolated from the FRAMA execution path.
...
bool PreloadFramaState()
  {
   if(g_frama_handle==INVALID_HANDLE || g_atr_handle==INVALID_HANDLE)
      return(false);
   const int frama_bars=BarsCalculated(g_frama_handle);
   const int atr_bars=BarsCalculated(g_atr_handle);
   if(frama_bars<FRAMA_PERIOD+2 || atr_bars<ATR_PERIOD+2)
      return(false);

   double frama_values[];
   double atr_values[];
   ArraySetAsSeries(frama_values,false);
   ArraySetAsSeries(atr_values,false);
   const int sample=16;
   if(CopyBuffer(g_frama_handle,0,1,sample,frama_values)!=sample ||
      CopyBuffer(g_atr_handle,0,1,sample,atr_values)!=sample)
      return(false);
   for(int i=0;i<sample;i++)
      if(!IsUsable(frama_values[i]) || !IsUsable(atr_values[i]) || atr_values[i]<=0.0)
         return(false);

   PrintFormat("FRAMA001_PRELOAD frama_bars=%d atr_bars=%d sample=%d latest_frama=%.8f latest_atr=%.8f",
               frama_bars,atr_bars,sample,frama_values[sample-1],atr_values[sample-1]);
   return(true);
  }

bool ReadClosedFramaAndAtr(double &current_frama,double &prior_frama,double &current_atr)
  {
   current_frama=EMPTY_VALUE;
   prior_frama=EMPTY_VALUE;
   ...
   ArraySetAsSeries(frama_values,false);
   ArraySetAsSeries(atr_values,false);
   if(CopyBuffer(g_frama_handle,0,1,2,frama_values)!=2 ||
      CopyBuffer(g_atr_handle,0,1,1,atr_values)!=1)
      return(false);
   prior_frama=frama_values[0];
   current_frama=frama_values[1];
   current_atr=atr_values[0];
   return(IsUsable(prior_frama) && IsUsable(current_frama) &&
          IsUsable(current_atr) && current_atr>0.0);
  }

bool ProcessFramaClosedBar(const datetime availability_time,SignalDecision &signal)
  {
   ZeroMemory(signal);
   MqlRates rates[];
   if(!LoadClosedRates(rates))
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=CLOSED_RATE_LOAD availability=%I64d",
                  (long)availability_time);
      return(false);
     }
   ...
   if(g_last_decision_time>0 && decision_time<g_last_decision_time)
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=DECISION_CLOCK_REGRESSION decision=%I64d last=%I64d",
   ...
   double current_frama=0.0;
   double prior_frama=0.0;
   double current_atr=0.0;
   if(!ReadClosedFramaAndAtr(current_frama,prior_frama,current_atr))
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=INDICATOR_LOAD decision=%I64d",
                  (long)decision_time);
      return(false);
     }

   int direction=0;
   if(rates[prior].close<=prior_frama && rates[release].close>current_frama)
      direction=1;
   else if(rates[prior].close>=prior_frama && rates[release].close<current_frama)
      direction=-1;
   if(direction==0)
      return(false);
   ...
   signal.frama=current_frama;
   signal.prior_frama=prior_frama;
   signal.structural_stop=stop;
   return(true);
  }
...
double FloorToTick(const double price)
  {
   ...
   if(GetLastError()!=0 || !IsUsable(tick_size) || tick_size<=0.0)
     {
      g_runtime_failed=true;
      Print("FRAMA001_FATAL reason=TICK_SIZE_PROPERTY");
      return(EMPTY_VALUE);
     }
...
bool CalculateVolume(const ENUM_ORDER_TYPE order_type,const double entry,
                      const double stop,double &volume)
  {
   ...
   if(!OrderCalcProfit(order_type,_Symbol,1.0,entry,stop,one_lot_profit) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=ORDER_CALC_PROFIT error=%d value=%.8f",
   ...
   if(account_error!=0 || !IsUsable(equity) || equity<=0.0 ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=MARGIN_ACCOUNT_STATE error=%d mode=%d equity=%.2f margin=%.2f call=%.2f stopout=%.2f",
   ...
   if(!IsUsable(raw_volume) || !IsUsable(minimum) || !IsUsable(maximum) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=VOLUME_PROPERTIES error=%d min=%.8f max=%.8f step=%.8f",
   ...
   if(!OrderCalcMargin(order_type,_Symbol,1.0,entry,margin_one_lot) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=MARGIN_ONE_LOT error=%d value=%.8f",
   ...
   for(int guard=0;guard<1024 && sized>=minimum-1e-9;guard++)
     {
      ...
      if(!OrderCalcMargin(order_type,_Symbol,sized,entry,exact_margin) ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("FRAMA001_FATAL reason=MARGIN_EXACT error=%d volume=%.8f value=%.8f",
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   if(!SymbolInfoTick(_Symbol,tick) || GetLastError()!=0 ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=SYMBOL_TICK error=%d bid=%.8f ask=%.8f",
   ...
   if(GetLastError()!=0 || stops_level<0)
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=STOPS_LEVEL_PROPERTY error=%d value=%I64d",
   ...
   if(!check_ok || check_error!=0 || check.retcode!=0)
     {
      g_runtime_failed=true;
      PrintFormat("FRAMA001_FATAL reason=ORDER_CHECK error=%d retcode=%u comment=%s",
...
bool ValidateFrozenInputs()
  {
   return(_Symbol==EXPECTED_SYMBOL && _Period==PERIOD_M15 &&
          ...
          InpHypothesisId==EXPECTED_HYPOTHESIS && InpVariantTag==EXPECTED_VARIANT &&
           InpMagic==5604102 && MathAbs(InpRiskPercent-0.25)<1e-12 &&
...
int OnInit()
  {
   if(!ValidateFrozenInputs())
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=FROZEN_INPUT_OR_SYMBOL_CONTRACT");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(!CurrentM15Open(g_last_bar_open))
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=INITIAL_M15_CLOCK");
      return(INIT_FAILED);
     }
   if(!EmitD0SeriesProof())
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=D0_SERIES_PROOF");
      return(INIT_FAILED);
     }
   long filling_mode=0;
   if(!SymbolInfoInteger(_Symbol,SYMBOL_FILLING_MODE,filling_mode) ||
     ...
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=FOK_NOT_SUPPORTED");
      return(INIT_FAILED);
     }
    g_atr_handle=iATR(_Symbol,PERIOD_M15,ATR_PERIOD);
    g_frama_handle=iFrAMA(_Symbol,PERIOD_M15,FRAMA_PERIOD,0,PRICE_CLOSE);
    if(g_atr_handle==INVALID_HANDLE || g_frama_handle==INVALID_HANDLE)
      {
       g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=INDICATOR_HANDLE");
       return(INIT_FAILED);
      }
    g_indicators_ready=PreloadFramaState();
   const double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   if(!IsUsable(equity) || equity<=0.0)
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=INITIAL_EQUITY");
      return(INIT_FAILED);
     }
   g_daily_day_key=DateKey(TimeCurrent());
   g_daily_start_equity=equity;
   g_peak_equity=equity;
    PrintFormat("FRAMA001_INIT hypothesis=%s variant=%s symbol=%s period=%d frama_period=%d atr_period=%d",
                InpHypothesisId,InpVariantTag,_Symbol,(int)_Period,FRAMA_PERIOD,ATR_PERIOD);
...
void OnDeinit(const int reason)
  {
   ...
   if(g_frama_handle!=INVALID_HANDLE)
      IndicatorRelease(g_frama_handle);
   PrintFormat("FRAMA001_SUMMARY hypothesis=%s closed_bars=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d runtime_failed=%s reason=%d",
...
void OnTick()
  {
   ...
   if(!g_indicators_ready)
     {
      ...
      if(!CurrentM15Open(warmup_open))
        {
         g_runtime_failed=true;
         Print("FRAMA001_FATAL reason=WARMUP_M15_CLOCK");
         return;
        }
   ...
   if(!CurrentM15Open(current_open))
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=M15_SCHEDULING_CLOCK");
      return;
     }
   ...
   if(current_open<g_last_bar_open)
     {
      g_runtime_failed=true;
       Print("FRAMA001_FATAL reason=BAR_CLOCK_REGRESSION");
      return;
     }
   ...
   if(ProcessFramaClosedBar(current_open,signal) && signal.fired)
