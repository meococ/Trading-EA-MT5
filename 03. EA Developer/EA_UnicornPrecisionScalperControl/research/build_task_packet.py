#!/usr/bin/env python3
"""Refresh the dirty-tree-bound HYP-UPSC-XAU-M5-002 control task packet."""
...
HYPOTHESIS_ID = "HYP-UPSC-XAU-M5-002"
EA_NAME = "EA_UnicornPrecisionScalperControl"
OVERRIDES = (
...
def latest_registry_row(registry: Path) -> tuple[str, dict[str, Any]]:
    ...
    if row.get("state") != "screened" or row.get("model") != 0:
        raise ValueError("latest HYP-UPSC-002 registry row is not screened Model 0")
    return raw, row
...
def build(root: Path, output: Path) -> dict[str, Any]:
    ...
    prereg = (
        ...
        / EA_NAME
        / "research"
        / "HYP-UPSC-XAU-M5-002_FROZEN_PREREG.md"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=(
            "03. EA Developer/EA_UnicornPrecisionScalperControl/research/preflight/"
            "HYP-UPSC-XAU-M5-002/task_packet.control.json"
        ),
    )
