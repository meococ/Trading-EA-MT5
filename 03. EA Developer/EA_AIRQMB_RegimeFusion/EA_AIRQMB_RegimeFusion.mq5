...
bool SubmitEntry(const TradeDecision &d)
  {
   ...
   // MqlTradeCheckResult.retcode is 0 ("Done") after a successful preflight;
   // TRADE_RETCODE_DONE/PLACED belong to MqlTradeResult after OrderSend.
   if(!check_ok)
     {
      g_reject_order_check++;
      g_last_reason=StringFormat("ORDER_CHECK_FALSE_%d_%u",check_error,check.retcode);
      return(false);
     }
...
int OnInit()
  {
   ...
   // Use the indicators' canonical default inputs.  Their tester-aware paths
   // suppress chart objects and alerts without changing mathematical buffers.
   // Keeping iCustom parameterless also prevents a stale EX5 input schema from
   // silently shifting a long variadic argument list.
