# HYP-XBT-MM-TRADETHROUGH-004 — frozen digest-bound XBTUSD market making

Frozen before any V4 strategy outcome is computed. V2 remains sealed one-day
engineering evidence. V3 proved indexed cross-day state continuity and is now
sealed after its engineering reports. V4 adds a deterministic in-stream digest
that MT5 recomputes while processing every event; no V3 economic outcome was
authorized or used. The sizing revision was originally required because the
official XBTUSD lot-size change made V2 structurally non-executable during the
declared DESIGN population.
...
the venue change. The final V4 rule keeps the smallest constant order quantity
...
- Every normalized daily binary carries a deterministic 64-bit digest over all
  event fields. MT5 recomputes and matches it before the daily gate can pass.
