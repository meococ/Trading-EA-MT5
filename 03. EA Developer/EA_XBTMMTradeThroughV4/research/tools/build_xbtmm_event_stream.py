...
def digest_mix(digest: int, value: int) -> int:
    return ((digest ^ (value & UINT64_MASK)) * FNV64_PRIME) & UINT64_MASK
...
def event_digest(
    ...
    fields = (
        time_us,
        kind,
        round(bid / DESIGN_TICK_SIZE),
        round(ask / DESIGN_TICK_SIZE),
        bid_size,
        ask_size,
        round(trade_price / DESIGN_TICK_SIZE),
        trade_size,
        side,
    )
    for value in fields:
...
def verify_binary_digest(path: Path) -> str:
    with path.open("rb") as handle:
        header_raw = handle.read(HEADER.size)
        if len(header_raw) != HEADER.size:
            raise ValueError("truncated event header")
        magic, schema, record_size, records, _first, _last, _quotes, _trades, expected = HEADER.unpack(
            header_raw
        )
        if magic != MAGIC or schema != SCHEMA_VERSION or record_size != RECORD.size:
            raise ValueError("event header identity mismatch")
        digest = FNV64_OFFSET
        for _ in range(records):
            raw = handle.read(RECORD.size)
            if len(raw) != RECORD.size:
                raise ValueError("truncated event payload")
            time_us, kind, bid, ask, bid_size, ask_size, trade_price, trade_size, side = RECORD.unpack(raw)
            digest = event_digest(
                digest,
                time_us,
                kind,
        ...
        if handle.read(1):
            raise ValueError("event payload has trailing bytes")
    if digest != expected:
        raise ValueError(f"event digest mismatch: {digest:016X} != {expected:016X}")
...
def build(quote_path: Path, trade_path: Path, out_path: Path, manifest_path: Path, symbol: str) -> dict:
    ...
    payload_digest64 = FNV64_OFFSET

    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("wb") as output:
        output.write(HEADER.pack(MAGIC, SCHEMA_VERSION, RECORD.size, 0, 0, 0, 0, 0, 0))
        while quote is not None or trade is not None:
            ...
            if take_trade:
                ...
                record_bid = current_quote["bid"] if current_quote is not None else 0.0
                record_ask = current_quote["ask"] if current_quote is not None else 0.0
                record_bid_size = current_quote["bid_size"] if current_quote is not None else 0
                record_ask_size = current_quote["ask_size"] if current_quote is not None else 0
                payload_digest64 = event_digest(
                    payload_digest64,
                    stamp,
                    TRADE,
                    record_bid,
    ...
    payload = {
        ...
        "output": {
            ...
            "event_digest64": f"{payload_digest64:016X}",
        },
        "integrity": {
