...
def paths_for(root: Path, utc_day: date) -> dict[str, Path]:
    ...
    return {
        ...
        "event": root / "events_v4" / year / month / f"{token}.xbtmm",
        "manifest": root / "manifests_v4" / year / month / f"{token}.event_manifest.json",
    }
...
def index_row(root: Path, manifest: dict) -> dict[str, object]:
    ...
    return {
        ...
        "event_digest64": manifest["output"]["event_digest64"],
...
def write_index(root: Path, start: date, end: date, manifests: list[dict], pilot: bool) -> dict:
    ...
    prefix = f"pilot_v4_{start:%Y%m%d}_{end:%Y%m%d}" if pilot else "design_2018_2021_v4"
    ...
    root_payload = "\n".join(
        f"{row['utc_day']}|{row['event_sha256']}|{row['event_digest64']}|{row['records']}" for row in rows
    )
