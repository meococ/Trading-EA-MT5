...
SRC = Path(__file__).parents[1] / "EA_OnlineRidge_EUR_H1_V12MLR1.mq5"
TEXT = SRC.read_text(encoding="utf-8")


def test_identity_and_binding():
    assert 'EXPECTED_HYPOTHESIS="HYP-ORLS-EURUSD-H1-002"' in TEXT
...
def test_runtime_buffer_bound_repair_only():
    assert 'const int need=26' in TEXT
    assert 'const int need=25' not in TEXT
