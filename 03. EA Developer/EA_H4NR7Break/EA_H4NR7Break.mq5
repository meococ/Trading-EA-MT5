//+------------------------------------------------------------------+
//| EA_H4NR7Break.mq5 — H4 NR7 compression breakout (RR=3 thick)     |
//| Symbol: USDJPY | Period: H4 | Magic: 880990                       |
//| Hypothesis: HYP-H4-NR7-BREAK-001                                   |
//| Closed-bar[1] only. Mon–Thu; weekend flat; risk 0.5%; RR=3.0.     |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_H4NR7Break"
...
input group "=== General ==="
input ulong    InpMagic         = 880990;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== NR7 ==="
input int      InpNRLookback    = 7;      // classic NR7
input double   InpSL_ATR_Buf    = 0.10;   // buffer beyond NR7 opposite
...
int OnInit()
{
   if(Period() != PERIOD_H4)
   {
      Print("[NR7] FAIL — chart must be H4");
      return INIT_FAILED;
   }
   ...
   Print("[NR7] HYP-H4-NR7-BREAK-001 RR=", InpTP_Ratio);
...
// bar[2] is NR7 among bars[2 .. 1+lookback]; breakout evaluated on closed bar[1]
bool DetectNR7Break(bool &isBuy, double &nrHigh, double &nrLow)
{
   isBuy = false;
   nrHigh = 0.0;
   nrLow = 0.0;
   ...
   for(int i = 3; i <= 1 + InpNRLookback; i++)
   {
...
void OnTick()
{
   ...
   bool isBuy = false;
   double nrHigh = 0.0, nrLow = 0.0;
   if(!DetectNR7Break(isBuy, nrHigh, nrLow))
      return;
   ...
   double entry = (isBuy ? SymbolInfoDouble(_Symbol, SYMBOL_ASK)
                         : SymbolInfoDouble(_Symbol, SYMBOL_BID));
   double slRaw = (isBuy ? nrLow - atr[0] * InpSL_ATR_Buf
                         : nrHigh + atr[0] * InpSL_ATR_Buf);
   ...
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, "NR7|brk", res))
      return;
