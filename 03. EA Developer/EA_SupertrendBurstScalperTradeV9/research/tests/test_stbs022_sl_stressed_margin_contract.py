...
V8 = ROOT / "03. EA Developer/EA_SupertrendBurstScalperTradeV8/EA_SupertrendBurstScalperTradeV8.mq5"
V9 = PACKAGE / "EA_SupertrendBurstScalperTradeV9.mq5"
...
def test_fresh_v9_identity_is_fail_closed():
    text = source(V9)
    assert '#property version   "9.00"' in text
    assert 'InpHypothesisId        = "HYP-STBS-XAUUSD-M15-022"' in text
    assert 'InpVariantTag          = "STBS_H1_FLIP_M15_BURST_TRADE_V9_SL_STRESSED_MARGIN"' in text
    assert "InpMagic               = 5604122" in text
    assert 'EA_NAME              = "EA_SupertrendBurstScalperTradeV9"' in text
    assert 'InpHypothesisId!="HYP-STBS-XAUUSD-M15-022"' in text
    assert 'InpVariantTag!="STBS_H1_FLIP_M15_BURST_TRADE_V9_SL_STRESSED_MARGIN"' in text
    assert "InpAuditOnly || !InpEnableTelemetry || InpMagic!=5604122" in text
...
def test_downward_only_broker_step_search_is_preserved():
    old = function_body(source(V8), "bool SelectMarginSafeVolume(")
    new = function_body(source(V9), "bool SelectMarginSafeVolume(")
    assert new == old
