//+------------------------------------------------------------------+
//| EA_M15SessionVWAPReclaim.mq5 — Session VWAP stretch→reclaim      |
//| Symbol: USDJPY | Period: M15 | Magic: 880971                     |
//|                                                                   |
//| Hypothesis: HYP-SESSION-VWAP-RECLAIM-M15-001                      |
//| Cumulative session VWAP from London open; fade stretch away then |
//| reclaim closed-bar[1] back through VWAP in London–NY. Independent|
//| of SB KZ / ORB / ITSM / AsianSweep. Closed-bar only.             |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15SessionVWAPReclaim"
...
input group "=== General ==="
input ulong    InpMagic         = 880971;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== Session VWAP ==="
input int      InpSessionStartH = 7;     // VWAP reset / session start (server)
input int      InpTradeStartH   = 8;
input int      InpTradeEndH     = 18;
input int      InpFlatHour      = 21;
input double   InpStretchATR    = 0.80;  // min |close-VWAP| / ATR to arm
input double   InpReclaimBufATR = 0.05;  // reclaim buffer through VWAP
...
double   g_vwapNum = 0.0;   // sum typical*tickvol
double   g_vwapDen = 0.0;   // sum tickvol
int      g_vwapDayKey = -1;
int      g_stretchDir = 0;  // +1 stretched above, -1 below
...
int OnInit()
{
   ...
   Print("[SVR] HYP-SESSION-VWAP-RECLAIM-M15-001");
...
void UpdateSessionVWAP(const datetime bar1Time, const int dayKey)
{
   MqlDateTime dt;
   TimeToStruct(bar1Time, dt);
   if(dayKey != g_vwapDayKey || dt.hour < InpSessionStartH)
   {
      if(dayKey != g_vwapDayKey)
      {
         g_vwapDayKey = dayKey;
         g_vwapNum = 0.0;
         g_vwapDen = 0.0;
   ...
   g_vwapNum += typical * (double)vol;
   g_vwapDen += (double)vol;
}
...
int GetSignal()
{
   if(g_vwapDen <= 0.0)
      return 0;
   double vwap = g_vwapNum / g_vwapDen;
   double atr[];
   ...
   double dist = (c1 - vwap) / atr[0];
   double buf = InpReclaimBufATR;

   // Arm stretch on closed bar
   if(dist >= InpStretchATR)
      g_stretchDir = +1;
   else if(dist <= -InpStretchATR)
      g_stretchDir = -1;

   if(g_stretchDir == 0)
      return 0;

   // Reclaim: was stretched above → close back below VWAP; opposite for below
   if(g_stretchDir > 0 && c2 > vwap && c1 <= vwap - buf * atr[0])
   {
      g_stretchDir = 0;
      return -1; // fade down through VWAP
   }
   if(g_stretchDir < 0 && c2 < vwap && c1 >= vwap + buf * atr[0])
   {
      g_stretchDir = 0;
...
void OnTick()
{
   ...
   UpdateSessionVWAP(bar1Time, dayKey);
   ...
   if(dt.hour >= InpFlatHour || dt.day_of_week == 5 ||
      ...
      return;
   if(dt.hour < InpTradeStartH || dt.hour >= InpTradeEndH)
      return;
