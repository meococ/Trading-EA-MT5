//+------------------------------------------------------------------+
//| EA_ChopMeanRevert.mq5 — Mean Reversion DURING Choppy Regimes     |
//| Symbol: USDJPY+  |  Period: M15                                   |
//|                                                                   |
//| PARADIGM INVERSION: ChopRegime trades trends when CI < 50.        |
//| This EA trades MEAN REVERSION when CI > 55 (choppy/ranging).     |
//| During ranging markets, price oscillates around a mean —          |
//| fade deviations from the fast EMA when in choppy regime.          |
//|                                                                   |
//| Signal: Price > fast EMA + threshold → SELL (fade), vice versa   |
//| Filter: CI > threshold (market IS ranging, mean reversion valid) |
//|                                                                   |
//| Novelty: Type #90 — Choppiness-gated mean reversion              |
//| SIGNALS ON BAR[1] ONLY — no repaint.                              |
//| Max | 2026-04-13 | v1.0                                          |
//+------------------------------------------------------------------+
#property copyright "Max — EA_ChopMeanRevert v1.0"
#property version   "1.00"
#property strict

input group "=== General ==="
input ulong    InpMagic         = 223001;
input int      InpDeviation     = 30;

input group "=== Choppiness Index ==="
input int      InpChopPeriod    = 14;
input double   InpChopMin       = 55.0;          // CI > this = choppy enough for MR

input group "=== Mean Reversion ==="
input int      InpMeanEMA       = 20;            // EMA to revert to
input double   InpDevATRMult    = 1.0;           // Price must deviate > N*ATR from EMA to trigger
input int      InpTrendEMA      = 50;            // Trend bias (optional — may disable for pure MR)
input bool     InpUseTrendBias  = false;          // Use trend EMA50? (false = pure mean reversion)
...
int OnInit()
{
   ...
   g_hMean  = iMA(_Symbol, PERIOD_CURRENT, InpMeanEMA, 0, MODE_EMA, PRICE_CLOSE);
   g_hTrend = iMA(_Symbol, PERIOD_CURRENT, InpTrendEMA, 0, MODE_EMA, PRICE_CLOSE);
   if(g_hATR14==INVALID_HANDLE||g_hATR1==INVALID_HANDLE||g_hMean==INVALID_HANDLE||g_hTrend==INVALID_HANDLE)
      return INIT_FAILED;
   g_dayStartBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   PrintFormat("[CHOPMR] v1.00 | ChopMin=%.1f MeanEMA=%d DevMult=%.1f TrendBias=%s",
               InpChopMin, InpMeanEMA, InpDevATRMult, InpUseTrendBias?"ON":"OFF");
...
int GetSignal()
{
   ...
   // Only trade when market is CHOPPY (ranging) — mean reversion zone
   if(ci < InpChopMin) return 0;  // Trending — don't mean-revert into a trend

   // EMA as mean
   double ema[];
   ArraySetAsSeries(ema, true);
   if(CopyBuffer(g_hMean, 0, 1, 1, ema) < 1) return 0;
   ...
   double deviation = close1 - ema[0];
   double threshold = atr[0] * InpDevATRMult;

   // Fade the deviation: price above mean → SELL (expect reversion down)
   //                     price below mean → BUY (expect reversion up)
   if(deviation > threshold)
   {
      // Optional trend bias: only sell if below trend EMA (counter-trend safety)
      if(InpUseTrendBias)
      {
         double trend[];
         ArraySetAsSeries(trend, true);
         if(CopyBuffer(g_hTrend, 0, 1, 1, trend) < 1) return 0;
         if(close1 > trend[0]) return 0;  // Don't fade in strong uptrend
      }
      return -1;  // SELL — fade upward deviation
   }
   if(deviation < -threshold)
   {
      if(InpUseTrendBias)
      {
         ...
         ArraySetAsSeries(trend, true);
         if(CopyBuffer(g_hTrend, 0, 1, 1, trend) < 1) return 0;
         if(close1 < trend[0]) return 0;  // Don't fade in strong downtrend
      }
      return +1;  // BUY — fade downward deviation
   }
...
void OnTick()
{
   ...
   req.comment=StringFormat("CMR|CI=%.1f",ComputeChoppiness(1));
   req.type_filling=ORDER_FILLING_FOK;
   if(!OrderSend(req,res)){req.type_filling=ORDER_FILLING_IOC;if(!OrderSend(req,res))return;}
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED)
   { g_tradesToday++; PrintFormat("[CHOPMR] %s %.2f @ %.2f CI=%.1f",isBuy?"BUY":"SELL",lot,res.price,ComputeChoppiness(1)); }
}
