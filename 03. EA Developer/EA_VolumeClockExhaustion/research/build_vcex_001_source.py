#!/usr/bin/env python3
"""Inert outcome-blind source probe for HYP-VCEX-EURUSD-M15-001.
...
surface is intentionally usable with synthetic OHLC+tick_volume rows.
"""
...
HYPOTHESIS_ID = "HYP-VCEX-EURUSD-M15-001"
EA_NAME = "EA_VolumeClockExhaustion"
FAMILY = "volume-clock-early-impulse-exhaustion-reversal"
ATTEMPT_ID = "VCEX001-SOURCE-001"

PLAN_REL = (
    "03. EA Developer/EA_VolumeClockExhaustion/research/"
    "HYP-VCEX-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN.md"
)
PLAN_SHA256 = "1E6E5DB88998743F5D56D4A10C5E518EB1612289A807FE729E64EA49D9B66FCE"
BUILDER_REL = (
    "03. EA Developer/EA_VolumeClockExhaustion/research/"
    "build_vcex_001_source.py"
)
TEST_REL = (
    "03. EA Developer/EA_VolumeClockExhaustion/research/tests/"
    "test_build_vcex_001_source.py"
)
EVIDENCE_ROOT_REL = (
    "03. EA Developer/EA_VolumeClockExhaustion/research/evidence/"
    "HYP-VCEX-EURUSD-M15-001_SOURCE_FEASIBILITY/"
    f"{ATTEMPT_ID}"
)
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_VolumeClockExhaustion/research/"
    "HYP-VCEX-EURUSD-M15-001_SOURCE_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
REVIEW_RECEIPT_SCHEMA = "vcex_001_source_implementation_review_receipt.v1"
...
EARLY_ATR_MULT = 0.45
EXHAUSTION_LATE_MULT = 0.30
TAU_MAX = 0.40
ATR_WINDOW = 14
HORIZON_BARS = 8
SIGNAL_HOUR_START = 7
SIGNAL_HOUR_END = 16  # inclusive
HALF_MASS = 0.5
MINUTES_PER_M15 = 15

# Binary64-safe Stage-0 boundary comparisons. Threshold constants stay frozen;
# helpers only absorb representation noise at inclusive equality / strict edges
# via a finite-only handful of IEEE-754 ULPs (no fixed relative floor).
...
def le_inclusive(left: float, right: float) -> bool:
    """Inclusive left <= right under a tiny finite-only ULP tolerance."""
...
def lt_strict(left: float, right: float) -> bool:
    """Strict left < right; non-finite / exact / ULP-equality at boundary fails."""
...
ALLOWED_DECODE_COLUMNS = ("time_utc", "open", "high", "low", "close", "tick_volume")
...
def validate_registry_snapshot(
    *,
    ...
    try:
        module = types.ModuleType("_vcex_verified_candidate_registry_validator")
...
def _finite_tick_volume(value: object) -> float:
    if type(value) not in {float, int} or isinstance(value, bool):
...
def build_complete_m15(rows: Iterable[Mapping[str, object]]) -> tuple[list[dict[str, object]], dict[str, int]]:
    ...
    for row in rows:
        ...
        _ohlc(row)
        if "tick_volume" not in row:
            raise ContractError("M1 row missing tick_volume")
    ...
    for bucket in sorted(groups):
        group = groups[bucket]
        expected = [bucket + timedelta(minutes=index) for index in range(MINUTES_PER_M15)]
        observed = [item[0] for item in group]
        if len(group) != MINUTES_PER_M15 or observed != expected:
            incomplete += 1
            continue
        ...
        minute_tv = [_finite_tick_volume(row["tick_volume"]) for _, row in group]
        complete.append(
            {
                "time_utc": bucket,
                ...
                "minute_open": tuple(minute_open),
                "minute_close": tuple(minute_close),
                "minute_tick_volume": tuple(minute_tv),
                "tick_volume": float(sum(minute_tv)),
            }
        )
    return complete, {
...
def volume_clock_features(
    *,
    minute_open: Sequence[float],
    minute_close: Sequence[float],
    minute_tick_volume: Sequence[float],
) -> dict[str, object] | None:
    """Compute h, tau, P_early, P_late from exact 15-minute volume clock."""

    if (
        len(minute_open) != MINUTES_PER_M15
        or len(minute_close) != MINUTES_PER_M15
        or len(minute_tick_volume) != MINUTES_PER_M15
    ):
        raise ContractError("volume-clock requires exactly 15 M1 minutes")
    total_tv = float(sum(float(value) for value in minute_tick_volume))
    if not math.isfinite(total_tv) or total_tv <= 0:
        return None
    cumulative = 0.0
    half_mass = HALF_MASS * total_tv
    half_index: int | None = None
    for index, volume in enumerate(minute_tick_volume):
        tv = float(volume)
        if not math.isfinite(tv) or tv < 0:
            raise ContractError("invalid minute tick_volume")
        cumulative += tv
    ...
    if half_index is None:
        return None
    tau = (half_index + 0.5) / float(MINUTES_PER_M15)
    p_early = float(minute_close[half_index]) - float(minute_open[0])
    p_late = float(minute_close[MINUTES_PER_M15 - 1]) - float(minute_close[half_index])
    if not math.isfinite(tau) or not math.isfinite(p_early) or not math.isfinite(p_late):
        raise ContractError("non-finite volume-clock features")
    return {
        "h": half_index,
        "tau": tau,
        "p_early": p_early,
...
def exhaustion_holds(*, p_early: float, p_late: float) -> bool:
    if p_early == 0.0:
        return False
    if (p_late * p_early) <= 0.0:
        return True
    # Strict < at frozen 0.30: exact same-sign ratio must fail; just below passes.
    return lt_strict(abs(p_late), EXHAUSTION_LATE_MULT * abs(p_early))


def compute_bar_features(bars: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    """Closed-bar ATR14 + volume-clock features for complete M15 series with gap resets."""
    ...
    for index, bar in enumerate(ordered):
        ...
        minute_open = bar.get("minute_open")
        minute_close = bar.get("minute_close")
        minute_tv = bar.get("minute_tick_volume")
        if (
            not isinstance(minute_open, (list, tuple))
            or not isinstance(minute_close, (list, tuple))
            or not isinstance(minute_tv, (list, tuple))
            or len(minute_open) != MINUTES_PER_M15
            or len(minute_close) != MINUTES_PER_M15
            or len(minute_tv) != MINUTES_PER_M15
        ):
            raise ContractError("M15 bar missing minute volume-clock arrays")
        clock = volume_clock_features(
            minute_open=[float(value) for value in minute_open],
            minute_close=[float(value) for value in minute_close],
            minute_tick_volume=[float(value) for value in minute_tv],
        )
        row: dict[str, object] = {
            "time_utc": at,
            ...
            "h": None if clock is None else clock["h"],
            "tau": None if clock is None else clock["tau"],
            "p_early": None if clock is None else clock["p_early"],
            "p_late": None if clock is None else clock["p_late"],
            "total_tv": None if clock is None else clock["total_tv"],
        }
...
def select_raw_signals(features: Sequence[Mapping[str, object]]) -> list[dict[str, object]]:
    ...
    for feature in ordered:
        ...
        tau = feature.get("tau")
        p_early = feature.get("p_early")
        p_late = feature.get("p_late")
        half_index = feature.get("h")
        if (
            type(atr) not in {int, float}
            or isinstance(atr, bool)
            or not math.isfinite(float(atr))
            or float(atr) <= 0
            or type(tau) not in {int, float}
            or isinstance(tau, bool)
            or not math.isfinite(float(tau))
            or type(p_early) not in {int, float}
            or isinstance(p_early, bool)
            or not math.isfinite(float(p_early))
            or type(p_late) not in {int, float}
            or isinstance(p_late, bool)
            or not math.isfinite(float(p_late))
            or type(half_index) is not int
            or isinstance(half_index, bool)
            or not (0 <= int(half_index) <= 14)
        ):
            continue
        if gt_strict(float(tau), TAU_MAX):
            continue
        if float(p_early) == 0.0:
            continue
        # Inclusive early >= 0.45*ATR: binary64 equality must pass.
        if not ge_inclusive(abs(float(p_early)), EARLY_ATR_MULT * float(atr)):
            continue
        if not exhaustion_holds(p_early=float(p_early), p_late=float(p_late)):
            continue
        direction = _sign(float(p_early))
        if direction == 0:
            continue
        stop_distance = max(float(atr), MIN_STOP_DISTANCE)
        cost_ratio = COST_ROUND_TURN / stop_distance
        true_dir = "SHORT" if direction > 0 else "LONG"  # TRUE = -sign(P_early)
        follow_dir = "LONG" if direction > 0 else "SHORT"  # FOLLOW_CONTROL = +sign(P_early)
        selected.append(
            {
                "time_utc": at,
...
def map_horizon(
    ...
    """Timestamp-only eight-bar horizon; never inspects post-entry OHLC."""
...
def assign_source_signal_id(decision: datetime) -> str:
    ...
    return f"VCEX001-SRC-{sha256_bytes(identity)[:16]}"
...
def build_matched_ledgers(
    ...
    for signal in sorted(raw_signals, key=lambda row: _as_utc(row["time_utc"])):
        ...
        features = {
            "h": int(signal["h"]),
            "tau": float(signal["tau"]),
            "p_early": float(signal["p_early"]),
            "p_late": float(signal["p_late"]),
        ...
        for arm, direction_key in (
            ...
            candidate_id = f"VCEX001-{arm}-{sha256_bytes(identity)[:16]}"
...
def evaluate_stage0_gates(
    *,
    ...
    gates = {
        "signal_domain_m15_completeness_at_least_0_99": ge_inclusive(domain_ratio, 0.99),
        "source_executable_horizon_ratio_at_least_0_99": ge_inclusive(horizon_ratio, 0.99),
        "eligible_cadence_2_to_5_per_elapsed_week": ge_inclusive(cadence, 2.0)
        and le_inclusive(cadence, 5.0),
        "follow_control_matched_true_timestamps": follow_match,
    ...
    def visit(item: object) -> None:
        if isinstance(item, Mapping):
            for key, child in item.items():
                ...
                if key in {"arm", "arms"} and type(child) is str and child not in ARM_NAMES:
                    if child in {"WALL_CLOCK_H7", "H7_WALL", "SHIFTED_TOD", "FADE"}:
                        raise ContractError(f"forbidden trial arm: {child}")
                visit(child)
...
def validate_review_receipt(
    ...
    *,
    ...
    receipt = parse_canonical_object(payload, label="VCEX implementation review receipt")
    expected = {
...
def _decode_parquet_allowed_only(payload: bytes) -> DecodedShard:
    """Decode only the allowlisted OHLC+tick_volume columns; never request forbidden fields."""
    ...
    try:
        ...
        forbidden_requested = [
            name
            for name in ("time_server", "spread", "real_volume", "utc_offset_h")
    ...
    *,
    ...
    try:
        ...
        for row in decoded.rows:
            ...
            if not (low <= open_price <= high and low <= close <= high):
                raise ValueError
            tick_volume = _finite_tick_volume(row["tick_volume"])
            cleaned.append(
                {
...
def _flatten_ledgers(
    ...
    for arm in ARM_NAMES:
        ...
        for row in rows:
            ...
            flattened.append(
                {
                    "schema_version": "vcex_001_source_ledger_row.v1",
                    "hypothesis_id": HYPOTHESIS_ID,
                    "attempt_id": ATTEMPT_ID,
                    "reviewed_registry_row_sha256": reviewed_row_sha256,
...
def _persist_success(
    ...
    _write_new_canonical(root / "vcex_001_source_report.json", enriched)
    ...
    for row in classifications:
        ...
        classification_rows.append(
            {
                "schema_version": "vcex_001_source_classification_row.v1",
                "hypothesis_id": HYPOTHESIS_ID,
                "attempt_id": ATTEMPT_ID,
                "reviewed_registry_row_sha256": reviewed_row_sha256,
                "attempt_started_sha256": started_sha,
    ...
    _write_new_jsonl(root / "vcex_001_source_classifications.jsonl", classification_rows)
    flattened = _flatten_ledgers(
        enriched,
        reviewed_row_sha256=reviewed_row_sha256,
        attempt_started_sha256=started_sha,
    )
    _write_new_jsonl(root / "vcex_001_source_ledger.jsonl", flattened)
    ...
    terminal = {
        "schema_version": "vcex_001_attempt_terminal.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "reviewed_registry_row_sha256": reviewed_row_sha256,
...
def _safe_unlink_attempt_terminal(root: Path) -> None:
    """Unlink only root/attempt_terminal.json when present (file/symlink/hardlink).

    Recovery after any exception must never preserve a suspect PASS terminal.
    Directory entries are refused; missing path is a no-op.
    """
...
def _persist_engineering_failure(root: Path, reviewed_row_sha256: str, error: Exception) -> None:
    """Write ENGINEERING_INVALID as the sole terminal; never keep a prior PASS.

    If a post-write failure left a suspect PASS/partial terminal under the
    one-use attempt root, unlink that exact path then exclusively write the
    engineering-invalid terminal hash-binding remaining artifacts. Loud failure
    if the suspect entry cannot be removed/replaced.
    """
...
def _immutable_m1_copy(m1_rows: Sequence[Mapping[str, object]]) -> tuple[dict[str, object], ...]:
    ...
    for row in m1_rows:
        ...
        _ohlc(item)
        _finite_tick_volume(item["tick_volume"])
        copied.append(item)
    return tuple(copied)
...
def scan_source_once(
    ...
    report = {
        ...
        "source_contract": {
            ...
            "tau_max": TAU_MAX,
            "early_atr_mult": EARLY_ATR_MULT,
            "exhaustion_late_mult": EXHAUSTION_LATE_MULT,
            "horizon_bars": HORIZON_BARS,
            "matched_arms": list(ARM_NAMES),
            "rejected_trials": ["WALL_CLOCK_H7"],
        },
        "arm_counts": {
