//+------------------------------------------------------------------+
//| EA_ADXDirectionalCross.mq5                                             |
//| HYP-ADX-XAUUSD-M15-001: native ADX14 DI polarity cross with rising strength           |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Untuned XAUUSD M15 native ADX14 DI polarity trend initiation"

input group "--- Frozen research authority ---"
input bool   InpResearchAutoMode=false;
input bool   InpEnableTelemetry=false;
input string InpHypothesisId="HYP-ADX-XAUUSD-M15-001";
input string InpVariantTag="ADX14_DI_CROSS_STRENGTH25_RISING";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5604401;
...
const string EA_NAME="EA_ADXDirectionalCross";
const string EXPECTED_HYPOTHESIS="HYP-ADX-XAUUSD-M15-001";
const string EXPECTED_VARIANT="ADX14_DI_CROSS_STRENGTH25_RISING";
...
struct SignalDecision
  {
   ...
   double prior_adx;
   double plus_di;
   double prior_plus_di;
   double minus_di;
   double prior_minus_di;
   double kvo;
   double kvo_signal;
   double ema100;
...
// Legacy KVO state remains compile-isolated from the ADX execution path.
...
bool PreloadKvoState()
  {
   ...
   PrintFormat("ADX001_LEGACY_PRELOAD synchronized=%I64d requested=%d copied=%d first=%I64d last=%I64d current=%I64d ready=%s",
   ...
   if(!LoadClosedRates(rates))
     {
      ...
      g_runtime_failed=true;
       PrintFormat("ADX001_FATAL reason=LEGACY_CLOSED_RATE_LOAD availability=%I64d",
    ...
    if(decision_time<g_kvo_last_time)
      {
       g_invalid_bars++;
       g_runtime_failed=true;
       PrintFormat("ADX001_FATAL reason=LEGACY_CLOCK_REGRESSION decision=%I64d last=%I64d",
       ...
       if(!AdvanceKvoBar(rates[release],direction,ko,kvo_signal,ema100))
         {
          g_invalid_bars++;
          g_runtime_failed=true;
          PrintFormat("ADX001_FATAL reason=LEGACY_STATE decision=%I64d",(long)decision_time);
          return(false);
         }
    ...
    if(!ReadClosedAtr(atr_release) || atr_release<=0.0)
      {
       g_invalid_bars++;
       g_runtime_failed=true;
       PrintFormat("ADX001_FATAL reason=LEGACY_ATR_LOAD decision=%I64d",
                   (long)decision_time);
       return(false);
      }
...
bool PreloadAdxState()
  {
   if(g_adx_handle==INVALID_HANDLE || g_atr_handle==INVALID_HANDLE)
      return(false);
   const int adx_bars=BarsCalculated(g_adx_handle);
   const int atr_bars=BarsCalculated(g_atr_handle);
   if(adx_bars<ADX_PERIOD+2 || atr_bars<ATR_PERIOD+2)
      return(false);
   ...
   ArraySetAsSeries(plus_values,false);
   ArraySetAsSeries(minus_values,false);
   ArraySetAsSeries(atr_values,false);
   const int sample=16;
   if(CopyBuffer(g_adx_handle,0,1,sample,adx_values)!=sample ||
      CopyBuffer(g_adx_handle,1,1,sample,plus_values)!=sample ||
      CopyBuffer(g_adx_handle,2,1,sample,minus_values)!=sample ||
      CopyBuffer(g_atr_handle,0,1,sample,atr_values)!=sample)
      return(false);
   for(int i=0;i<sample;i++)
      if(!IsUsable(adx_values[i]) || !IsUsable(plus_values[i]) ||
         !IsUsable(minus_values[i]) || !IsUsable(atr_values[i]) ||
         adx_values[i]<0.0 || plus_values[i]<0.0 || minus_values[i]<0.0 ||
         atr_values[i]<=0.0)
         return(false);

   PrintFormat("ADX001_PRELOAD adx_bars=%d atr_bars=%d sample=%d latest_adx=%.8f latest_plus_di=%.8f latest_minus_di=%.8f latest_atr=%.8f",
               adx_bars,atr_bars,sample,adx_values[sample-1],
               plus_values[sample-1],minus_values[sample-1],atr_values[sample-1]);
   return(true);
  }

bool ReadClosedAdxAndAtr(double &current_adx,double &prior_adx,
                         double &current_plus,double &prior_plus,
                         double &current_minus,double &prior_minus,
                         double &current_atr)
  {
   current_adx=EMPTY_VALUE;
   prior_adx=EMPTY_VALUE;
   current_plus=EMPTY_VALUE;
   prior_plus=EMPTY_VALUE;
   current_minus=EMPTY_VALUE;
   prior_minus=EMPTY_VALUE;
   ...
   ArraySetAsSeries(minus_values,false);
   ArraySetAsSeries(atr_values,false);
   if(CopyBuffer(g_adx_handle,0,1,2,adx_values)!=2 ||
      CopyBuffer(g_adx_handle,1,1,2,plus_values)!=2 ||
      CopyBuffer(g_adx_handle,2,1,2,minus_values)!=2 ||
      CopyBuffer(g_atr_handle,0,1,1,atr_values)!=1)
      return(false);
   prior_adx=adx_values[0];
   current_adx=adx_values[1];
   prior_plus=plus_values[0];
   current_plus=plus_values[1];
   prior_minus=minus_values[0];
   current_minus=minus_values[1];
   current_atr=atr_values[0];
   return(IsUsable(prior_adx) && IsUsable(current_adx) &&
          IsUsable(prior_plus) && IsUsable(current_plus) &&
          IsUsable(prior_minus) && IsUsable(current_minus) &&
          IsUsable(current_atr) && prior_adx>=0.0 && current_adx>=0.0 &&
          prior_plus>=0.0 && current_plus>=0.0 &&
          prior_minus>=0.0 && current_minus>=0.0 && current_atr>0.0);
  }

bool ProcessAdxClosedBar(const datetime availability_time,SignalDecision &signal)
  {
   ZeroMemory(signal);
   MqlRates rates[];
   if(!LoadClosedRates(rates))
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=CLOSED_RATE_LOAD availability=%I64d",
                  (long)availability_time);
      return(false);
     }
   ...
   if(g_last_decision_time>0 && decision_time<g_last_decision_time)
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=DECISION_CLOCK_REGRESSION decision=%I64d last=%I64d",
   ...
   double prior_adx=0.0;
   double current_plus=0.0;
   double prior_plus=0.0;
   double current_minus=0.0;
   double prior_minus=0.0;
   double current_atr=0.0;
   if(!ReadClosedAdxAndAtr(current_adx,prior_adx,current_plus,prior_plus,
                           current_minus,prior_minus,current_atr))
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=INDICATOR_LOAD decision=%I64d",
                  (long)decision_time);
      return(false);
     }

   int direction=0;
   if(prior_plus<=prior_minus && current_plus>current_minus &&
      current_adx>=25.0 && current_adx>prior_adx)
      direction=1;
   else if(prior_plus>=prior_minus && current_plus<current_minus &&
           current_adx>=25.0 && current_adx>prior_adx)
      direction=-1;
   if(direction==0)
      return(false);
   ...
   signal.adx=current_adx;
   signal.prior_adx=prior_adx;
   signal.plus_di=current_plus;
   signal.prior_plus_di=prior_plus;
   signal.minus_di=current_minus;
   signal.prior_minus_di=prior_minus;
   signal.structural_stop=stop;
   return(true);
  }
...
double FloorToTick(const double price)
  {
   ...
   if(GetLastError()!=0 || !IsUsable(tick_size) || tick_size<=0.0)
     {
      g_runtime_failed=true;
      Print("ADX001_FATAL reason=TICK_SIZE_PROPERTY");
      return(EMPTY_VALUE);
     }
...
bool ClosePositionTicket(const ulong ticket,const string reason)
  {
   if(ticket==0 || !PositionSelectByTicket(ticket))
     {
      g_runtime_failed=true;
       PrintFormat("ADX001_FATAL reason=CLOSE_POSITION_SELECT ticket=%I64u",ticket);
      return(false);
     }
   ResetLastError();
   ...
   if(property_error!=0 ||
     ...
     {
      g_runtime_failed=true;
       PrintFormat("ADX001_FATAL reason=CLOSE_POSITION_PROPERTIES ticket=%I64u error=%d type=%d volume=%.8f",
   ...
   if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
      g_runtime_failed=true;
       PrintFormat("ADX001_CLOSE_REJECT ticket=%I64u retcode=%u error=%d reason=%s",
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(ticket==0 || !PositionSelectByTicket(ticket))
        {
         g_runtime_failed=true;
         PrintFormat("ADX001_FATAL reason=POSITION_ENUMERATION index=%d",i);
         return(false);
        }
      ResetLastError();
      const string symbol=PositionGetString(POSITION_SYMBOL);
      ...
      if(GetLastError()!=0 ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("ADX001_FATAL reason=POSITION_PROPERTIES ticket=%I64u",ticket);
         return(false);
        }
      if(symbol!=_Symbol || magic!=InpMagic)
         continue;
      if(flatten_time)
        {
          if(!ClosePositionTicket(ticket,"ADX_WEEKEND_OR_END"))
            return(false);
         continue;
        }
      if(update_exit)
        {
         ...
         if(shift<0 || GetLastError()!=0)
           {
            g_runtime_failed=true;
             PrintFormat("ADX001_FATAL reason=ENTRY_BAR_RECOVERY ticket=%I64u",ticket);
            return(false);
           }
          if(shift>=MAX_HOLD_BARS && !ClosePositionTicket(ticket,"ADX_TIME_EXIT"))
            return(false);
        }
     }
   return(true);
...
bool CalculateVolume(const ENUM_ORDER_TYPE order_type,const double entry,
                      const double stop,double &volume)
  {
   ...
   if(!OrderCalcProfit(order_type,_Symbol,1.0,entry,stop,one_lot_profit) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=ORDER_CALC_PROFIT error=%d value=%.8f",
   ...
   if(account_error!=0 || !IsUsable(equity) || equity<=0.0 ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=MARGIN_ACCOUNT_STATE error=%d mode=%d equity=%.2f margin=%.2f call=%.2f stopout=%.2f",
   ...
   if(!IsUsable(raw_volume) || !IsUsable(minimum) || !IsUsable(maximum) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=VOLUME_PROPERTIES error=%d min=%.8f max=%.8f step=%.8f",
   ...
   if(!OrderCalcMargin(order_type,_Symbol,1.0,entry,margin_one_lot) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=MARGIN_ONE_LOT error=%d value=%.8f",
   ...
   for(int guard=0;guard<1024 && sized>=minimum-1e-9;guard++)
     {
      ...
      if(!OrderCalcMargin(order_type,_Symbol,sized,entry,exact_margin) ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("ADX001_FATAL reason=MARGIN_EXACT error=%d volume=%.8f value=%.8f",
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   if(!SymbolInfoTick(_Symbol,tick) || GetLastError()!=0 ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=SYMBOL_TICK error=%d bid=%.8f ask=%.8f",
   ...
   if(GetLastError()!=0 || stops_level<0)
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=STOPS_LEVEL_PROPERTY error=%d value=%I64d",
                  GetLastError(),stops_level);
      return(false);
     }
   const double minimum_distance=(double)stops_level*_Point;
   const double stop_reference_distance=
      (signal.direction>0 ? tick.bid-stop : stop-tick.ask);
   const double target_reference_distance=
      (signal.direction>0 ? target-tick.bid : tick.ask-target);
   if(!IsUsable(stop_reference_distance) ||
      !IsUsable(target_reference_distance) ||
      stop_reference_distance+1e-12<minimum_distance ||
      target_reference_distance+1e-12<minimum_distance)
     {
      g_geometry_rejects++;
      return(false);
     }
   ...
   if(!check_ok || check_error!=0 || check.retcode!=0)
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=ORDER_CHECK error=%d retcode=%u comment=%s bid=%.8f ask=%.8f entry=%.8f sl=%.8f tp=%.8f stop_distance=%.8f target_distance=%.8f minimum_distance=%.8f",
                  check_error,check.retcode,check.comment,tick.bid,tick.ask,
                  entry,stop,target,stop_reference_distance,
                  target_reference_distance,minimum_distance);
      return(false);
     }
   ResetLastError();
   ...
   if(!send_ok && !definitive_no_fill)
     {
      g_runtime_failed=true;
      PrintFormat("ADX001_FATAL reason=ENTRY_SEND_TRANSPORT decision=%I64d retcode=%u error=%d order=%I64u deal=%I64u",
   ...
   if(!AcceptedRetcode(result.retcode))
     {
      if(!definitive_no_fill)
        {
         g_runtime_failed=true;
         PrintFormat("ADX001_FATAL reason=ENTRY_REJECT_UNKNOWN decision=%I64d retcode=%u error=%d order=%I64u deal=%I64u",
                     (long)signal.decision_time,result.retcode,send_error,
                     result.order,result.deal);
         return(false);
        }
      ...
      if(!OwnedPositionCount(positions) || !OwnedOrderCount(orders) ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("ADX001_FATAL reason=ENTRY_REJECT_AMBIGUOUS decision=%I64d retcode=%u error=%d positions=%d orders=%d",
                     (long)signal.decision_time,result.retcode,send_error,
                     positions,orders);
         return(false);
        }
   ...
   if(!OwnedPositionCount(positions) || !OwnedOrderCount(orders))
     {
      g_runtime_failed=true;
      g_entry_rejects++;
       Print("ADX001_FATAL reason=OWNED_INVENTORY_UNCERTAIN");
      return;
     }
...
bool ValidateFrozenInputs()
  {
   return(_Symbol==EXPECTED_SYMBOL && _Period==PERIOD_M15 &&
          ...
          InpHypothesisId==EXPECTED_HYPOTHESIS && InpVariantTag==EXPECTED_VARIANT &&
           InpMagic==5604401 && MathAbs(InpRiskPercent-0.25)<1e-12 &&
...
int OnInit()
  {
   if(!ValidateFrozenInputs())
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=FROZEN_INPUT_OR_SYMBOL_CONTRACT");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(!CurrentM15Open(g_last_bar_open))
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=INITIAL_M15_CLOCK");
      return(INIT_FAILED);
     }
   if(!EmitD0SeriesProof())
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=D0_SERIES_PROOF");
      return(INIT_FAILED);
     }
   long filling_mode=0;
   if(!SymbolInfoInteger(_Symbol,SYMBOL_FILLING_MODE,filling_mode) ||
     ...
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=FOK_NOT_SUPPORTED");
      return(INIT_FAILED);
     }
    ...
    if(g_atr_handle==INVALID_HANDLE || g_adx_handle==INVALID_HANDLE)
      {
       g_runtime_failed=true;
       Print("ADX001_FATAL reason=INDICATOR_HANDLE");
       return(INIT_FAILED);
      }
    g_indicators_ready=PreloadAdxState();
   const double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   if(!IsUsable(equity) || equity<=0.0)
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=INITIAL_EQUITY");
      return(INIT_FAILED);
     }
   g_daily_day_key=DateKey(TimeCurrent());
   g_daily_start_equity=equity;
   g_peak_equity=equity;
    PrintFormat("ADX001_INIT hypothesis=%s variant=%s symbol=%s period=%d adx_period=%d atr_period=%d",
                InpHypothesisId,InpVariantTag,_Symbol,(int)_Period,ADX_PERIOD,ATR_PERIOD);
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("ADX001_SUMMARY hypothesis=%s closed_bars=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d geometry_rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d runtime_failed=%s reason=%d",
               InpHypothesisId,g_closed_bars,g_raw_signals,g_long_signals,
               g_short_signals,g_entries_accepted,g_entry_rejects,
               g_geometry_rejects,g_close_requests,g_clock_rejects,g_invalid_bars,
...
void OnTick()
  {
   ...
   if(!g_indicators_ready)
     {
      ...
      if(!CurrentM15Open(warmup_open))
        {
         g_runtime_failed=true;
         Print("ADX001_FATAL reason=WARMUP_M15_CLOCK");
         return;
        }
   ...
   if(!CurrentM15Open(current_open))
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=M15_SCHEDULING_CLOCK");
      return;
     }
   ...
   if(current_open<g_last_bar_open)
     {
      g_runtime_failed=true;
       Print("ADX001_FATAL reason=BAR_CLOCK_REGRESSION");
      return;
     }
   ...
   if(ProcessAdxClosedBar(current_open,signal) && signal.fired)
