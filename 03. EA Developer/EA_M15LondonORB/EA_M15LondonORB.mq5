//+------------------------------------------------------------------+
//| EA_M15LondonORB.mq5 — London opening-range breakout (M15)        |
//| Symbol: USDJPY | Period: M15 | Magic: 880940                     |
//|                                                                   |
//| Hypothesis: HYP-LONDON-ORB-M15-001                                |
//| Independent of Spark Asian-range, HourOpen micro-ORB, InsideBar, |
//| VolExp, Chop, TickVol, GoldJPY, ITSM/SB/USBILL.                   |
//|                                                                   |
//| Build Hi/Lo on closed M15 bars in London first-hour window;       |
//| lock; enter on closed bar[1] break with body + D1 EMA50.          |
//| Mon–Thu a priori; weekend flat; risk 0.5%. No day mining.         |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15LondonORB"
...
input group "=== General ==="
input ulong    InpMagic         = 880940;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== London ORB ==="
input int      InpOrbStart      = 9;      // Inclusive hour (server)
input int      InpOrbEnd        = 10;     // Exclusive hour — lock here
...
input int      InpTradeStart    = 10;     // Breakout window start
input int      InpTradeEnd      = 16;     // Breakout window end (exclusive)
...
int OnInit()
{
   ...
   PrintFormat("[LORB] HYP-LONDON-ORB-M15-001 | ORB[%d,%d) trade[%d,%d) TP=%.2fR risk=%.2f%%",
...
void OnTick()
{
   ...
   if(IsSuccessfulRetcode(res.retcode))
   {
      ...
      g_brokeToday = true;
      g_holdBars = 0;
      PrintFormat("[LORB] %s lot=%.2f @ %.5f ORB[%.5f,%.5f]",
