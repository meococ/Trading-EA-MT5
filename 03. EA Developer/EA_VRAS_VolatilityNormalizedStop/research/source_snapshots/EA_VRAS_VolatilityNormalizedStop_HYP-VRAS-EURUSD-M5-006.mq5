...
#property strict
#property description "HYP-VRAS-EURUSD-M5-006 matched stop-geometry experiment"
...
bool WriteRunMeta()
  {
   ...
   string payload=StringFormat(
      ...
      "\"risk_rejections\":%I64d,\"account_halt\":%s}}",
...
bool ValidateInputs()
  {
   if(_Symbol!="EURUSD" || _Period!=PERIOD_M5)
      return false;
   if(InpHypothesisId!="HYP-VRAS-EURUSD-M5-006" || InpMagic!=5600756)
      return false;
   if(InpUseVolatilityNormalizedStop)
     {
...
int OnInit()
  {
   ...
   if(!OpenTelemetry())
      return INIT_FAILED;
   PrintFormat("VRAS HYP006 init variant=%s normalized_stop=%s closed_bar=true promotion=false",
               InpVariantTag,InpUseVolatilityNormalizedStop ? "true" : "false");
