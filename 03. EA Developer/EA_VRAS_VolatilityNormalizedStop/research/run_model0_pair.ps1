...
$preflight=Join-Path $PSScriptRoot 'preflight\HYP-VRAS-EURUSD-M5-006'
$alpha=Join-Path $repo '02. AlphaFactory\alpha.ps1'
$selected=if($Arm -eq 'pair'){@('control','challenger')}else{@($Arm)}
foreach($name in $selected){
  & (Join-Path $PSScriptRoot 'build_matched_pair_receipts.ps1') -TargetArm $name|Write-Host
  ...
  Write-Host "[VRAS-HYP006] Starting $name Model-0 run..."
