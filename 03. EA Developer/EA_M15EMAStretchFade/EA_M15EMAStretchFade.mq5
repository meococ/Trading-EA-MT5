//+------------------------------------------------------------------+
//| EA_M15EMAStretchFade.mq5 — EMA stretch mean-reversion fade       |
//| Symbol: USDJPY | Period: M15 | Magic: 880944                     |
//|                                                                   |
//| Hypothesis: HYP-EMA-STRETCH-FADE-M15-001                           |
//| When closed bar[1] |Close-EMA20|/ATR14 >= threshold, fade toward  |
//| EMA. Pure stretch MR — not ADR% exhaust (S680/S681), not CI-gated |
//| ChopMeanRevert (S652), not PivotBounce, not ORB/PDH/NY.           |
//|                                                                   |
//| Closed-bar[1] only. Mon–Thu Europe; weekend flat; risk 0.5%.      |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15EMAStretchFade"
...
input group "=== General ==="
input ulong    InpMagic         = 880944;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== Stretch Fade ==="
input int      InpEMAPeriod     = 20;
input int      InpATRPeriod     = 14;
input double   InpStretchATR    = 1.50;
input double   InpMaxStretchATR = 4.00;
...
input double   InpSL_BufferATR  = 0.25;
input double   InpTP_Ratio      = 1.00;
...
int OnInit()
{
   ...
   PrintFormat("[ESF] HYP-EMA-STRETCH-FADE-M15-001 | stretch>=%.2f ATR trade[%d,%d)",
               InpStretchATR, InpTradeStart, InpTradeEnd);
...
int GetSignal(const int hour, double &emaOut, double &atrOut)
{
   emaOut = 0.0;
   atrOut = 0.0;
   ...
   double stretch = (bC - ema[0]) / atr[0];
   atrOut = atr[0];
   emaOut = ema[0];

   if(MathAbs(stretch) > InpMaxStretchATR)
      return 0;
   if(stretch >= InpStretchATR)
      return -1;
   if(stretch <= -InpStretchATR)
      return +1;
   return 0;
...
void OnTick()
{
   ...
   int signal = GetSignal(dt.hour, ema, atr);
   if(signal == 0)
      return;
   ...
   double slRaw = (isBuy ? bL - atr * InpSL_BufferATR
                         : bH + atr * InpSL_BufferATR);
   ...
   double emaDist = MathAbs(ema - entry);
   double tpDist = slDist * InpTP_Ratio;
   if(emaDist >= slDist * 0.80)
      tpDist = MathMin(emaDist, slDist * 2.0);
   ...
   if(IsSuccessfulRetcode(res.retcode))
   {
      g_tradesToday++;
      g_holdBars = 0;
      PrintFormat("[ESF] %s lot=%.2f @ %.5f ema=%.5f atr=%.5f",
                  isBuy ? "BUY" : "SELL", lot, res.price, ema, atr);
   }
}
