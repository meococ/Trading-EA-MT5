...
input string InpForensicNativeLossSchedule="";           // Empty, FROZEN_7_LOSERS_V1 or FROZEN_7_LOSER_OUTCOMES_V1
input int    InpForensicNativeCaseIndex=0;                // 0 all, 1..7 one frozen reference case (diagnostic only)
   ...
   for(int i=0;i<7;i++)
     {
      ...
      // Capture while the frozen entry/exit/SL/TP objects still exist. The old
      // deferred request deleted them first and allowed a fast-forward frame to
      // be mistaken for the requested event.
...
int OnInit()
  {
   if(InpForensicShotWidth<320 || InpForensicShotHeight<240 ||
      ...
      InpForensicNativeCaseIndex<0 || InpForensicNativeCaseIndex>7 ||
