...
bool AttachVisualIndicators()
  {
   ...
   g_visual_mbb=iCustom(_Symbol,PERIOD_M5,"AlphaFactory\\Modern_Bollinger_Bands_GBB",
      mbb_contract,
      InpMbbLengthMode,InpMbbFixedLength,InpMbbBasisMode,InpMbbBandMode,InpMbbStdevMultiplier,
      InpMbbRobustUpperPct,InpMbbRobustLowerPct,InpMbbRobustWindowMult,InpMbbRobustWindowFloor,
      InpMbbKamaFast,InpMbbKamaSlow,InpMbbKerLength,InpMbbRankLength,InpMbbTrendEnter,InpMbbTrendExit,
      InpMbbSqueezeThreshold,InpMbbSqueezeMinBars,InpMbbBasisTouchFraction,
   ...
   g_visual_tb=iCustom(_Symbol,PERIOD_M5,"AlphaFactory\\TB_Smart_Money_Concept_2026",
      tb_contract,TB_PROFILE_EA_CUSTOM,InpTbSwingLength,InpTbDisplacementAtr,InpTbCellsKept,InpTbVoidsKept,
   ...
   // QQE uses the same 12-argument call proven by the parent EA.  Its own
   // tester fast path suppresses alerts, so no extra display arguments are
   // needed (and an extended vararg call was rejected by MT5 in VISUAL-003).
   g_visual_qqe=iCustom(_Symbol,PERIOD_M5,"AlphaFactory\\QQE_MOD",
      ...
      InpQqeSecondarySource,InpQqeBollingerLength,InpQqeBollingerMultiplier);
