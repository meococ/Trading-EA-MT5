...
$preregPath = Join-Path $repoRoot '03. EA Developer\EA_RegimeStructureFusionForensics\research\structural_event\HYP-RSF-EURUSD-M5-STRUCTURAL-EVENT-VISUAL-005_FROZEN_PREREG.md'
...
$selectionPath = Join-Path $repoRoot '03. EA Developer\EA_RegimeStructureFusion\research\structural_event\HYP-RSF-EURUSD-M5-STRUCTURAL-EVENT-004_POSTMORTEM.json'
...
$receipt = [ordered]@{
    ...
    note = 'One-case short-window paired native MT5 visual replay for the terminal STRUCTURAL-EVENT-004 result. Diagnostic Model1 only; all generated performance numbers have no economic authority.'
}

$json = $receipt | ConvertTo-Json -Depth 16
[System.IO.File]::WriteAllText($receiptPath, $json + "`n", [System.Text.UTF8Encoding]::new($false))
Write-Output ("STRUCTURAL_EVENT_VISUAL_RECEIPT_OK path={0} sha256={1} git_status_sha256={2}" -f `
    $receiptPath,
