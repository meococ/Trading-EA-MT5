//+------------------------------------------------------------------+
//| EA_KAMATrend.mq5 — Kaufman Adaptive Moving Average + CI Filter    |
//| Symbol: USDJPY+  |  Period: M15                                   |
//|                                                                   |
//| PARADIGM: Self-adapting period based on efficiency ratio           |
//| KAMA = EMA where smoothing constant adapts to noise level.        |
//| Efficiency Ratio (ER) = |direction| / volatility over N bars.     |
//| In trending markets: ER→1, KAMA fast (like 2-bar EMA).           |
//| In choppy markets: ER→0, KAMA slow (like 30-bar EMA).            |
//|                                                                   |
//| Key difference from fixed EMA: KAMA IS its own regime filter.     |
//| No separate CI needed — the adaptive period embeds regime info.   |
//| Signal: price crosses KAMA + momentum confirmation.               |
//|                                                                   |
//| Novelty: Type #95 — KAMA with embedded regime adaptation          |
//| SIGNALS ON BAR[1] ONLY — no repaint.                              |
//| Max | 2026-04-13 | v1.0                                          |
//+------------------------------------------------------------------+
#property copyright "Max — EA_KAMATrend v1.0"
#property version   "1.00"
#property strict

input group "=== General ==="
input ulong    InpMagic         = 228001;
input int      InpDeviation     = 30;

input group "=== KAMA ==="
input int      InpKAMAPeriod    = 10;            // ER lookback period
input int      InpFastPeriod    = 2;             // Fast constant (trending)
input int      InpSlowPeriod    = 30;            // Slow constant (choppy)

input group "=== Momentum Confirmation ==="
input int      InpMomPeriod     = 5;             // Bars to confirm momentum direction
input double   InpMinERThresh   = 0.25;          // Min ER to take trades (filter extreme chop)
...
double   g_kama = 0;
double   g_prevKama = 0;
double   g_er = 0;       // Efficiency ratio
...
int OnInit()
{
   ...
   if(g_hATR14==INVALID_HANDLE||g_hTrend==INVALID_HANDLE) return INIT_FAILED;
...
//+------------------------------------------------------------------+
//| Update KAMA value using bar[1] data                               |
//| ER = |Close[1] - Close[N+1]| / Sum(|Close[i] - Close[i+1]|)     |
...
void UpdateKAMA()
{
   ...
   double volatility = 0;
   for(int i = 1; i <= InpKAMAPeriod; i++)
      volatility += MathAbs(iClose(_Symbol, PERIOD_CURRENT, i) - iClose(_Symbol, PERIOD_CURRENT, i + 1));

   if(volatility > 0)
      g_er = direction / volatility;
   else
      g_er = 0;

   // Smoothing constant
   double fastSC = 2.0 / (InpFastPeriod + 1);
   double slowSC = 2.0 / (InpSlowPeriod + 1);
   double sc = g_er * (fastSC - slowSC) + slowSC;
   sc = sc * sc;  // Square for extra smoothing
...
int GetSignal()
{
   if(g_warmup < InpKAMAPeriod + InpMomPeriod + 10) return 0;

   // ER threshold — skip extreme chop
   if(g_er < InpMinERThresh) return 0;

   double close1 = iClose(_Symbol, PERIOD_CURRENT, 1);
   double close2 = iClose(_Symbol, PERIOD_CURRENT, 2);

   // Price cross KAMA
   bool crossAbove = (close2 <= g_prevKama && close1 > g_kama);
   bool crossBelow = (close2 >= g_prevKama && close1 < g_kama);

   if(!crossAbove && !crossBelow) return 0;

   // Momentum confirmation: close[1] vs close[1+InpMomPeriod]
   double closeMom = iClose(_Symbol, PERIOD_CURRENT, 1 + InpMomPeriod);
   bool momUp = (close1 > closeMom);
   bool momDn = (close1 < closeMom);
   ...
   if(crossAbove && momUp && close1 > trend[0]) return +1;
   if(crossBelow && momDn && close1 < trend[0]) return -1;
...
void OnTick()
{
   ...
   req.comment=StringFormat("KAMA|ER=%.2f",g_er);
   req.type_filling=ORDER_FILLING_FOK;
   if(!OrderSend(req,res)){req.type_filling=ORDER_FILLING_IOC;if(!OrderSend(req,res))return;}
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED)
   { g_tradesToday++; PrintFormat("[KAMA] %s %.2f @ %.2f ER=%.2f",isBuy?"BUY":"SELL",lot,res.price,g_er); }
}
