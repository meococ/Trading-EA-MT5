//+------------------------------------------------------------------+
//| EA_MultiAssetTSMOMD1V2.mq5                                      |
//| HYP-MULTI-TSMOM-D1-002 - frozen nine-asset design baseline      |
//+------------------------------------------------------------------+
...
input ulong  InpMagic=260812004;
input int    InpDeviationPoints=20;

const string HYPOTHESIS_ID="HYP-MULTI-TSMOM-D1-002";
...
bool     g_pending_basket_ready=false;
int      g_day_key=0;
...
void ProcessMonday(const datetime now)
  {
   ...
   // The first Monday H1 tick can still be in the broker's closed session.
   // Do not consume the weekly decision until the old basket is fully closed.
   if(!CloseAllOwned("MTS_WEEKLY"))
     {
      g_rebalance_retries++;
      g_next_rebalance_retry=now+15;
      if(g_rebalance_retries<=20)
         PrintFormat("MTS_REBALANCE_RETRY phase=close monday_key=%d now=%I64d owned=%d",
                     monday_key,(long)now,OwnedPositionCount());
      return;
     }

   // Freeze the closed-D1 basket once for this Monday. Execution retries reuse it.
   if(!g_pending_basket_ready)
     {
      if(!PrepareMondayBasket(now))
        {
         g_last_monday_key=monday_key; // fail-closed source skip, not an execution retry
         return;
        }
      g_pending_basket_ready=true;
     }
   ...
   if(planned>0 && accepted==planned)
     {
      g_last_monday_key=monday_key;
      g_completed_rebalances++;
      PrintFormat("MTS_REBALANCE_COMPLETE decision=%I64d equity=%.2f planned=%d accepted=%d cumulative_entries=%I64d usd_factor=%.8f",
                  (long)now,AccountInfoDouble(ACCOUNT_EQUITY),planned,accepted,
                  g_entries_accepted,UsdFactorExposure(g_weight));
      return;
     }

   // Never carry a partial target basket. Unwind accepted legs, then retry the
   // identical frozen basket at the next available Monday H1 tick.
   if(accepted>0)
     {
      g_partial_unwinds++;
...
int OnInit()
  {
   if(_Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_H1 || !InpResearchAutoMode ||
      InpMagic!=260812004 || InpDeviationPoints!=20 ||
...
// Frozen successor: same signal/portfolio; retry next Monday tradable H1 tick.
//+------------------------------------------------------------------+
