//+------------------------------------------------------------------+
//| EA_M15ADRCont.mq5 — ADR extreme continuation (not exhaustion MR) |
//| Symbol: USDJPY | Period: M15 | Magic: 880942                     |
//|                                                                   |
//| Hypothesis: HYP-ADR-CONT-M15-001                                  |
//| Opposite of dead ADRExhaust fade (S680/S681). STRATEGY_LOG S681  |
//| notes ADR reach favors continuation over MR on USDJPY M15.       |
//| Independent of PDH break / London ORB / NY ORB / ITSM / SB.       |
//|                                                                   |
//| ADR from closed D1 shifts 1..N only. Signal on closed M15 bar[1]. |
//| Mon–Thu; weekend flat; risk 0.5%.                                 |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15ADRCont"
...
input group "=== General ==="
input ulong    InpMagic         = 880942;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== ADR Continuation ==="
input int      InpADR_Period    = 14;
input double   InpADR_Threshold = 1.00;   // Today's range >= thresh * ADR
input double   InpExtremePct    = 0.15;   // Close within X of day extreme
...
input double   InpSL_ATR_Mult   = 1.00;
...
int OnInit()
{
   ...
   PrintFormat("[ADRC] HYP-ADR-CONT-M15-001 | ADR thr=%.2f extreme=%.2f trade[%d,%d)",
               InpADR_Threshold, InpExtremePct, InpTradeStart, InpTradeEnd);
...
//+------------------------------------------------------------------+
// Closed D1 bars only (shift >= 1).
double CalcADR()
{
   double sum = 0.0;
   int n = 0;
   for(int i = 1; i <= InpADR_Period; i++)
   {
   ...
   if(n < MathMax(5, InpADR_Period / 2))
      return 0.0;
   return sum / n;
}

//+------------------------------------------------------------------+
int GetSignal(const int hour)
{
   if(g_tradedToday)
      return 0;
   ...
   // Today's range from forming D1[0] after closed M15 bar[1] contributed.
   double dayH = iHigh(_Symbol, PERIOD_D1, 0);
   double dayL = iLow(_Symbol, PERIOD_D1, 0);
   if(dayH <= 0.0 || dayL <= 0.0 || dayH <= dayL)
      return 0;
   double dayRange = dayH - dayL;
   if(dayRange < adr * InpADR_Threshold)
      return 0;
   ...
   double band = dayRange * InpExtremePct;
   int dir = 0;
   // Continuation at extreme (opposite of ADRExhaust MR).
   if(bC >= dayH - band)
      dir = +1;
   else if(bC <= dayL + band)
      dir = -1;
   else
      return 0;
...
void OnTick()
{
   ...
   if(IsSuccessfulRetcode(res.retcode))
   {
      ...
      g_tradedToday = true;
      g_holdBars = 0;
      PrintFormat("[ADRC] %s lot=%.2f @ %.5f", isBuy ? "BUY" : "SELL", lot, res.price);
   }
}
