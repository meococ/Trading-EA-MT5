...
#property description "H1 Supertrend flip economic baseline with account-safe margin and lifecycle-v3 evidence."

input string InpHypothesisId        = "HYP-STBS-XAUUSD-M15-020";
input string InpVariantTag          = "STBS_H1_FLIP_M15_BURST_TRADE_V7_ACCOUNT_SAFE";
input bool   InpAuditOnly           = false;
input bool   InpEnableTelemetry     = true;
input long   InpMagic               = 5604120;
...
const string EA_NAME              = "EA_SupertrendBurstScalperTradeV7";
...
bool WriteRunMeta()
{
   ...
   const string payload=StringFormat(
      ...
      "\"margin_emergencies\":%I64d,\"forced_stopouts\":%I64d,"
      ...
      g_margin_emergencies,g_forced_stopouts,g_entries_submitted,g_entry_rejects,
...
bool PositionOpenedByThisEa(const ulong position_id)
{
                                ...
                                double &open_volume,double &close_volume)
{
   ...
   if(target_time_msc<=0 || !PositionOpenedByThisEa(position_id) ||
   ...
   return open_volume>0.0 && close_volume<=open_volume+1e-8;
}
...
bool LogLifecycleDeal(const ulong deal)
{
   ...
   if(HistoryDealGetString(deal,DEAL_SYMBOL)!=_Symbol ||
      ...
      ((entry_kind==DEAL_ENTRY_OUT || entry_kind==DEAL_ENTRY_OUT_BY) &&
       !PositionOpenedByThisEa(position_id)))
      return true;
   const double volume=HistoryDealGetDouble(deal,DEAL_VOLUME);
   ...
   if(position_id==0 || volume<=0.0 || price<=0.0 || !MathIsValidNumber(volume) ||
...
MarginSafetyResult EvaluateMarginCandidate(EntryPlan &plan,const double volume)
{
   ...
   if(!InpAuditOnly)
      PrintFormat("STBS_MARGIN_CHECK|volume=%.8f|required=%.8f|projected_free=%.8f|projected_level=%.8f|mode=%d|call=%.8f|stop=%.8f|threshold=%.8f|safe=%s",
                  volume,required_margin,check.margin_free,check.margin_level,(int)so_mode,
                  so_call,so_stop,threshold,safe ? "true" : "false");
   if(!safe)
      return MARGIN_SAFETY_UNSAFE;
   plan.volume=volume;
...
MarginSafetyResult EvaluateActualMargin()
{
   ...
   PrintFormat("STBS_MARGIN_ACTUAL|equity=%.8f|margin=%.8f|free=%.8f|level=%.8f|mode=%d|call=%.8f|stop=%.8f|threshold=%.8f|safe=%s",
               equity,margin,free_margin,margin_level,(int)so_mode,so_call,so_stop,
               threshold,safe ? "true" : "false");
...
bool SubmitEntry(const int direction,const double atr,const datetime decision_time)
{
   ...
   PrintFormat("STBS_ENTRY_REQUEST|decision=%s|direction=%s|atr=%.8f|entry=%.8f|sl=%.8f|tp=%.8f|volume=%.8f|required_margin=%.8f|projected_free=%.8f|projected_level=%.8f",
               TimeToString(decision_time,TIME_DATE|TIME_SECONDS),
               direction>0 ? "LONG" : "SHORT",atr,plan.entry,plan.stop,
               plan.target,plan.volume,plan.required_margin,
               plan.projected_margin_free,plan.projected_margin_level);
   g_expected_direction=direction;
...
void ConsumeFlipEvent(const MqlRates &bar,const int prior_state,const datetime next_time)
{
   ...
   {
      g_gap_events++;
      PrintFormat("STBS_SIGNAL|source_epoch=%I64d|decision_epoch=%I64d|direction=%s|exact_next=false|consumed=true|audit=%s",
   ...
   if(InpAuditOnly)
      ...
      PrintFormat("STBS_SIGNAL|source=%s|decision=%s|source_epoch=%I64d|decision_epoch=%I64d|direction=%s|exact_next=true|atr_ready=%s|geometry_ready=%s|margin_ready=%s|atr=%.8f|entry=%.8f|sl=%.8f|tp=%.8f|volume=%.8f|projected_level=%.8f|audit=false",
                  TimeToString(bar.time,TIME_DATE|TIME_SECONDS),
                  TimeToString(next_time,TIME_DATE|TIME_SECONDS),
                  (long)ServerToUtc(bar.time),(long)ServerToUtc(next_time),
                  direction>0 ? "LONG" : "SHORT",atr_ready ? "true" : "false",
                  geometry_ready ? "true" : "false",margin_ready ? "true" : "false",atr,
                  geometry_ready ? probe.entry : 0.0,
                  geometry_ready ? probe.stop : 0.0,
                  geometry_ready ? probe.target : 0.0,
                  margin_ready ? probe.volume : 0.0,
                  margin_ready ? probe.projected_margin_level : 0.0);
   if(InpAuditOnly)
      return;
   if(g_runtime_failed)
      return;
...
bool ReconcileExecutionState(const bool advance_visibility_timeout=false)
{
   ...
   const MarginSafetyResult actual_margin=EvaluateActualMargin();
...
int OnInit()
{
   if(!MQLInfoInteger(MQL_TESTER) || MQLInfoInteger(MQL_OPTIMIZATION) ||
      _Symbol!="XAUUSD" || _Period!=PERIOD_M15 ||
       InpHypothesisId!="HYP-STBS-XAUUSD-M15-020" ||
       InpVariantTag!="STBS_H1_FLIP_M15_BURST_TRADE_V7_ACCOUNT_SAFE" ||
       InpAuditOnly || !InpEnableTelemetry || InpMagic!=5604120 ||
   ...
   if(InpEnableTelemetry && (!OpenTelemetry() || !RecoverTelemetryPositionContext()))
   {
      g_runtime_failed=true;
                         ...
                         const MqlTradeResult &result)
{
       ...
       HistoryDealGetString(transaction.deal,DEAL_SYMBOL)==_Symbol)
   {
      ...
      const long deal_magic=(long)HistoryDealGetInteger(transaction.deal,DEAL_MAGIC);
      const ENUM_DEAL_REASON deal_reason=
         (ENUM_DEAL_REASON)HistoryDealGetInteger(transaction.deal,DEAL_REASON);
      const bool owned_deal=(entry_kind==DEAL_ENTRY_IN && deal_magic==InpMagic) ||
                            ((entry_kind==DEAL_ENTRY_OUT || entry_kind==DEAL_ENTRY_OUT_BY) &&
                             PositionOpenedByThisEa(position_id));
      if(owned_deal)
      {
         if((entry_kind==DEAL_ENTRY_OUT || entry_kind==DEAL_ENTRY_OUT_BY) &&
            ...
            !RegisterForcedStopout(transaction.deal,position_id))
            FailRuntime("stopout_registration_failed");
         const bool defer_close=(entry_kind==DEAL_ENTRY_OUT || entry_kind==DEAL_ENTRY_OUT_BY) &&
                                !PositionWasLoggedOpen(position_id);
         if(defer_close)
         {
            PrintFormat("STBS_DEAL_DEFER|deal=%I64u|position=%I64u|reason=%s",
                        transaction.deal,position_id,EnumToString(deal_reason));
         }
         else
         {
         if(!LogLifecycleDeal(transaction.deal))
            FailRuntime("lifecycle_deal_logging_failed");
         PrintFormat("STBS_DEAL|deal=%I64u|order=%I64u|position=%I64u|entry=%d|type=%d|reason=%s|volume=%.8f|price=%.8f|profit=%.8f|commission=%.8f|swap=%.8f",
                     transaction.deal,transaction.order,position_id,(int)entry_kind,
