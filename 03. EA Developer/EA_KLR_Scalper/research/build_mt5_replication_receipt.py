#!/usr/bin/env python3
"""Build hash-bound manual AlphaFactory receipts for the frozen KLR pair."""
...
HYPOTHESIS_ID = "HYP-KLR-MT5-REPLICATION-M5-XAU-001"
...
ACCEPTANCE = {
    ...
    "min_cost_pf_x2": 1.0,
    "max_monte_carlo_p95_dd_pct": 5.0,
}
STANDARD_INCLUDES = (
    ...
    "03. EA Developer/EA_KLR_Scalper/research/generated/KLR_DTWEXBGS_Data.mqh",
)
...
def normalized_overrides(require_usd: bool) -> str:
    values = dict(COMMON_INPUTS)
    values["InpRequireUsdGate"] = "true" if require_usd else "false"
...
def latest_registry_row(registry: Path) -> tuple[str, dict[str, Any]]:
    ...
    if row.get("state") != "screened" or row.get("model") != 0:
        raise ValueError("latest KLR replication registry row is not screened Model 0")
    return raw, row
...
def build(role: str, control_run: Path | None) -> tuple[Path, str]:
    ...
    # Pre-create every path that can affect `git status --untracked-files=all`.
    for name in (
    ...
    required.extend(root / Path(*item.split("/")) for item in STANDARD_INCLUDES)
    ...
    require_usd = role == "challenger"
    overrides = normalized_overrides(require_usd)
    task = {
    ...
    for index, relative in enumerate(STANDARD_INCLUDES, start=1):
        evidence.append(
    ...
    receipt = {
        ...
        "note": "Frozen KLR matched-pair diagnostic. Cost evidence is explicitly unverified and cannot support promotion.",
    }
