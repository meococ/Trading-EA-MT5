...
Removing the per-volume print alone would still exceed the former combined one-MiB raw cap. Raising the cap alone would retain more than 17 MiB of spam and also fail. HYP025 inherits the same two already-reviewed engineering controls:
...
2. Freeze an explicit combined raw journal cap of `4,194,304` bytes through the HYP025 task packet, receipt and AlphaFactory normalized manifest. Other hypotheses that omit the field retain the one-MiB default.
