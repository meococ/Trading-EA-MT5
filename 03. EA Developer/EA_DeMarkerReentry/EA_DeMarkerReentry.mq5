//+------------------------------------------------------------------+
//| EA_DeMarkerReentry.mq5                                             |
//| HYP-DMR-XAUUSD-M15-002: native DeMarker14 0.30/0.70 re-entry           |
//+------------------------------------------------------------------+
#property strict
#property version   "1.10"
#property description "Untuned XAUUSD M15 native DeMarker14 exhaustion re-entry"

input group "--- Frozen research authority ---"
input bool   InpResearchAutoMode=false;
input bool   InpEnableTelemetry=false;
input string InpHypothesisId="HYP-DMR-XAUUSD-M15-002";
input string InpVariantTag="DEMARKER14_030_070_REENTRY_BROKER_VALID_GEOMETRY";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5604302;
...
const string EA_NAME="EA_DeMarkerReentry";
const string EXPECTED_HYPOTHESIS="HYP-DMR-XAUUSD-M15-002";
const string EXPECTED_VARIANT="DEMARKER14_030_070_REENTRY_BROKER_VALID_GEOMETRY";
const string EXPECTED_SYMBOL="XAUUSD";
const int    ATR_PERIOD=14;
const int    DEMARKER_PERIOD=14;
...
int g_demarker_handle=INVALID_HANDLE;
bool g_indicators_ready=false;

// Legacy KVO state remains compile-isolated from the DeMarker execution path.
...
bool PreloadKvoState()
  {
   ...
   PrintFormat("DMR002_LEGACY_PRELOAD synchronized=%I64d requested=%d copied=%d first=%I64d last=%I64d current=%I64d ready=%s",
   ...
   if(!LoadClosedRates(rates))
     {
      ...
      g_runtime_failed=true;
       PrintFormat("DMR002_FATAL reason=LEGACY_CLOSED_RATE_LOAD availability=%I64d",
    ...
    if(decision_time<g_kvo_last_time)
      {
       g_invalid_bars++;
       g_runtime_failed=true;
       PrintFormat("DMR002_FATAL reason=LEGACY_CLOCK_REGRESSION decision=%I64d last=%I64d",
       ...
       if(!AdvanceKvoBar(rates[release],direction,ko,kvo_signal,ema100))
         {
          g_invalid_bars++;
          g_runtime_failed=true;
          PrintFormat("DMR002_FATAL reason=LEGACY_STATE decision=%I64d",(long)decision_time);
          return(false);
         }
    ...
    if(!ReadClosedAtr(atr_release) || atr_release<=0.0)
      {
       g_invalid_bars++;
       g_runtime_failed=true;
       PrintFormat("DMR002_FATAL reason=LEGACY_ATR_LOAD decision=%I64d",
                   (long)decision_time);
       return(false);
      }
...
bool PreloadDeMarkerState()
  {
   if(g_demarker_handle==INVALID_HANDLE || g_atr_handle==INVALID_HANDLE)
      return(false);
   const int demarker_bars=BarsCalculated(g_demarker_handle);
   const int atr_bars=BarsCalculated(g_atr_handle);
   if(demarker_bars<DEMARKER_PERIOD+2 || atr_bars<ATR_PERIOD+2)
      return(false);

   double demarker_values[];
   double atr_values[];
   ArraySetAsSeries(demarker_values,false);
   ArraySetAsSeries(atr_values,false);
   const int sample=16;
   if(CopyBuffer(g_demarker_handle,0,1,sample,demarker_values)!=sample ||
      CopyBuffer(g_atr_handle,0,1,sample,atr_values)!=sample)
      return(false);
   for(int i=0;i<sample;i++)
      if(!IsUsable(demarker_values[i]) || !IsUsable(atr_values[i]) || atr_values[i]<=0.0)
         return(false);

   PrintFormat("DMR002_PRELOAD demarker_bars=%d atr_bars=%d sample=%d latest_demarker=%.8f latest_atr=%.8f",
               demarker_bars,atr_bars,sample,demarker_values[sample-1],atr_values[sample-1]);
   return(true);
  }

bool ReadClosedDeMarkerAndAtr(double &current_demarker,double &prior_demarker,double &current_atr)
  {
   current_demarker=EMPTY_VALUE;
   prior_demarker=EMPTY_VALUE;
   ...
   ArraySetAsSeries(demarker_values,false);
   ArraySetAsSeries(atr_values,false);
   if(CopyBuffer(g_demarker_handle,0,1,2,demarker_values)!=2 ||
      CopyBuffer(g_atr_handle,0,1,1,atr_values)!=1)
      return(false);
   prior_demarker=demarker_values[0];
   current_demarker=demarker_values[1];
   current_atr=atr_values[0];
   return(IsUsable(prior_demarker) && IsUsable(current_demarker) &&
          IsUsable(current_atr) && current_atr>0.0);
  }

bool ProcessDeMarkerClosedBar(const datetime availability_time,SignalDecision &signal)
  {
   ZeroMemory(signal);
   MqlRates rates[];
   if(!LoadClosedRates(rates))
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=CLOSED_RATE_LOAD availability=%I64d",
                  (long)availability_time);
      return(false);
     }
   ...
   if(g_last_decision_time>0 && decision_time<g_last_decision_time)
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=DECISION_CLOCK_REGRESSION decision=%I64d last=%I64d",
   ...
   double current_demarker=0.0;
   double prior_demarker=0.0;
   double current_atr=0.0;
   if(!ReadClosedDeMarkerAndAtr(current_demarker,prior_demarker,current_atr))
     {
      g_invalid_bars++;
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=INDICATOR_LOAD decision=%I64d",
                  (long)decision_time);
      return(false);
     }

   int direction=0;
   if(prior_demarker<=0.30 && current_demarker>0.30)
      direction=1;
   else if(prior_demarker>=0.70 && current_demarker<0.70)
      direction=-1;
   if(direction==0)
      return(false);
   ...
   signal.demarker=current_demarker;
   signal.prior_demarker=prior_demarker;
   signal.structural_stop=stop;
   return(true);
  }
...
double FloorToTick(const double price)
  {
   ...
   if(GetLastError()!=0 || !IsUsable(tick_size) || tick_size<=0.0)
     {
      g_runtime_failed=true;
      Print("DMR002_FATAL reason=TICK_SIZE_PROPERTY");
      return(EMPTY_VALUE);
     }
...
bool ClosePositionTicket(const ulong ticket,const string reason)
  {
   if(ticket==0 || !PositionSelectByTicket(ticket))
     {
      g_runtime_failed=true;
       PrintFormat("DMR002_FATAL reason=CLOSE_POSITION_SELECT ticket=%I64u",ticket);
      return(false);
     }
   ResetLastError();
   ...
   if(property_error!=0 ||
     ...
     {
      g_runtime_failed=true;
       PrintFormat("DMR002_FATAL reason=CLOSE_POSITION_PROPERTIES ticket=%I64u error=%d type=%d volume=%.8f",
   ...
   if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
      g_runtime_failed=true;
       PrintFormat("DMR002_CLOSE_REJECT ticket=%I64u retcode=%u error=%d reason=%s",
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(ticket==0 || !PositionSelectByTicket(ticket))
        {
         g_runtime_failed=true;
         PrintFormat("DMR002_FATAL reason=POSITION_ENUMERATION index=%d",i);
         return(false);
        }
      ResetLastError();
      const string symbol=PositionGetString(POSITION_SYMBOL);
      ...
      if(GetLastError()!=0 ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("DMR002_FATAL reason=POSITION_PROPERTIES ticket=%I64u",ticket);
         return(false);
        }
      if(symbol!=_Symbol || magic!=InpMagic)
         continue;
      if(flatten_time)
        {
          if(!ClosePositionTicket(ticket,"DMR_WEEKEND_OR_END"))
            return(false);
         continue;
        }
      if(update_exit)
        {
         ...
         if(shift<0 || GetLastError()!=0)
           {
            g_runtime_failed=true;
             PrintFormat("DMR002_FATAL reason=ENTRY_BAR_RECOVERY ticket=%I64u",ticket);
            return(false);
           }
          if(shift>=MAX_HOLD_BARS && !ClosePositionTicket(ticket,"DMR_TIME_EXIT"))
            return(false);
        }
     }
   return(true);
...
bool CalculateVolume(const ENUM_ORDER_TYPE order_type,const double entry,
                      const double stop,double &volume)
  {
   ...
   if(!OrderCalcProfit(order_type,_Symbol,1.0,entry,stop,one_lot_profit) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=ORDER_CALC_PROFIT error=%d value=%.8f",
   ...
   if(account_error!=0 || !IsUsable(equity) || equity<=0.0 ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=MARGIN_ACCOUNT_STATE error=%d mode=%d equity=%.2f margin=%.2f call=%.2f stopout=%.2f",
   ...
   if(!IsUsable(raw_volume) || !IsUsable(minimum) || !IsUsable(maximum) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=VOLUME_PROPERTIES error=%d min=%.8f max=%.8f step=%.8f",
   ...
   if(!OrderCalcMargin(order_type,_Symbol,1.0,entry,margin_one_lot) ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=MARGIN_ONE_LOT error=%d value=%.8f",
   ...
   for(int guard=0;guard<1024 && sized>=minimum-1e-9;guard++)
     {
      ...
      if(!OrderCalcMargin(order_type,_Symbol,sized,entry,exact_margin) ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("DMR002_FATAL reason=MARGIN_EXACT error=%d volume=%.8f value=%.8f",
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   if(!SymbolInfoTick(_Symbol,tick) || GetLastError()!=0 ||
     ...
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=SYMBOL_TICK error=%d bid=%.8f ask=%.8f",
   ...
   if(GetLastError()!=0 || stops_level<0)
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=STOPS_LEVEL_PROPERTY error=%d value=%I64d",
   ...
   if(!check_ok || check_error!=0 || check.retcode!=0)
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=ORDER_CHECK error=%d retcode=%u comment=%s bid=%.8f ask=%.8f entry=%.8f sl=%.8f tp=%.8f stop_distance=%.8f target_distance=%.8f minimum_distance=%.8f",
   ...
   if(!send_ok && !definitive_no_fill)
     {
      g_runtime_failed=true;
      PrintFormat("DMR002_FATAL reason=ENTRY_SEND_TRANSPORT decision=%I64d retcode=%u error=%d order=%I64u deal=%I64u",
   ...
   if(!AcceptedRetcode(result.retcode))
     {
      if(!definitive_no_fill)
        {
         g_runtime_failed=true;
         PrintFormat("DMR002_FATAL reason=ENTRY_REJECT_UNKNOWN decision=%I64d retcode=%u error=%d order=%I64u deal=%I64u",
                     (long)signal.decision_time,result.retcode,send_error,
                     result.order,result.deal);
         return(false);
        }
      ...
      if(!OwnedPositionCount(positions) || !OwnedOrderCount(orders) ||
        ...
        {
         g_runtime_failed=true;
         PrintFormat("DMR002_FATAL reason=ENTRY_REJECT_AMBIGUOUS decision=%I64d retcode=%u error=%d positions=%d orders=%d",
                     (long)signal.decision_time,result.retcode,send_error,
                     positions,orders);
         return(false);
        }
   ...
   if(!OwnedPositionCount(positions) || !OwnedOrderCount(orders))
     {
      g_runtime_failed=true;
      g_entry_rejects++;
       Print("DMR002_FATAL reason=OWNED_INVENTORY_UNCERTAIN");
      return;
     }
...
bool ValidateFrozenInputs()
  {
   return(_Symbol==EXPECTED_SYMBOL && _Period==PERIOD_M15 &&
          ...
          InpHypothesisId==EXPECTED_HYPOTHESIS && InpVariantTag==EXPECTED_VARIANT &&
           InpMagic==5604302 && MathAbs(InpRiskPercent-0.25)<1e-12 &&
...
int OnInit()
  {
   if(!ValidateFrozenInputs())
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=FROZEN_INPUT_OR_SYMBOL_CONTRACT");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(!CurrentM15Open(g_last_bar_open))
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=INITIAL_M15_CLOCK");
      return(INIT_FAILED);
     }
   if(!EmitD0SeriesProof())
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=D0_SERIES_PROOF");
      return(INIT_FAILED);
     }
   long filling_mode=0;
   if(!SymbolInfoInteger(_Symbol,SYMBOL_FILLING_MODE,filling_mode) ||
     ...
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=FOK_NOT_SUPPORTED");
      return(INIT_FAILED);
     }
    g_atr_handle=iATR(_Symbol,PERIOD_M15,ATR_PERIOD);
    g_demarker_handle=iDeMarker(_Symbol,PERIOD_M15,DEMARKER_PERIOD);
    if(g_atr_handle==INVALID_HANDLE || g_demarker_handle==INVALID_HANDLE)
      {
       g_runtime_failed=true;
       Print("DMR002_FATAL reason=INDICATOR_HANDLE");
       return(INIT_FAILED);
      }
    g_indicators_ready=PreloadDeMarkerState();
   const double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   if(!IsUsable(equity) || equity<=0.0)
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=INITIAL_EQUITY");
      return(INIT_FAILED);
     }
   g_daily_day_key=DateKey(TimeCurrent());
   g_daily_start_equity=equity;
   g_peak_equity=equity;
    PrintFormat("DMR002_INIT hypothesis=%s variant=%s symbol=%s period=%d demarker_period=%d atr_period=%d",
                InpHypothesisId,InpVariantTag,_Symbol,(int)_Period,DEMARKER_PERIOD,ATR_PERIOD);
...
void OnDeinit(const int reason)
  {
   ...
   if(g_demarker_handle!=INVALID_HANDLE)
      IndicatorRelease(g_demarker_handle);
   PrintFormat("DMR002_SUMMARY hypothesis=%s closed_bars=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d geometry_rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d runtime_failed=%s reason=%d",
...
void OnTick()
  {
   ...
   if(!g_indicators_ready)
     {
      ...
      if(!CurrentM15Open(warmup_open))
        {
         g_runtime_failed=true;
         Print("DMR002_FATAL reason=WARMUP_M15_CLOCK");
         return;
        }
   ...
   if(!CurrentM15Open(current_open))
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=M15_SCHEDULING_CLOCK");
      return;
     }
   ...
   if(current_open<g_last_bar_open)
     {
      g_runtime_failed=true;
       Print("DMR002_FATAL reason=BAR_CLOCK_REGRESSION");
      return;
     }
   ...
   if(ProcessDeMarkerClosedBar(current_open,signal) && signal.fired)
