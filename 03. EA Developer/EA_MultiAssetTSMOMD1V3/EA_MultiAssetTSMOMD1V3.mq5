//+------------------------------------------------------------------+
//| EA_MultiAssetTSMOMD1V3.mq5                                      |
//| HYP-MULTI-TSMOM-D1-003 - frozen nine-asset design baseline      |
//+------------------------------------------------------------------+
...
input ulong  InpMagic=260812005;
input int    InpDeviationPoints=20;

const string HYPOTHESIS_ID="HYP-MULTI-TSMOM-D1-003";
...
datetime g_next_rebalance_retry=0;
int      g_last_monday_key=0;
int      g_pending_monday_key=0;
datetime g_pending_decision_time=0;
bool     g_pending_snapshot_ready=false;
bool     g_pending_volume_plan_ready=false;
bool     g_weekly_execution_phase=false;
bool     g_common_readiness_armed=false;
int      g_day_key=0;
...
long g_risk_kills=0;
long g_market_closed_rejects=0;
long g_rebalance_retries=0;
long g_partial_unwinds=0;
long g_completed_rebalances=0;
long g_failed_rebalances=0;
long g_stale_quote_waits=0;
long g_closed_session_waits=0;
long g_partial_close_events=0;
long g_partial_open_events=0;
long g_orders_without_common_readiness=0;
long g_basket_recomputations=0;
   ...
   for(uint session=0;session<32;session++)
     {
      ...
      if(!TimeToStruct(from,from_stamp) || !TimeToStruct(to,to_stamp))
         continue;
      const int from_second=from_stamp.hour*3600+from_stamp.min*60+from_stamp.sec;
      const int to_second=to_stamp.hour*3600+to_stamp.min*60+to_stamp.sec;
      if(from_second==to_second)
         return true;
      if(from_second<to_second && second_of_day>=from_second && second_of_day<to_second)
         return true;
      if(from_second>to_second && (second_of_day>=from_second || second_of_day<to_second))
         return true;
     }
...
bool CommonMarketReady(const datetime retry_time)
  {
   bool ready=true;
   for(int i=0;i<ASSET_COUNT;i++)
     {
      ...
      if(!SymbolInfoTick(g_symbols[i],tick) || !ValidTick(tick) ||
         tick.time>retry_time || retry_time-tick.time>60)
        {
         g_stale_quote_waits++;
         ready=false;
         continue;
        }
      if(!IsTradeSessionOpen(g_symbols[i],retry_time))
        {
...
bool CloseOwnedPosition(const ulong ticket,const string comment)
  {
   if(g_weekly_execution_phase && !g_common_readiness_armed)
     {
      g_orders_without_common_readiness++;
      return false;
     }
   ...
   ResetLastError();
    if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
       g_close_rejects++;
       if(result.retcode==TRADE_RETCODE_DONE_PARTIAL && g_weekly_execution_phase)
          g_partial_close_events++;
       if(result.retcode==TRADE_RETCODE_MARKET_CLOSED)
          g_market_closed_rejects++;
       return false;
      }
   g_closes_accepted++;
   return true;
...
bool CloseAllOwned(const string comment)
  {
   bool accepted=true;
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(ticket==0 || !PositionSelectByTicket(ticket))
       ...
       if(PositionGetInteger(POSITION_MAGIC)!=(long)InpMagic)
          continue;
       if(!CloseOwnedPosition(ticket,comment))
          accepted=false;
      }
   return accepted && OwnedPositionCount()==0;
  }

bool SubmitPlannedEntry(const int index)
  {
   if(!g_common_readiness_armed)
     {
      g_orders_without_common_readiness++;
      return false;
     }
...
bool PrepareMondaySnapshot(const datetime decision_time)
  {
   g_monday_attempts++;
...
void ProcessMonday(const datetime now)
  {
   ...
   if(stamp.day_of_week!=1)
     {
      if(g_pending_monday_key>0 && g_pending_monday_key!=g_last_monday_key &&
         DateKey(now)>g_pending_monday_key)
        {
         ...
         PrintFormat("MTS_REBALANCE_EXPIRED monday_key=%d now=%I64d",
                     g_pending_monday_key,(long)now);
        }
      return;
     }
   ...
   if(monday_key!=g_pending_monday_key)
     {
      ...
      g_pending_snapshot_ready=false;
      g_pending_volume_plan_ready=false;
      g_next_rebalance_retry=0;
      g_risk_locked=false;
      g_week_start_equity=AccountInfoDouble(ACCOUNT_EQUITY);
     }
   else if(g_next_rebalance_retry>0 && now<g_next_rebalance_retry)
      return;

   // Freeze signal, volatility and weights once at the first Monday tick.
   // They are never recomputed while waiting for a common tradable market.
   if(!g_pending_snapshot_ready)
     {
      if(!PrepareMondaySnapshot(g_pending_decision_time))
        {
         g_failed_rebalances++;
         g_last_monday_key=monday_key;
         return;
        }
      g_pending_snapshot_ready=true;
     }

   // No close or open request is sent before every symbol has a fresh quote
   // and its published Monday trading session is open at this causal retry.
   if(!CommonMarketReady(now))
     {
      g_rebalance_retries++;
      g_next_rebalance_retry=now+15;
      return;
     }

   g_weekly_execution_phase=true;
   g_common_readiness_armed=true;
   const int old_positions=OwnedPositionCount();
   if(!CloseAllOwned("MTS_WEEKLY"))
     {
      const int remaining=OwnedPositionCount();
      if(old_positions>0 && remaining>0 && remaining<old_positions)
         g_partial_close_events++;
      g_common_readiness_armed=false;
      g_weekly_execution_phase=false;
      g_rebalance_retries++;
      g_next_rebalance_retry=now+15;
      return;
     }

   // Convert frozen weights to lots exactly once, after the old basket is flat,
   // using only the current common-ready quotes and current account equity.
   if(!g_pending_volume_plan_ready)
     {
      if(!PlanVolumes())
        {
         g_common_readiness_armed=false;
      ...
      g_pending_volume_plan_ready=true;
     }
   ...
   for(int i=0;i<ASSET_COUNT;i++)
     {
      if(g_target_volume[i]<=0.0 || g_signal[i]==0.0)
         continue;
      planned++;
      if(SubmitPlannedEntry(i))
         accepted++;
     }

   if(planned>0 && accepted==planned && OwnedPositionCount()==planned)
     {
      g_common_readiness_armed=false;
      g_weekly_execution_phase=false;
      g_last_monday_key=monday_key;
      ...
      PrintFormat("MTS_REBALANCE_COMPLETE decision=%I64d execution=%I64d latency_sec=%I64d equity=%.2f planned=%d accepted=%d cumulative_entries=%I64d usd_factor=%.8f",
                  (long)g_pending_decision_time,(long)now,latency,
                  AccountInfoDouble(ACCOUNT_EQUITY),planned,accepted,
                  g_entries_accepted,UsdFactorExposure(g_weight));
      return;
     }

   // Any surviving position after a failed all-leg submit is a partial-open
   // event. Unwind it, continue only for diagnostics, and invalidate economics.
   if(OwnedPositionCount()>0)
     {
      g_partial_open_events++;
      g_partial_unwinds++;
      CloseAllOwned("MTS_RETRY_UNWIND");
     }
   g_common_readiness_armed=false;
   g_weekly_execution_phase=false;
   g_rebalance_retries++;
   g_next_rebalance_retry=now+15;
   if(g_rebalance_retries<=20)
      PrintFormat("MTS_REBALANCE_RETRY phase=open monday_key=%d now=%I64d planned=%d accepted=%d owned=%d",
                  monday_key,(long)now,planned,accepted,OwnedPositionCount());
  }
...
int OnInit()
  {
   if(_Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_H1 || !InpResearchAutoMode ||
      InpMagic!=260812005 || InpDeviationPoints!=20 ||
   ...
   PrintFormat("MTS_ACCOUNT_MARGIN mode=%d call=%.8f stopout=%.8f leverage=%I64d",
...
void OnDeinit(const int reason)
  {
   ...
   const double atomic_ratio=(g_valid_baskets>0 ?
                              (double)g_completed_rebalances/(double)g_valid_baskets : 0.0);
   const bool engineering_pass=(g_monday_attempts==208 && source_pass &&
                                atomic_ratio>=0.95 && g_partial_close_events==0 &&
                                g_partial_open_events==0 && g_partial_unwinds==0 &&
                                g_orders_without_common_readiness==0 &&
                                g_basket_recomputations==0);
   PrintFormat("MTS_EXEC_SUMMARY hypothesis_id=%s attempted=%I64d valid=%I64d completed_atomic=%I64d atomic_ratio=%.8f failed=%I64d retries=%I64d stale_quote_waits=%I64d closed_session_waits=%I64d market_closed_rejects=%I64d partial_close=%I64d partial_open=%I64d partial_unwinds=%I64d orders_without_common_readiness=%I64d basket_recomputations=%I64d max_latency_sec=%I64d engineering_gate_pass=%s",
               HYPOTHESIS_ID,g_monday_attempts,g_valid_baskets,g_completed_rebalances,
               atomic_ratio,g_failed_rebalances,g_rebalance_retries,g_stale_quote_waits,
               g_closed_session_waits,g_market_closed_rejects,g_partial_close_events,
               g_partial_open_events,g_partial_unwinds,g_orders_without_common_readiness,
               g_basket_recomputations,g_max_rebalance_latency_seconds,
               (string)engineering_pass);
   PrintFormat("MTS_ECON_SUMMARY hypothesis_id=%s ticks=%I64d entries_requested=%I64d entries_accepted=%I64d closes_requested=%I64d closes_accepted=%I64d below_min_volume=%I64d margin_scaled=%I64d order_check_rejects=%I64d order_send_rejects=%I64d close_rejects=%I64d market_closed_rejects=%I64d rebalance_retries=%I64d partial_unwinds=%I64d completed_rebalances=%I64d risk_kills=%I64d deal_profit=%.2f deal_swap=%.2f deal_commission=%.2f deal_net=%.2f",
                ...
                g_order_check_rejects,g_order_send_rejects,g_close_rejects,
                g_market_closed_rejects,g_rebalance_retries,g_partial_unwinds,
                g_completed_rebalances,g_risk_kills,
                g_deal_profit,g_deal_swap,g_deal_commission,
                g_deal_profit+g_deal_swap+g_deal_commission);
  }

// Frozen successor: same alpha; common-session/fresh-quote atomicity barrier.
//+------------------------------------------------------------------+
