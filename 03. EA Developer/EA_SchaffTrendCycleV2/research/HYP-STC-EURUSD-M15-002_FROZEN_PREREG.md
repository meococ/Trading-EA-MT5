# HYP-STC-EURUSD-M15-002 â€” frozen untuned baseline

Status: `FROZEN_PRE_BASELINE`

## Engineering revision lineage

Parent `HYP-STC-EURUSD-M15-001` is terminally killed as
`KILL_ENGINEERING_PRELOAD_READINESS_PRECHECK`: its sole MT5 attempt stopped in
`OnInit` with zero bars, ticks, signals, orders and returns, so no economic
result was opened. HYP002 changes only fresh identity/magic and removes the
premature `SERIES_SYNCHRONIZED` sample. Exact population, endpoints, recurrence,
indicator, signal, execution, risk, cost, DESIGN and sealed-window rules remain
unchanged. Detailed preload diagnostics are added; no wait/retry or relaxed
history rule is allowed.

Parent failure packet:
`03. EA Developer/EA_SchaffTrendCycle/research/HYP-STC-EURUSD-M15-001_FAILURE_PACKET.json`,
SHA256 `2611B601B65BFAFD749FE740A9DA0CAFA5A5DF073B5995A99A40922070D7ECF4`.
...
- Recurrence is initialized once from the exact native-M15 preload population:
  ...
  completed M15 bar. Missing, late or incomplete `CopyRates` prehistory fails
  closed and no rolling-window reseeding is allowed. The origin proof is frozen in
  `HYP-STC-EURUSD-M15-002_PRELOAD_ORIGIN_PROOF.json`.
