#!/usr/bin/env python3
"""Outcome-blind native-H1 Vortex-14 polarity-crossover source analyzer."""
...
HYPOTHESIS_ID="HYP-VTX-XAUUSD-H1-001"
ATTEMPT_ID="VTX001-SOURCE-ATTEMPT-001"
PREREG_SHA256="A21B969D644CC935552ED4AF5C8DD1D1EDAF14F97BD518C532F944CBBB957B78"
...
EVENT_KEYS={"hypothesis_id","source_bar_time_utc","decision_time_utc","direction","prior_vi_plus","prior_vi_minus","vi_plus","vi_minus"}
...
def validate_selected_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data=frame.loc[:,REQUIRED_COLUMNS].copy(); data["time_utc"]=pd.to_datetime(data["time_utc"],utc=True,errors="raise")
    if ((data["time_utc"]<DESIGN_START)|(data["time_utc"]>=DESIGN_END)).any(): raise ValueError("reader materialized rows outside the frozen design window")
    data=data.reset_index(drop=True)
    if len(data)<MIN_ROWS: raise ValueError(f"design rows {len(data)} below {MIN_ROWS}")
...
def calculate_vortex(data: pd.DataFrame) -> dict[str,pd.Series]:
    high=data["high"].astype(float); low=data["low"].astype(float); close=data["close"].astype(float)
    finite=np.isfinite(np.column_stack((high,low,close))).all(axis=1)
    row_valid=pd.Series(finite&(high.to_numpy()>low.to_numpy())&(close.to_numpy()>=low.to_numpy())&(close.to_numpy()<=high.to_numpy()),index=data.index,dtype=bool)
    pair_valid=row_valid&row_valid.shift(1,fill_value=False)
    tr=pd.concat(((high-low).abs(),(high-close.shift(1)).abs(),(low-close.shift(1)).abs()),axis=1).max(axis=1)
    vm_plus=(high-low.shift(1)).abs(); vm_minus=(low-high.shift(1)).abs()
    valid14=pair_valid.astype(int).rolling(PERIOD,min_periods=PERIOD).sum().eq(PERIOD)
    tr14=tr.rolling(PERIOD,min_periods=PERIOD).sum(); plus14=vm_plus.rolling(PERIOD,min_periods=PERIOD).sum(); minus14=vm_minus.rolling(PERIOD,min_periods=PERIOD).sum()
    vi_plus=pd.Series(np.nan,index=data.index,dtype=float); vi_minus=pd.Series(np.nan,index=data.index,dtype=float)
    valid=valid14&np.isfinite(tr14)&np.isfinite(plus14)&np.isfinite(minus14)&(tr14>0)
    vi_plus.loc[valid]=plus14.loc[valid]/tr14.loc[valid]; vi_minus.loc[valid]=minus14.loc[valid]/tr14.loc[valid]
    feature_valid=valid&valid.shift(1,fill_value=False)&pd.Series(np.isfinite(np.column_stack((vi_plus,vi_minus,vi_plus.shift(1),vi_minus.shift(1)))).all(axis=1),index=data.index,dtype=bool)
    return {"tr":tr,"vm_plus":vm_plus,"vm_minus":vm_minus,"vi_plus":vi_plus,"vi_minus":vi_minus,"feature_valid":feature_valid}
...
def analyze_frame(frame: pd.DataFrame) -> tuple[list[dict[str,Any]],dict[str,Any]]:
    ...
    for column in ("high","low","close"): data[column]=pd.to_numeric(data[column],errors="raise")
    indicator=calculate_vortex(data); plus=indicator["vi_plus"]; minus=indicator["vi_minus"]; usable=indicator["feature_valid"]
    raw_long=usable&(plus.shift(1)<=minus.shift(1))&(plus>minus); raw_short=usable&(plus.shift(1)>=minus.shift(1))&(plus<minus)
    conflicts=raw_long&raw_short; raw_long&=~conflicts; raw_short&=~conflicts; raw_mask=raw_long|raw_short
    exact_next=(data["time_utc"].shift(-1)-data["time_utc"])==pd.Timedelta(hours=1); event_mask=raw_mask&exact_next
    events=[]
    for index in data.index[event_mask]:
        bar_time=data.at[index,"time_utc"]
        events.append({"hypothesis_id":HYPOTHESIS_ID,"source_bar_time_utc":bar_time.isoformat().replace("+00:00","Z"),"decision_time_utc":(bar_time+pd.Timedelta(hours=1)).isoformat().replace("+00:00","Z"),"direction":"LONG" if bool(raw_long.loc[index]) else "SHORT","prior_vi_plus":finite_float(plus.shift(1).loc[index]),"prior_vi_minus":finite_float(minus.shift(1).loc[index]),"vi_plus":finite_float(plus.loc[index]),"vi_minus":finite_float(minus.loc[index])})
    raw_count=int(raw_mask.sum()); count=len(events); gap_rejected=raw_count-count; elapsed_weeks=(DESIGN_END-DESIGN_START).total_seconds()/604800.0; cadence=count/elapsed_weeks
    ...
    feature_coverage=int(usable.iloc[WARMUP_ROWS:].sum())/max(len(data)-WARMUP_ROWS,1); next_coverage=count/max(raw_count,1)
    ...
    gates={"minimum_design_rows":len(data)>=MIN_ROWS,"feature_coverage":feature_coverage>=MIN_FEATURE_COVERAGE,"raw_event_exact_next_coverage":next_coverage>=MIN_NEXT_COVERAGE,"minimum_events":count>=MIN_EVENTS,"pooled_cadence":MIN_CADENCE<=cadence<=MAX_CADENCE,"direction_balance":long_share>=MIN_DIRECTION_SHARE and short_share>=MIN_DIRECTION_SHARE,"year_concentration":max_year_share<=MAX_YEAR_SHARE,"each_year_cadence":all(MIN_YEAR_CADENCE<=row["cadence_per_week"]<=MAX_YEAR_CADENCE for row in yearly.values()),"zero_direction_conflicts":int(conflicts.sum())==0}
    passed=all(gates.values())
    report={"schema_version":"vortex14_source_report.v1","hypothesis_id":HYPOTHESIS_ID,"attempt_id":ATTEMPT_ID,"epistemic_scope":"OUTCOME_BLIND_VORTEX14_POLARITY_AND_CADENCE_ONLY","window":{"from":DESIGN_START.isoformat(),"to_exclusive":DESIGN_END.isoformat()},"parameters":{"timeframe":"H1","period":14,"scale":"unscaled_ratio"},"funnel":{"design_rows":int(len(data)),"feature_usable_rows":int(usable.sum()),"raw_events":raw_count,"executable_events":count,"gap_rejected_events":gap_rejected,"direction_conflicts":int(conflicts.sum()),"long_events":longs,"short_events":shorts},"metrics":{"elapsed_weeks":elapsed_weeks,"feature_coverage":feature_coverage,"raw_event_exact_next_coverage":next_coverage,"event_cadence_per_week":cadence,"long_share":long_share,"short_share":short_share,"max_year_event_share":max_year_share},"yearly":yearly,"gates":gates,"all_gates_pass":passed,"verdict":"SCREENED_SOURCE_PASS_MQL5_DIRECT_VORTEX_BUILD_AUTHORIZED" if passed else "PARK_SOURCE_FEASIBILITY_EXACT_VORTEX14_POLARITY","prohibitions":{"post_event_ohlc_read":False,"returns_computed":False,"trades_simulated":False,"economics_executed":False,"validation_opened":False,"holdout_opened":False,"mql5_build_authorized_by_attempt":passed,"economic_build_authorized":False,"native_ivortex_claim_authorized":False,"live_trading_authorized":False}}
...
def validate_manifest(manifest_path: Path,data_path: Path) -> None:
    ...
    if len(matches)!=1 or matches[0].get("sha256")!=DATA_SHA256: raise ValueError("manifest does not bind frozen H1 data")
...
def claim_attempt(output_dir: Path,authority: dict[str,str]) -> tuple[str,Path]:
    ...
    output_dir.mkdir(parents=True,exist_ok=True); marker_path=output_dir/"attempt_started.json"; started=datetime.now(timezone.utc).isoformat().replace("+00:00","Z")
    marker={"schema_version":"vortex14_attempt_started.v1","hypothesis_id":HYPOTHESIS_ID,"attempt_id":ATTEMPT_ID,"started_at_utc":started,"process_id":os.getpid(),"registry_sha256":authority["registry_sha256"],"latest_hypothesis_row_sha256":authority["latest_row_sha256"],"analyzer_sha256":sha256_file(Path(__file__).resolve()),"status":"ATTEMPT_CLAIMED_SOURCE_NOT_YET_OPENED"}
    try:
...
def execute(root: Path) -> dict[str,Any]:
    prereg=root/"03. EA Developer/EA_VortexPolarity/research/HYP-VTX-XAUUSD-H1-001_FROZEN_PREREG.md"; manifest=root/"02. AlphaFactory/data/fivepercent/FiveAssetFoundation/DATA-FIVEPERCENT-5ASSET-MULTITF-004/manifest.json"; data_path=root/"02. AlphaFactory/data/fivepercent/FiveAssetFoundation/DATA-FIVEPERCENT-5ASSET-MULTITF-004/XAUUSD/XAUUSD_H1_ALL_AVAILABLE_20260801.parquet"; registry=root/"04. Memory/research/CANDIDATE_REGISTRY.jsonl"; output_dir=root/"03. EA Developer/EA_VortexPolarity/research/evidence/HYP-VTX-XAUUSD-H1-001/VTX001-SOURCE-ATTEMPT-001"
    ...
    raw=pd.read_parquet(data_path,columns=list(REQUIRED_COLUMNS),filters=[("time_utc",">=",DESIGN_START.to_pydatetime()),("time_utc","<",DESIGN_END.to_pydatetime())],engine="pyarrow"); selected=validate_selected_frame(raw)
    events,report=analyze_frame(selected); assert_outcome_blind(events,report); replay_events,replay_report=analyze_frame(selected)
