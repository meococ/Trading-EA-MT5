//+------------------------------------------------------------------+
//| EA_H1ATRRegimeMom.mq5 — H1 ATR-regime directional momentum         |
//| Symbol: USDJPY | Period: H1 | Magic: 880950                        |
//|                                                                    |
//| Hypothesis: HYP-H1-ATR-REGIME-MOM-001                               |
//| Elevated vol (ATR14 / SMA50(ATR) >= gate) + close vs EMA50         |
//| continuation on closed H1 bar[1]. Not stretch-fade, not VolExp     |
//| M15 single-bar break, not Chop CI, not ORB/PDH/NY/ITSM/SB.         |
//|                                                                    |
//| Mon–Thu a priori; weekend flat; risk 0.5%. No day/threshold mining.|
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_H1ATRRegimeMom"
...
input group "=== General ==="
input ulong    InpMagic         = 880950;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== ATR Regime + Direction ==="
input int      InpATRPeriod     = 14;
input int      InpATRSlow       = 50;     // SMA of ATR (regime baseline)
input double   InpATRRatioMin   = 1.20;   // elevated-vol gate (frozen a priori)
input int      InpEMAPeriod     = 50;
input double   InpBodyRatio     = 0.35;   // min body/range on signal bar

input group "=== Session (server hours) ==="
input int      InpTradeStart    = 1;      // H1: allow most of day
...
input int      InpMaxHoldBars   = 24;     // H1 bars
...
int OnInit()
{
   if(Period() != PERIOD_H1)
   {
      Print("[H1ARM] FAIL — chart must be H1");
      return INIT_FAILED;
   }
   g_trade.SetExpertMagicNumber((long)InpMagic);
   g_trade.SetDeviationInPoints(InpDeviation);
   g_hATR = iATR(_Symbol, PERIOD_H1, InpATRPeriod);
   g_hEMA = iMA(_Symbol, PERIOD_H1, InpEMAPeriod, 0, MODE_EMA, PRICE_CLOSE);
   if(g_hATR == INVALID_HANDLE || g_hEMA == INVALID_HANDLE)
      return INIT_FAILED;
   g_dayStartBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   PrintFormat("[H1ARM] HYP-H1-ATR-REGIME-MOM-001 | ATRRatio>=%.2f EMA=%d TP=%.2fR risk=%.2f%%",
               InpATRRatioMin, InpEMAPeriod, InpTP_Ratio, InpRiskPct);
...
//+------------------------------------------------------------------+
// Closed H1 bar[1] only. Regime = ATR[1] / mean(ATR[1..slow]).
int GetSignal()
{
   int need = InpATRSlow + 2;
   double atr[];
   ...
   if(atrSlow <= 0.0)
      return 0;
   double ratio = atr[0] / atrSlow;
   if(ratio < InpATRRatioMin)
      return 0;
...
void OnTick()
{
   ...
   if(IsSuccessfulRetcode(res.retcode))
   {
      g_tradesToday++;
      g_holdBars = 0;
      PrintFormat("[H1ARM] %s lot=%.2f @ %.5f ATR=%.5f",
                  isBuy ? "BUY" : "SELL", lot, res.price, atr[0]);
   }
}
