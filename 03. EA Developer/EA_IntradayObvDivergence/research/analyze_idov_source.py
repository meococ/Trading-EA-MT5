...
HYPOTHESIS_ID = "HYP-IDOV-XAUUSD-M5-001"
ATTEMPT_ID = "IDOV001-SOURCE-001"
...
PREREG_PATH = RESEARCH_DIR / "HYP-IDOV-XAUUSD-M5-001_FROZEN_SOURCE_PREREG.md"
REVIEW_PATH = RESEARCH_DIR / "HYP-IDOV-XAUUSD-M5-001_PRE_SOURCE_REVIEW.md"
TEST_PATH = RESEARCH_DIR / "tests/test_analyze_idov_source.py"
...
def claim_attempt() -> None:
    ...
    write_exclusive(START_PATH, json_bytes({
        "schema_version": "idov_source_attempt_started.v1",
...
def measure_session(group: pd.DataFrame) -> dict | None:
    ...
    volumes = ordered["tick_volume"].astype(float).to_numpy()
    if (len(closes) != 192 or np.any(~np.isfinite(closes)) or np.any(closes <= 0.0)
            or np.any(~np.isfinite(volumes)) or np.any(volumes <= 0.0)):
        return None
    changes = closes[1:] - closes[:-1]
    signed_flow = float(np.sum(np.sign(changes) * volumes[1:]))
    session_return = math.log(float(closes[-1]) / float(closes[0]))
    if not math.isfinite(signed_flow) or not math.isfinite(session_return):
        return None
    return {
        "decision_index": int(ordered.index[-1]),
        "signed_tick_volume_flow": signed_flow,
        "session_return": session_return,
        "up_bars": int(np.count_nonzero(changes > 0.0)),
        "down_bars": int(np.count_nonzero(changes < 0.0)),
        "flat_bars": int(np.count_nonzero(changes == 0.0)),
    }


def select_direction(measure: dict) -> str | None:
    if measure["session_return"] < 0.0 and measure["signed_tick_volume_flow"] > 0.0:
        return "LONG"
    if measure["session_return"] > 0.0 and measure["signed_tick_volume_flow"] < 0.0:
        return "SHORT"
    return None
...
def extract_events(frame: pd.DataFrame) -> tuple[list[dict], dict]:
    ...
    for day, group in intraday.groupby("utc_date", sort=True):
        ...
        raw.append({
            ...
            "direction": direction,
            "signed_tick_volume_flow": measure["signed_tick_volume_flow"],
            "session_return": measure["session_return"],
            "up_bars": measure["up_bars"],
            "down_bars": measure["down_bars"],
            "flat_bars": measure["flat_bars"],
...
def analyze(frame: pd.DataFrame, validation: dict) -> tuple[dict, list[dict]]:
    ...
    return {
        "schema_version": "idov_source_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "outcomes_opened": False,
        "economics_evaluated": False,
        "parameters": {"session_utc": "00:00-15:55", "flow": "obv_signed_tick_volume",
                       "event": "strict_price_flow_divergence_fade"},
...
def execute() -> dict:
    ...
    try:
        ...
        write_exclusive(RECEIPT_PATH, json_bytes({
            "schema_version": "idov_source_attempt_receipt.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
        ...
        write_exclusive(TERMINAL_PATH, json_bytes({
            "schema_version": "idov_source_attempt_terminal.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
            "status": "COMPLETE",
