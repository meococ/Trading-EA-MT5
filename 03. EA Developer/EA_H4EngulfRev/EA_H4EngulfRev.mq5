//+------------------------------------------------------------------+
//| EA_H4EngulfRev.mq5 — H4 body-engulf reversal accept RR=3         |
//| Symbol: USDJPY | Period: H4 | Magic: 880992                       |
//| Hypothesis: HYP-H4-ENGULF-REV-001                                  |
//| Closed-bar[1] only. Mon–Thu; weekend flat; risk 0.5%; RR=3.0.     |
//| Not M15 EngulfTrend retune; not Outside+WR7.                      |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_H4EngulfRev"
...
input group "=== Engulf ==="
input double   InpMinBodyATR    = 0.25;   // engulf body min vs ATR
...
int OnInit()
{
   if(Period() != PERIOD_H4)
   {
      Print("[ENGULF] FAIL — chart must be H4");
      return INIT_FAILED;
   }
   ...
   Print("[ENGULF] HYP-H4-ENGULF-REV-001 RR=", InpTP_Ratio);
...
// bar[2] body engulfs bar[3] body; bar[1] accepts mid → trade engulf direction
bool DetectEngulfAccept(bool &isBuy, double &slExtreme)
{
   isBuy = false;
   slExtreme = 0.0;
   ...
   double o3 = iOpen(_Symbol, PERIOD_H4, 3);
   double c3 = iClose(_Symbol, PERIOD_H4, 3);
   double c1 = iClose(_Symbol, PERIOD_H4, 1);
   if(o2 == c2 || o3 == c3)
      return false;

   double body2Hi = MathMax(o2, c2);
   double body2Lo = MathMin(o2, c2);
   double body3Hi = MathMax(o3, c3);
   double body3Lo = MathMin(o3, c3);
   if(!(body2Hi > body3Hi && body2Lo < body3Lo))
      return false;

   double atr[];
   ArraySetAsSeries(atr, true);
   if(CopyBuffer(g_hATR, 0, 2, 1, atr) < 1 || atr[0] <= 0.0)
      return false;
   if((body2Hi - body2Lo) < atr[0] * InpMinBodyATR)
      return false;

   double mid = (body2Hi + body2Lo) * 0.5;
   bool bullEngulf = (c2 > o2 && c3 < o3);
   bool bearEngulf = (c2 < o2 && c3 > o3);
   if(bullEngulf && c1 >= mid)
   {
      isBuy = true;
      slExtreme = l2;
      return true;
   }
   if(bearEngulf && c1 <= mid)
   {
      isBuy = false;
      slExtreme = h2;
      return true;
...
void OnTick()
{
   ...
   if(!DetectEngulfAccept(isBuy, slExtreme))
      return;
   ...
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, "ENGULF|accept", res))
      return;
