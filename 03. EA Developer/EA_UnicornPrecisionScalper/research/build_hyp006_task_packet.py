#!/usr/bin/env python3
"""Refresh the dirty-tree-bound HYP-UPS-XAU-M5-006 control task packet."""
...
OVERRIDES = (
    ...
    "InpMagic=5600717;InpMaxAccountDrawdownPct=5.50;InpMaxConsecutiveLosses=2;"
    ...
    "InpTargetRR=2.50;InpUseEventAnchoredSweepState=true"
)
ACCEPTANCE = {
...
def latest_registry_row(registry: Path) -> tuple[str, dict[str, Any]]:
    ...
    if row.get("state") != "screened" or row.get("model") != 0:
        raise ValueError("latest HYP-006 registry row is not screened Model 0")
    return raw, row
...
def build(root: Path, output: Path) -> dict[str, Any]:
    ...
    prereg = (
        ...
        / EA_NAME
        / "research"
        / "HYP-UPS-XAU-M5-006_FROZEN_PREREG.md"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "HYP-UPS-XAU-M5-006/task_packet.control.json"
        ),
    )
