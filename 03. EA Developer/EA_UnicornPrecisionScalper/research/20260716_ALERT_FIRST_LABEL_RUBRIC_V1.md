# ALERT_FIRST_LABEL_RUBRIC_V1

Status: `FROZEN_PRE_OUTCOME_REVIEW_RUBRIC`  
Frozen: 2026-07-16, before any reviewer output or outcome join  
...
`20260716_153059` from source contract `UPS_ALERT_FIRST_CASEBOOK_V1_2`.
...
files keyed by `event_id`. The overlay adds `label_valid_breaker`, which was
mistakenly absent from the v1 casebook blank-label columns even though the
report-to-code fidelity gate requires it. This schema correction does not
change the detector or authorize a new signal.
...
`label_valid_breaker`, `label_core_setup_accept`, `entry_readiness`,
