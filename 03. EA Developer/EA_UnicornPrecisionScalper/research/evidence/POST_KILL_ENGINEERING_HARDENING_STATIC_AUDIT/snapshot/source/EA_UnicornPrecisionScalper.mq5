...
input bool   InpUseEventAnchoredSweepState=false;
...
int OnInit()
  {
   ...
   g_last_bar=iTime(_Symbol,PERIOD_M5,0);
   g_new_bar_ready=false;
   if(g_last_bar<=0)
      return INIT_FAILED;
   if(!OpenLifecycleTelemetry())
      return INIT_FAILED;
   PrintFormat("UPS init mode=%s hypothesis=%s engineering=%s telemetry=%s",
...
int EntryPositionCountSince(const datetime from_time)
  {
...
double RealizedNetSince(const datetime from_time)
  {
   double net=0.0;
   if(!HistorySelect(from_time,TimeCurrent()))
      return 0.0;
...
int ConsecutiveLosingPositionsSince(const datetime from_time)
  {
   if(!HistorySelect(from_time,TimeCurrent()))
      return 0;
   ulong position_ids[];
...
bool RiskGuardsAllow()
  {
   ...
   if(account_dd>=InpMaxAccountDrawdownPct)
      return false;
   int day_trades=EntryPositionCountSince(StartOfDay(TimeCurrent()));
   int streak=ConsecutiveLosingPositionsSince(StartOfWeek(TimeCurrent()));
   double day_net=RealizedNetSince(StartOfDay(TimeCurrent()));
   ...
   if(day_trades>=InpMaxTradesPerDay || streak>=InpMaxConsecutiveLosses || (-day_net/day_start*100.0)>=InpMaxDailyLossPct)
      return false;
   double week_net=RealizedNetSince(StartOfWeek(TimeCurrent()));
...
bool PreflightMarketOrder(const int direction,const double volume,const double entry,
  ...
  {
   if(!BrokerTradingAllows(direction) || ForeignSymbolPositionExists())
      return false;
