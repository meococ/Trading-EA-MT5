#!/usr/bin/env python3
"""Refresh the dirty-tree-bound HYP-UPS-XAU-M5-004 control task packet."""
...
HYPOTHESIS_ID = "HYP-UPS-XAU-M5-004"
...
def latest_registry_row(registry: Path) -> tuple[str, dict[str, Any]]:
    ...
    if row.get("state") != "screened" or row.get("model") != 0:
        raise ValueError("latest HYP-004 registry row is not screened Model 0")
    return raw, row
...
def build(root: Path, output: Path) -> dict[str, Any]:
    ...
    prereg = (
        ...
        / EA_NAME
        / "research"
        / "HYP-UPS-XAU-M5-004_FROZEN_PREREG.md"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "HYP-UPS-XAU-M5-004/task_packet.control.json"
        ),
    )
