...
input int    InpStructurePivotStrength=2;
...
const string ENGINEERING_STATUS="FIDELITY_M15_STRUCTURE_ALERT_ONLY_NO_RUN_AUTHORITY";
const string CASEBOOK_CONTRACT_ID="ALERT_FIRST_CASEBOOK_V1";
const string CASEBOOK_SOURCE_CONTRACT_ID="UPS_ALERT_FIRST_CASEBOOK_V1_4";
...
double g_pending_risk_budget_account=0.0;
double g_planned_risk_budget_account=0.0;
bool g_force_close_risk_overshoot=false;
...
bool WriteRunMeta()
  {
   ...
   string payload=StringFormat(
      ...
      TradingMutationAllowed() ? "true" : "false");
...
bool WriteAlertCasebookMeta()
  {
   ...
   FileWrite(handle,
             "alert_first_casebook_meta.v1",CASEBOOK_CONTRACT_ID,CASEBOOK_SOURCE_CONTRACT_ID,
             InpExpectedSourceSha256,g_run_id,
             ...
             InpSweepStateBars,InpUseEventAnchoredSweepState ? "1" : "0",
             InpStructurePivotStrength,InpBreakerLookback,
...
bool OpenAlertCasebook()
  {
   ...
   FileWrite(g_casebook_handle,
             "schema_version","contract_id","source_contract_id","source_sha256","run_id","event_id",
             ...
             "fvg_high","overlap_ratio","h4_bias","d1_bias","m15_structure","pd_ok",
             "spread_points","label_true_sweep_liquidity",
             "label_true_displacement","label_true_mss_bos_close","label_true_breaker_valid",
...
bool WriteAlertCasebook(const SignalPlan &signal,const double spread_points)
  {
   ...
   FileWrite(g_casebook_handle,
             "alert_first_casebook.v1",CASEBOOK_CONTRACT_ID,CASEBOOK_SOURCE_CONTRACT_ID,
             InpExpectedSourceSha256,g_run_id,event_id,
             ...
             signal.m15_structure,signal.pd_ok ? "1" : "0",DoubleToString(spread_points,1),
             "","","","","","","","","","");
   FileFlush(g_casebook_handle);
   g_casebook_rows++;
   return true;
...
bool TradingMutationAllowed()
  {
   return MQLInfoInteger(MQL_TESTER) &&
          InpResearchAutoMode && InpAllowRetiredResearchExecution;
  }
   ...
   switch(reason)
     {
      ...
      case REJECT_M15_STRUCTURE:return "M15_STRUCTURE";
...
bool ValidateInputs()
  {
   ...
   if(InpEnableAlertCasebook && !IsSha256Hex(InpExpectedSourceSha256))
      return InputError("alert casebook requires a 64-character source SHA256 binding");
   ...
   if(InpStructurePivotStrength<1 || InpStructurePivotStrength>5 ||
      InpBreakerLookback<1 || InpMinDisplacementAtr<=0.0 || InpStrongDisplacementAtr<InpMinDisplacementAtr)
...
int OnInit()
  {
   ...
   trade.SetDeviationInPoints((ulong)MathMax(1,InpEstimatedSlippagePoints));
   ...
   ulong owned=0;
   if(!TryOwnedPositionTicket(owned))
     {
      Print("UPS init failed: position enumeration unavailable.");
      return INIT_FAILED;
     }
   if(!TradingMutationAllowed())
   ...
   else if(owned!=0)
     {
      ulong position_id=(ulong)PositionGetInteger(POSITION_IDENTIFIER);
       if(!CaptureInitialRiskFromPosition(position_id))
          g_force_close_risk_overshoot=true;
...
bool IsConfirmedSwing(const MqlRates &bars[],const int index,const int strength,const bool find_high)
  {
   int size=ArraySize(bars);
   if(strength<1 || index-strength<0 || index+strength>=size)
      return false;
   double value=(find_high ? bars[index].high : bars[index].low);
   for(int offset=1;offset<=strength;offset++)
     {
      double older=(find_high ? bars[index-offset].high : bars[index-offset].low);
      double newer=(find_high ? bars[index+offset].high : bars[index+offset].low);
      if(find_high && (value<=older || value<=newer))
         return false;
      if(!find_high && (value>=older || value>=newer))
         return false;
     }
...
int ClosedSwingStructureState(const ENUM_TIMEFRAMES timeframe,const int strength)
  {
   MqlRates bars[];
   ArraySetAsSeries(bars,false);
   const int requested=32;
   int copied=CopyRates(_Symbol,timeframe,1,requested,bars);
   if(copied<requested)
      return 0;
   double older_high=0.0;
   double newer_high=0.0;
   double older_low=0.0;
   double newer_low=0.0;
   int high_count=0;
   int low_count=0;
   for(int index=strength;index<copied-strength;index++)
     {
      if(IsConfirmedSwing(bars,index,strength,true))
        {
         older_high=newer_high;
         newer_high=bars[index].high;
         high_count++;
        }
      if(IsConfirmedSwing(bars,index,strength,false))
        {
         older_low=newer_low;
      ...
      if(direction>0 && rates[j].low<prior_low && rates[j].close>prior_low)
        {
         bool invalidated=false;
          if(InpUseEventAnchoredSweepState)
             // Every bar from the sweep through the decision bar is closed
             // here. Include the displacement/FVG bars (0 and 1), not only
             // the older bars ending at `left`.
             for(int k=0;k<j;k++)
      ...
      if(direction<0 && rates[j].high>prior_high && rates[j].close<prior_high)
        {
         bool invalidated=false;
          if(InpUseEventAnchoredSweepState)
             // Symmetric full closed-bar invalidation for bearish sweeps.
             for(int k=0;k<j;k++)
...
SignalPlan EvaluateClosedSignal()
  {
   ...
   if(h4_bias==0 || d1_bias==-h4_bias)
      return result;
   int m15_structure=ClosedSwingStructureState(PERIOD_M15,InpStructurePivotStrength);
   result.m15_structure=m15_structure;
   result.reject_reason=REJECT_M15_STRUCTURE;
   if(m15_structure!=h4_bias)
      return result;
...
bool TryOwnedPositionTicket(ulong &owned_ticket)
  {
   owned_ticket=0;
...
bool ForeignSymbolPositionExists(bool &scan_ok)
  {
   scan_ok=false;
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
...
bool ForeignSymbolOrderExists(bool &scan_ok)
  {
   scan_ok=false;
...
double RiskSizedVolume(const int direction,const double entry,const double stop,
                       double &risk_account,double &risk_budget_account)
  {
   ...
   if(total_loss_one_lot<=0.0)
      return 0.0;
   risk_budget_account=AccountInfoDouble(ACCOUNT_EQUITY)*InpRiskPercent/100.0;
   risk_account=risk_budget_account;
   double volume=NormalizeVolume(risk_budget_account/total_loss_one_lot);
   if(volume<=0.0)
      return 0.0;
   ...
   if(normalized_total_loss>risk_budget_account+0.01)
      return 0.0;
...
bool PreflightMarketOrder(const int direction,const double volume,const double entry,
  ...
  {
   bool position_scan_ok=false;
   bool order_scan_ok=false;
   bool foreign_position=ForeignSymbolPositionExists(position_scan_ok);
   bool foreign_order=ForeignSymbolOrderExists(order_scan_ok);
   if(!BrokerTradingAllows(direction) || !position_scan_ok || !order_scan_ok ||
      foreign_position || foreign_order)
      return false;
   ...
   request.sl=stop;
   request.tp=target;
   request.deviation=(ulong)MathMax(1,InpEstimatedSlippagePoints);
...
bool OpenFromSignal(const SignalPlan &signal)
  {
   ...
   double risk_budget_account=0.0;
   double volume=RiskSizedVolume(signal.direction,entry,stop,risk_account,risk_budget_account);
   if(volume<=0.0)
      return false;
   ...
   g_pending_risk_budget_account=risk_budget_account;
   g_force_close_risk_overshoot=false;
   ...
   if(!sent || (trade.ResultRetcode()!=TRADE_RETCODE_DONE && trade.ResultRetcode()!=TRADE_RETCODE_DONE_PARTIAL))
     {
      ...
      g_pending_risk_budget_account=0.0;
      SetExecutionState(EXEC_RECOVERING_ERROR,"broker rejected market request");
      return false;
     }
   ulong owned=0;
   if(!TryOwnedPositionTicket(owned))
      SetExecutionState(EXEC_RECOVERING_ERROR,"position enumeration failed after market request");
   else if(owned!=0)
     {
      SetExecutionState(EXEC_MANAGING_POSITION,"position visible after market request");
      // Reconcile the broker fill immediately. Waiting for the next tick would
      // leave a fill whose actual stop-loss money risk exceeds the frozen
      // budget open longer than necessary.
      ManageOwnedPosition(owned);
     }
   else
...
void ManageOwnedPosition(const ulong ticket)
  {
   if(ticket==0 || !PositionSelectByTicket(ticket))
      return;
   double entry=PositionGetDouble(POSITION_PRICE_OPEN);
   ...
   ulong position_id=(ulong)PositionGetInteger(POSITION_IDENTIFIER);
   if((g_initial_risk_position_identifier!=position_id || g_planned_risk_account<=0.0) &&
      !CaptureInitialRiskFromPosition(position_id))
      g_force_close_risk_overshoot=true;
   if(g_force_close_risk_overshoot)
     {
      if(!TradeResultCompleted(trade.PositionClose(ticket)))
         Print("UPS risk-overshoot emergency close failed: ",trade.ResultRetcodeDescription());
      return;
     }
...
bool CaptureInitialRiskFromPosition(const ulong position_id)
  {
   ...
   if(entry<=0.0 || stop<=0.0 || volume<=0.0 || distance<=_Point*0.5)
      return false;
   ENUM_POSITION_TYPE position_type=(ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
   ENUM_ORDER_TYPE order_type=(position_type==POSITION_TYPE_SELL ? ORDER_TYPE_SELL : ORDER_TYPE_BUY);
   double stop_pnl=0.0;
   if(!OrderCalcProfit(order_type,_Symbol,volume,entry,stop,stop_pnl) || stop_pnl>=0.0)
      return false;
   double execution_cost_one_lot=EstimatedExecutionCostOneLot(order_type,entry);
   if(!MathIsValidNumber(execution_cost_one_lot) || execution_cost_one_lot<0.0 ||
      execution_cost_one_lot==DBL_MAX)
      return false;
   double actual_risk_account=MathAbs(stop_pnl)+volume*execution_cost_one_lot;
   if(!MathIsValidNumber(actual_risk_account) || actual_risk_account<=0.0)
      return false;
   double risk_budget_account=g_planned_risk_budget_account;
   if(risk_budget_account<=0.0)
      risk_budget_account=AccountInfoDouble(ACCOUNT_EQUITY)*InpRiskPercent/100.0;
   g_initial_risk_position_identifier=position_id;
   g_initial_risk_distance=distance;
   g_planned_risk_points=distance/_Point;
   g_planned_risk_account=actual_risk_account;
   g_planned_risk_budget_account=risk_budget_account;
   if(actual_risk_account>risk_budget_account+0.01)
     {
      PrintFormat("UPS actual-fill risk overshoot actual=%.2f budget=%.2f position=%I64u",
                  actual_risk_account,risk_budget_account,position_id);
      g_force_close_risk_overshoot=true;
     }
   return true;
  }
      ...
      if(g_pending_risk_points>0.0 && g_pending_risk_account>0.0)
        {
         ...
         g_planned_risk_budget_account=g_pending_risk_budget_account;
         g_pending_risk_points=0.0;
         g_pending_risk_account=0.0;
         g_pending_risk_budget_account=0.0;
        }
      ...
      if(position_id==g_position_identifier)
        {
         ...
         g_planned_risk_budget_account=0.0;
         g_force_close_risk_overshoot=false;
        }
...
void OnTick()
  {
   ...
   if(!TryOwnedPositionTicket(owned))
     {
      SetExecutionState(EXEC_RECOVERING_ERROR,"position enumeration failed on tick");
      return;
     }
   ...
   if(!g_new_bar_ready)
      return;
   ulong entry_owned=0;
   if(!TryOwnedPositionTicket(entry_owned) || entry_owned!=0 ||
      !SessionOpen(g_last_bar) || !NewsGuardAllows())
      return;
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol,tick))
      return;
