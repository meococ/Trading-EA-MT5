#!/usr/bin/env python3
"""Build hash-bound manual AlphaFactory receipts for MZMS HYP-006 XAU."""
...
def build(role: str, control_run: Path | None) -> tuple[Path, str]:
    ...
    cost = {
        ...
        "from": FROM_DATE,
        "to": TO_DATE,
        "spread_policy": "MT5 Model 0 historical tick spread with frozen 35-point entry ceiling",
    ...
    task = {
        ...
        "news_guard": "DISABLED_UNIFORMLY_NO_XAU_FULL_WINDOW_PIT_CALENDAR",
        "promotion_eligible": False,
    }
    ...
    receipt = {
        ...
        "note": (
            "Owner-directed 2018-YTD XAUUSD transfer diagnostic. News guard is disabled "
            "uniformly because no point-in-time XAU calendar covers the window. No promotion authority."
        ),
    }
