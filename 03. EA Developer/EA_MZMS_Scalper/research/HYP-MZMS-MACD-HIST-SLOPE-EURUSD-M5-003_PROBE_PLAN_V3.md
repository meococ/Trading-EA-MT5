# HYP-MZMS-MACD-HIST-SLOPE-EURUSD-M5-003 - Frozen Probe Plan V3

Status: PRE-SOURCE / PRE-OUTCOME

Frozen at: 2026-07-21T09:17:30Z

Amendment boundary: V1/ID 001 was parked before any market-price outcome due
to a news-SHA transcription error. V2/ID 002 then opened outcomes, but its
post-run causal audit found same-M1-timestamp re-entry and inclusion of the M1
bar beginning at the 75-minute horizon. V2 is invalid instrumentation and its
metrics cannot kill, rescue or tune the strategy. V3 changes only hypothesis
identity and offline simulator ordering: an entry timestamp must be strictly
later than the prior exit timestamp; the M1 bar beginning at the 75-minute
horizon is excluded; a hard-flatten boundary is acted on at that M1 open before
its high/low. Signal, windows, arms, costs and acceptance gates are unchanged.

## Identity and authority

- EA: `EA_MZMS_Scalper`
- Hypothesis: `HYP-MZMS-MACD-HIST-SLOPE-EURUSD-M5-003`
...
- A new entry is allowed only when its timestamp is strictly later than the
  prior position exit timestamp; equality is rejected as causally ambiguous.
- The bar whose open equals the 75-minute horizon is outside the holding path.
- A hard-flatten boundary exits at that M1 open before reading the bar high/low.
