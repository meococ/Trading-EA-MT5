...
$outDir = Join-Path $eaRoot 'research\preflight\HYP-JCDR-EURUSD-M5-004'
...
# Materialize the path before taking git status.  Git status records path/state,
# not file content, so the final packet can bind its own pathname without a
...
# Authority is hypothesis-local.  Other append-only registry amendments may
# follow HYP004, so select the latest HYP004 row instead of assuming that the
# final physical line belongs to this hypothesis.
$rows = @(Get-Content -LiteralPath $registryPath)
$latestIndex = -1
$latest = $null
for ($i = $rows.Count - 1; $i -ge 0; $i--) {
    $candidate = $rows[$i] | ConvertFrom-Json
    if ([string]$candidate.hypothesis_id -ceq 'HYP-JCDR-EURUSD-M5-004') {
        $latestIndex = $i
        $latest = $candidate
        break
    }
...
if ($latestIndex -lt 0 -or $null -eq $latest) {
    throw 'No HYP-JCDR-EURUSD-M5-004 registry row exists.'
}
if ([string]$latest.state -cne 'screened' -or
    [bool]$latest.validation.source_probe_run_authorized -ne $true -or
    [bool]$latest.validation.model0_authorized -ne $true -or
    [bool]$latest.validation.mt5_authorized -ne $true) {
    throw 'Latest HYP004 registry row is not the one-shot screened execution authority.'
}
...
$preregPath = '03. EA Developer/EA_JumpClusterDecayReversal/research/HYP-JCDR-EURUSD-M5-004_ROLE_AWARE_SOURCE_PREREG.md'
$costPath = '03. EA Developer/EA_JumpClusterDecayReversal/research/HYP-JCDR-EURUSD-M5-004_COLLECTION_ONLY_COST_SOURCE_MANIFEST.json'
$overrides = 'InpAnalysisFrom=2016.01.04;InpAnalysisTo=2020.12.31;InpExpectedSymbol=EURUSD;InpHypothesisId=HYP-JCDR-EURUSD-M5-004;InpResearchAutoMode=true;InpVariantTag=JCDR_ROLE_AWARE_SOURCE_V1'

$packet = [ordered]@{
    schema_version = 'alphafactory_research_task_packet.v1'
    authority = 'DATA_ACQUISITION_ONLY_NO_MODEL0_PERFORMANCE'
    hypothesis_id = 'HYP-JCDR-EURUSD-M5-004'
    run_role = 'control'
    ...
    # Data-acquisition authority forbids lifecycle/performance sidecar gates.
    # AlphaFactory still snapshots this exporter's StateTelemetry/RunMeta files
    # through its default source-data patterns and the EA's own fatal run gate.
    required_sidecars = @()
