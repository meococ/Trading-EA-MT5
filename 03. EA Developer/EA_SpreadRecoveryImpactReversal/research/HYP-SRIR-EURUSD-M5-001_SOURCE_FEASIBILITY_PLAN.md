# HYP-SRIR-EURUSD-M5-001 — Source-Feasibility Plan
...
4. Source dates are the exact chronologically ordered dates in the bound
   manifest. The first 20 source dates are immutable burn-in and are excluded
   from every formation, spread-availability and cadence denominator.
5. For source date `d` and exact UTC M5 slot `s`, the frozen baseline is the
   median of `block_spread_points` for slot `s` on exactly the prior 20 manifest
   source dates. Current-date and future values are excluded. If any of those
   20 prior-date blocks is incomplete or spread-unavailable, the baseline is
   unavailable; it is never filled from an older or later date.

The shock scan domain is Monday through Friday and exact M5 opens from `07:00`
through `15:40` UTC inclusive after the 20-source-date burn-in. The latest
...
5. Frozen prior-20-date baseline availability at least `0.99` among complete
...
forbidden as denominator. The 20-date burn-in prevents structural warm-up from
