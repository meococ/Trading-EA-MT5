#!/usr/bin/env python3
"""Account-compatible bounded-journal correctness audit for HYP-STBS-XAUUSD-M15-016."""
...
HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-016"
INNER_HYPOTHESIS_ID = HYPOTHESIS_ID
PARENT_HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-015"
ATTEMPT_ID = "STBS016-MODEL0-AUDIT-001"
...
DIFF_PROOF = ROOT / "03. EA Developer" / EA_NAME / "research" / f"{HYPOTHESIS_ID}_V4_V5_DIFF_PROOF.md"
...
EXPECTED_PREREG_SHA = "5C5B47E5FEB387F134EA0FCF6DD90973625674D103A13752A7F77F8C4DA8B421"
...
def read_registry_authority() -> tuple[dict, str, str]:
    ...
    if latest is None or latest_raw is None:
        raise RuntimeError("HYP016 authority row is missing")
    parent_rows = [raw for raw in rows if json.loads(raw).get("hypothesis_id") == PARENT_HYPOTHESIS_ID]
    if not parent_rows:
        raise RuntimeError("terminal HYP015 parent row is missing")
    parent = json.loads(parent_rows[-1])
    parent_sha = hashlib.sha256(parent_rows[-1]).hexdigest().upper()
    if not (
        parent.get("state") == "killed"
        and parent.get("verdict") == "KILL_HARNESS_JOURNAL_DELTA_HARDCAP_TRUNCATED_AND_INVALID_10K_MARGIN_CONTRACT_NO_ECONOMIC_VERDICT"
        and parent_sha == "E611136E39A8BC6336DFC06995458239EB231BBE5456C1B9B4E685404658E472"
    ):
        raise RuntimeError("terminal HYP015 parent state/hash/verdict mismatch")
    ...
    if v.get("parent_hyp015_terminal_row_sha256") != "E611136E39A8BC6336DFC06995458239EB231BBE5456C1B9B4E685404658E472":
        raise RuntimeError("terminal HYP015 parent binding mismatch")
...
def assert_bound_inputs(latest: dict) -> None:
    ...
    for path_field, sha_field in (
        ...
        ("parent_hyp015_failure_path", "parent_hyp015_failure_sha256"),
        ("parent_hyp015_post_failure_review_path", "parent_hyp015_post_failure_review_sha256"),
        ("parent_hyp015_attempt_terminal_path", "parent_hyp015_attempt_terminal_sha256"),
    ):
...
def assert_prelaunch_integrity(
    ...
    if not hyp_rows or hashlib.sha256(hyp_rows[-1]).hexdigest().upper() != latest_row_sha:
        raise RuntimeError("latest HYP016 authority row changed before launch")
