#!/usr/bin/env python3
"""Fresh outer-only canonical-override correctness audit for HYP-STBS-XAUUSD-M15-017."""
...
HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-017"
INNER_HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-016"
PARENT_HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-016"
ATTEMPT_ID = "STBS017-MODEL0-AUDIT-001"
...
CLI_OVERRIDES = (
    ...
    "InpMoneyFreeEquityFloorPct=1.0;"
    "InpVariantTag=STBS_H1_FLIP_M15_BURST_AUDIT_V5_ACCOUNT_SAFE"
)
EFFECTIVE_OVERRIDES = (
    "InpAuditOnly=true;"
    f"InpHypothesisId={INNER_HYPOTHESIS_ID};InpMagic={MAGIC};"
...
DIFF_PROOF = ROOT / "03. EA Developer" / EA_NAME / "research" / "HYP-STBS-XAUUSD-M15-016_V4_V5_DIFF_PROOF.md"
EA_CONTRACT = ROOT / "03. EA Developer" / EA_NAME / "ALPHAFACTORY_EA_CONTRACT.json"
COST_MANIFEST = ROOT / "03. EA Developer" / "EA_SupertrendBurstScalperTradeV4" / "research" / "HYP-STBS-XAUUSD-M15-014_COLLECTION_ONLY_COST_MANIFEST.json"
...
EXPECTED_SOURCE_SHA = "3822EED82C8D484CE8010A496767271DED20528158D68509B46EF934B043D918"
EXPECTED_PREREG_SHA = "806C87D2322EC0016711269FE5BEFB48EE9DE2F5119C4CC5539A2163B96BAD83"
EXPECTED_DIFF_PROOF_SHA = "9516B9587F0EA8AA01DDC78E8F4C7F671A8CECB777C64E8F4A3CE3C60158F55C"
...
EXPECTED_ACCOUNT_SHA = "0A603E7B316F58B39FEA0A1710FE6F250E544909DA2B91967C93AD984317A073"
...
def read_registry_authority() -> tuple[dict, str, str]:
    ...
    if latest is None or latest_raw is None:
        raise RuntimeError("HYP017 authority row is missing")
    parent_rows = [raw for raw in rows if json.loads(raw).get("hypothesis_id") == PARENT_HYPOTHESIS_ID]
    if not parent_rows:
        raise RuntimeError("terminal HYP016 parent row is missing")
    parent = json.loads(parent_rows[-1])
    parent_sha = hashlib.sha256(parent_rows[-1]).hexdigest().upper()
    if not (
        parent.get("state") == "killed"
        and parent.get("verdict") == "KILL_HARNESS_UNNORMALIZED_OVERRIDE_RECEIPT_PRECOMPILE_NO_MT5_NO_ECONOMIC_VERDICT"
        and parent_sha == "3CED3A5D29DEE9A5DC424C104DCBC92DC06F2D6CC0CDEE845E3643E581DFCCCC"
    ):
        raise RuntimeError("terminal HYP016 parent state/hash/verdict mismatch")
    ...
    if v.get("parent_hyp016_terminal_row_sha256") != "3CED3A5D29DEE9A5DC424C104DCBC92DC06F2D6CC0CDEE845E3643E581DFCCCC":
        raise RuntimeError("terminal HYP016 parent binding mismatch")
    if v.get("cli_overrides") != CLI_OVERRIDES or v.get("effective_overrides") != EFFECTIVE_OVERRIDES:
        raise RuntimeError("CLI/effective override authority mismatch")
...
def assert_bound_inputs(latest: dict) -> None:
    expected = {
        ...
        DIFF_PROOF: EXPECTED_DIFF_PROOF_SHA,
    ...
    for path_field, sha_field in (
        ...
        ("reviewed_diff_proof_path", "reviewed_diff_proof_sha256"),
        ...
        ("parent_hyp016_failure_path", "parent_hyp016_failure_sha256"),
        ("parent_hyp016_post_failure_review_path", "parent_hyp016_post_failure_review_sha256"),
        ("parent_hyp016_attempt_terminal_path", "parent_hyp016_attempt_terminal_sha256"),
    ):
...
def ensure_clean_stable_sidecars() -> None:
    ...
    if found:
        raise RuntimeError("pre-existing stable HYP016 sidecar(s): " + "; ".join(sorted(found)))
...
def assert_prelaunch_integrity(
    ...
    if not hyp_rows or hashlib.sha256(hyp_rows[-1]).hexdigest().upper() != latest_row_sha:
        raise RuntimeError("latest HYP017 authority row changed before launch")
...
def parse_kv_payload(line: str, marker: str) -> dict[str, str]:
    ...
    for part in payload.split("|"):
        if not part or "=" not in part:
            raise RuntimeError(f"bare/empty {marker} payload fragment")
...
def validate_journal_text(text: str) -> dict:
    ...
    expected_summary = {
        "hypothesis": INNER_HYPOTHESIS_ID, "reason": "0", "raw": "690", "executable": "683", "gaps": "7",
    ...
    if set(summaries[0]) != set(expected_summary):
        raise RuntimeError("summary field allowlist mismatch")
    ...
    for item in exact:
        if set(item) != {
            "source_epoch", "decision_epoch", "direction", "exact_next",
            "atr_ready", "geometry_ready", "margin_ready", "volume",
            "projected_free", "required_free", "audit",
        }:
            raise RuntimeError("exact signal field allowlist mismatch")
        ...
        try:
            ...
            projected_free = float(item["projected_free"])
            required_free = float(item["required_free"])
        except (KeyError, ValueError) as exc:
            raise RuntimeError("non-numeric exact signal field") from exc
        if decision_epoch != source_epoch + 3600:
            raise RuntimeError("exact signal clock mismatch")
        if not all(math.isfinite(value) for value in (volume, projected_free, required_free)):
            raise RuntimeError("non-finite margin evidence")
        if volume <= 0.0 or abs(required_free - 93600.0) > 1e-6 or projected_free < required_free:
            raise RuntimeError("account-safe margin evidence mismatch")
    for item in gaps:
        if set(item) != {
            "source_epoch", "decision_epoch", "direction", "exact_next", "consumed", "audit",
        }:
            raise RuntimeError("gap signal field allowlist mismatch")
        if item.get("consumed") != "true" or item.get("audit") != "true":
            raise RuntimeError("gap signal was not consumed in audit mode")
...
def validate_run(
    ...
    journal_contract = manifest.get("data_quality_contract", {})
    journal_delta = manifest.get("data_quality_journal_delta", {})
    if not (
        journal_contract.get("max_journal_delta_bytes") == 1048576
        and journal_delta.get("path") == "logs/tester_journal_delta.log"
        and journal_delta.get("files_read") == 2
        and journal_delta.get("truncated") is False
        and isinstance(journal_delta.get("bytes_read"), int)
        and 0 < journal_delta["bytes_read"] < 1048576
    ):
        raise RuntimeError("journal completeness/cap contract mismatch")
    ...
    if not (
        ...
        and funding.swap == 0.0 and funding.order_id is None and funding.profit == 100000.0
        and funding.balance == 100000.0
    ):
...
def execute() -> dict:
    ...
    try:
        ...
        cmd = [
            ...
            "-Overrides", CLI_OVERRIDES, "-HypothesisId", HYPOTHESIS_ID, "-RunRole", "control",
            "-TelemetryTier", "off", "-Deposit", "100000", "-Leverage", "100",
