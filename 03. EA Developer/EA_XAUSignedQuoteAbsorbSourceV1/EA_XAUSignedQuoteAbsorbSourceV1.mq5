//+------------------------------------------------------------------+
//| EA_XAUSignedQuoteAbsorbSourceV1.mq5                              |
//| HYP-XAU-SIGNED-QUOTE-ABSORB-002 - outcome-blind source gate      |
//+------------------------------------------------------------------+
...
void RegisterSignal(const bool is_long,
                    ...
                    const double spread)
  {
   ...
   PrintFormat("SQA_SOURCE_SIGNAL hypothesis_id=%s decision=%s side=%s net_pressure=%d events=%d mid_move=%.5f atr=%.5f spread=%.5f orders_sent=0",
               HYPOTHESIS_ID,TimeToString(decision_time,TIME_DATE|TIME_SECONDS),
               is_long ? "LONG" : "SHORT",net_pressure,events,mid_move,atr,spread);
  }
