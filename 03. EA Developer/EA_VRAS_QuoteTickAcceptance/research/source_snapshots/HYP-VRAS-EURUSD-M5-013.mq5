...
//| HYP-VRAS-EURUSD-M5-013 - UTC-normalized quote-tick collection   |
...
#property strict
#property description "HYP-013 UTC-normalized quote acceptance collection only"
#property description "Closed-bar arm + causal OnTick FSM; never places trades"

input bool   InpCollectionOnly=true;
input string InpHypothesisId="HYP-VRAS-EURUSD-M5-013";
...
bool IdentityOk()
  {
   if(!InpCollectionOnly)
      return false;
   if(InpHypothesisId!="HYP-VRAS-EURUSD-M5-013")
      return false;
   if(_Symbol!="EURUSD")
      return false;
...
//+------------------------------------------------------------------+
long NormalizeTickUtcMsc(const long raw_time_msc)
  {
   long offset_ms=((long)TimeCurrent()-(long)TimeGMT())*1000;
   long normalized=raw_time_msc-offset_ms;
...
int OnInit()
  {
   if(!IdentityOk())
     {
      Print("QTA identity fail-closed: require EURUSD M5 HYP-VRAS-EURUSD-M5-013 exact defaults collection-only");
      return INIT_FAILED;
     }
   ...
   if(g_state==ST_ACTIVE)
     {
      ...
      if(SymbolInfoTick(_Symbol,tick))
        {
         t_msc=NormalizeTickUtcMsc(tick.time_msc);
         bid=tick.bid;
         ask=tick.ask;
        }
      if(t_msc<=0)
         t_msc=(long)TimeGMT()*1000;
...
void OnTick()
  {
   ...
   if(!SymbolInfoTick(_Symbol,tick))
      return;
   long time_msc=NormalizeTickUtcMsc(tick.time_msc);
   double bid=tick.bid;
   double ask=tick.ask;
