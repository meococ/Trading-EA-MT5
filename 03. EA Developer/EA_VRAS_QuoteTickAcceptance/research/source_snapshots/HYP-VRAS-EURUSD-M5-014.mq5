...
//| HYP-VRAS-EURUSD-M5-014 - unique UTC quote-tick collection       |
...
input string InpHypothesisId="HYP-VRAS-EURUSD-M5-014";
...
bool IdentityOk()
  {
   if(!InpCollectionOnly)
      return false;
   if(InpHypothesisId!="HYP-VRAS-EURUSD-M5-014")
      return false;
   if(_Symbol!="EURUSD")
      return false;
...
int OnInit()
  {
   if(!IdentityOk())
     {
      Print("QTA identity fail-closed: require EURUSD M5 HYP-VRAS-EURUSD-M5-014 exact defaults collection-only");
      return INIT_FAILED;
     }
