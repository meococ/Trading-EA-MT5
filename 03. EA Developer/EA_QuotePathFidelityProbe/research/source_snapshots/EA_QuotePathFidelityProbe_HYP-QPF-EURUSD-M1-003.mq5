...
//| HYP-QPF-EURUSD-M1-003 - explicit-input quote-path reissue        |
//+------------------------------------------------------------------+
...
input string InpHypothesisId="HYP-QPF-EURUSD-M1-003";
...
bool IdentityOk()
  {
   return InpCollectionOnly &&
          InpHypothesisId=="HYP-QPF-EURUSD-M1-003" &&
...
int OnInit()
  {
   if(!IdentityOk())
     {
      Print("QPF_IDENTITY_FAIL require exact HYP-QPF-EURUSD-M1-003 EURUSD M1 collection-only inputs");
      return INIT_FAILED;
     }
   if(!OpenTelemetry())
      return INIT_FAILED;
