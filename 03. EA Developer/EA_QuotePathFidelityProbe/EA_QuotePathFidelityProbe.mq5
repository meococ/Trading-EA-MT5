...
//| HYP-QPF-EURUSD-M1-004 - D0-proof quote-path reissue              |
//+------------------------------------------------------------------+
...
input string InpHypothesisId="HYP-QPF-EURUSD-M1-004";
...
bool IdentityOk()
  {
   return InpCollectionOnly &&
          InpHypothesisId=="HYP-QPF-EURUSD-M1-004" &&
...
int OnInit()
  {
   if(!IdentityOk())
     {
      Print("QPF_IDENTITY_FAIL require exact HYP-QPF-EURUSD-M1-004 EURUSD M1 collection-only inputs");
      return INIT_FAILED;
     }
   if(!EmitD0SeriesProof())
     {
      Print("QPF_D0_SERIES_PROOF_FAIL");
      return INIT_FAILED;
     }
   if(!OpenTelemetry())
      return INIT_FAILED;
