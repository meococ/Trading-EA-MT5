...
const string HYPOTHESIS_ID="HYP-ICT-FVG-FIDEXEC-EURUSD-M5-003";
...
void SavePersistentRiskState()
  {
   ...
   GlobalVariableSet(PersistentKey("posid"),(double)g_position_identifier);
...
void LoadPersistentRiskState()
  {
   ...
   if(GlobalVariableCheck(PersistentKey("posid")))
      g_position_identifier=(ulong)GlobalVariableGet(PersistentKey("posid"));
...
bool RestoreOwnedPositionState()
  {
   ...
   if(!stored_stop_valid)
     {
      if(target>0.0 && InpTargetRR>0.0)
         g_initial_stop=entry-(double)direction*MathAbs(target-entry)/InpTargetRR;
      else
         g_initial_stop=current_stop;
     }
   g_initial_risk_price=MathAbs(g_initial_entry-g_initial_stop);
   ...
   if(g_planned_risk_account<=0.0 && g_initial_risk_price>0.0)
     {
      double one_lot_risk=0.0;
      if(OrderCalcProfit(g_entry_order_type,_Symbol,1.0,g_initial_entry,g_initial_stop,one_lot_risk))
         g_planned_risk_account=MathAbs(one_lot_risk)*PositionGetDouble(POSITION_VOLUME);
     }
   SavePersistentRiskState();
   return true;
  }
