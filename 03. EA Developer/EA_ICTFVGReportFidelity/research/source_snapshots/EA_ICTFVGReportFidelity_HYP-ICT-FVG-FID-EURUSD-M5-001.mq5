...
const string HYPOTHESIS_ID="HYP-ICT-FVG-FID-EURUSD-M5-001";
...
bool NewsBlocked(const datetime server_time)
  {
   // No hash-bound historical high-impact calendar exists in this workspace.
   // Requiring the guard therefore fails closed instead of pretending safety.
   return InpRequireNewsGuard;
  }
...
bool CanOpenNow(const datetime server_time)
  {
   ...
   if(!InpResearchAutoMode || NewsBlocked(server_time))
      return false;
   if(EligibleSession(server_time)==0 || UtcMinute(server_time)>=InpFlattenUtcHour*60)
      return false;
   if(g_trades_today>=InpMaxTradesPerDay || DailyLossHit() || AccountDrawdownHit() || CooldownActive())
      return false;
   if(OwnedPositionTicket()!=0 || ForeignSymbolExposure())
      return false;
...
bool TryOpenTrade(const int direction,const double raw_stop,const string reason)
  {
   ...
   if(!OrderCheck(request,check) || (check.retcode!=TRADE_RETCODE_DONE && check.retcode!=TRADE_RETCODE_PLACED))
     {
      PrintFormat("ICTFVG OrderCheck rejected retcode=%u comment=%s",check.retcode,check.comment);
      g_risk_rejections++;
      return false;
     }
   ...
   if(!sent)
     {
      PrintFormat("ICTFVG entry failed retcode=%u %s",trade.ResultRetcode(),trade.ResultRetcodeDescription());
      g_initial_entry=0.0;
      g_initial_stop=0.0;
...
void DetectSweep(const MqlRates &bars[],const int count,const int date_key,const int session_id)
  {
   ...
   if(!FindLatestM15BreakLevel(direction,break_level))
      return;
   AddSweepSetup(direction,date_key,session_id,bars[0],break_level);
  }
...
void AdvanceRetest(SetupState &setup,const MqlRates &closed_bar,const int date_key)
  {
   ...
   g_retests++;
   double adx=0.0;
   if(!ClosedM15Adx(adx) || adx<=InpMinAdx)
     {
      g_adx_rejections++;
      ClearSetup(setup);
      return;
...
bool WriteRunMeta()
  {
   ...
   string payload=StringFormat(
      "{\"schema_version\":\"alphafactory_run_meta.v1\",\"run_id\":\"%s\",\"ea_name\":\"%s\",\"symbol\":\"%s\",\"telemetry_profile\":\"%s\",\"hypothesis_id\":\"%s\",\"signal_mode\":%d,\"promotion_eligible\":false,\"news_status\":\"UNMET\",\"diagnostic\":{\"days_seen\":%I64d,\"sweeps\":%I64d,\"displacements\":%I64d,\"fvgs\":%I64d,\"mss\":%I64d,\"pre_mss_mitigations\":%I64d,\"retests\":%I64d,\"adx_rejections\":%I64d,\"spread_rejections\":%I64d,\"risk_rejections\":%I64d,\"entries_attempted\":%I64d,\"entries_opened\":%I64d,\"fill_risk_closes\":%I64d}}",
      ...
      g_retests,g_adx_rejections,g_spread_rejections,g_risk_rejections,
...
void ManageOwnedPosition()
  {
   ...
   if(now_key!=g_position_entry_utc_key || minute>=InpFlattenUtcHour*60 || AccountDrawdownHit())
     {
...
bool ValidateInputs()
  {
   ...
   if(InpRequireNewsGuard)
     {
      Print("Historical news guard requested but no hash-bound calendar is installed; fail closed.");
      return false;
     }
...
int OnInit()
  {
   ...
   if(!OpenLifecycleTelemetry())
      return INIT_FAILED;
   PrintFormat("ICTFVG init hypothesis=%s mode=%s auto=%s news=UNMET promotion=false",
               HYPOTHESIS_ID,
               InpSignalMode==SIGNAL_REPORT_FIDELITY ? "FULL_FIDELITY" : "CONTROL_SWEEP",
...
void OnTick()
  {
   ...
   if(current_bar<=0 || current_bar==g_last_m5_bar)
      return;
