...
enum ENUM_SIGNAL_MODE
  {
   ...
   SIGNAL_HUMAN_CONTEXT_POLICY=3,
   SIGNAL_TICK_INITIATION_COLLECTION=4,
   SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION=5,
   SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION=6,
   SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION=7
  };
...
#include "HumanContextEngine.mqh"
...
struct SetupState
  {
   ...
   bool level_path_active;
   double level_path_level;
   datetime level_path_start_time;
   datetime level_path_last_tick_time;
   int level_path_side;
   bool level_path_first_favorable;
   datetime level_path_first_favorable_time;
   long level_path_valid_ticks;
   long level_path_invalid_ticks;
   long level_path_adverse_reentries;
   bool resilience_active;
   double resilience_level;
   long resilience_start_msc;
   long resilience_last_valid_msc;
   int resilience_side;
   bool resilience_first_favorable;
   long resilience_first_favorable_msc;
   long resilience_favorable_ms;
   long resilience_adverse_ms;
   long resilience_max_gap_ms;
   long resilience_valid_ticks;
   long resilience_invalid_ticks;
  };
...
TickPathProfile g_live_tick_profile;
TickPathProfile g_closed_tick_profile;
...
int g_human_context_handle=INVALID_HANDLE;
int g_tick_initiation_handle=INVALID_HANDLE;
int g_level_path_handle=INVALID_HANDLE;
int g_level_resilience_handle=INVALID_HANDLE;
string g_run_id="";
string g_lifecycle_name="";
string g_human_context_name="";
string g_tick_initiation_name="";
string g_level_path_name="";
string g_level_resilience_name="";
string g_run_meta_name="";
bool g_level_path_slot_active[MAX_SETUPS];
int g_active_level_path_indices[MAX_SETUPS];
int g_active_level_path_count=0;
bool g_resilience_slot_active[MAX_SETUPS];
int g_active_resilience_indices[MAX_SETUPS];
int g_active_resilience_count=0;
...
long g_human_context_snapshots=0;
long g_human_context_invalid=0;
long g_human_policy_rejections=0;
long g_tick_profiles_logged=0;
long g_tick_profiles_defined=0;
long g_tick_sign_agree=0;
long g_tick_sign_nonagree=0;
long g_tick_profile_identity_invalid=0;
long g_level_paths_logged=0;
long g_level_paths_defined=0;
long g_level_paths_orderly=0;
long g_level_paths_repeated_churn=0;
long g_level_path_identity_invalid=0;
long g_resilience_logged=0;
long g_resilience_defined=0;
long g_resilience_favorable_dominant=0;
long g_resilience_adverse_dominant=0;
long g_resilience_identity_invalid=0;
long g_resilience_duration_invalid=0;
...
void ResetTickPathProfile(TickPathProfile &profile,const datetime bar_time)
  {
   ZeroMemory(profile);
   profile.bar_time=bar_time;
  }

void AccumulateTickPath(TickPathProfile &profile,const MqlTick &tick)
  {
   if(!MathIsValidNumber(tick.bid) || !MathIsValidNumber(tick.ask) ||
      tick.bid<=0.0 || tick.ask<=0.0 || tick.ask<tick.bid)
     {
      profile.invalid_ticks++;
      return;
     }
   ...
   if(!profile.has_mid)
     {
      profile.has_mid=true;
      profile.first_mid=mid;
      profile.last_mid=mid;
      profile.first_spread=spread;
      profile.last_spread=spread;
      profile.max_spread=spread;
      return;
     }
   if(mid>profile.last_mid)
      profile.up_ticks++;
   else if(mid<profile.last_mid)
      profile.down_ticks++;
   else
      profile.flat_ticks++;
   profile.path_length+=MathAbs(mid-profile.last_mid);
   profile.last_mid=mid;
   profile.last_spread=spread;
   profile.max_spread=MathMax(profile.max_spread,spread);
  }

bool TickPathImbalance(const TickPathProfile &profile,double &imbalance)
  {
   long nonzero=profile.up_ticks+profile.down_ticks;
   if(!profile.has_mid || nonzero<=0)
     {
      imbalance=0.0;
      return false;
     }
   imbalance=(double)(profile.up_ticks-profile.down_ticks)/(double)nonzero;
   return MathIsValidNumber(imbalance);
  }

bool TickPathSignAgrees(const int direction,const double imbalance)
  {
   return (direction>0 && imbalance>0.0) ||
          (direction<0 && imbalance<0.0);
  }

void SealTickPathOnNewBar(const datetime current_bar,const MqlTick &tick)
  {
   if(current_bar<=0)
      return;
   if(g_live_tick_profile.bar_time<=0)
     {
      ResetTickPathProfile(g_live_tick_profile,current_bar);
      AccumulateTickPath(g_live_tick_profile,tick);
      return;
     }
   if(current_bar==g_live_tick_profile.bar_time)
     {
      AccumulateTickPath(g_live_tick_profile,tick);
      return;
     }
   if(current_bar>g_live_tick_profile.bar_time)
     {
      g_closed_tick_profile=g_live_tick_profile;
      ResetTickPathProfile(g_live_tick_profile,current_bar);
      AccumulateTickPath(g_live_tick_profile,tick);
     }
  }
...
datetime NthSunday(const int year,const int month,const int occurrence,const int hour)
  {
   datetime value=MakeDateTime(year,month,1,hour);
   MqlDateTime parts;
   TimeToStruct(value,parts);
   int days_to_sunday=(7-parts.day_of_week)%7;
   return value+(days_to_sunday+7*(occurrence-1))*86400;
  }
...
bool IsUsDstServerTime(const datetime server_time)
  {
   MqlDateTime parts;
   TimeToStruct(server_time,parts);
   datetime utc_guess=server_time-InpServerUtcOffsetWinterHours*3600;
   datetime start=NthSunday(parts.year,3,2,7);
   datetime finish=NthSunday(parts.year,11,1,6);
   return utc_guess>=start && utc_guess<finish;
  }
...
bool IsBrokerDstServerTime(const datetime server_time)
  {
   ...
   TimeToStruct(server_time,parts);
   if(parts.year>=2024)
      return IsUsDstServerTime(server_time);
   return IsEuropeDstServerTime(server_time);
  }

datetime ServerToUtc(const datetime server_time)
  {
   int offset=InpServerUtcOffsetWinterHours+(IsBrokerDstServerTime(server_time) ? 1 : 0);
...
void RegisterActiveLevelPathSlot(const int slot)
  {
   if(InpSignalMode!=SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION ||
      slot<0 || slot>=MAX_SETUPS)
      return;
   for(int index=0;index<g_active_level_path_count;index++)
      if(g_active_level_path_indices[index]==slot)
        {
         g_level_path_slot_active[slot]=true;
         return;
        }
   if(g_active_level_path_count>=MAX_SETUPS)
     {
      Print("ICTFVG level-path registry capacity exhausted");
      return;
     }
   g_level_path_slot_active[slot]=true;
   g_active_level_path_indices[g_active_level_path_count++]=slot;
  }

void AccumulateSetupLevelPath(SetupState &setup,const MqlTick &tick)
  {
   if(!setup.level_path_active || setup.stage!=SETUP_SWEPT ||
      tick.time<setup.level_path_start_time)
      return;
   if(!MathIsValidNumber(tick.bid) || !MathIsValidNumber(tick.ask) ||
      !MathIsValidNumber(setup.level_path_level) ||
      tick.bid<=0.0 || tick.ask<=0.0 || tick.ask<tick.bid)
     {
      setup.level_path_invalid_ticks++;
      return;
     }
   setup.level_path_valid_ticks++;
   setup.level_path_last_tick_time=tick.time;
   double mid=0.5*(tick.bid+tick.ask);
   double directed_distance=setup.direction*(mid-setup.level_path_level);
   int next_side=setup.level_path_side;
   if(directed_distance>0.0)
      next_side=1;
   else if(directed_distance<0.0)
      next_side=-1;
   else
      return;
   if(next_side==1 && !setup.level_path_first_favorable)
     {
      setup.level_path_first_favorable=true;
      setup.level_path_first_favorable_time=tick.time;
     }
   if(setup.level_path_first_favorable &&
      setup.level_path_side==1 && next_side==-1)
      setup.level_path_adverse_reentries++;
   setup.level_path_side=next_side;
  }

void AccumulateActiveLevelPaths(const MqlTick &tick)
  {
   if(InpSignalMode!=SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION)
      return;
   int write_index=0;
   for(int read_index=0;read_index<g_active_level_path_count;read_index++)
     {
      int slot=g_active_level_path_indices[read_index];
      if(slot<0 || slot>=MAX_SETUPS || !g_level_path_slot_active[slot])
         continue;
      g_active_level_path_indices[write_index++]=slot;
      AccumulateSetupLevelPath(g_setups[slot],tick);
     }
   g_active_level_path_count=write_index;
  }

void RegisterActiveResilienceSlot(const int slot)
  {
   if((InpSignalMode!=SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION &&
       InpSignalMode!=SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION) ||
      slot<0 || slot>=MAX_SETUPS)
      return;
   for(int index=0;index<g_active_resilience_count;index++)
      if(g_active_resilience_indices[index]==slot)
        {
         g_resilience_slot_active[slot]=true;
         return;
        }
   if(g_active_resilience_count>=MAX_SETUPS)
     {
      Print("ICTFVG resilience registry capacity exhausted");
      return;
     }
   g_resilience_slot_active[slot]=true;
   g_active_resilience_indices[g_active_resilience_count++]=slot;
  }

void AccumulateSetupResilience(SetupState &setup,const MqlTick &tick)
  {
   if(!setup.resilience_active || setup.stage!=SETUP_SWEPT ||
      tick.time_msc<setup.resilience_start_msc)
      return;
   if(tick.time_msc<=0 || !MathIsValidNumber(tick.bid) ||
      !MathIsValidNumber(tick.ask) ||
      !MathIsValidNumber(setup.resilience_level) ||
      tick.bid<=0.0 || tick.ask<=0.0 || tick.ask<tick.bid)
     {
      setup.resilience_invalid_ticks++;
      return;
     }
   if(setup.resilience_first_favorable &&
      tick.time_msc<setup.resilience_last_valid_msc)
     {
      setup.resilience_invalid_ticks++;
      return;
     }
   setup.resilience_valid_ticks++;
   double mid=0.5*(tick.bid+tick.ask);
   double directed_distance=setup.direction*(mid-setup.resilience_level);
   int next_side=setup.resilience_side;
   if(directed_distance>0.0)
      next_side=1;
   else if(directed_distance<0.0)
      next_side=-1;
   if(!setup.resilience_first_favorable)
     {
      if(next_side!=1)
         return;
      setup.resilience_first_favorable=true;
      setup.resilience_first_favorable_msc=tick.time_msc;
      setup.resilience_last_valid_msc=tick.time_msc;
      setup.resilience_side=1;
      return;
     }
   long delta_msc=tick.time_msc-setup.resilience_last_valid_msc;
   if(delta_msc>setup.resilience_max_gap_ms)
      setup.resilience_max_gap_ms=delta_msc;
   if(setup.resilience_side==1)
      setup.resilience_favorable_ms+=delta_msc;
   else if(setup.resilience_side==-1)
      setup.resilience_adverse_ms+=delta_msc;
   setup.resilience_side=next_side;
   setup.resilience_last_valid_msc=tick.time_msc;
  }

void AccumulateActiveResilience(const MqlTick &tick)
  {
   if(InpSignalMode!=SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION &&
      InpSignalMode!=SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION)
      return;
   int write_index=0;
   for(int read_index=0;read_index<g_active_resilience_count;read_index++)
     {
      int slot=g_active_resilience_indices[read_index];
      if(slot<0 || slot>=MAX_SETUPS || !g_resilience_slot_active[slot])
         continue;
      g_active_resilience_indices[write_index++]=slot;
      AccumulateSetupResilience(g_setups[slot],tick);
     }
   g_active_resilience_count=write_index;
  }

void ClearSetup(SetupState &setup)
  {
   int slot=setup.slot_index;
   if(slot>=0 && slot<MAX_SETUPS)
     {
      g_level_path_slot_active[slot]=false;
      g_resilience_slot_active[slot]=false;
     }
   ZeroMemory(setup);
   setup.stage=SETUP_EMPTY;
   setup.slot_index=slot;
  }

bool AddSweepSetup(const int direction,const int date_key,const int session_id,
                   const MqlRates &bar,const double swept_pivot_level,
                   const double m15_break_level,
   ...
   for(int index=0;index<MAX_SETUPS;index++)
     {
      ...
      g_setups[index].sweep_high=bar.high;
       ...
       g_setups[index].sweep_close=bar.close;
       g_setups[index].swept_pivot_level=swept_pivot_level;
       g_setups[index].m15_break_level=m15_break_level;
        if(InpSignalMode==SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION)
         {
          g_setups[index].level_path_active=true;
          g_setups[index].level_path_level=(direction>0 ? bar.low : bar.high);
          g_setups[index].level_path_start_time=bar.time+PeriodSeconds(PERIOD_M5);
           RegisterActiveLevelPathSlot(index);
          }
        if(InpSignalMode==SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION)
          {
           g_setups[index].resilience_active=true;
           g_setups[index].resilience_level=(direction>0 ? bar.low : bar.high);
           g_setups[index].resilience_start_msc=
              1000*(long)(bar.time+PeriodSeconds(PERIOD_M5));
           RegisterActiveResilienceSlot(index);
          }
        if(InpSignalMode==SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION)
          {
           g_setups[index].resilience_active=true;
           g_setups[index].resilience_level=swept_pivot_level;
           g_setups[index].resilience_start_msc=
              1000*(long)(bar.time+PeriodSeconds(PERIOD_M5));
           RegisterActiveResilienceSlot(index);
          }
...
bool LogHumanContextDecision(const int direction,
                             ...
                             const double raw_stop,const string reason)
  {
   bool log_enabled=InpEnableTelemetry && g_human_context_handle!=INVALID_HANDLE;
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol,tick))
     {
      if(log_enabled)
         g_human_context_invalid++;
      return false;
     }
   double entry=(direction>0 ? tick.ask : tick.bid);
   HumanContextSnapshot context;
   bool valid=BuildHumanContextSnapshot(direction,decision_server_time,
                                        sweep_high,sweep_low,sweep_close,
                                        confirmation_bar,entry,raw_stop,
                                        tick.ask-tick.bid,context);
   bool policy_accept=valid &&
                      (context.state==HUMAN_CONTEXT_EXTERNAL_SWEEP_WITH_ROOM ||
                       context.state==HUMAN_CONTEXT_INTERNAL_SWEEP_WITH_ROOM);
   if(log_enabled)
     {
   if(valid)
      g_human_context_snapshots++;
   else
      g_human_context_invalid++;
   long event_sequence=g_human_context_snapshots+g_human_context_invalid;
   string event_id=StringFormat("%s_%I64d_%s_%d_%I64d",g_run_id,
                                (long)decision_server_time,reason,direction,
                                event_sequence);
   FileWrite(g_human_context_handle,
             event_id,TimeToString(decision_server_time,TIME_DATE|TIME_SECONDS),
             reason,direction,DoubleToString(entry,_Digits),
             DoubleToString(raw_stop,_Digits),
             DoubleToString(MathAbs(entry-raw_stop),_Digits),
             valid ? "1" : "0",HumanContextStateName(context.state),
             DoubleToString(context.h1_range_low,_Digits),
             DoubleToString(context.h1_range_high,_Digits),
             DoubleToString(context.h1_range_location,12),
             DoubleToString(context.h4_range_low,_Digits),
             DoubleToString(context.h4_range_high,_Digits),
             DoubleToString(context.h4_range_location,12),
             context.h1_structure,context.h4_structure,
             context.h1_aligned ? "1" : "0",
             context.h4_aligned ? "1" : "0",
             DoubleToString(context.h1_pivot_high,_Digits),
             DoubleToString(context.h1_pivot_low,_Digits),
             DoubleToString(context.h4_pivot_high,_Digits),
             DoubleToString(context.h4_pivot_low,_Digits),
             DoubleToString(context.previous_day_high,_Digits),
             DoubleToString(context.previous_day_low,_Digits),
             DoubleToString(context.previous_week_high,_Digits),
             DoubleToString(context.previous_week_low,_Digits),
             DoubleToString(context.asia_high,_Digits),
             DoubleToString(context.asia_low,_Digits),
             context.nearest_pool_type,
             DoubleToString(context.nearest_pool_price,_Digits),
             DoubleToString(context.nearest_pool_pips,8),
             context.directional_pool_count,DoubleToString(context.room_r,8),
             context.room_to_target ? "1" : "0",
             context.external_sweep ? "1" : "0",
             context.external_swept_count,
             DoubleToString(context.partial_h1_body_atr,8),
             DoubleToString(context.partial_h4_body_atr,8),
             DoubleToString(context.confirmation_body_atr,8),
             context.directional_run_bars,
             DoubleToString(context.h1_extension_atr,8),
             DoubleToString(context.h4_extension_atr,8),
             DoubleToString(context.spread_to_risk,8));
   FileFlush(g_human_context_handle);
     }
   return policy_accept;
  }

bool LogTickInitiationDecision(const int direction,
                               const MqlRates &confirmation_bar)
  {
   g_tick_profiles_logged++;
   bool profile_identity_valid=
      (g_closed_tick_profile.bar_time==confirmation_bar.time);
   if(!profile_identity_valid)
      g_tick_profile_identity_invalid++;
   double imbalance=0.0;
   bool imbalance_defined=profile_identity_valid &&
                          TickPathImbalance(g_closed_tick_profile,imbalance);
   bool sign_agree=imbalance_defined &&
                   TickPathSignAgrees(direction,imbalance);
   if(imbalance_defined)
     {
   ...
   if(InpEnableTelemetry && g_tick_initiation_handle!=INVALID_HANDLE)
     {
      long nonzero=g_closed_tick_profile.up_ticks+
                   g_closed_tick_profile.down_ticks;
      double net_mid_change=(g_closed_tick_profile.has_mid ?
                             g_closed_tick_profile.last_mid-
                             g_closed_tick_profile.first_mid : 0.0);
      datetime decision_time=confirmation_bar.time+PeriodSeconds(PERIOD_M5);
      string event_id=StringFormat("%s_%I64d_TICK_INIT_%d_%I64d",g_run_id,
                                   (long)decision_time,direction,
                                   g_tick_profiles_logged);
      FileWrite(g_tick_initiation_handle,
                event_id,
                TimeToString(decision_time,TIME_DATE|TIME_SECONDS),
                TimeToString(confirmation_bar.time,TIME_DATE|TIME_SECONDS),
                TimeToString(g_closed_tick_profile.bar_time,
                             TIME_DATE|TIME_SECONDS),
                direction,
                g_closed_tick_profile.valid_ticks,
                g_closed_tick_profile.invalid_ticks,
                g_closed_tick_profile.up_ticks,
                g_closed_tick_profile.down_ticks,
                g_closed_tick_profile.flat_ticks,
                nonzero,
                imbalance_defined ? DoubleToString(imbalance,12) : "",
                sign_agree ? "1" : "0",
                DoubleToString(g_closed_tick_profile.first_mid,_Digits),
                DoubleToString(g_closed_tick_profile.last_mid,_Digits),
                DoubleToString(g_closed_tick_profile.path_length,12),
                DoubleToString(net_mid_change,12),
                DoubleToString(g_closed_tick_profile.first_spread,12),
                DoubleToString(g_closed_tick_profile.last_spread,12),
                DoubleToString(g_closed_tick_profile.max_spread,12),
                profile_identity_valid ? "1" : "0",
                "MODEL0_REAL_TICK_ONLINE_V1");
      FileFlush(g_tick_initiation_handle);
     }
   return sign_agree;
  }

void LogLevelPathDecision(SetupState &setup,const MqlRates &confirmation_bar)
  {
   g_level_paths_logged++;
   datetime decision_time=confirmation_bar.time+PeriodSeconds(PERIOD_M5);
   bool interval_identity_valid=
      setup.level_path_active &&
      setup.level_path_start_time==setup.sweep_time+PeriodSeconds(PERIOD_M5) &&
      setup.level_path_last_tick_time>=setup.level_path_start_time &&
      setup.level_path_last_tick_time<decision_time;
   if(!interval_identity_valid)
      g_level_path_identity_invalid++;
   bool path_defined=interval_identity_valid &&
                     setup.level_path_valid_ticks>0 &&
                     setup.level_path_first_favorable;
   string path_label="UNDEFINED";
   if(path_defined)
     {
      g_level_paths_defined++;
      if(setup.level_path_adverse_reentries<=1)
        {
         path_label="ORDERLY";
         g_level_paths_orderly++;
        }
      else
        {
         path_label="REPEATED_CHURN";
         g_level_paths_repeated_churn++;
        }
     }
   if(InpEnableTelemetry && g_level_path_handle!=INVALID_HANDLE)
     {
      string event_id=StringFormat("%s_%I64d_LEVEL_PATH_%d_%I64d",g_run_id,
                                   (long)decision_time,setup.direction,
                                   g_level_paths_logged);
      string last_tick_time=(setup.level_path_last_tick_time>0 ?
         TimeToString(setup.level_path_last_tick_time,TIME_DATE|TIME_SECONDS) : "");
      string first_favorable_time=(setup.level_path_first_favorable_time>0 ?
         TimeToString(setup.level_path_first_favorable_time,TIME_DATE|TIME_SECONDS) : "");
      FileWrite(g_level_path_handle,
                event_id,
                TimeToString(decision_time,TIME_DATE|TIME_SECONDS),
                TimeToString(setup.sweep_time,TIME_DATE|TIME_SECONDS),
                TimeToString(confirmation_bar.time,TIME_DATE|TIME_SECONDS),
                setup.direction,
                DoubleToString(setup.level_path_level,_Digits),
                TimeToString(setup.level_path_start_time,TIME_DATE|TIME_SECONDS),
                last_tick_time,
                setup.level_path_valid_ticks,
                setup.level_path_invalid_ticks,
                ...
                setup.level_path_adverse_reentries,
                path_label,
                setup.level_path_side,
                interval_identity_valid ? "1" : "0",
                "MODEL0_REAL_TICK_SETUP_LEVEL_PATH_V1");
      FileFlush(g_level_path_handle);
     }
  }

void LogLevelResilienceDecision(SetupState &setup,const MqlRates &confirmation_bar)
  {
   g_resilience_logged++;
   datetime decision_time=confirmation_bar.time+PeriodSeconds(PERIOD_M5);
   long decision_msc=1000*(long)decision_time;
   bool interval_identity_valid=
      setup.resilience_active &&
      setup.resilience_start_msc==1000*(long)(setup.sweep_time+PeriodSeconds(PERIOD_M5)) &&
      setup.resilience_last_valid_msc>=setup.resilience_start_msc &&
      setup.resilience_last_valid_msc<decision_msc;
   if(!interval_identity_valid)
      g_resilience_identity_invalid++;
   bool duration_identity_valid=false;
   if(interval_identity_valid && setup.resilience_first_favorable &&
      setup.resilience_first_favorable_msc>=setup.resilience_start_msc)
     {
      long tail_msc=decision_msc-setup.resilience_last_valid_msc;
      if(tail_msc>setup.resilience_max_gap_ms)
         setup.resilience_max_gap_ms=tail_msc;
      if(setup.resilience_side==1)
         setup.resilience_favorable_ms+=tail_msc;
      else if(setup.resilience_side==-1)
         setup.resilience_adverse_ms+=tail_msc;
      long total_msc=setup.resilience_favorable_ms+setup.resilience_adverse_ms;
      duration_identity_valid=(total_msc>0 &&
         total_msc==decision_msc-setup.resilience_first_favorable_msc);
     }
   if(!duration_identity_valid)
      g_resilience_duration_invalid++;
   string resilience_label="UNDEFINED";
   if(duration_identity_valid &&
      setup.resilience_favorable_ms>setup.resilience_adverse_ms)
     {
      resilience_label="FAVORABLE_DOMINANT";
      g_resilience_defined++;
      g_resilience_favorable_dominant++;
     }
   else if(duration_identity_valid &&
           setup.resilience_adverse_ms>setup.resilience_favorable_ms)
     {
      resilience_label="ADVERSE_DOMINANT";
      g_resilience_defined++;
      g_resilience_adverse_dominant++;
     }
   if(InpEnableTelemetry && g_level_resilience_handle!=INVALID_HANDLE)
     {
      string event_id=StringFormat("%s_%I64d_LEVEL_RESILIENCE_%d_%I64d",g_run_id,
                                   (long)decision_time,setup.direction,
                                   g_resilience_logged);
      long total_msc=setup.resilience_favorable_ms+setup.resilience_adverse_ms;
      FileWrite(g_level_resilience_handle,
                event_id,
                TimeToString(decision_time,TIME_DATE|TIME_SECONDS),
                decision_msc,
                TimeToString(setup.sweep_time,TIME_DATE|TIME_SECONDS),
                TimeToString(confirmation_bar.time,TIME_DATE|TIME_SECONDS),
                setup.direction,
                DoubleToString(setup.resilience_level,_Digits),
                setup.resilience_start_msc,
                setup.resilience_last_valid_msc,
                setup.resilience_valid_ticks,
                setup.resilience_invalid_ticks,
                setup.resilience_first_favorable_msc,
                setup.resilience_favorable_ms,
                setup.resilience_adverse_ms,
                total_msc,
                setup.resilience_max_gap_ms,
                resilience_label,
                setup.resilience_side,
                interval_identity_valid ? "1" : "0",
                duration_identity_valid ? "1" : "0",
                "MODEL0_REAL_TICK_TIME_MSC_LEVEL_RESILIENCE_V1");
      FileFlush(g_level_resilience_handle);
     }
  }
...
void DetectSweep(const MqlRates &bars[],const int count,const int date_key,const int session_id)
  {
   ...
   if(direction==0)
      return;
   double swept_pivot_level=(direction>0 ? pivot_low : pivot_high);
   ...
   if(InpSignalMode==SIGNAL_HIGH_RECALL_CONTROL)
     {
      ...
      LogHumanContextDecision(direction,bars[0].time+PeriodSeconds(PERIOD_M5),
                              bars[0].high,bars[0].low,bars[0].close,
                              bars[0],stop,"ICTFVG_CONTROL_SWEEP");
      TryOpenTrade(direction,stop,"ICTFVG_CONTROL_SWEEP");
      return;
     }

   if(InpSignalMode==SIGNAL_HUMAN_CONTEXT_POLICY)
     {
      g_sweeps++;
      double stop=(direction>0 ? bars[0].low-PipsToPrice(InpStopBufferPips)
                               : bars[0].high+PipsToPrice(InpStopBufferPips));
      bool policy_accept=LogHumanContextDecision(
         direction,bars[0].time+PeriodSeconds(PERIOD_M5),
         bars[0].high,bars[0].low,bars[0].close,
         bars[0],stop,"ICTFVG_HUMAN_CONTEXT_POLICY");
      if(!policy_accept)
        {
         g_human_policy_rejections++;
         return;
        }
      TryOpenTrade(direction,stop,"ICTFVG_HUMAN_CONTEXT_POLICY");
      return;
     }

   bool context_collection_mode=(InpSignalMode==SIGNAL_CONTEXT_STATE ||
                                 InpSignalMode==SIGNAL_TICK_INITIATION_COLLECTION);
   if(InpSignalMode==SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION)
      context_collection_mode=true;
   if(InpSignalMode==SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION)
      context_collection_mode=true;
   if(InpSignalMode==SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION)
      context_collection_mode=true;
   if(context_collection_mode)
     {
      g_sweeps++;
      ...
      AddSweepSetup(direction,date_key,session_id,bars[0],swept_pivot_level,
                    0.0,false);
      return;
     }
...
void AdvanceContextState(SetupState &setup,const MqlRates &bars[],const int count,
                         const int date_key)
  {
   ...
   LogHumanContextDecision(setup.direction,
                           bars[0].time+PeriodSeconds(PERIOD_M5),
                           setup.sweep_high,setup.sweep_low,setup.sweep_close,
                           bars[0],stop,"ICTFVG_CONTEXT_STATE");
   if(InpSignalMode==SIGNAL_TICK_INITIATION_COLLECTION)
     {
      LogTickInitiationDecision(setup.direction,bars[0]);
      ClearSetup(setup);
      return;
     }
   if(InpSignalMode==SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION)
     {
      LogLevelPathDecision(setup,bars[0]);
      ClearSetup(setup);
      return;
     }
   if(InpSignalMode==SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION)
     {
      LogLevelResilienceDecision(setup,bars[0]);
      ClearSetup(setup);
      return;
     }
   if(InpSignalMode==SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION)
...
void AdvanceRetest(SetupState &setup,const MqlRates &closed_bar,const int date_key)
  {
   ...
   g_adx_passes++;
   LogHumanContextDecision(setup.direction,
                           closed_bar.time+PeriodSeconds(PERIOD_M5),
                           setup.sweep_high,setup.sweep_low,setup.sweep_close,
                           closed_bar,setup.stop,"ICTFVG_FULL_FIDELITY");
...
bool WriteRunMeta()
  {
   ...
   string payload=StringFormat(
      ...
      g_context_timeouts,g_context_confirmations,g_human_context_snapshots,
      g_human_context_invalid,g_human_policy_rejections,g_tick_profiles_logged,
      g_tick_profiles_defined,g_tick_sign_agree,g_tick_sign_nonagree,
      g_tick_profile_identity_invalid,g_level_paths_logged,g_level_paths_defined,
      g_level_paths_orderly,g_level_paths_repeated_churn,
      g_level_path_identity_invalid,g_resilience_logged,g_resilience_defined,
      g_resilience_favorable_dominant,g_resilience_adverse_dominant,
      g_resilience_identity_invalid,g_resilience_duration_invalid);
   FileWriteString(handle,payload);
   FileClose(handle);
   return true;
  }

bool OpenHumanContextTelemetry()
  {
   if(!InpEnableTelemetry)
      return true;
   g_human_context_name=StringFormat("%s_HumanContext_%s.csv",_Symbol,g_run_id);
   g_human_context_handle=FileOpen(g_human_context_name,
                                   FILE_WRITE|FILE_CSV|FILE_ANSI,',');
   if(g_human_context_handle==INVALID_HANDLE)
      return false;
   FileWrite(g_human_context_handle,
             "event_id","decision_time","reason","direction","entry","stop",
             "risk_price","valid","context_state",
             "h1_range_low","h1_range_high","h1_range_location",
             "h4_range_low","h4_range_high","h4_range_location",
             "h1_structure","h4_structure","h1_aligned","h4_aligned",
             "h1_pivot_high","h1_pivot_low","h4_pivot_high","h4_pivot_low",
             "previous_day_high","previous_day_low","previous_week_high",
             "previous_week_low","asia_high","asia_low","nearest_pool_type",
             "nearest_pool_price","nearest_pool_pips","directional_pool_count",
             "room_r","room_to_target","external_sweep","external_swept_count",
             "partial_h1_body_atr","partial_h4_body_atr",
             "confirmation_body_atr","directional_run_bars",
             "h1_extension_atr","h4_extension_atr","spread_to_risk");
   FileFlush(g_human_context_handle);
   return true;
  }

bool OpenTickInitiationTelemetry()
  {
   if(!InpEnableTelemetry)
      return true;
   g_tick_initiation_name=StringFormat("%s_TickInitiation_%s.csv",_Symbol,
                                       g_run_id);
   g_tick_initiation_handle=FileOpen(g_tick_initiation_name,
                                     FILE_WRITE|FILE_CSV|FILE_ANSI,',');
   if(g_tick_initiation_handle==INVALID_HANDLE)
      return false;
   FileWrite(g_tick_initiation_handle,
             "event_id","decision_time","confirmation_bar_time",
             "profile_bar_time","direction","valid_ticks","invalid_ticks",
             "up_ticks","down_ticks","flat_ticks","nonzero_ticks",
             "imbalance","sign_agree","first_mid","last_mid",
             "path_length","net_mid_change","first_spread","last_spread",
             "max_spread","profile_identity_valid","tick_provenance");
   FileFlush(g_tick_initiation_handle);
   return true;
  }

bool OpenLevelPathTelemetry()
  {
   if(!InpEnableTelemetry)
      return true;
   g_level_path_name=StringFormat("%s_LevelPath_%s.csv",_Symbol,g_run_id);
   g_level_path_handle=FileOpen(g_level_path_name,
                                FILE_WRITE|FILE_CSV|FILE_ANSI,',');
   if(g_level_path_handle==INVALID_HANDLE)
      return false;
   FileWrite(g_level_path_handle,
...
bool OpenLevelResilienceTelemetry()
  {
   if(!InpEnableTelemetry)
      return true;
   g_level_resilience_name=StringFormat("%s_LevelResilience_%s.csv",_Symbol,g_run_id);
   g_level_resilience_handle=FileOpen(g_level_resilience_name,
                                      FILE_WRITE|FILE_CSV|FILE_ANSI,',');
   if(g_level_resilience_handle==INVALID_HANDLE)
      return false;
   FileWrite(g_level_resilience_handle,
...
bool OpenLifecycleTelemetry()
  {
   ...
   return OpenHumanContextTelemetry() && OpenTickInitiationTelemetry() &&
          OpenLevelPathTelemetry() && OpenLevelResilienceTelemetry() &&
          WriteRunMeta();
  }
...
bool ValidateInputs()
  {
   ...
   if(InpRiskPercent<=0.0 || InpRiskPercent>1.0 || InpPivotStrength<1 ||
      ...
      InpContextCloseFraction>=0.5 || InpHumanRangeBars<5 ||
      InpHumanPivotStrength<1 || InpHumanPivotLookback<30 ||
      InpHumanAtrPeriod<5 || InpStopBufferPips<=0.0 ||
...
string SignalModeName()
  {
   ...
   if(InpSignalMode==SIGNAL_HUMAN_CONTEXT_POLICY)
      return "HUMAN_CONTEXT_POLICY";
   if(InpSignalMode==SIGNAL_TICK_INITIATION_COLLECTION)
      return "TICK_INITIATION_COLLECTION";
   if(InpSignalMode==SIGNAL_REPEATED_LEVEL_CHURN_COLLECTION)
      return "REPEATED_LEVEL_CHURN_COLLECTION";
   if(InpSignalMode==SIGNAL_TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION)
      return "TIME_WEIGHTED_LEVEL_RESILIENCE_COLLECTION";
   if(InpSignalMode==SIGNAL_PIVOT_RECLAIM_DWELL_COLLECTION)
      return "PIVOT_RECLAIM_DWELL_COLLECTION";
...
int OnInit()
  {
   ...
   g_active_level_path_count=0;
   g_active_resilience_count=0;
   for(int index=0;index<MAX_SETUPS;index++)
     {
      ZeroMemory(g_setups[index]);
      g_setups[index].stage=SETUP_EMPTY;
      g_setups[index].slot_index=index;
      g_level_path_slot_active[index]=false;
      g_resilience_slot_active[index]=false;
     }
...
void OnDeinit(const int reason)
  {
   ...
   if(g_human_context_handle!=INVALID_HANDLE)
     {
      FileFlush(g_human_context_handle);
      FileClose(g_human_context_handle);
      g_human_context_handle=INVALID_HANDLE;
     }
   if(g_tick_initiation_handle!=INVALID_HANDLE)
     {
      FileFlush(g_tick_initiation_handle);
      FileClose(g_tick_initiation_handle);
      g_tick_initiation_handle=INVALID_HANDLE;
     }
   if(g_level_path_handle!=INVALID_HANDLE)
     {
      FileFlush(g_level_path_handle);
      FileClose(g_level_path_handle);
      g_level_path_handle=INVALID_HANDLE;
     }
   if(g_level_resilience_handle!=INVALID_HANDLE)
     {
      FileFlush(g_level_resilience_handle);
      FileClose(g_level_resilience_handle);
      g_level_resilience_handle=INVALID_HANDLE;
     }
...
void OnTick()
  {
   ...
   if(current_bar<=0)
      return;
   SealTickPathOnNewBar(current_bar,tick);
   if(current_bar==g_last_m5_bar)
     {
      AccumulateActiveLevelPaths(tick);
      AccumulateActiveResilience(tick);
      return;
     }
