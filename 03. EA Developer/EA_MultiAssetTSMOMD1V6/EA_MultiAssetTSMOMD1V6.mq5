...
//+------------------------------------------------------------------+
//| EA_MultiAssetTSMOMD1V6.mq5                                      |
//| HYP-MULTI-TSMOM-D1-006 - calendar-365 multi-asset TSMOM         |
//+------------------------------------------------------------------+
#property copyright "AlphaFactory research"
#property version   "1.00"
#property strict
#property description "Eight-asset FX/XAU calendar-365 time-series momentum"
...
const string HYPOTHESIS_ID="HYP-MULTI-TSMOM-D1-006";
const string EXPECTED_SYMBOL="EURUSD_AFD_TSMOM_V6";
...
string g_symbols[ASSET_COUNT]={"EURUSD_AFD_TSMOM_V6",
                               "GBPUSD_AFD_TSMOM_V6",
                               "AUDUSD_AFD_TSMOM_V6",
                               "NZDUSD_AFD_TSMOM_V6",
                               "USDJPY_AFD_TSMOM_V6",
...
bool ValidateTradeProfitSpecs()
  {
   for(int i=0;i<ASSET_COUNT;i++)
     {
      const double point=SymbolInfoDouble(g_symbols[i],SYMBOL_POINT);
      const double contract=SymbolInfoDouble(g_symbols[i],SYMBOL_TRADE_CONTRACT_SIZE);
      const double volume=SymbolInfoDouble(g_symbols[i],SYMBOL_VOLUME_MIN);
      if(!IsFinitePositive(point) || !IsFinitePositive(contract) ||
         !IsFinitePositive(volume))
         return false;
      const double open=(i==4 ? 100.0 : (i==7 ? 1000.0 : 1.0));
      const double close=open+point*10.0;
      double calculated=0.0;
      ResetLastError();
      if(!OrderCalcProfit(ORDER_TYPE_BUY,g_symbols[i],volume,open,close,calculated) ||
         !IsFinitePositive(calculated))
        {
         PrintFormat("MTS006_PROFIT_SPEC_FAIL symbol=%s calculated=%.8f error=%d",
                     g_symbols[i],calculated,GetLastError());
         return false;
        }
      double expected=volume*contract*(close-open);
      if(!IsDirectQuoteUsd(i))
         expected/=close;
      const double relative_error=MathAbs(calculated-expected)/expected;
      PrintFormat("MTS006_PROFIT_SPEC symbol=%s base=%s profit=%s margin=%s calc_mode=%I64d contract=%.8f expected=%.8f calculated=%.8f relative_error=%.8f",
                  g_symbols[i],SymbolInfoString(g_symbols[i],SYMBOL_CURRENCY_BASE),
                  SymbolInfoString(g_symbols[i],SYMBOL_CURRENCY_PROFIT),
                  SymbolInfoString(g_symbols[i],SYMBOL_CURRENCY_MARGIN),
                  SymbolInfoInteger(g_symbols[i],SYMBOL_TRADE_CALC_MODE),contract,
                  expected,calculated,relative_error);
      if(!MathIsValidNumber(relative_error) || relative_error>0.05)
        {
         return false;
        }
     }
...
bool ValidateTradeStaticSpecs()
  {
   for(int i=0;i<ASSET_COUNT;i++)
     {
      string expected_base="USD";
      string expected_profit="USD";
      string expected_margin="USD";
      if(i==0) { expected_base="EUR"; expected_margin="EUR"; }
      if(i==1) { expected_base="GBP"; expected_margin="GBP"; }
      if(i==2) { expected_base="AUD"; expected_margin="AUD"; }
      if(i==3) { expected_base="NZD"; expected_margin="NZD"; }
      if(i==4) expected_profit="JPY";
      if(i==5) expected_profit="CAD";
      if(i==6) expected_profit="CHF";
      const long expected_calc=(i==7 ? 4 : 0);
      const double expected_contract=(i==7 ? 100.0 : 100000.0);
      const string actual_base=SymbolInfoString(g_symbols[i],SYMBOL_CURRENCY_BASE);
      const string actual_profit=SymbolInfoString(g_symbols[i],SYMBOL_CURRENCY_PROFIT);
      const string actual_margin=SymbolInfoString(g_symbols[i],SYMBOL_CURRENCY_MARGIN);
      const long actual_calc=SymbolInfoInteger(g_symbols[i],SYMBOL_TRADE_CALC_MODE);
      const double actual_contract=SymbolInfoDouble(g_symbols[i],SYMBOL_TRADE_CONTRACT_SIZE);
      if(actual_base!=expected_base || actual_profit!=expected_profit ||
         actual_margin!=expected_margin || actual_calc!=expected_calc ||
         MathAbs(actual_contract-expected_contract)>1e-9)
        {
         PrintFormat("MTS006_STATIC_TRADE_SPEC_FAIL symbol=%s base=%s/%s profit=%s/%s margin=%s/%s calc=%I64d/%I64d contract=%.8f/%.8f",
                     g_symbols[i],actual_base,expected_base,actual_profit,expected_profit,
                     actual_margin,expected_margin,actual_calc,expected_calc,
...
bool IsActive(const int index,const datetime decision_time)
  {
   return index>=0 && index<ASSET_COUNT && decision_time>=0;
  }
   ...
   if(transition_ok && TargetsReached())
     {
      ...
      PrintFormat("MTS006_REBALANCE_COMPLETE decision=%I64d execution=%I64d latency_sec=%I64d usd_factor=%.8f",
...
void EmitFinancingExposure(const datetime now,const bool force,const string reason)
  {
   ...
   PrintFormat("MTS006_FINANCE_EXPOSURE epoch=%I64d day=%d reason=%s fx_usd=%.2f xau_usd=%.2f",
               (long)now,day,reason,fx,xau);
  }

int OnInit()
  {
   const ulong expected_magic=(InpLongOnlyComparator ? 260812010 : 260812009);
   if(_Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_H1 || !InpResearchAutoMode ||
     ...
     {
      PrintFormat("MTS006_IDENTITY_FAIL symbol=%s period=%d currency=%s",
   ...
   for(int i=0;i<ASSET_COUNT;i++)
     {
      ...
      if(!SymbolSelect(g_symbols[i],true) || GetLastError()!=0)
        {
         PrintFormat("MTS006_SYMBOL_SELECT_FAIL symbol=%s error=%d",
                     g_symbols[i],GetLastError());
         return INIT_FAILED;
        }
     }
   if(!ValidateTradeStaticSpecs())
     {
      Print("MTS006_TRADE_SPEC_FAIL");
      return INIT_FAILED;
     }
   if(!ValidateTradeProfitSpecs())
     {
      Print("MTS006_PROFIT_SPEC_FAIL_CLOSED");
      return INIT_FAILED;
     }
   if(!EmitSeriesProof())
     {
      PrintFormat("MTS006_DATA_PROOF_FAIL symbol=%s error=%d",_Symbol,GetLastError());
      return INIT_FAILED;
     }
   ...
   PrintFormat("MTS006_READY hypothesis_id=%s variant=%s primary=%s lookback_calendar_days=365 vol_returns=60 universe=FX7_XAU",
...
void OnTradeTransaction(const MqlTradeTransaction &transaction,
                        ...
                        const MqlTradeResult &result)
  {
   ...
   const double one_spread_cost=OneSpreadCostUsd(index,symbol,deal_type,volume,
                                                 price,spread_points);
   PrintFormat("MTS006_DEAL_COST epoch=%I64d deal=%I64u symbol=%s class=%s entry=%I64d type=%I64d volume=%.8f price=%.8f spread_points=%I64d one_spread_cost_usd=%.8f native_profit=%.8f native_swap=%.8f native_commission=%.8f",
...
void OnDeinit(const int reason)
  {
   ...
   for(int i=0;i<ASSET_COUNT;i++)
      PrintFormat("MTS006_SYMBOL_SOURCE symbol=%s missing_weeks=%I64d long=%I64d short=%I64d",
                  g_symbols[i],g_missing_weeks[i],g_long_signals[i],g_short_signals[i]);
   PrintFormat("MTS006_SOURCE_SUMMARY hypothesis_id=%s attempts=%I64d source_valid=%I64d weight_valid=%I64d y2018=%I64d y2019=%I64d y2020=%I64d y2021=%I64d reason=%d",
   ...
   PrintFormat("MTS006_ECON_TELEMETRY ticks=%I64d deal_profit=%.2f deal_swap=%.2f deal_commission=%.2f native_net=%.2f",
