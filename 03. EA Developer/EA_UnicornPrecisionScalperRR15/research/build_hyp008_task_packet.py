#!/usr/bin/env python3
"""Refresh the dirty-tree-bound HYP-UPS-XAU-M5-008 control task packet."""
...
OVERRIDES = (
    ...
    "InpMagic=5600719;InpMaxAccountDrawdownPct=5.50;InpMaxConsecutiveLosses=2;"
    ...
    "InpTargetRR=1.50;InpUseEventAnchoredSweepState=true"
)
ACCEPTANCE = {
...
def build_include_closure(root: Path) -> tuple[list[dict[str, str]], str]:
    entries: list[dict[str, str]] = []
    records: list[str] = []
    for relative in INCLUDE_PATHS:
        path = (root / Path(*relative.split("/"))).resolve()
        if not path.is_file():
            raise ValueError(f"required terminal include is missing: {path}")
        digest = sha256_file(path)
        entries.append({"path": relative, "sha256": digest})
        records.append(f"{str(path).lower()}\t{digest}")
    return entries, sha256_bytes("\n".join(sorted(records)).encode("utf-8"))
...
def latest_registry_row(registry: Path) -> tuple[str, dict[str, Any]]:
    ...
    if row.get("state") != "screened" or row.get("model") != 0:
        raise ValueError("latest HYP-008 registry row is not screened Model 0")
    return raw, row
...
def build(root: Path, output: Path) -> dict[str, Any]:
    ...
    prereg = (
        ...
        / EA_NAME
        / "research"
        / "HYP-UPS-XAU-M5-008_FROZEN_PREREG.md"
    )
    ...
    include_closure, include_closure_sha256 = build_include_closure(root)
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default=(
            "03. EA Developer/EA_UnicornPrecisionScalperRR15/research/preflight/"
            "HYP-UPS-XAU-M5-008/task_packet.control.json"
        ),
    )
