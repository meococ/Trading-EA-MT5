#!/usr/bin/env python3
"""Outcome-blind MFI14 extreme-reentry source analyzer."""
...
HYPOTHESIS_ID = "HYP-MFI-XAUUSD-M5-001"
ATTEMPT_ID = "MFI001-SOURCE-ATTEMPT-001"
PREREG_SHA256 = "F89BE96DE95E484A7734F89FC565F3113521C41215F40CCAEC66A0D326D57295"
...
def calculate_mfi(data: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
    prices = data[["high", "low", "close"]].to_numpy(dtype=float)
    finite_prices = np.isfinite(prices).all(axis=1)
    price_valid = pd.Series(
        finite_prices
        & (data["high"].to_numpy(dtype=float) > data["low"].to_numpy(dtype=float))
        & (data["close"].to_numpy(dtype=float) <= data["high"].to_numpy(dtype=float))
        & (data["close"].to_numpy(dtype=float) >= data["low"].to_numpy(dtype=float)),
        index=data.index,
        dtype=bool,
    )
    ...
    row_valid = price_valid & volume_valid
    typical_price = (data["high"] + data["low"] + data["close"]) / 3.0
    raw_flow = typical_price * data["tick_volume"]
    prior_tp = typical_price.shift(1)
    pair_valid = row_valid & row_valid.shift(1, fill_value=False)
    positive = pd.Series(
        np.where(pair_valid & (typical_price > prior_tp), raw_flow, 0.0), index=data.index
    )
    negative = pd.Series(
        np.where(pair_valid & (typical_price < prior_tp), raw_flow, 0.0), index=data.index
    )
    all_inputs_valid = (
        pair_valid.astype(int)
        .rolling(MFI_PERIOD, min_periods=MFI_PERIOD)
        .sum()
        .eq(MFI_PERIOD)
    )
    positive14 = positive.rolling(MFI_PERIOD, min_periods=MFI_PERIOD).sum()
    negative14 = negative.rolling(MFI_PERIOD, min_periods=MFI_PERIOD).sum()
    mfi = pd.Series(np.nan, index=data.index, dtype=float)
    both = all_inputs_valid & (positive14 > 0) & (negative14 > 0)
    only_positive = all_inputs_valid & (positive14 > 0) & (negative14 == 0)
    only_negative = all_inputs_valid & (negative14 > 0) & (positive14 == 0)
    ratio = positive14[both] / negative14[both]
    mfi.loc[both] = 100.0 - 100.0 / (1.0 + ratio)
    mfi.loc[only_positive] = 100.0
    mfi.loc[only_negative] = 0.0
    return typical_price, mfi, positive14, negative14
...
def analyze_frame(frame: pd.DataFrame) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    _, mfi, _, _ = calculate_mfi(data)
    event_feature_usable = np.isfinite(mfi) & np.isfinite(mfi.shift(1))
    exact_next = (data["time_utc"].shift(-1) - data["time_utc"]) == pd.Timedelta(minutes=5)
    source_usable = event_feature_usable & exact_next
    long_signal = source_usable & (mfi.shift(1) <= LOWER_LEVEL) & (mfi > LOWER_LEVEL)
    short_signal = source_usable & (mfi.shift(1) >= UPPER_LEVEL) & (mfi < UPPER_LEVEL)
    conflicts = long_signal & short_signal
    long_signal &= ~conflicts
    short_signal &= ~conflicts
    event_mask = long_signal | short_signal
        ...
        events.append(
            {
                ...
                "prior_mfi14": finite_float(mfi.shift(1).loc[index]),
                "mfi14": finite_float(mfi.loc[index]),
                "tick_volume": int(data.at[index, "tick_volume"]),
            }
        )

    count = len(events)
    longs = int(long_signal.sum())
    shorts = int(short_signal.sum())
    ...
    warmup = MFI_PERIOD + 1
    feature_coverage = int(event_feature_usable.iloc[warmup:].sum()) / max(len(data) - warmup, 1)
    next_denominator = max(int(event_feature_usable.iloc[:-1].sum()), 1)
    next_coverage = int((event_feature_usable & exact_next).iloc[:-1].sum()) / next_denominator
    ...
    gates = {
        ...
        "exact_next_m5_coverage": next_coverage >= MIN_NEXT_COVERAGE,
    ...
    report = {
        "schema_version": "mfi_source_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        ...
        "parameters": {"period": MFI_PERIOD, "lower": LOWER_LEVEL, "upper": UPPER_LEVEL},
        "funnel": {
            "design_rows": int(len(data)),
            "event_feature_usable_rows": int(event_feature_usable.sum()),
        ...
        "metrics": {
            ...
            "exact_next_m5_coverage": next_coverage,
        ...
        "verdict": (
            "SCREENED_SOURCE_PASS_MQL5_IMFI_BUILD_AUTHORIZED"
            if passed
            else "PARK_SOURCE_FEASIBILITY_EXACT_MFI_REENTRY"
        ),
        "prohibitions": {
...
def claim_attempt(output_dir: Path, authority: dict[str, str]) -> tuple[str, Path]:
    ...
    marker = {
        "schema_version": "mfi_attempt_started.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
...
def execute(root: Path) -> dict[str, Any]:
    prereg = root / "03. EA Developer/EA_MFIExtremeReentry/research/HYP-MFI-XAUUSD-M5-001_FROZEN_PREREG.md"
    ...
    output_dir = root / "03. EA Developer/EA_MFIExtremeReentry/research/evidence/HYP-MFI-XAUUSD-M5-001/MFI001-SOURCE-ATTEMPT-001"
    ...
    if jsonl_bytes(events) != jsonl_bytes(replay_events) or json_bytes(report) != json_bytes(
        replay_report
    ):
    ...
    report_path = output_dir / "mfi_001_source_report.json"
    ledger_path = output_dir / "mfi_001_event_ledger.jsonl"
    atomic_write(report_path, report_bytes)
    atomic_write(ledger_path, ledger_bytes)
    receipt = {
        "schema_version": "mfi_source_receipt.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "started_at_utc": started,
    ...
    terminal = {
        "schema_version": "mfi_attempt_terminal.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
