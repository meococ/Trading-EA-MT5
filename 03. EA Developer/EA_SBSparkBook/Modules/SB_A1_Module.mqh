...
void SB_OnTick(string symbol, ulong magic, double riskPct, double maxLot)
{
   ...
   // --- A1 weekend-flat (must run on Friday before SkipFriday) ---
   if(SB_USE_WEEKEND_FLAT && dow == 5)
   {
      bool cutoff = false;
      if(dt.hour > SB_FRIDAY_FLAT_HOUR) cutoff = true;
      else if(dt.hour == SB_FRIDAY_FLAT_HOUR && dt.min >= SB_FRIDAY_FLAT_MINUTE)
         cutoff = true;
      if(cutoff)
      {
