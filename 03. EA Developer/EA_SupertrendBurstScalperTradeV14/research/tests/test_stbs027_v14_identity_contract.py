...
CURRENT_V14 = PACKAGE / "EA_SupertrendBurstScalperTradeV14.mq5"
FAILURE_ARCHIVE = (
    ROOT
...
COST_MANIFEST = PACKAGE / "research/HYP-STBS-XAUUSD-M15-027_RESEARCH_COST_SOURCE_MANIFEST.json"
...
def normalize_v12_to_v14(text: str) -> str:
    replacements = {
        ...
        '#property description "H1 Supertrend V12 identity clone with unchanged bounded telemetry and execution."':
            '#property description "H1 Supertrend V14 identity clone with post-claim reconciliation harness."',
        "HYP-STBS-XAUUSD-M15-025": "HYP-STBS-XAUUSD-M15-027",
        "STBS_H1_FLIP_M15_BURST_TRADE_V12_IDENTITY_CLONE":
            "STBS_H1_FLIP_M15_BURST_TRADE_V14_NONREPAINT_ROOT",
        "5604125": "5604127",
        "EA_SupertrendBurstScalperTradeV12": "EA_SupertrendBurstScalperTradeV14",
    }
...
def test_v14_is_exact_v12_identity_only_clone():
    assert normalize_v12_to_v14(PARENT_V12.read_text(encoding="utf-8")) == CURRENT_V14.read_text(encoding="utf-8")
    assert sha256(PARENT_V12) == "D96F55A26F277CFC3FDC4E23A11A84C74598C111639E629CEC1877AC3F7704C5"
    assert sha256(CURRENT_V14) == "F60A9469D1A6FE2D62F5E83DECB953862C68AF9E3D154EA0AE488C072B4A4DA4"


def test_v14_identity_is_fail_closed():
    text = CURRENT_V14.read_text(encoding="utf-8")
    for needle in (
        ...
        "InpMagic               = 5604127",
        'EA_NAME              = "EA_SupertrendBurstScalperTradeV14"',
        'InpHypothesisId!="HYP-STBS-XAUUSD-M15-027"',
        'InpVariantTag!="STBS_H1_FLIP_M15_BURST_TRADE_V14_NONREPAINT_ROOT"',
...
def test_nondecision_spam_remains_absent():
    assert "STBS_MARGIN_STRESS_UNSAFE" not in CURRENT_V14.read_text(encoding="utf-8")


def test_generic_cost_builder_accepts_same_outer_inner_identity_and_rejects_adapter(tmp_path: Path):
    spec = importlib.util.spec_from_file_location("stbs027_cost_builder", COST_BUILDER)
    ...
    payload = {
        ...
        "variant_tag": "STBS_H1_FLIP_M15_BURST_TRADE_V14_NONREPAINT_ROOT",
        "magic": 5604127,
        "audit_only": False,
...
def test_execute_claim_precedes_first_bound_helper_read_and_failure_is_terminal(tmp_path: Path):
    ...
    packet_relative = (
        "03. EA Developer/EA_SupertrendBurstScalperTradeV14/research/preflight/"
        "HYP-STBS-XAUUSD-M15-027/V1/task_packet.control.json"
    )
    row = {
        "hypothesis_id": "HYP-STBS-XAUUSD-M15-027",
        "state": "screened",
        ...
        "validation": {
            ...
            "mt5_attempt_id": "STBS027-MODEL0-TRAIN-001",
            "mt5_attempt_limit": 1,
    ...
    command = [
        ...
        "-EaName", "EA_SupertrendBurstScalperTradeV14",
        "-HypothesisId", "HYP-STBS-XAUUSD-M15-027",
    ...
    attempt = isolated / (
        ...
        "HYP-STBS-XAUUSD-M15-027/STBS027-MODEL0-TRAIN-001"
    )
