...
BOUNDED_DIFF_PROOF = (
    PACKAGE / "research" / f"{HYPOTHESIS_ID}_V13_V14_NONREPAINT_ROOT_DIFF_PROOF.md"
)
COMPACT_TELEMETRY_TEST = (
    PACKAGE / "research" / "tests" / "test_stbs027_v14_identity_contract.py"
)
PARENT_PACKAGE = ROOT / "03. EA Developer" / "EA_SupertrendBurstScalperTradeV13"
PARENT_FAILURE_PACKET = (
    PARENT_PACKAGE / "research" / "HYP-STBS-XAUUSD-M15-026_RUNTIME_NONREPAINT_PATH_FAILURE.md"
)
PARENT_POST_FAILURE_REVIEW = (
    PARENT_PACKAGE / "research" / "HYP-STBS-XAUUSD-M15-026_INDEPENDENT_POST_FAILURE_REVIEW.md"
)
...
RESERVED_POST_PACKET_REVIEW_STATUS_LINE = (
    '?? "03. EA Developer/EA_SupertrendBurstScalperTradeV14/research/'
    'HYP-STBS-XAUUSD-M15-027_POST_PACKET_REVIEW.md"'
)
PACKET_ATTEMPT_ROOT = (
    PACKAGE / "research" / "evidence" / HYPOTHESIS_ID / "STBS027-PACKET-BUILD-001"
)
...
PACKET_ATTEMPT_ID = "STBS027-PACKET-BUILD-001"
...
OVERRIDES = (
    ...
    "InpHypothesisId=HYP-STBS-XAUUSD-M15-027;InpMagic=5604127;"
...
def latest_registry_identity(registry_raw: bytes | None = None) -> tuple[str, str, dict]:
    ...
    if row.get("hypothesis_id") != HYPOTHESIS_ID or row.get("state") != "probe":
        raise RuntimeError("latest registry row is not the frozen HYP027 packet-only probe authority")
    if (
        ...
        raise RuntimeError("HYP027 authority source/prereg binding is invalid")
    if row.get("verdict") != "FROZEN_HYP027_PACKET_BUILDER_SEMANTIC_AUTHORITY":
        raise RuntimeError("latest HYP027 authority has the wrong packet-builder verdict")
    metrics = row.get("metrics")
    validation = row.get("validation")
    if not isinstance(metrics, dict) or not isinstance(validation, dict):
        raise RuntimeError("HYP027 packet-builder authority metrics/validation are absent")
    if row.get("run_ids") != [] or any(
        metrics.get(name) != expected for name, expected in PROBE_ZERO_METRICS.items()
    ):
        raise RuntimeError("HYP027 packet-builder authority counters are not pristine")
    if (
        ...
        or validation.get("exact_inner_mql_identity") != "HYP-STBS-XAUUSD-M15-027"
        ...
        raise RuntimeError(
            "HYP027 packet-builder authority is not exact packet-only authority"
        )
    parent_raw = None
    parent_row = None
