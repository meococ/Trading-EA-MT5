# HYP027 pre-execution harness addendum

Status: FROZEN BEFORE HYP027 PROBE OR PACKET BUILD
...
- Outer registry, task, receipt, attempt and Alpha run-manifest identity: `HYP-STBS-XAUUSD-M15-027`.
- Inner MQL5 override, lifecycle and RunMeta identity: `HYP-STBS-XAUUSD-M15-027`.
- EA/source: identity-only clone `EA_SupertrendBurstScalperTradeV14`, SHA-256 `F60A9469D1A6FE2D62F5E83DECB953862C68AF9E3D154EA0AE488C072B4A4DA4`.
- Magic and variant are `5604127` and `STBS_H1_FLIP_M15_BURST_TRADE_V14_NONREPAINT_ROOT`.

All identity surfaces must equal HYP027: Alpha invocation, exact MQL5 override, RunMeta, lifecycle sidecars, task packet, contract receipt, run manifest and attempt chain. The generic verified-cost builder is intentionally unchanged and must reject any outer/inner identity divergence.

## Two-stage authority

1. The initial HYP027 row is `state=probe`, authority `PACKET_BUILD_ONLY_NO_EXECUTION_NO_ECONOMICS`.
2. It authorizes only `STBS027-PACKET-BUILD-001`; every compile, MT5, trade, outcome, performance, economics, validation, holdout and deployment permission is false.
...
4. Only a reviewed ordinary `probe -> screened` successor may bind packet/start/terminal, this exact addendum path/SHA and authorize `STBS027-MODEL0-TRAIN-001`.
