...
#property description "H1 Supertrend V14 identity clone with run-root non-repaint manifest."

input string InpHypothesisId        = "HYP-STBS-XAUUSD-M15-027";
input string InpVariantTag          = "STBS_H1_FLIP_M15_BURST_TRADE_V14_NONREPAINT_ROOT";
input bool   InpAuditOnly           = false;
input bool   InpEnableTelemetry     = true;
input long   InpMagic               = 5604127;
...
const double FROZEN_ROUND_TURN_COMMISSION_ACCOUNT_PER_LOT = 4.40;
const string EA_NAME              = "EA_SupertrendBurstScalperTradeV14";
...
struct EntryPlan
{
   ...
   double stressed_loss;
   double stressed_equity;
   double stressed_margin;
   double stressed_free_margin;
   double stressed_margin_level;
};
...
MarginSafetyResult EvaluateMarginCandidate(EntryPlan &plan,const double volume)
{
   ...
   // Admission reserves the worst fill allowed by the frozen deviation and the
   // complete research-proxy round-turn commission before stressing to the
   // already-frozen SL.  Gaps beyond that envelope remain a runtime fatal
   // backstop rather than an economic exit.
   ResetLastError();
   const double point=SymbolInfoDouble(_Symbol,SYMBOL_POINT);
   if(GetLastError()!=0 || !MathIsValidNumber(point) || point<=0.0 ||
      InpDeviationPoints<0 ||
      !MathIsValidNumber(FROZEN_ROUND_TURN_COMMISSION_ACCOUNT_PER_LOT) ||
      FROZEN_ROUND_TURN_COMMISSION_ACCOUNT_PER_LOT<=0.0)
      return MARGIN_SAFETY_FATAL;
   const double adverse_deviation=point*(double)InpDeviationPoints;
   const double worst_fill=(plan.order_type==ORDER_TYPE_BUY)
                           ? plan.entry+adverse_deviation
                           : plan.entry-adverse_deviation;
   if(!MathIsValidNumber(worst_fill) || worst_fill<=0.0 ||
      (plan.order_type==ORDER_TYPE_BUY && worst_fill<=plan.stop) ||
      (plan.order_type==ORDER_TYPE_SELL && worst_fill>=plan.stop))
      return MARGIN_SAFETY_FATAL;
   double stop_profit_before_charge=0.0;
   double worst_fill_required_margin=0.0;
   double stop_required_margin=0.0;
   if(!OrderCalcProfit(plan.order_type,_Symbol,volume,worst_fill,plan.stop,stop_profit_before_charge) ||
      !MathIsValidNumber(stop_profit_before_charge) || stop_profit_before_charge>=0.0 ||
      !OrderCalcMargin(plan.order_type,_Symbol,volume,worst_fill,worst_fill_required_margin) ||
      !MathIsValidNumber(worst_fill_required_margin) || worst_fill_required_margin<=0.0 ||
      !OrderCalcMargin(plan.order_type,_Symbol,volume,plan.stop,stop_required_margin) ||
      !MathIsValidNumber(stop_required_margin) || stop_required_margin<=0.0)
      return MARGIN_SAFETY_FATAL;
   const double reserved_charge=volume*FROZEN_ROUND_TURN_COMMISSION_ACCOUNT_PER_LOT;
   const double stressed_profit=stop_profit_before_charge-reserved_charge;
   if(!MathIsValidNumber(reserved_charge) || reserved_charge<=0.0 ||
      !MathIsValidNumber(stressed_profit) || stressed_profit>=stop_profit_before_charge)
      return MARGIN_SAFETY_FATAL;
   const double candidate_required_margin=MathMax(required_margin,
                                          MathMax(worst_fill_required_margin,stop_required_margin));
   if(!MathIsValidNumber(candidate_required_margin) || candidate_required_margin<=0.0)
      return MARGIN_SAFETY_FATAL;
   if(candidate_required_margin>equity*InpMaxNewPositionMarginPct/100.0 ||
      candidate_required_margin>free_margin)
      return MARGIN_SAFETY_UNSAFE;
   const double stressed_equity=check.equity+stressed_profit;
   const double stressed_margin=MathMax(check.margin,
                                        current_margin+candidate_required_margin);
   const double extra_margin=MathMax(0.0,stressed_margin-check.margin);
   const double stressed_check_free=check.margin_free+stressed_profit-extra_margin;
   const double stressed_balance_free=stressed_equity-stressed_margin;
   if(!MathIsValidNumber(stressed_equity) || stressed_equity<=0.0 ||
      !MathIsValidNumber(stressed_margin) || stressed_margin<=0.0 ||
      !MathIsValidNumber(stressed_check_free) || stressed_check_free<0.0 ||
      !MathIsValidNumber(stressed_balance_free) || stressed_balance_free<0.0)
      return MARGIN_SAFETY_UNSAFE;
   const double stressed_margin_level=100.0*stressed_equity/stressed_margin;
   if(!MathIsValidNumber(stressed_margin_level) || stressed_margin_level<=0.0)
      return MARGIN_SAFETY_FATAL;
   ...
   else
   {
      ...
      if(stressed_equity>protected_level)
      {
         const double remaining_headroom=stressed_equity-protected_level;
         const double reserve=MathMax(remaining_headroom*InpMoneyHeadroomReserveFactor,
                                      stressed_equity*InpMoneyFreeEquityFloorPct/100.0);
...
int OnInit()
{
   if(!MQLInfoInteger(MQL_TESTER) || MQLInfoInteger(MQL_OPTIMIZATION) ||
      _Symbol!="XAUUSD" || _Period!=PERIOD_M15 ||
       InpHypothesisId!="HYP-STBS-XAUUSD-M15-027" ||
       InpVariantTag!="STBS_H1_FLIP_M15_BURST_TRADE_V14_NONREPAINT_ROOT" ||
       InpAuditOnly || !InpEnableTelemetry || InpMagic!=5604127 ||
