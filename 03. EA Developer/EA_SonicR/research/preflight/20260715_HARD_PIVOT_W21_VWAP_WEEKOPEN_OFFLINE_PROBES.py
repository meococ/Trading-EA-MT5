#!/usr/bin/env python3
"""HARD PIVOT W21 — session-VWAP reclaim CONT + weekly-open break CONT after W1–W20 ALL_KILL.

FORBIDDEN densify: W1–W20 / FVG / R-series / swing / Donch / Outside / VR /
...
compfade / volregime / fix-impulse / session-CONT location-accept densify /
mean-revert fade densify.

NEW edge-source classes (≠ fade densify; ≠ session CONT densify; ≠ H4-retest):
  A. HYP-FX3-H1-SESSION-VWAP-RECLAIM-CONT-001
     Session cumulative typical-price VWAP pierce → close reclaim CONT.
     Institutional VWAP microstructure. ≠ Asia-mid reclaim; ≠ FVG; ≠ location S/R.
  B. HYP-FX3-H1-WEEKLY-OPEN-BREAK-CONT-001
     First H1 close beyond current week open → CONT (weekly auction open pivot).
     ≠ W16 prior-week HL; ≠ W12 weekday D1 gap; ≠ prior2d range.
  BOOK. HYP-BOOK-VWAP-WEEKOPEN-APRIORI-001
...
STEM = "20260715_HARD_PIVOT_W21_VWAP_WEEKOPEN"
...
OUT_CLEAN_VN = READ / "20260715_CLEAN_BOOK_AND_HARD_PIVOT_W21_VN_ACTION_BRIEF.md"
OUT_FREEZE = READ / f"{STEM}_APRIORI_FREEZE.md"
OUT_COMBO_VN = READ / "20260715_COST_GRADE_AND_HARD_PIVOT_W21_VN_ACTION_BRIEF.md"
...
def probe_session_vwap_reclaim_cont(h1):
    """A: session typical-price VWAP pierce → close reclaim CONT."""
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    used = set()
    # per-symbol session accumulators
    state = {
        s: {"day": None, "pv": 0.0, "vol": 0.0, "pierced_side": 0, "used_day": None} for s in FX3
    }
    for i in range(30, len(clock) - 1):
        ...
        for sym in FX3:
            ...
            if st["day"] != day_key:
                ...
                st["vol"] = 0.0
                st["pierced_side"] = 0
            # only accumulate from session start hour
            if sig_dt.hour < A_VWAP_START:
                continue
            o, h, l, c = float(d["o"][j]), float(d["h"][j]), float(d["l"][j]), float(d["c"][j])
            typ = (h + l + c) / 3.0
            w = max(h - l, 1e-12)  # range as volume proxy
            st["pv"] += typ * w
            st["vol"] += w
            if st["vol"] <= 0:
                continue
            ...
            if not np.isfinite(atr) or atr <= 0:
                continue
            # track pierce
            if c < vwap - A_PIERCE_ATR * atr:
                st["pierced_side"] = -1
            elif c > vwap + A_PIERCE_ATR * atr:
                st["pierced_side"] = 1
            if st["pierced_side"] == 0:
                continue
            if st["used_day"] == day_key:
                continue
            if sym in open_syms:
                continue
            ...
            if cool in used:
                continue
            side = 0
            # reclaim: pierced below then close back above VWAP → long CONT
            if st["pierced_side"] < 0 and c > vwap:
                side = 1
            elif st["pierced_side"] > 0 and c < vwap:
                side = -1
            if side == 0:
                continue
            ent_i = asof_idx(d, ts)
            ...
            if side > 0:
                sl = min(l, entry - A_SL_MIN_ATR * atr_e) - A_SL_PAD * atr_e
            else:
                sl = max(h, entry + A_SL_MIN_ATR * atr_e) + A_SL_PAD * atr_e
...
def probe_weekly_open_break_cont(h1):
    """B: first H1 close beyond current week open → CONT."""
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    used = set()
    week_open = {s: None for s in FX3}  # (iso_year, iso_week) -> open px
    fired = {s: None for s in FX3}  # week key already traded
    for i in range(40, len(clock) - 1):
        ...
        if not (B_SESSION[0] <= sig_dt.hour < B_SESSION[1]):
            continue
        # skip Monday first 2h (open discovery); trade Tue–Thu primarily + Mon after 09
        if sig_dt.weekday() == 0 and sig_dt.hour < 9:
            continue
        iso = sig_dt.isocalendar()
        ...
        for sym in FX3:
            ...
            if j is None or j < 20:
                continue
            # establish week open = first bar open of this ISO week seen
            if week_open[sym] is None or week_open[sym][0] != wk:
                # find earliest bar of this week on this symbol
                # use current bar open if Monday, else search back
                wo = None
                for k in range(j, max(0, j - 120), -1):
                    kdt = datetime.fromtimestamp(int(d["t"][k]), tz=timezone.utc)
                    kiso = kdt.isocalendar()
                    if (kiso.year, kiso.week) != wk:
                        break
                    wo = float(d["o"][k])
                if wo is None:
                    wo = float(d["o"][j])
            ...
            side = 0
            if c > wo + B_BREAK_ATR * atr:
                side = 1
            elif c < wo - B_BREAK_ATR * atr:
                side = -1
            if side == 0:
                continue
            ent_i = asof_idx(d, ts)
            ...
            atr_e = d["atr"][ent_i] if np.isfinite(d["atr"][ent_i]) else atr
            if side > 0:
                sl = min(wo, entry - B_SL_MIN_ATR * atr_e) - B_SL_PAD * atr_e
            else:
                sl = max(wo, entry + B_SL_MIN_ATR * atr_e) + B_SL_PAD * atr_e
            sl_dist = abs(entry - sl)
            if sl_dist <= 1e-12:
                continue
...
def evaluate_book(ta, tb):
    for t in ta:
        t["sleeve"], t["priority"] = "A_VWAP", A_PRIORITY
        t["pnl_haircut"] = float(t["pnl"]) - BASE_COST
    for t in tb:
        t["sleeve"], t["priority"] = "B_WEEKOPEN", B_PRIORITY
        t["pnl_haircut"] = float(t["pnl"]) - BASE_COST
    ...
    a, b = by.get("A_VWAP", []), by.get("B_WEEKOPEN", [])
    ...
    return _jsonable(
        {
            "hypothesis_id": "HYP-BOOK-VWAP-WEEKOPEN-APRIORI-001",
            "setup": "a priori book: session-VWAP reclaim CONT + weekly-open break CONT",
            "symbol": "EURUSD+GBPUSD+USDJPY",
            "timeframe": "H1",
            "metrics": m,
            "haircuts": hc,
...
def write_freeze():
    body = "\n".join(
        [
            "# A priori freeze — HARD PIVOT W21 session-VWAP reclaim + weekly-open break",
            "",
            "Date: 2026-07-15",
            "Status: `APRIORI_FREEZE__PRE_METRICS`",
            f"A target MFE ≈ ${A_RR * CASH_R:.0f}; B ≈ ${B_RR * CASH_R:.0f} ≫ ${BASE_COST}.",
            "",
            "## Edge-source pivot (why ≠ W1–W20)",
            "W1–W20 ALL_KILL across session-CONT, fade boards, vol-regime, fix-flow.",
            "W21: INSTITUTIONAL VWAP RECLAIM CONT + WEEKLY AUCTION OPEN BREAK CONT.",
            "≠ fade densify; ≠ session location CONT densify; ≠ H4-retest densify.",
            "",
            "## Objects",
            "- HYP-FX3-H1-SESSION-VWAP-RECLAIM-CONT-001",
            "- HYP-FX3-H1-WEEKLY-OPEN-BREAK-CONT-001",
            "- HYP-BOOK-VWAP-WEEKOPEN-APRIORI-001",
            "",
            "## Forbidden",
            "W1–W20 densify; FVG; R-series; session-CONT densify; fade densify;",
            "H4-retest; volregime/fix densify; week-HL densify; ORB densify.",
            "",
            "## Screen",
...
def write_preregs():
    ...
    specs = [
        (
            "20260715_H_FX3_H1_SESSION_VWAP_RECLAIM_CONT_001_PREREG.md",
            "HYP-FX3-H1-SESSION-VWAP-RECLAIM-CONT-001",
            [
                f"- Session typ-price VWAP pierce≥{A_PIERCE_ATR}*ATR → close reclaim CONT; RR={A_RR}.",
                "- Institutional VWAP. != Asia-mid reclaim; != FVG; != location S/R densify.",
            ],
        ),
        (
            "20260715_H_FX3_H1_WEEKLY_OPEN_BREAK_CONT_001_PREREG.md",
            "HYP-FX3-H1-WEEKLY-OPEN-BREAK-CONT-001",
            [
                f"- First H1 close beyond week open by {B_BREAK_ATR}*ATR → CONT; RR={B_RR}.",
                "- Weekly auction open. != W16 week HL; != W12 D1 gap; != prior2d.",
            ],
        ),
        (
            "20260715_H_BOOK_VWAP_WEEKOPEN_APRIORI_001_PREREG.md",
            "HYP-BOOK-VWAP-WEEKOPEN-APRIORI-001",
        ...
        p.write_text(
            "\n".join(
                [
                    ...
                    "- Lane: `hard_pivot_w21_vwap_weekopen_20260715`",
                    *extra,
        ...
        rows.append(
            {
                ...
                "parent_candidate": "hard_pivot_w21_vwap_weekopen",
                "feature_family": "hard_pivot_w21_vwap_weekopen",
                "lane": "hard_pivot_w21_vwap_weekopen_20260715",
...
def cost_grade_lines(qfsi_spot=None):
    ...
    if qfsi_spot:
        lines.append(
            f"- W21 spot-check: `{qfsi_spot.get('verdict')}` "
...
def write_docs(results, book, receipt, any_surv, freeze_sha, qfsi_spot):
    ...
    OUT_PANEL.write_text(
        "\n".join(
            [
                "# 3-critic — W21 session-VWAP reclaim + weekly-open break",
                "",
                "| Critic | Call |",
                "|---|---|",
                "| Sonic trader | GO — VWAP microstructure + weekly auction open ≠ fade / ≠ session CONT densify |",
                "| Quant | GO — new edge family after W20 ALL_KILL; MFE≫$12; honest +$12 |",
    ...
    OUT_DESIGN.write_text(
        "\n".join(
            [
                "# Design — HARD PIVOT W21 VWAP + week-open",
                "",
                f"Freeze sha={freeze_sha[:16]}…",
                "",
                "After W1–W20 ALL_KILL.",
                "NEW edge-source classes:",
                "1) Session VWAP reclaim CONT (institutional) ≠ Asia-mid / ≠ FVG.",
                "2) Weekly-open break CONT (auction open pivot) ≠ W16 week HL / ≠ W12 gap.",
    ...
    OUT_DEDUP.write_text(
        "\n".join(
            [
                ...
                "| Object | ≠ |",
                "|---|---|",
                "| Session VWAP reclaim CONT | Asia-mid reclaim; FVG; location S/R; W20 volregime/fix |",
                "| Weekly-open break CONT | W16 prior-week HL; W12 weekday D1 gap; prior2d; H4-retest |",
                "",
    ...
    OUT_MD.write_text(
        "\n".join(
            [
                f"# HARD PIVOT W21 offline probes — `{status}`",
                "",
                f"Receipt `{receipt}`",
    ...
    OUT_VN.write_text(
        "\n".join(
            [
                "# VN — HARD PIVOT W21 VWAP + week-open",
                "",
                f"`{status}`",
                "",
                *table,
                "",
                "Edge mới: session-VWAP reclaim CONT + weekly-open break CONT — ≠ fade / ≠ session CONT densify / ≠ H4-retest.",
    ...
    OUT_CLOSE.write_text(
        "\n".join(
            [
                f"# Session closeout — HARD PIVOT W21 — `{status}`",
                "",
                f"Receipt `{receipt}` freeze sha={freeze_sha[:16]}…",
                "R-series densify PAUSED. Model0 withheld if ALL_KILL.",
                "Next: next independent class outside W1–W21 if ALL_KILL;",
    ...
    OUT_CLEAN_VN.write_text(
        "\n".join(
            [
                "# VN action brief — Clean book + HARD PIVOT W21",
                "",
                ...
                "",
                f"## HARD PIVOT W21 — `{status}`",
                *table,
                "",
                "- Edge: session-VWAP reclaim CONT + weekly-open break CONT.",
                "- Không densify W1–W20 / H4-retest / fade / session CONT / FVG / R-series.",
                f"Receipt `{receipt}`",
...
def patch_hot(results, book, receipt, any_surv, freeze_sha, qfsi_spot):
    ...
    block = "\n".join(
        [
            f"- **HARD PIVOT W21 VWAP/WEEKOPEN + COST-GRADE CLOSEOUT (2026-07-15 ~{stamp} ICT) — "
    ...
    text = re.sub(
        r"^Updated:.*$",
        f"Updated: 2026-07-15 ~{stamp} ICT | HARD PIVOT W21 vwap/weekopen + cost-grade; "
...
def main():
    ...
    try:
        ...
        pnls_a, detail_a, trades_a = probe_session_vwap_reclaim_cont(h1)
        pnls_b, detail_b, trades_b = probe_weekly_open_break_cont(h1)
        ra = pack_result(
            "HYP-FX3-H1-SESSION-VWAP-RECLAIM-CONT-001",
            "Session VWAP reclaim CONT RR2.50",
            pnls_a,
            detail_a,
        )
        rb = pack_result(
            "HYP-FX3-H1-WEEKLY-OPEN-BREAK-CONT-001",
            "Weekly-open break CONT RR2.50",
            pnls_b,
            detail_b,
        )
        ...
        payload = {
            "schema_version": "hard_pivot_w21_vwap_weekopen_offline.v1",
            ...
            "edge_source_note": (
                "INSTITUTIONAL_VWAP + WEEKLY_AUCTION_OPEN: "
                "session typical-price VWAP reclaim CONT + weekly-open break CONT; "
