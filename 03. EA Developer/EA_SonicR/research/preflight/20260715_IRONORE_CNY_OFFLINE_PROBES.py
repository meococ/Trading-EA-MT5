#!/usr/bin/env python3
"""Iron ore + CNY strength — offline probes (HARD PIVOT W24 exo).

A priori frozen (do not mine / do not densify):
  O1 HYP-AUDUSD-H1-IRONORE-TOT-CONT-001
  O2 HYP-AUDUSD-H1-CNYSTRENGTH-DEMAND-CONT-001
  O3 HYP-BOOK-IRONORE-CNY-APRIORI-001

Surface: Yahoo TIO=F iron ore + USDCNY inverted CNY strength (lag +1d).
CNH offshore proved sparse this pass → USDCNY proxy.

Independent of: XLK/XLF · CuGold · oil · VIX siblings · killed FRED boards ·
W1–W23 OHLC densify · R-series.
...
IRON_PANEL = EXO / "panels" / "yahoo_tio_ironore_d1_v1.csv"
CNY_PANEL = EXO / "panels" / "yahoo_usdcny_cny_strength_d1_v1.csv"
ACQ_MANIFEST = EXO / "manifests" / "20260715_IRONORE_CNY_ACQUISITION_V1.json"
CONTRACT = EXO / "contracts" / "20260715_IRONORE_CNY_AVAILABLE_AT_UTC_CONTRACT_V1.json"

OUT_JSON = PRE / "20260715_IRONORE_CNY_OFFLINE_PROBES.json"
OUT_MD = READ / "20260715_IRONORE_CNY_OFFLINE_PROBES.md"
OUT_DEDUP = READ / "20260715_IRONORE_CNY_DEDUP_CLEARANCE.md"
OUT_CLOSE = READ / "20260715_IRONORE_CNY_SESSION_CLOSEOUT.md"
OUT_VN = READ / "20260715_IRONORE_CNY_VN_ACTION_BRIEF.md"
OUT_ACQ = READ / "20260715_IRONORE_CNY_ACQUISITION_READOUT.md"
OUT_DESIGN = READ / "20260715_IRONORE_CNY_DESIGN_MEMO.md"
...
def probe_book(o1: dict, o2: dict) -> dict[str, Any]:
    ...
    verdict, notes = joint_verdict(m, hc)
    return {
        "hypothesis_id": "HYP-BOOK-IRONORE-CNY-APRIORI-001",
        "class": "nested_ironore_cny_apriori_book",
        "symbol": "AUDUSD",
        "tf": "H1",
        "funnel": {
            "n_o1": o1["metrics"]["n"],
...
def append_registry(objects: list[dict], receipt: str) -> None:
    ...
    for o in objects:
        ...
        rec = {
            ...
            "lane": "ironore_cny_exo_20260715",
            "setup_type": o["hypothesis_id"],
            "symbol": o.get("symbol") or "AUDUSD",
            "timeframe": o.get("tf") or "H1",
            "window": "2021.01.01-2025.12.31",
            "model": None,
            "readout_path": "03. EA Developer/EA_SonicR/research/readouts/20260715_IRONORE_CNY_OFFLINE_PROBES.md",
            "run_ids": [],
...
def write_docs(payload: dict, objects: list[dict]) -> None:
    OUT_DEDUP.write_text(
        """# De-dup clearance — Iron ore ToT + CNY strength

Status: `INTAKE_CLEARED / INDEPENDENT` (a priori)

| Object | Vs killed shelf |
|---|---|
| O1 AUDUSD TIO iron-ore ToT CONT | ≠ WTI/Brent oil ToT; ≠ HG/GC CuGold; ≠ XLK/XLF sector; ≠ W1–W23 OHLC densify |
| O2 AUDUSD CNY-strength demand CONT | ≠ VIXCLS risk-off; ≠ MOVE/HY/DTWEX siblings; ≠ SPX−DGS10; ≠ G10 overnight FRED |
| O3 nested book iron∪cny | a priori union; not post-hoc sleeve mine |

Banned densify: iron z thr · CNY z thr · displace ATR/RR · sector/cugold revive · oil revive · VIX shopping · W1–W23 / R10–R31 / FVG.
""",
        encoding="utf-8",
    )
...
""",
    ...
    OUT_MD.write_text(
        f"""# Offline probes — Iron ore ToT + CNY strength

Date: 2026-07-15  
Status: `{status}`  
Iron panel SHA: `{payload["panel_sha256"]["iron"]}`  
CNY panel SHA: `{payload["panel_sha256"]["cny"]}`  
...
""",
    ...
    OUT_CLOSE.write_text(
        f"""# Session closeout — Iron ore + CNY exo (W24)
...
Yahoo TIO=F iron ore + USDCNY→CNY strength (lag +1d).  
CNH offshore: unavailable this pass (n≈1) → pick-next USDCNY executed.  
...
Receipt: `{payload["receipt_sha256"]}`  
Artifacts: `preflight/20260715_IRONORE_CNY_OFFLINE_PROBES.json`
...
1. Do **not** densify iron z / CNY z / displace ATR/RR.
2. Keep R-series / W1–W23 densify paused.
3. Keep Real QFSI accumulate for cost frontier (still GAP).
4. Next object outside ironore-cny / sector-cugold / oil / VIX-sibling /
   W1–W23 killboard — or Owner ChatGPT login for Deep Research (parallel, not stop).
...
""",
    ...
    OUT_VN.write_text(
        f"""# VN — Iron ore + CNY (W24 exo) + cost note

## Surface
- CNH offshore: **không lấy được** (Yahoo n≈1) → USDCNY strength proxy.
- Đã SHA-freeze: Yahoo TIO=F iron ore + USDCNY→CNY strength (lag +1d).
...
## Không làm
- Densify iron / CNY z / ATR/RR.
- Densify sector-cugold / oil / VIX / W14/FVG / W1–W23 / R-series.
...
""",
    ...
    OUT_ACQ.write_text(
        f"""# Acquisition readout — Iron ore + CNY strength
...
- Yahoo TIO=F iron ore daily → panel lag +1d  
  SHA `{payload["panel_sha256"]["iron"]}`
- Yahoo USDCNY=X → CNY strength (1/USDCNY) panel lag +1d  
  SHA `{payload["panel_sha256"]["cny"]}`
...
def main() -> None:
    assert IRON_PANEL.is_file(), "run acquire_freeze_ironore_cny_v1.py first"
    assert CNY_PANEL.is_file(), "run acquire_freeze_ironore_cny_v1.py first"
    if not mt5.initialize():
        raise RuntimeError(f"MT5 init fail: {mt5.last_error()}")
    try:
        h1 = load_h1("AUDUSD")
        iron_z = build_z_lookup(IRON_PANEL, "tio_close")
        cny_z = build_z_lookup(CNY_PANEL, "cny_usd_strength")
        o1 = probe_ratio_cont(
            h1,
            iron_z,
            hypothesis_id="HYP-AUDUSD-H1-IRONORE-TOT-CONT-001",
            class_name="iron_ore_tot_cont",
            thesis="TIO_iron_ore_AUD_terms_of_trade",
            value_name="ironore",
        )
        o2 = probe_ratio_cont(
            h1,
            cny_z,
            hypothesis_id="HYP-AUDUSD-H1-CNYSTRENGTH-DEMAND-CONT-001",
            class_name="cny_strength_china_demand_cont",
            thesis="CNY_strength_china_demand_AUD",
            value_name="cnystr",
        )
        o3 = probe_book(o1, o2)
        ...
        payload = {
            "schema": "ironore_cny_offline_probes.v1",
            "created_at_utc": utc_now(),
            "campaign_status": status,
            "wave": "W24_IRONORE_CNY_EXO",
            "window": {
                "from": FROM.isoformat(),
                "to": TO.isoformat(),
            ...
            "panel_sha256": {
                "iron": sha256_file(IRON_PANEL),
                "cny": sha256_file(CNY_PANEL),
            },
            ...
            "cnh_status": "UNAVAILABLE_THIS_PASS__PICK_NEXT_USDCNY_STRENGTH",
            "objects": objects,
