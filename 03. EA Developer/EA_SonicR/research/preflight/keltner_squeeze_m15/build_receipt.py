# -*- coding: utf-8 -*-
"""Build contract receipt + stubs for HYP-KELTNER-SQUEEZE-M15-001."""
...
EA = ROOT / "03. EA Developer" / "EA_KeltnerSqueeze" / "EA_KeltnerSqueeze.mq5"
OUT_DIR = (
    ROOT
    / "03. EA Developer"
    / "EA_SonicR"
...
STUB = OUT_DIR / "receipt_stubs_HYP_KELTNER_SQUEEZE_M15_001"
RECEIPT = OUT_DIR / "20260714_HYP_KELTNER_SQUEEZE_M15_001_CONTRACT_RECEIPT.json"

# S654-aligned a priori (defaults): Mon+Wed+Thu Europe — freeze via overrides
# so binding is explicit even if EA defaults drift.
# alpha.ps1 normalizes overrides to alphabetical key order — match that here.
_OVERRIDE_MAP = {
    "InpEndHour": "14",
    "InpExitHour": "22",
    "InpMaxPerDay": "2",
...
OVERRIDES = ";".join(f"{k}={_OVERRIDE_MAP[k]}" for k in sorted(_OVERRIDE_MAP))
...
def main() -> int:
    ...
    task = {
        "hypothesis_id": "HYP-KELTNER-SQUEEZE-M15-001",
        "ea_name": "EA_KeltnerSqueeze",
        "symbol": "USDJPY",
        "period": "M15",
        "from": "2021.01.01",
        "to": "2025.12.31",
        ...
        "overrides": OVERRIDES,
        "run_role": "control",
        "seed": "S654 KeltnerSqueeze Mon+Wed+Thu Europe PF~1.15",
    }
    prereg = {
        "hypothesis_id": "HYP-KELTNER-SQUEEZE-M15-001",
        "prereg_path": "03. EA Developer/EA_SonicR/research/preregs/20260714_H_KELTNER_SQUEEZE_M15_001_PREREG.md",
        "frozen": True,
    }
    cost = {
    ...
    include_note = (
        "Packet-bound include closure note for HYP-KELTNER-SQUEEZE-M15-001. "
        "Single-file EA; no custom includes beyond terminal Standard Library."
    )
    ...
    receipt = {
        ...
        "note": "Model 0 Keltner Squeeze S654 seed; independent of VolExp/Chop.",
    }
    RECEIPT.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    rec_sha = sha256_file(RECEIPT)
    (OUT_DIR / "20260714_HYP_KELTNER_SQUEEZE_M15_001_CONTRACT_RECEIPT.sha256.txt").write_text(
        rec_sha + "\n", encoding="utf-8"
    )
