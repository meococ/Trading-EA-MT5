#!/usr/bin/env python3
"""HARD PIVOT W14 — H4 swing-break H1-retest CONT + Asia-quiet London break after W1–W13 ALL_KILL.

FORBIDDEN densify: W1–W12 / FVG / R-series / swing / Donch / Outside / VR /
absorb3 / weekday-gap / inventory / prior2d / usdconsensus / NR7 / H4-engulf / ORB/IB / closeloc.

NEW classes (outside W1–W13 densify bans):
  A. HYP-FX3-H4-SWING-BREAK-H1-RETEST-CONT-001
     H4 fractal swing break → first H1 retest of break level CONT.
     ≠ W3 H4disp accept; ≠ W9 H4 swing first-retest densify; ≠ W10 mid-reclaim.
  B. HYP-FX3-H1-ASIA-QUIET-LONDON-BREAK-CONT-001
     Asia TR quiet (≤0.55*ATR) then London break of Asia HL CONT.
     ≠ NR7; ≠ W8 London box; ≠ W5 Asia sweep reclaim; ≠ XAU compress starve densify.
  BOOK. HYP-BOOK-H4RETEST-ASIAQUIET-APRIORI-001
...
STEM = "20260715_HARD_PIVOT_W14_H4RETEST_ASIAQUIET"
...
OUT_CLEAN_VN = READ / "20260715_CLEAN_BOOK_AND_HARD_PIVOT_W14_VN_ACTION_BRIEF.md"
OUT_FREEZE = READ / f"{STEM}_APRIORI_FREEZE.md"
OUT_COMBO_VN = READ / "20260715_COST_GRADE_AND_HARD_PIVOT_W14_VN_ACTION_BRIEF.md"
...
B_ASIA = (0, 7)  # UTC
B_LONDON = (7, 12)
B_QUIET_ATR = 0.55  # Asia TR <= this * ATR(H1 at Asia end)
B_SL_PAD = 0.20
B_SL_MIN_ATR = 1.25
B_RR = 2.75
B_HOLD = 40
...
def h4_fractal_swings(h4, L):
    swings = []
    tt, hh, ll = h4["t"], h4["h"], h4["l"]
    for j in range(L, len(tt) - L):
        if all(hh[j] > hh[j - k] for k in range(1, L + 1)) and all(
            hh[j] > hh[j + k] for k in range(1, L + 1)
        ):
            swings.append((int(tt[j + L]), float(hh[j]), "hi"))
        if all(ll[j] < ll[j - k] for k in range(1, L + 1)) and all(
            ll[j] < ll[j + k] for k in range(1, L + 1)
        ):
            swings.append((int(tt[j + L]), float(ll[j]), "lo"))
...
def probe_h4_swing_break_h1_retest(h1, h4):
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    swings = {s: h4_fractal_swings(h4[s], A_SWING_L) for s in FX3}
    ...
    for i in range(80, len(clock) - 1):
        ...
        for sym in FX3:
            ...
            if j4 < A_SWING_L + 2:
                continue
            while si[sym] < len(swings[sym]) and swings[sym][si[sym]][0] <= int(d4["t"][j4]):
                si[sym] += 1
            if si[sym] <= 0:
                continue
            _conf_ts, level, kind = swings[sym][si[sym] - 1]
            c4 = float(d4["c"][j4])
            atr4 = d4["atr"][j4]
            if not np.isfinite(atr4) or atr4 <= 0:
                continue
            if kind == "hi" and c4 > level:
                arms[sym] = {"level": level, "side": 1, "armed_ts": int(d4["t"][j4])}
            elif kind == "lo" and c4 < level:
                arms[sym] = {"level": level, "side": -1, "armed_ts": int(d4["t"][j4])}
            arm = arms[sym]
...
def probe_asia_quiet_london_break(h1):
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    ...
    for i in range(40, len(clock) - 1):
        ...
        for sym in FX3:
            ...
            side = 0
            if c > a_hi and h > a_hi:
                side = 1
            elif c < a_lo and l < a_lo:
                side = -1
            if side == 0:
                continue
            ent_i = asof_idx(d, ts)
            if ent_i is None:
                continue
            entry = float(d["o"][ent_i])
            atr_e = d["atr"][ent_i] if np.isfinite(d["atr"][ent_i]) else atr1
            if side > 0:
                sl = a_lo - B_SL_PAD * atr_e
                sl = min(sl, entry - B_SL_MIN_ATR * atr_e)
            else:
                sl = a_hi + B_SL_PAD * atr_e
...
def evaluate_book(ta, tb):
    for t in ta:
        t["sleeve"], t["priority"] = "A_H4RETEST", A_PRIORITY
        t["pnl_haircut"] = float(t["pnl"]) - BASE_COST
    for t in tb:
        t["sleeve"], t["priority"] = "B_ASIAQUIET", B_PRIORITY
        t["pnl_haircut"] = float(t["pnl"]) - BASE_COST
    ...
    a, b = by.get("A_H4RETEST", []), by.get("B_ASIAQUIET", [])
    ...
    return _jsonable(
        {
            "hypothesis_id": "HYP-BOOK-H4RETEST-ASIAQUIET-APRIORI-001",
            "setup": "a priori book: H4 swing-break H1-retest + Asia-quiet London break",
            "symbol": "EURUSD+GBPUSD+USDJPY",
            "timeframe": "H1",
            "metrics": m,
            "haircuts": hc,
...
def write_freeze():
    body = "\n".join(
        [
            "# A priori freeze — HARD PIVOT W13 H4-retest + Asia-quiet",
            "",
            "Date: 2026-07-15",
            ...
            "",
            "## Objects",
            "- HYP-FX3-H4-SWING-BREAK-H1-RETEST-CONT-001",
            "- HYP-FX3-H1-ASIA-QUIET-LONDON-BREAK-CONT-001",
            "- HYP-BOOK-H4RETEST-ASIAQUIET-APRIORI-001",
            "",
            "## Forbidden",
            "W1–W13 densify; FVG; R-series; swing/Donch/Outside/VR; absorb3; weekday-gap; prior2d; usdconsensus;",
...
def write_preregs():
    ...
    specs = [
        (
            "20260715_H_FX3_H4_SWING_BREAK_H1_RETEST_CONT_001_PREREG.md",
            "HYP-FX3-H4-SWING-BREAK-H1-RETEST-CONT-001",
            [
                f"- H4 fractal swing break then first H1 retest CONT; RR={A_RR}; hold<={A_HOLD}.",
                "- != W3 H4disp accept; != W9 H4 swing first-retest densify; != W10 mid-reclaim.",
            ],
        ),
        (
            "20260715_H_FX3_H1_ASIA_QUIET_LONDON_BREAK_CONT_001_PREREG.md",
            "HYP-FX3-H1-ASIA-QUIET-LONDON-BREAK-CONT-001",
            [
                f"- Asia TR<={B_QUIET_ATR}*ATR then London break Asia HL; RR={B_RR}; hold<={B_HOLD}.",
                "- != NR7; != W8 London box; != W5 Asia sweep reclaim.",
            ],
        ),
        (
            "20260715_H_BOOK_H4RETEST_ASIAQUIET_APRIORI_001_PREREG.md",
            "HYP-BOOK-H4RETEST-ASIAQUIET-APRIORI-001",
            [
        ...
        p.write_text(
            "\n".join(
                [
                    ...
                    "- Lane: `hard_pivot_w14_h4retest_asiaquiet_20260715`",
                    *extra,
        ...
        rows.append(
            {
                ...
                "parent_candidate": "hard_pivot_w14_h4retest_asiaquiet",
                "feature_family": "hard_pivot_w14_h4retest_asiaquiet",
                "lane": "hard_pivot_w14_h4retest_asiaquiet_20260715",
...
def write_docs(results, book, receipt, any_surv, freeze_sha):
    ...
    OUT_PANEL.write_text(
        "\n".join(
            [
                "# 3-critic — W13 H4-retest + Asia-quiet",
                "",
                "| Critic | Call |",
                "|---|---|",
                "| Sonic trader | GO — H4 retest CONT != W3 accept; Asia-quiet London != W8 box/W5 reclaim |",
    ...
    OUT_DESIGN.write_text(
        "\n".join(
            [
                "# Design — HARD PIVOT W13 H4-retest + Asia-quiet",
                "",
                f"Freeze sha={freeze_sha[:16]}…",
                "",
                "After W1–W13 ALL_KILL (dense CONT shelf starved edge under +$12).",
    ...
    OUT_DEDUP.write_text(
        "\n".join(
            [
                ...
                "| Object | ≠ |",
                "|---|---|",
                "| H4 swing-break H1-retest | W3 H4disp accept; W9 H4 swing first-retest; W10 mid-reclaim |",
                "| Asia-quiet London break | NR7; W8 London box; W5 Asia sweep reclaim; XAU compress densify |",
                "",
    ...
    OUT_VN.write_text(
        "\n".join(
            [
                "# VN — HARD PIVOT W13 H4-retest + Asia-quiet",
                "",
                f"`{status}`",
                "",
                *table,
                "",
                "Không densify W1–W13 / FVG / R-series / prior2d / usdconsensus / absorb3.",
                f"Receipt `{receipt}`",
                "GOAL unmet.",
                "",
            ]
    ...
    OUT_CLOSE.write_text(
        "\n".join(
            [
                ...
                "Next: next independent class outside W1–W14 if ALL_KILL;",
    ...
    OUT_CLEAN_VN.write_text(
        "\n".join(
            [
                ...
                *table,
                "",
                "- H4-retest CONT + Asia-quiet London break; MFE ≫ $12 a priori.",
        ...
        "\n".join(
            [
                ...
                *cg,
                ...
                "",
                f"## Track 2 — HARD PIVOT W14 `{status}`",
                *table,
                f"Receipt W14 `{receipt}`",
                "GOAL unmet.",
                "",
            ]
        ),
...
def patch_hot(results, book, receipt, any_surv, freeze_sha):
    ...
    block = "\n".join(
        [
            f"- **HARD PIVOT W14 H4RETEST/ASIAQUIET + COST-GRADE CLOSEOUT (2026-07-15 ~{stamp} ICT) — "
    ...
    text = re.sub(
        r"^Updated:.*$",
        f"Updated: 2026-07-15 ~{stamp} ICT | HARD PIVOT W14 h4retest/asiaquiet + cost-grade; "
...
def main():
    ...
    try:
        ...
        pnls_a, detail_a, trades_a = probe_h4_swing_break_h1_retest(h1, h4)
        pnls_b, detail_b, trades_b = probe_asia_quiet_london_break(h1)
        ra = pack_result(
            "HYP-FX3-H4-SWING-BREAK-H1-RETEST-CONT-001",
            "H4 swing-break H1 first-retest CONT RR3",
            pnls_a,
            detail_a,
        )
        rb = pack_result(
            "HYP-FX3-H1-ASIA-QUIET-LONDON-BREAK-CONT-001",
            "Asia-quiet London HL break CONT RR2.75",
            pnls_b,
            detail_b,
        )
        ...
        payload = {
            "schema_version": "hard_pivot_w14_h4retest_asiaquiet_offline.v1",
