#!/usr/bin/env python3
"""Structural MULTISYM — EUR/GBP/XAU offline probes (escape USDJPY TF saturation).
...
def main() -> int:
    ...
    payload = {
        "schema_version": "sonic_structural_offline_probes.multisym",
        "created_at_utc": utc_now(),
        "status": "OFFLINE_FIRST_MULTISYM_COMPLETE",
        "honesty": "NOT_MODEL0_NOT_CONFIRMED_NOT_GOAL",
        "escape": "USDJPY_TF_SATURATION_EXPLICIT",
        "server": server,
        "login": login,
        "dedup": "readouts/20260714_STRUCTURAL_MULTISYM_DEDUP_CLEARANCE.md",
        "probes": probes,
    ...
    out = PRE / "20260714_20260714_STRUCTURAL_MULTISYM_OFFLINE_PROBES.json"
    write_json(out, payload)
    sha = sha256_file(out)

    lines = [
        "# Structural rebuild offline probes MULTISYM (EUR/GBP/XAU)",
        "",
    ...
    (READ / "20260714_20260714_STRUCTURAL_MULTISYM_OFFLINE_PROBES.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
                ...
                json.dumps(
                    {
                        ...
                        "state": state,
                        "verdict": p["verdict"],
                        "reason": f"offline MULTISYM {p['metrics']}; {p['kill_notes']}",
                        "updated_at": "2026-07-14",
                        "lane": "structural_multisym_20260714",
                        "symbol": p["symbol"],
                        "timeframe": p["tf"],
