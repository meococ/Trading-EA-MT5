#!/usr/bin/env python3
"""CLEAN BOOK — offline a priori +$12 stress for HYP-BOOK-CLEAN-APRIORI-RR2SPARK-001.

Universe/caps frozen BEFORE metrics in:
  readouts/20260715_CLEAN_BOOK_APRIORI_UNIVERSE_FREEZE.md

NOT Phase-0 ceremony. Does not clear contamination attestation.
No sleeve substitution after metrics. Model 0 WITHHELD (book-level offline).
Tester +$12 ≠ Real QFSI.
"""
...
HYP_ID = "HYP-BOOK-CLEAN-APRIORI-RR2SPARK-001"
OUT_JSON = PRE / "20260715_CLEAN_BOOK_APRIORI_RR2SPARK_STRESS.json"
OUT_MD = READ / "20260715_CLEAN_BOOK_APRIORI_RR2SPARK_STRESS.md"
FREEZE_MD = READ / "20260715_CLEAN_BOOK_APRIORI_UNIVERSE_FREEZE.md"
...
def main() -> int:
    ...
    payload = {
        "schema_version": "clean_book_apriori_rr2spark_stress.v1",
        ...
        "model0": "WITHHELD_BOOK_LEVEL_OFFLINE_NOT_EA_CHALLENGER",
        "phase0_contamination": "STILL_CONTAMINATED_NOT_CLEARED_BY_THIS_HYP",
        "non_claims": [
            ...
            "no_invented_spreads",
            "no_model0_book_glue",
            "sibling_strategy_shift_stress_is_history_only",
        ],
    }
    ...
    md = f"""# CLEAN BOOK — offline a priori +$12 stress
...
"""
    ...
    # Registry: idea→killed/parked under this hyp after offline screen.
    p_verdict = primary["goal_screen"]["verdict"]
    e_verdict = extended["goal_screen"]["verdict"]
    goal_pass = (
        primary["goal_screen"]["pf_gt_1_30_after_12"]
        and primary["goal_screen"]["tpw_2_to_5"]
        and primary["caps"]["caps_pass"]
    )
    state = "probe" if goal_pass else "killed"
    reg = {
        "record_type": "candidate",
        "schema_version": 1,
        "hypothesis_id": HYP_ID,
        "state": state,
        "verdict": "OFFLINE_GOAL_SCREEN_PASS" if goal_pass else "KILLED_AT_OFFLINE_PROBE",
        "reason": (
            f"clean-book a priori freeze then stress; PRIMARY={p_verdict}; "
            f"EXTENDED={e_verdict}; model0=WITHHELD; not Phase-0"
        ),
        "parent_candidate": "HYP-SB-MAXKZ2-RR2-FRICTION-001",
        "feature_family": "clean_book_apriori_rr2_spark_not_phase0",
        "lane": "strategy_shift_clean_book_20260715",
        "setup_type": "PRIMARY RR2+Spark + EXTENDED ITSM; caps frozen pre-metrics",
        "symbol": "USDJPY+multi",
        "timeframe": "M15",
        "window": "2021.01.01-2025.12.31",
        "model": "offline_book_probe_only",
        "source_provenance": (
            "Clean-lane book hyp after STRATEGY SHIFT; does not clear "
            "PHASE0-CONTAMINATION-20260711-001"
        ),
        "prereg_path": None,
        "readout_path": str(OUT_MD.as_posix()),
        "run_ids": ["20260714_194548", "20260714_193358", "20260714_003920"],
        "metrics": {
            "primary_n": primary["pooled_after_heat"]["n"],
            "primary_pf_12": primary["pooled_after_heat"]["pf_haircut"],
            "primary_tpw": primary["pooled_after_heat"]["tpw"],
            "extended_n": extended["pooled_after_heat"]["n"],
            "extended_pf_12": extended["pooled_after_heat"]["pf_haircut"],
            "extended_tpw": extended["pooled_after_heat"]["tpw"],
        },
        "validation": {
            "offline_probe": state.upper(),
            "model0": "WITHHELD_BOOK_LEVEL_OFFLINE_NOT_EA_CHALLENGER",
            "freeze_sha256": payload["freeze_sha256"],
            "stress_sha256": result_sha,
            "phase0": "STILL_CONTAMINATED_NOT_CLEARED",
        },
        ...
        json.dumps(
            {
                ...
                "freeze_sha256": payload["freeze_sha256"],
                "primary": primary["goal_screen"],
                "extended": extended["goal_screen"],
                "model0": payload["model0"],
