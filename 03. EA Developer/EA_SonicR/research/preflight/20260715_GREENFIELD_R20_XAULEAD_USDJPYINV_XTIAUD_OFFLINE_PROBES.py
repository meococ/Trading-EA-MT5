#!/usr/bin/env python3
"""Round 20 greenfield — XAU→NZD lead + USDJPY→XAU inverse + XTI→AUD.

HARD FORBIDDEN: R19 CRYPTO3/CADJPY-riskon/EURCHF; ETH VR densify; R18 AUD-imp/
pivot/co-mom; R17 VR/semivar/UTC0; R16 US30-break/CPI/US30-lead; R15–R10;
Parkinson/range-compress; residual/corr/ON-ratio; fade/MR; unpark/exit/FRED.

A priori freeze (nested critic GO after reboard; compress + US30→EUR densify NO-GO):
...
STEM = "20260715_GREENFIELD_R20_XAULEAD_USDJPYINV_XTIAUD"
...
OUT_CLEAN_VN = READ / "20260715_CLEAN_BOOK_AND_R20_VN_ACTION_BRIEF.md"
...
# 1 XAU → NZD impulse lead
XN_IMP = 0.70
XN_SL = 1.40
XN_RR = 2.00
XN_HOLD = 10

# 2 USDJPY → XAU inverse
UX_IMP = 0.70
UX_SL = 1.45
UX_RR = 2.00
UX_HOLD = 10

# 3 XTI → AUD impulse lead
XA_IMP = 0.60
XA_SL = 1.40
XA_RR = 2.00
XA_HOLD = 10
...
def probe_lead_follower(
    ...
    """Closed-bar lead impulse → follower CONT next open (optional inverse side)."""
    closed, open_pos = [], []
    last_day = None
    for i in range(2, len(follower["t"]) - 1):
        ts = int(follower["t"][i])
        open_pos = manage_exits(open_pos, {fsym: follower}, ts, closed, hold)
        dt = datetime.fromtimestamp(ts, tz=timezone.utc)
        if dt.weekday() >= 5:
            continue
        if open_pos:
            continue
        sig_ts = int(follower["t"][i - 1])
        li = asof_idx(leader, sig_ts)
        if li is None:
            continue
        atr_l = leader["atr"][li]
        if not np.isfinite(atr_l) or atr_l <= 0:
            continue
        body_l = float(leader["c"][li]) - float(leader["o"][li])
        if abs(body_l) < imp_k * atr_l:
            continue
        side = (1 if body_l > 0 else -1) * (-1 if inverse else 1)
        atr = follower["atr"][i - 1]
        if not np.isfinite(atr) or atr <= 0:
            continue
        day = dt.date()
        if day == last_day:
            continue
        entry = float(follower["o"][i])
        sl = entry - side * sl_k * atr
        tp = entry + side * rr * sl_k * atr
        lots = risk_lots(fsym, entry, sl)
        open_pos.append(
            {
                "sym": fsym,
                "side": side,
        ...
        last_day = day
    flush_open(open_pos, {fsym: follower}, closed)
    ...
    with REG.open("a", encoding="utf-8") as f:
        for r in results:
            row = {
                ...
                "feature_family": "greenfield_r20_xaulead_usdjpyinv_xtiaud",
                "lane": "strategy_shift_r20_greenfield_20260715",
                "setup_type": r["setup"],
                "symbol": r["symbol"],
                ...
                "source_provenance": (
                    "R20 NON-FADE outside R10–R19; nested critic GO after "
                    "compress/US30-EUR densify reboard"
                ),
                "prereg_path": None,
...
def write_docs(results, receipt, any_surv, qnote, clean_book_note, freeze_sha):
    ...
    OUT_PANEL.write_text(
        "\n".join(
            [
                "# 3-critic panel — Round 20 XAU→NZD / USDJPY→XAU inv / XTI→AUD",
                "",
                "Date: 2026-07-15",
                "Nested critic: first board NO-GO (compress densify); second NO-GO "
                "(US30→EUR densify of R16 lead); final board **GO** (lead self-merge).",
                "",
                "## Named (NON-FADE)",
                "1. `NZDUSD_XAU_IMPULSE_LEAD_CONT`",
                "2. `XAUUSD_USDJPY_INVERSE_LEAD_CONT`",
                "3. `AUDUSD_XTI_IMPULSE_LEAD_CONT`",
                "",
                "| Critic | Stance |",
                "|---|---|",
                "| Sonic trader | SOFT — metal→NZD / yen→gold inv / oil→AUD CONT channels |",
                "| Quant | PASS — distinct lead×follower; no R10–R19 hard ban |",
                "| MQL5/MT5 | PASS — closed-bar as-of lead → next-open CONT |",
                "",
    ...
    OUT_DESIGN.write_text(
        "\n".join(
            [
                ...
                "",
                f"## 1 `HYP-NZDUSD-H1-XAUUSD-IMPULSE-LEAD-CONT-001`",
                f"XAU |body|≥{XN_IMP}×ATR → NZDUSD same-sign CONT; "
                f"SL={XN_SL} RR={XN_RR} hold≤{XN_HOLD}. NOT XAU-ROC / AUD→NZD densify.",
                "",
                f"## 2 `HYP-XAUUSD-H1-USDJPY-INVERSE-LEAD-CONT-001`",
                f"USDJPY |body|≥{UX_IMP}×ATR → XAU opposite CONT; "
                f"SL={UX_SL} RR={UX_RR}. NOT XAU ROC densify.",
                "",
                f"## 3 `HYP-AUDUSD-H1-XTI-IMPULSE-LEAD-CONT-001`",
                f"XTI |body|≥{XA_IMP}×ATR → AUDUSD same-sign CONT; "
                f"SL={XA_SL} RR={XA_RR}. NOT CADJPY XTI×NAS / COM3 densify.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    OUT_DEDUP.write_text(
        "\n".join(
            [
                "# Dedup — Round 20 XAU→NZD / USDJPY→XAU inv / XTI→AUD",
                "",
                "| Object | Cleared vs |",
                "|---|---|",
                "| NZDUSD ← XAU impulse lead CONT | ≠ AUD→NZD impulse lead (R18); "
                "≠ XAU ROC TSMOM (R13); ≠ US30 lead (R16); ≠ Parkinson compress |",
                "| XAU ← USDJPY inverse lead CONT | ≠ XAU ROC densify; ≠ EURCHF NAS "
                "sentiment; ≠ CADJPY risk-on; ≠ metal-ratio / yen-β resid |",
                "| AUDUSD ← XTI impulse lead CONT | ≠ CADJPY XTI×NAS dual (R19); "
                "≠ AUD COM3; ≠ AUD→NZD; ≠ WTI→USDCAD ToT clones |",
                "",
                "Rejected a priori: GBPJPY range-compress (Parkinson densify); "
                "US30→EURUSD (R16 US30-lead densify).",
                "R10–R19 densify boards: **FORBIDDEN**.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    OUT_CLOSE.write_text(
        "\n".join(
            [
                "# Session closeout — Round 20 XAU→NZD / USDJPY→XAU inv / XTI→AUD",
                "",
                f"Status: `{status}`",
                f"Receipt: `{receipt}`",
                ...
                "R19 CRYPTO3/riskon/EURCHF / ETH VR / R18–R10 / R1–R9 / unpark / exit / FRED.",
                "Next: next true greenfield outside R20 — QFSI parallel only for cost.",
    ...
    OUT_VN.write_text(
        "\n".join(
            [
                "# VN brief — Round 20 XAU→NZD / USDJPY→XAU inv / XTI→AUD",
                "",
                f"Thời điểm: 2026-07-15 ~{stamp} ICT",
                "Ngoài R10–R19. **NON-FADE.** Reboard sau NO-GO compress + US30→EUR.",
                "",
                f"## Kết quả — `{status}`",
                *table,
                "",
                ...
                "",
                "## Quyết định",
                "- Không densify XAU→NZD / USDJPY→XAU-inv / XTI→AUD / R10–R19.",
                "- Best shelf RR2 `194548`. Cost GAP. QFSI parallel only.",
                "- Next: greenfield ngoài R20 **hoặc** research-grade cost.",
                "",
    ...
    OUT_CLEAN_VN.write_text(
        "\n".join(
            [
                "# VN brief — Clean book + Round 20",
                "",
                f"Thời điểm: 2026-07-15 ~{stamp} ICT",
                "Lane: clean-book + discovery R20 NON-FADE (metal/yen/oil leads). GOAL: chưa đạt.",
                "",
                "## 1. Clean book `HYP-BOOK-CLEAN-APRIORI-RR2SPARK-001`",
                clean_book_note,
                "- Model 0 book-level: **WITHHELD**.",
                "",
                "## 2. Discovery Round 20 — XAU→NZD / USDJPY→XAU inv / XTI→AUD",
                *table,
                ...
                "",
                "## Cấm",
                "Densify R1–R20 / VR rescue / CRYPTO3 breadth-k / fade-session / "
                "unpark / exit / FRED / Phase-0.",
                "",
                "## Next agent",
                "Giữ QFSI; greenfield ngoài R20 (NON-FADE); cost khi Owner drop deal-export.",
...
def patch_hot(results, receipt, any_surv, qnote, clean_note, freeze_sha):
    ...
    block = [
        "",
        f"- **GREENFIELD ROUND20 XAULEAD/USDJPYINV/XTIAUD CLOSEOUT (2026-07-15 ~{stamp} ICT) — "
        ...
        "  Nested critic GO after compress + US30→EUR densify reboard "
    ...
    if lines and lines[0].startswith("# Hot Cache"):
        lines[1] = (
            f"Updated: 2026-07-15 ~{stamp} ICT | R20 "
    ...
    while i < len(lines):
        ln = lines[i]
        if ln.startswith("- **GREENFIELD ROUND20 XAULEAD/USDJPYINV/XTIAUD CLOSEOUT"):
            i += 1
            while i < len(lines):
                nxt = lines[i]
...
def freeze_contract_sha() -> str:
    contract = {
        "xau_nzd": {"imp": XN_IMP, "sl": XN_SL, "rr": XN_RR, "hold": XN_HOLD},
        "usdjpy_xau_inv": {"imp": UX_IMP, "sl": UX_SL, "rr": UX_RR, "hold": UX_HOLD},
        "xti_aud": {"imp": XA_IMP, "sl": XA_SL, "rr": XA_RR, "hold": XA_HOLD},
        "cost": BASE_COST,
        "window": "2021.01.01-2025.12.31",
        "forbidden": "R10_R19_densify__ETH_VR__compress__US30_EUR_lead_swap",
    }
    return sha256_bytes(
...
def main():
    ...
    try:
        ...
        xau = enrich(load("XAUUSD", mt5.TIMEFRAME_H1))
        uj = enrich(load("USDJPY", mt5.TIMEFRAME_H1))
        aud = enrich(load("AUDUSD", mt5.TIMEFRAME_H1))
        xti = enrich(load("XTIUSD", mt5.TIMEFRAME_H1))
        results = [
            pack_result(
                "HYP-NZDUSD-H1-XAUUSD-IMPULSE-LEAD-CONT-001",
                "nzdusd_h1_xau_impulse_lead_cont",
                "NZDUSD",
                "H1",
                *probe_lead_follower(
                    nzd, xau, "NZDUSD", XN_IMP, XN_SL, XN_RR, XN_HOLD, False
                ),
            ),
            pack_result(
                "HYP-XAUUSD-H1-USDJPY-INVERSE-LEAD-CONT-001",
                "xauusd_h1_usdjpy_inverse_lead_cont",
                "XAUUSD",
                "H1",
                *probe_lead_follower(
                    xau, uj, "XAUUSD", UX_IMP, UX_SL, UX_RR, UX_HOLD, True
                ),
            ),
            pack_result(
                "HYP-AUDUSD-H1-XTI-IMPULSE-LEAD-CONT-001",
                "audusd_h1_xti_impulse_lead_cont",
                "AUDUSD",
                "H1",
                *probe_lead_follower(
                    aud, xti, "AUDUSD", XA_IMP, XA_SL, XA_RR, XA_HOLD, False
                ),
            ),
        ]
        any_surv = any(r["verdict"] == "PROBE_SURVIVOR" for r in results)
        payload = {
            "schema_version": "greenfield_r20_xaulead_usdjpyinv_xtiaud.v1",
            "generated_at_utc": utc_now(),
            "window": "2021.01.01-2025.12.31",
            "cost_apriori_usd": BASE_COST,
            "freeze_sha256": freeze_sha,
            "hard_constraint": "NON_FADE__NO_R10_R19_DENSIFY__NO_ETH_VR_RESCUE",
            "gates": {
                "n_min": 80,
                "pf_min": 1.30,
                "tpw_min": 2.0,
        ...
        print(json.dumps({"status": payload["status"], "receipt": receipt, "results": [
            {"id": r["hypothesis_id"], "verdict": r["verdict"], "m": r["metrics"],
             "x15": r["haircuts"]["x1_5"]["pf"]} for r in results
        ]}, indent=2))
    finally:
        mt5.shutdown()
