#!/usr/bin/env python3
"""HARD PIVOT W19 — H4 EMA z-score fade + compression false-break fade after W1–W18 ALL_KILL.

FORBIDDEN densify: W1–W18 / FVG / R-series / swing / Donch / Outside / VR /
...
failbreak / relexp / trendday / ADR-fade / impulse-decay /
session-CONT location-accept densify.

NEW edge-source classes (≠ CONT densify; ≠ W18 ADR/impulse densify):
  A. HYP-FX3-H1-H4-EMA-ZSCORE-FADE-001
     |H1 close - H4 EMA20| / H4 ATR ≥ Z → fade toward EMA. Statistical stretch.
     ≠ W18 ADR day-range fade; ≠ location S/R accept; ≠ session CONT.
  B. HYP-FX3-H1-COMPRESSION-FALSEBREAK-FADE-001
     ≥6-bar compression (mean TR ≤ 0.55 ATR) → first expansion bar FADE (trap).
     ≠ W7 expansion CONT; ≠ W17 rel-expansion CONT; ≠ ORB/NR7 densify.
  BOOK. HYP-BOOK-ZSCORE-COMPFADE-APRIORI-001
...
STEM = "20260715_HARD_PIVOT_W19_ZSCORE_COMPFADE"
...
OUT_CLEAN_VN = READ / "20260715_CLEAN_BOOK_AND_HARD_PIVOT_W19_VN_ACTION_BRIEF.md"
OUT_FREEZE = READ / f"{STEM}_APRIORI_FREEZE.md"
OUT_COMBO_VN = READ / "20260715_COST_GRADE_AND_HARD_PIVOT_W19_VN_ACTION_BRIEF.md"
...
def h4_asof(h4, ts):
    """Last closed H4 bar at or before ts (H4 open <= ts-1s conceptually: use searchsorted right-1)."""
...
def probe_h4_ema_zscore_fade(h1, h4):
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    used = set()
    for i in range(40, len(clock) - 1):
        ...
        for sym in FX3:
            ...
            if j is None or j < 30:
                continue
            hj = h4_asof(h4[sym], sig_ts)
            if hj is None or hj < A_EMA + 5:
                continue
            ema = h4[sym]["ema"][hj]
            atr4 = h4[sym]["atr"][hj]
            if not (np.isfinite(ema) and np.isfinite(atr4) and atr4 > 0):
                continue
            c = float(d["c"][j])
            z = (c - ema) / atr4
            if abs(z) < A_Z:
            ...
            if not np.isfinite(atr) or atr <= 0:
                atr = atr4 / 4.0 if atr4 > 0 else None
            ...
            # SL beyond stretch extreme
            if side > 0:
                sl = min(float(d["l"][j]), entry - A_SL_MIN_ATR * atr_e) - A_SL_PAD * atr_e
                sl = min(sl, entry - A_SL_MIN_ATR * atr_e)
            else:
                sl = max(float(d["h"][j]), entry + A_SL_MIN_ATR * atr_e) + A_SL_PAD * atr_e
                sl = max(sl, entry + A_SL_MIN_ATR * atr_e)
            sl_dist = abs(entry - sl)
            if sl_dist <= 1e-12:
                continue
            tp_rr = entry + side * A_RR * sl_dist
            # prefer EMA magnet if closer and same side
            if side > 0:
                tp = min(tp_rr, ema) if ema > entry else tp_rr
            else:
                tp = max(tp_rr, ema) if ema < entry else tp_rr
            if side * (tp - entry) <= 0:
                tp = tp_rr
...
def probe_compression_falsebreak_fade(h1):
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    used = set()
    armed = {s: None for s in FX3}  # compression end idx
    for i in range(40, len(clock) - 1):
        ts = int(clock[i])
        ...
        for sym in FX3:
            d = h1[sym]
            j = asof_idx(d, sig_ts)
            if j is None or j < B_COMP_N + 10:
                continue
            atr = d["atr"][j]
            if not np.isfinite(atr) or atr <= 0:
                continue
            # detect compression on bars j-COMP_N .. j-1 (exclude signal bar)
            trs = []
            for k in range(B_COMP_N):
                idx = j - B_COMP_N + k
                trs.append(float(d["h"][idx] - d["l"][idx]))
            mean_tr = float(np.mean(trs))
            compressed = mean_tr <= B_COMP_ATR * atr
            if compressed:
                armed[sym] = {
                    "hi": max(float(d["h"][j - B_COMP_N + k]) for k in range(B_COMP_N)),
                    "lo": min(float(d["l"][j - B_COMP_N + k]) for k in range(B_COMP_N)),
                    "atr": atr,
                    "end_j": j - 1,
                }
            arm = armed[sym]
            ...
            if cool in used:
                continue
            # first expansion break of compression box
            ...
            if c > arm["hi"] and h > arm["hi"]:
                side = -1  # breakout up → fade short
            elif c < arm["lo"] and l < arm["lo"]:
                side = 1  # breakout down → fade long
            if side == 0:
                continue
            # only first break after arm
            if j > arm["end_j"] + 4:
                armed[sym] = None
                continue
            ent_i = asof_idx(d, ts)
            ...
            if sl_dist <= 1e-12:
                continue
            # target back into compression mid
            mid = 0.5 * (arm["hi"] + arm["lo"])
            tp_rr = entry + side * B_RR * sl_dist
            if side > 0:
...
def evaluate_book(ta, tb):
    for t in ta:
        t["sleeve"], t["priority"] = "A_ZSCORE", A_PRIORITY
        t["pnl_haircut"] = float(t["pnl"]) - BASE_COST
    for t in tb:
        t["sleeve"], t["priority"] = "B_COMPFADE", B_PRIORITY
        t["pnl_haircut"] = float(t["pnl"]) - BASE_COST
    ...
    a, b = by.get("A_ZSCORE", []), by.get("B_COMPFADE", [])
    ...
    return _jsonable(
        {
            "hypothesis_id": "HYP-BOOK-ZSCORE-COMPFADE-APRIORI-001",
            "setup": "a priori book: H4 EMA z-score fade + compression false-break fade",
            "symbol": "EURUSD+GBPUSD+USDJPY",
            "timeframe": "H1",
            "metrics": m,
            "haircuts": hc,
...
def write_freeze():
    body = "\n".join(
        [
            "# A priori freeze — HARD PIVOT W19 H4 EMA z-score fade + compression false-break fade",
            "",
            "Date: 2026-07-15",
            "Status: `APRIORI_FREEZE__PRE_METRICS`",
            f"A target MFE ≈ ${A_RR * CASH_R:.0f}; B ≈ ${B_RR * CASH_R:.0f} ≫ ${BASE_COST}.",
            "",
            "## Edge-source pivot (why ≠ W1–W18)",
            "W1–W17 session-CONT / location-accept die thin @$12; W18 ADR/impulse mean-revert also ALL_KILL.",
            "W19: statistical H4 z-score fade + compression false-break trap fade.",
            "≠ W18 ADR day-range; ≠ W18 impulse-decay bars; ≠ W7/W17 expansion CONT.",
            "",
            "## Objects",
            "- HYP-FX3-H1-H4-EMA-ZSCORE-FADE-001",
            "- HYP-FX3-H1-COMPRESSION-FALSEBREAK-FADE-001",
            "- HYP-BOOK-ZSCORE-COMPFADE-APRIORI-001",
            "",
            "## Forbidden",
            "W1–W18 densify; FVG; R-series; session-CONT densify; ADR-fade densify; impulse-decay densify;",
            "H4-retest; Asia-quiet; expansion CONT densify; NR7/ORB densify.",
            "",
            "## Screen",
...
def write_preregs():
    ...
    specs = [
        (
            "20260715_H_FX3_H1_H4_EMA_ZSCORE_FADE_001_PREREG.md",
            "HYP-FX3-H1-H4-EMA-ZSCORE-FADE-001",
            [
                f"- |H1 close - H4 EMA20|/H4 ATR ≥ {A_Z} → fade to EMA; RR={A_RR}.",
                "- Statistical stretch. != W18 ADR day-range fade; != location accept CONT.",
            ],
        ),
        (
            "20260715_H_FX3_H1_COMPRESSION_FALSEBREAK_FADE_001_PREREG.md",
            "HYP-FX3-H1-COMPRESSION-FALSEBREAK-FADE-001",
            [
                f"- {B_COMP_N}-bar compress (TR≤{B_COMP_ATR}*ATR) → first expansion FADE; RR={B_RR}.",
                "- False-break trap. != W7 expansion CONT; != W17 rel-expansion CONT; != NR7 densify.",
            ],
        ),
        (
            "20260715_H_BOOK_ZSCORE_COMPFADE_APRIORI_001_PREREG.md",
            "HYP-BOOK-ZSCORE-COMPFADE-APRIORI-001",
        ...
        p.write_text(
            "\n".join(
                [
                    ...
                    "- Lane: `hard_pivot_w19_zscore_compfade_20260715`",
                    *extra,
        ...
        rows.append(
            {
                ...
                "parent_candidate": "hard_pivot_w19_zscore_compfade",
                "feature_family": "hard_pivot_w19_zscore_compfade",
                "lane": "hard_pivot_w19_zscore_compfade_20260715",
...
def cost_grade_lines(qfsi_spot=None):
    ...
    if qfsi_spot:
        lines.append(
            f"- W19 spot-check: `{qfsi_spot.get('verdict')}` "
...
def write_docs(results, book, receipt, any_surv, freeze_sha, qfsi_spot):
    ...
    OUT_PANEL.write_text(
        "\n".join(
            [
                "# 3-critic — W19 H4 z-score fade + compression false-break fade",
                "",
                "| Critic | Call |",
                "|---|---|",
                "| Sonic trader | GO — statistical z-score + false-break trap ≠ CONT densify / ≠ W18 ADR-impulse |",
                "| Quant | GO — new edge family after W18 ALL_KILL; MFE≫$12; honest +$12 |",
    ...
    OUT_DESIGN.write_text(
        "\n".join(
            [
                "# Design — HARD PIVOT W19 z-score + compression-fade",
                "",
                f"Freeze sha={freeze_sha[:16]}…",
                "",
                "After W1–W18 ALL_KILL (CONT thin; W18 mean-revert thin).",
                "NEW edge-source classes:",
                "1) H4 EMA z-score stretch fade (statistical) ≠ W18 ADR day-range.",
                "2) Compression false-break fade (trap) ≠ W7/W17 expansion CONT.",
    ...
    OUT_DEDUP.write_text(
        "\n".join(
            [
                ...
                "| Object | ≠ |",
                "|---|---|",
                "| H4 EMA z-score fade | W18 ADR-complete fade; location S/R accept; session CONT |",
                "| Compression false-break fade | W7 expansion CONT; W17 rel-expansion CONT; NR7/ORB densify |",
                "",
    ...
    OUT_MD.write_text(
        "\n".join(
            [
                f"# HARD PIVOT W19 offline probes — `{status}`",
                "",
                f"Receipt `{receipt}`",
    ...
    OUT_VN.write_text(
        "\n".join(
            [
                "# VN — HARD PIVOT W19 z-score + compression-fade",
                "",
                f"`{status}`",
                "",
                *table,
                "",
                "Edge mới: H4 z-score fade + false-break trap — ≠ CONT / ≠ W18 ADR-impulse densify.",
    ...
    OUT_CLOSE.write_text(
        "\n".join(
            [
                f"# Session closeout — HARD PIVOT W19 — `{status}`",
                "",
                f"Receipt `{receipt}` freeze sha={freeze_sha[:16]}…",
                "R-series densify PAUSED. Model0 withheld if ALL_KILL.",
                "Next: next independent class outside W1–W19 if ALL_KILL;",
    ...
    OUT_CLEAN_VN.write_text(
        "\n".join(
            [
                "# VN action brief — Clean book + HARD PIVOT W19",
                "",
                ...
                "",
                f"## HARD PIVOT W19 — `{status}`",
                *table,
                "",
                "- Edge: H4 EMA z-score fade + compression false-break fade.",
                "- Không densify W1–W18 / H4-retest / FVG / R-series.",
                f"Receipt `{receipt}`",
...
def patch_hot(results, book, receipt, any_surv, freeze_sha, qfsi_spot):
    ...
    block = "\n".join(
        [
            f"- **HARD PIVOT W19 ZSCORE/COMPFADE + COST-GRADE CLOSEOUT (2026-07-15 ~{stamp} ICT) — "
    ...
    text = re.sub(
        r"^Updated:.*$",
        f"Updated: 2026-07-15 ~{stamp} ICT | HARD PIVOT W19 zscore/compfade + cost-grade; "
...
def main():
    ...
    try:
        ...
        pnls_a, detail_a, trades_a = probe_h4_ema_zscore_fade(h1, h4)
        pnls_b, detail_b, trades_b = probe_compression_falsebreak_fade(h1)
        ra = pack_result(
            "HYP-FX3-H1-H4-EMA-ZSCORE-FADE-001",
            "H4 EMA z-score stretch fade RR2.25",
            pnls_a,
            detail_a,
        )
        rb = pack_result(
            "HYP-FX3-H1-COMPRESSION-FALSEBREAK-FADE-001",
            "Compression false-break fade RR2.50",
            pnls_b,
            detail_b,
        )
        ...
        payload = {
            "schema_version": "hard_pivot_w19_zscore_compfade_offline.v1",
            ...
            "edge_source_note": (
                "STAT_STRETCH + FALSEBREAK_TRAP: H4 EMA z-score fade + compression false-break fade; "
                "≠ session-CONT densify; ≠ W18 ADR/impulse densify"
            ),
            "results": results,
            "book": book,
            ...
            "flags": {
                ...
                "W1_W18_DENSIFY_FORBIDDEN": True,
                "W18_ADR_IMPULSE_DENSIFY_FORBIDDEN": True,
