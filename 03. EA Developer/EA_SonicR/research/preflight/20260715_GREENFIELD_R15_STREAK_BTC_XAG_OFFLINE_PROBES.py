#!/usr/bin/env python3
"""Round 15 greenfield — NON-FADE only; outside R10–R14 densify.

HARD FORBIDDEN: fade/MR/session densify R10–R12; R13 NFP/CUSUM/XAU-k;
R14 gap-k / TV-imb / NAS-ROC; R1–R9 / unpark / exit / FRED.

A priori (lead self-merge):
  1) HYP-FX3-H1-BODYSTREAK-CONT-001
     — 3 same-sign H1 bodies → CONTINUE (≠ impulse/ADR/tickvol FADE)
  2) HYP-BTCUSD-H4-D1-TSMOM-THICK-001
     — crypto multi-day mom thick (≠ FX3/XAU/NAS densify)
  3) HYP-XAGUSD-H4-D1-TSMOM-THICK-001
     — silver multi-day mom thick (≠ XAU densify; ≠ XAU/XAG ratio MR)
...
STEM = "20260715_GREENFIELD_R15_STREAK_BTC_XAG"
...
OUT_CLEAN_VN = READ / "20260715_CLEAN_BOOK_AND_R15_VN_ACTION_BRIEF.md"
...
def probe_bodystreak_cont(h1):
    closed, open_pos = [], []
    clock = h1["EURUSD"]["t"]
    last_day, day_count = None, 0
    for i in range(ST_N + 2, len(clock) - 2):
        ts = int(clock[i])
        open_pos = manage_exits(open_pos, {s: h1[s] for s in FX3}, ts, closed, ST_HOLD)
        ...
        if dt.weekday() >= 5:
            continue
        if day_count >= ST_MAX_DAY or open_pos:
            continue
        chosen = None
        for sym in FX3:
            d = h1[sym]
            j = int(np.searchsorted(d["t"], ts, side="left"))
            if j < ST_N - 1 or j >= len(d["t"]) or d["t"][j] != ts:
                continue
            atr = d["atr"][j]
            ...
            signs = []
            ok = True
            for k in range(j - ST_N + 1, j + 1):
                body = float(d["c"][k]) - float(d["o"][k])
                if abs(body) < ST_BODY_ATR * atr:
                    ok = False
                    break
                signs.append(1 if body > 0 else -1)
...
def probe_sym_d1_tsmom_thick(sym, d1, h4):
    closed, open_pos = [], []
    d1_side = np.zeros(len(d1["t"]), dtype=int)
    for i in range(MOM_ROC + 2, len(d1["c"])):
        atr = d1["atr"][i]
        if not np.isfinite(atr) or atr <= 0:
            continue
        roc = float(d1["c"][i] - d1["c"][i - MOM_ROC])
        if abs(roc) < MOM_ROC_ATR * atr:
            continue
    ...
    for i in range(1, len(h4["t"]) - 1):
        ts = int(h4["t"][i])
        open_pos = manage_exits(open_pos, {sym: h4}, ts, closed, MOM_HOLD)
        dt = datetime.fromtimestamp(ts, tz=timezone.utc)
        if dt.weekday() >= 5:
            continue
        if open_pos:
        ...
        if not np.isfinite(atr) or atr <= 0:
            continue
        entry = float(h4["o"][i])
        sl = entry - side * MOM_SL * atr
        tp = entry + side * MOM_RR * MOM_SL * atr
        lots = risk_lots(sym, entry, sl)
        open_pos.append(
            {
                "sym": sym,
    ...
    with REG.open("a", encoding="utf-8") as f:
        for r in results:
            row = {
                ...
                "feature_family": "greenfield_r15_streak_btc_xag",
                "lane": "strategy_shift_r15_greenfield_20260715",
                "setup_type": r["setup"],
                "symbol": r["symbol"],
                "timeframe": r["timeframe"],
                "window": "2021.01.01-2025.12.31",
                "model": "offline_closed_bar_probe",
                "source_provenance": "R15 NON-FADE outside R10–R14 densify; lead self-merge",
                "prereg_path": None,
...
def write_docs(results, receipt, any_surv, qnote, clean_book_note):
    ...
    OUT_PANEL.write_text(
        "\n".join(
            [
                "# 3-critic panel — Round 15 streak / BTC / XAG",
                "",
                "Date: 2026-07-15",
                "Nested: lead self-merge `cursor-grok-4.5-high-fast`.",
                "",
                "## Named (NON-FADE)",
                "1. `FX3_BODYSTREAK_CONT`",
                "2. `BTC_D1_TSMOM_H4_THICK`",
                "3. `XAG_D1_TSMOM_H4_THICK`",
                "",
                "| Critic | Stance |",
                "|---|---|",
                "| Sonic trader | PASS — streak persist + crypto/silver trend books |",
                "| Quant | SOFT — BTC microstructure/cost; XAG cadence; streak may overfit N |",
                "| MQL5/MT5 | PASS — closed-bar streak; D1→H4 as-of |",
                "",
    ...
    OUT_DESIGN.write_text(
        "\n".join(
            [
                "# Design — Round 15 streak / BTC / XAG thick-mom",
                "",
                "Date: 2026-07-15",
                f"## 1 `HYP-FX3-H1-BODYSTREAK-CONT-001`",
                f"{ST_N} consecutive H1 bodies ≥{ST_BODY_ATR}×ATR same sign → CONTINUE;",
                f"SL={ST_SL} RR={ST_RR} hold≤{ST_HOLD}.",
                "",
                f"## 2 `HYP-BTCUSD-H4-D1-TSMOM-THICK-001`",
                f"D1 |ROC{MOM_ROC}|≥{MOM_ROC_ATR}×ATR → H4 thick; SL={MOM_SL} RR={MOM_RR}.",
                "",
                f"## 3 `HYP-XAGUSD-H4-D1-TSMOM-THICK-001`",
                f"Same thick-mom contract on XAGUSD.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    OUT_DEDUP.write_text(
        "\n".join(
            [
                "# Dedup — Round 15 streak / BTC / XAG",
                "",
                "| Object | Cleared vs |",
                "|---|---|",
                "| Body streak CONT | ≠ impulse FADE; ≠ ADR exhaust fade; ≠ tickvol climax fade |",
                "| BTC D1 TSMOM thick | ≠ FX3/XAU/NAS ROC densify |",
                "| XAG D1 TSMOM thick | ≠ XAU densify; ≠ XAU/XAG ratio zMR |",
                "",
                "R10–R14 densify: **FORBIDDEN**.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    OUT_MD.write_text(
        "\n".join(
            [
                "# Offline probes — Round 15 streak / BTC / XAG",
                "",
    ...
    OUT_CLOSE.write_text(
        "\n".join(
            [
                "# Session closeout — Round 15 streak / BTC / XAG",
                "",
                f"Status: `{status}`",
                f"Receipt: `{receipt}`",
                "Model 0: WITHHELD" if not any_surv else "Model 0: AUTHORIZED for survivors only",
                "Do **not** densify streak-N / BTC-ROC / XAG-ROC / R10–R14.",
                "Next: next true greenfield outside R15 — QFSI parallel only for cost.",
    ...
    OUT_VN.write_text(
        "\n".join(
            [
                "# VN brief — Round 15 streak / BTC / XAG",
                "",
                f"Thời điểm: 2026-07-15 ~{stamp} ICT",
                "Ngoài R10–R14. **NON-FADE only.**",
                "",
                f"## Kết quả — `{status}`",
                *table,
                "",
                ...
                "",
                "## Quyết định",
                "- Không densify streak-N / BTC-ROC / XAG-ROC / R10–R14.",
                "- Best shelf RR2 `194548`. Cost GAP. QFSI parallel only.",
                "- Next: greenfield ngoài R15 **hoặc** research-grade cost.",
                "",
    ...
    OUT_CLEAN_VN.write_text(
        "\n".join(
            [
                "# VN brief — Clean book + Round 15 (post R14)",
                "",
                f"Thời điểm: 2026-07-15 ~{stamp} ICT",
                "Lane: clean-book + discovery R15 NON-FADE. GOAL: chưa đạt.",
                "",
                "## 1. Clean book `HYP-BOOK-CLEAN-APRIORI-RR2SPARK-001`",
                clean_book_note,
                "- Model 0 book-level: **WITHHELD**.",
                "",
                "## 2. Discovery Round 15 — streak / BTC / XAG",
                *table,
                ...
                "",
                "## Cấm",
                "Densify R1–R15 / fade-session / unpark / exit / FRED / Phase-0 ceremony.",
                "",
                "## Next agent",
                "Giữ QFSI; greenfield ngoài R15 (NON-FADE); cost khi Owner drop deal-export.",
...
def patch_hot(results, receipt, any_surv, qnote, clean_note):
    ...
    block = [
        "",
        f"- **GREENFIELD ROUND15 STREAK/BTC/XAG CLOSEOUT (2026-07-15 ~{stamp} ICT) — "
    ...
    if lines and lines[0].startswith("# Hot Cache"):
        lines[1] = (
            f"Updated: 2026-07-15 ~{stamp} ICT | R15 "
    ...
    while i < len(lines):
        ln = lines[i]
        if ln.startswith("- **GREENFIELD ROUND15 STREAK/BTC/XAG CLOSEOUT"):
            i += 1
            while i < len(lines):
                nxt = lines[i]
...
def main():
    ...
    try:
        ...
        btc_d1 = enrich(load("BTCUSD", mt5.TIMEFRAME_D1))
        btc_h4 = enrich(load("BTCUSD", mt5.TIMEFRAME_H4))
        xag_d1 = enrich(load("XAGUSD", mt5.TIMEFRAME_D1))
        xag_h4 = enrich(load("XAGUSD", mt5.TIMEFRAME_H4))
        results = [
            pack_result(
                "HYP-FX3-H1-BODYSTREAK-CONT-001",
                "fx3_h1_bodystreak_cont",
                "FX3",
                "H1",
                *probe_bodystreak_cont(h1),
            ),
            pack_result(
                "HYP-BTCUSD-H4-D1-TSMOM-THICK-001",
                "btcusd_h4_d1_tsmom_thick",
                "BTCUSD",
                "H4",
                *probe_sym_d1_tsmom_thick("BTCUSD", btc_d1, btc_h4),
            ),
            pack_result(
                "HYP-XAGUSD-H4-D1-TSMOM-THICK-001",
                "xagusd_h4_d1_tsmom_thick",
                "XAGUSD",
                "H4",
                *probe_sym_d1_tsmom_thick("XAGUSD", xag_d1, xag_h4),
            ),
        ]
        any_surv = any(r["verdict"] == "PROBE_SURVIVOR" for r in results)
        payload = {
            "schema_version": "greenfield_r15_streak_btc_xag.v1",
