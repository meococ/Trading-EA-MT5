//+------------------------------------------------------------------+
//| EA_H1PrevDayBreak.mq5                                            |
//| HYP-PDBREAK-EURUSD-H1-001: closed-bar PDH/PDL break continuation |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "EURUSD H1 PDH/PDL break continuation. Not fade. Not LFH. Not Asia range."
...
input string InpHypothesisId="HYP-PDBREAK-EURUSD-H1-001";
input string InpVariantTag="PDH_PDL_BREAK_CONT";

input group "--- Frozen execution and risk ---"
input long   InpMagic=16081661;
...
input double InpMaxSLATR=15.00;
input double InpMinCostMultiple=6.0;
input int    InpMaxSpreadPoints=30;
input int    InpPriorDayLookback=120;
input int    InpMinPriorDayBars=12;

const string EA_NAME="EA_H1PrevDayBreak";
const string EXPECTED_HYPOTHESIS="HYP-PDBREAK-EURUSD-H1-001";
const string EXPECTED_VARIANT="PDH_PDL_BREAK_CONT";
...
bool CloseOwned(const string reason,const datetime current_bar_open)
  {
   ...
   if(!ScanOwnedPosition(ticket,found))
     {
      ...
      Print("PDB001_FATAL reason=POSITION_SCAN");
      return(false);
     }
   if(!found)
      return(true);
   ...
   if(!sent || (retcode!=TRADE_RETCODE_DONE && retcode!=TRADE_RETCODE_DONE_PARTIAL))
     {
      g_close_rejects++;
      PrintFormat("PDB001_CLOSE_REJECT reason=%s ticket=%I64u retcode=%u",
   ...
   if(g_entry_bar_open>0 && current_bar_open!=g_entry_bar_open)
      g_hold_h1_closes++;
   PrintFormat("PDB001_CLOSE reason=%s ticket=%I64u retcode=%u hold_h1=%s entry_bar=%I64d current_bar=%I64d",
               reason,ticket,retcode,
...
bool BuildSignal(const datetime availability_time,SignalDecision &signal)
  {
   ...
   const bool long_brk=(bar.close>pdh);
   const bool short_brk=(bar.close<pdl);
   if(long_brk==short_brk)
      return(false);
   if(g_last_setup_prior_day==prior_day)
     {
      g_level_skips++;
      return(false);
     }
   ...
   signal.signal_high=pdh;
   signal.signal_low=pdl;
   ...
   if(InpEnableTelemetry)
      PrintFormat("PDB001_SIGNAL decision=%I64d dir=%s prior=%d pdh=%.5f pdl=%.5f c=%.5f atr=%.5f",
                  (long)bar.time,(signal.direction>0 ? "LONG" : "SHORT"),prior_day,
...
bool SubmitEntry(const SignalDecision &signal,const datetime current_bar_open)
  {
   ...
   if(ForeignSymbolExposure(scan_ok))
     {
      if(!scan_ok)
        {
         g_runtime_failed=true;
         Print("PDB001_FATAL reason=FOREIGN_SCAN");
         return(false);
        }
      return(false);
     }
   ...
   const double raw_stop=(signal.direction>0
                          ? signal.pdl-InpSLATRBuffer*signal.atr
                          : signal.pdh+InpSLATRBuffer*signal.atr);
   ...
   if(!IsFinite(risk_distance) || risk_distance<InpMinSLATR*signal.atr ||
     ...
     {
      g_level_skips++;
      PrintFormat("PDB001_LEVEL_SKIP dir=%s entry=%.5f stop=%.5f risk=%.5f atr=%.5f",
   ...
   if(risk_distance<InpMinCostMultiple*spread_price)
     {
      g_cost_rejects++;
      PrintFormat("PDB001_ENTRY_REJECT reason=COST_WINDOW risk=%.5f spread=%.5f",
   ...
   if(volume<=0.0)
     {
      g_volume_rejects++;
      PrintFormat("PDB001_VOLUME_REJECT reason=RISK_LOT_ZERO equity=%.2f one_lot_loss=%.2f",
                  equity,one_lot_loss);
      return(false);
     }
   ...
   if(!sent || (retcode!=TRADE_RETCODE_DONE && retcode!=TRADE_RETCODE_DONE_PARTIAL))
     {
      g_entry_rejects++;
      PrintFormat("PDB001_ENTRY_REJECT dir=%s vol=%.2f entry=%.5f sl=%.5f tp=%.5f retcode=%u",
                  (signal.direction>0 ? "LONG" : "SHORT"),volume,entry,sl,tp,retcode);
      return(false);
     }
   g_entries++;
   g_daily_entries++;
   g_last_setup_prior_day=signal.prior_day;
   ...
   g_initial_sl=sl;
   g_initial_tp=tp;
   PrintFormat("PDB001_ENTRY decision=%I64d dir=%s vol=%.2f entry=%.5f sl=%.5f tp=%.5f prior=%d pdh=%.5f pdl=%.5f retcode=%u",
               (long)signal.decision_time,(signal.direction>0 ? "LONG" : "SHORT"),
               volume,g_entry_price,sl,tp,signal.prior_day,signal.pdh,signal.pdl,retcode);
   return(true);
  }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult &result)
  {
   ...
   const bool hold_h1=(g_entry_bar_open>0 && current_bar_open!=g_entry_bar_open);
   PrintFormat("PDB001_EXIT deal=%I64u reason=%s profit=%.2f commission=%.2f swap=%.2f hold_h1=%s",
...
int OnInit()
  {
   if(_Period!=PERIOD_H1 || !SymbolAllowed() ||
      ...
      InpMinCostMultiple<=0.0 || InpPriorDayLookback<InpMinPriorDayBars+2)
      return(INIT_PARAMETERS_INCORRECT);

   g_atr_handle=iATR(_Symbol,PERIOD_H1,InpATRPeriod);
   if(g_atr_handle==INVALID_HANDLE)
     {
      Print("PDB001_FATAL reason=ATR_HANDLE");
      return(INIT_FAILED);
     }
   g_trade.SetExpertMagicNumber(InpMagic);
   g_trade.SetDeviationInPoints(InpDeviationPoints);
   ...
   if(!CurrentBarOpen(g_last_bar_open) || g_last_bar_open<=0)
      return(INIT_FAILED);
   PrintFormat("PDB001_INIT ea=%s hyp=%s symbol=%s tf=H1 last_entry_h=%d friday_flat_h=%d tpR=%.2f",
               EA_NAME,InpHypothesisId,_Symbol,InpLastEntryHour,InpFridayFlattenHour,InpTargetR);
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("PDB001_SUMMARY reason=%d failed=%s closed_bars=%I64d signals=%I64d long=%I64d short=%I64d entries=%I64d entry_rej=%I64d spread_rej=%I64d cost_rej=%I64d risk_skip=%I64d vol_rej=%I64d window_skip=%I64d level_skip=%I64d closes=%I64d hold_h1_closes=%I64d close_rej=%I64d invalid=%I64d",
               ...
               g_closes,g_hold_h1_closes,g_close_rejects,g_invalid_inputs);
  }
