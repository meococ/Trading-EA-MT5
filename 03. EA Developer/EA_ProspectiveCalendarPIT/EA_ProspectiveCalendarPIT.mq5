...
{
   int h=OpenApp("calendar_pit_v15.jsonl",false,"");
...
{
   int h=OpenApp("calendar_pit_v15.csv",true,CSV_HDR);
   if(h==INVALID_HANDLE) return false;
...
void SaveCountries()
{
   int h=FileOpen(FOLDER+"countries_v15.txt",FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
...
void SaveCatalog()
{
   ...
   int h=FileOpen(FOLDER+"catalog_state_v15.txt",FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
   if(h==INVALID_HANDLE) return;
   FileWriteString(h,"schema="+SCHEMA_VER+"\n");
   ...
   int c=FileOpen(FOLDER+"event_catalog_v15.csv",FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
...
bool SaveOcc()
{
   int h=FileOpen(FOLDER+"occurrence_v15.txt",FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
   if(h==INVALID_HANDLE) return false;
...
bool LoadCountries()
{
   int h=FileOpen(FOLDER+"countries_v15.txt",FILE_READ|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
...
bool LoadCatalog()
{
   int h=FileOpen(FOLDER+"catalog_state_v15.txt",FILE_READ|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
   if(h==INVALID_HANDLE) return true;
   int declared=-1;
...
bool LoadOcc()
{
   int h=FileOpen(FOLDER+"occurrence_v15.txt",FILE_READ|FILE_TXT|FILE_ANSI|FILE_COMMON,0,CP_UTF8);
   if(h==INVALID_HANDLE) return true;
   int declared=-1;
...
int OnInit()
{
   ...
   {
      Print("EA_ProspectiveCalendarPIT v1.5.0 tester fail-closed");
      return(INIT_FAILED);
   }
   FolderCreate("calendar_pit",FILE_COMMON);
   ResetMemory();
   if(!LoadCountries()){ Print("v1.5 corrupt countries"); return(INIT_FAILED); }
   if(!LoadCatalog()){ Print("v1.5 corrupt catalog"); return(INIT_FAILED); }
   if(!LoadOcc()){ Print("v1.5 corrupt occurrences"); return(INIT_FAILED); }
   if(HasDupEventIds()){ Print("v1.5 duplicate/zero event_id"); return(INIT_FAILED); }
