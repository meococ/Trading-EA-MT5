//+------------------------------------------------------------------+
//|                                      EA_EventAggressorFlow.mq5  |
//| HYP-EVENT-AGGFLOW-EURUSD-TICK-013                              |
//| Frozen CME 6E first-15-second aggressive-flow event scalp.     |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Hash-bound CME 6E first-wave flow continuation DESIGN baseline"

#include <Trade/Trade.mqh>
#include "resources/event_aggflow_013_table.mqh"

input string InpHypothesisId       = "HYP-EVENT-AGGFLOW-EURUSD-TICK-013";
input string InpVariantTag         = "CME6E_FIRST15S_FLOW_CONTINUATION_V1";
input bool   InpResearchAutoMode   = false;
input bool   InpEnableAudit        = true;
input bool   InpReverseComparator  = false;
input ulong  InpMagic              = 5604413;
input double InpRiskPercent        = 0.25;
input double InpSizingStopPips     = 15.0;
input double InpMaxLots            = 1.0;
input int    InpDeviationPoints    = 100;

const long   AF_ENTRY_DELAY_MSC=15000;
const long   AF_EXIT_DELAY_MSC=75000;
...
bool ValidateFrozenTable()
  {
   if(ArraySize(AF_FLOW_EVENT_ID)!=AF_FLOW_EVENT_COUNT ||
      ArraySize(AF_FLOW_UTC_MSC)!=AF_FLOW_EVENT_COUNT ||
      ArraySize(AF_FLOW_SERVER_MSC)!=AF_FLOW_EVENT_COUNT ||
      ArraySize(AF_FLOW_SIGNED)!=AF_FLOW_EVENT_COUNT)
      return(false);

   string canonical="";
   for(int i=0;i<AF_FLOW_EVENT_COUNT;i++)
     {
      if(AF_FLOW_EVENT_ID[i]!=StringFormat("EVT%04d",i+1))
         return(false);
      if(i>0 && AF_FLOW_SERVER_MSC[i]<=AF_FLOW_SERVER_MSC[i-1])
         return(false);
      const long offset=AF_FLOW_SERVER_MSC[i]-AF_FLOW_UTC_MSC[i];
      if(offset!=7200000 && offset!=10800000)
         return(false);
      if(AF_FLOW_SIGNED[i]>0)
         g_table_buy++;
      else if(AF_FLOW_SIGNED[i]<0)
         g_table_sell++;
      else
         g_table_zero++;
      canonical+=StringFormat("%s,%I64d,%I64d,%I64d\n",
                              AF_FLOW_EVENT_ID[i],AF_FLOW_UTC_MSC[i],
                              AF_FLOW_SERVER_MSC[i],AF_FLOW_SIGNED[i]);
     }
   if(g_table_buy!=156 || g_table_sell!=170 || g_table_zero!=3)
      return(false);
   string actual="";
   if(!Sha256Utf8(canonical,actual) || actual!=AF_FLOW_TABLE_SHA256)
     {
      PrintFormat("EVENTAGGFLOW013_TABLE_HASH_FAIL expected=%s actual=%s",
                  AF_FLOW_TABLE_SHA256,actual);
      return(false);
     }
   PrintFormat("EVENTAGGFLOW013_SOURCE_BIND source_sha256=%s table_sha256=%s events=%d buy=%d sell=%d zero=%d",
               AF_FLOW_SOURCE_SHA256,actual,AF_FLOW_EVENT_COUNT,
...
void FailRuntime(const string reason)
  {
   if(!g_runtime_failed)
      Print("EVENTAGGFLOW013_RUNTIME_FAIL reason=",reason);
...
void WriteEventRow(const string status,const int event_index,const int direction,
                   ...
                   const string detail)
  {
   ...
   FileWrite(g_csv,status,g_role,InpHypothesisId,InpVariantTag,
             AF_FLOW_EVENT_ID[event_index],AF_FLOW_UTC_MSC[event_index],
             AF_FLOW_SERVER_MSC[event_index],AF_FLOW_SIGNED[event_index],direction,
             AF_FLOW_SERVER_MSC[event_index]+AF_ENTRY_DELAY_MSC,
             AF_FLOW_SERVER_MSC[event_index]+AF_EXIT_DELAY_MSC,
...
bool OpenEvent(const int event_index,const MqlTick &tick)
  {
   const int source_direction=(AF_FLOW_SIGNED[event_index]>0 ? 1 : -1);
   ...
   const string comment=StringFormat("AF013_%s_%s",g_role,AF_FLOW_EVENT_ID[event_index]);
   bool sent=false;
   if(direction>0)
   ...
   PrintFormat("EVENTAGGFLOW013_ENTRY role=%s event=%s tick_msc=%I64d target_msc=%I64d flow=%I64d direction=%d lots=%.2f fill=%.5f spread_pips=%.3f",
               g_role,AF_FLOW_EVENT_ID[event_index],tick.time_msc,
               AF_FLOW_SERVER_MSC[event_index]+AF_ENTRY_DELAY_MSC,
               AF_FLOW_SIGNED[event_index],direction,lots,g_entry_fill,spread_pips);
   return(true);
  }
...
bool CloseActiveEvent(const MqlTick &tick)
  {
   ...
   PrintFormat("EVENTAGGFLOW013_CLOSE role=%s event=%s entry_msc=%I64d exit_msc=%I64d target_msc=%I64d raw=%.2f base=%.2f x1_5=%.2f x2=%.2f",
               g_role,AF_FLOW_EVENT_ID[event_index],g_entry_tick_msc,tick.time_msc,
               AF_FLOW_SERVER_MSC[event_index]+AF_EXIT_DELAY_MSC,
...
void WriteRunMeta(const int reason)
  {
   ...
   if(handle==INVALID_HANDLE)
     {
      Print("EVENTAGGFLOW013_RUNMETA_FAIL error=",GetLastError());
      return;
     }
   const int accounted=g_completed+g_zero_flow+g_missed_tick+g_overlap_skips+g_entry_rejects;
   string json=StringFormat(
      "{\"schema_version\":\"event_aggflow_013_run_meta.v1\",\"hypothesis_id\":\"%s\",\"role\":\"%s\",\"source_sha256\":\"%s\",\"table_sha256\":\"%s\",\"events\":%d,\"next_event\":%d,\"accounted\":%d,\"completed\":%d,\"zero_flow\":%d,\"missed_tick\":%d,\"overlap_skips\":%d,\"entry_rejects\":%d,\"exit_rejects\":%d,\"max_concurrent\":%d,\"active_event\":%d,\"runtime_failed\":%s,\"failure_reason\":\"%s\",\"deinit_reason\":%d}",
      InpHypothesisId,g_role,AF_FLOW_SOURCE_SHA256,AF_FLOW_TABLE_SHA256,
      AF_FLOW_EVENT_COUNT,g_next_event,accounted,g_completed,g_zero_flow,
...
int OnInit()
  {
   if(_Symbol!="EURUSD" || InpHypothesisId!="HYP-EVENT-AGGFLOW-EURUSD-TICK-013" ||
      !InpResearchAutoMode || !InpEnableAudit || !MQLInfoInteger(MQL_TESTER))
     {
      PrintFormat("EVENTAGGFLOW013_INIT_FAIL symbol=%s hypothesis=%s auto=%s audit=%s tester=%s",
   ...
   if(InpRiskPercent!=0.25 || InpSizingStopPips!=15.0 || InpMaxLots!=1.0 ||
      InpDeviationPoints!=100 || !ValidateFrozenTable())
     {
      Print("EVENTAGGFLOW013_INIT_FAIL frozen input/table mismatch");
      return(INIT_FAILED);
     }
   g_role=(InpReverseComparator ? "REVERSE" : "PRIMARY");
   g_csv_name=StringFormat("EURUSD_EVENTAGGFLOW013_Trades_%s.csv",g_role);
   g_meta_name=StringFormat("EVENTAGGFLOW013_RunMeta_%s.json",g_role);
   g_csv=FileOpen(g_csv_name,FILE_COMMON|FILE_WRITE|FILE_CSV|FILE_ANSI,',');
   if(g_csv==INVALID_HANDLE)
     {
      Print("EVENTAGGFLOW013_INIT_FAIL audit file error=",GetLastError());
      return(INIT_FAILED);
     }
   FileWrite(g_csv,"status","role","hypothesis_id","variant","event_id",
             "event_utc_msc","event_server_msc","signed_flow","direction",
   ...
   PrintFormat("EVENTAGGFLOW013_INIT_OK role=%s source_sha256=%s table_sha256=%s",
               g_role,AF_FLOW_SOURCE_SHA256,AF_FLOW_TABLE_SHA256);
...
void OnTick()
  {
   ...
   if(g_active_event>=0)
     {
      const long exit_target=AF_FLOW_SERVER_MSC[g_active_event]+AF_EXIT_DELAY_MSC;
   ...
   while(g_next_event<AF_FLOW_EVENT_COUNT)
     {
      const int event_index=g_next_event;
      const long entry_target=AF_FLOW_SERVER_MSC[event_index]+AF_ENTRY_DELAY_MSC;
      const long exit_target=AF_FLOW_SERVER_MSC[event_index]+AF_EXIT_DELAY_MSC;
      if(tick.time_msc<entry_target)
         break;
      g_next_event++;
      if(AF_FLOW_SIGNED[event_index]==0)
        {
         g_zero_flow++;
         WriteTerminalSkip("SKIP_ZERO",event_index,0,tick.time_msc,"ZERO_TIE_OR_NO_SOURCE");
         continue;
        }
      const int source_direction=(AF_FLOW_SIGNED[event_index]>0 ? 1 : -1);
      const int direction=(InpReverseComparator ? -source_direction : source_direction);
...
void OnDeinit(const int reason)
  {
   ...
   if(g_next_event!=AF_FLOW_EVENT_COUNT)
      FailRuntime("NOT_ALL_EVENTS_REACHED");
   const int accounted=g_completed+g_zero_flow+g_missed_tick+g_overlap_skips+g_entry_rejects;
   if(accounted!=AF_FLOW_EVENT_COUNT)
   ...
   WriteRunMeta(reason);
   PrintFormat("EVENTAGGFLOW013_SUMMARY role=%s events=%d accounted=%d completed=%d zero=%d missed=%d overlap=%d entry_reject=%d exit_reject=%d max_concurrent=%d runtime_failed=%s reason=%s",
               g_role,AF_FLOW_EVENT_COUNT,accounted,g_completed,g_zero_flow,
               g_missed_tick,g_overlap_skips,g_entry_rejects,g_exit_rejects,
