# DBNV3 RECOVERY ACQUISITION PLAN - HYP-EVENT-AGGFLOW-EURUSD-TICK-006
...
The legacy 0.54 runtime remains untouched for historical reproducibility. HYP005 proved the inherited file decodes as DBNv3 trades with 526 records, then stopped before remote access because its reused foundation hard-pinned SDK 0.54. HYP006 therefore uses a reviewed self-contained 0.55.1 client/key loader and DBNv3 validator; it does not import the legacy acquisition foundation.

## Recovery and resume invariants

Before remote resume, the HYP005 tool must:

- bind its exact immutable file SHA256 and focused-test SHA256, including self-contained SDK 0.55.1 client and DBNv3 full-stream validation, through the
