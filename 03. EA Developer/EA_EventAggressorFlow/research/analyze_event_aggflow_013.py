"""Independent frozen-gate analyzer for HYP013 PRIMARY and REVERSE ledgers."""
...
EXPECTED_SOURCE_HASH = "65AA6558629F6FF224E5DB6FD218B1DC9A4EC6A7B07DE886C3FE13E5922A106C"
EXPECTED_TABLE_HASH = "620FEB6A51CFB69B9BFA9C47E2BC4C208B6C6E3D9645A1C61CE062AC1B09D8BA"
DEGRADED = {"EVT0198", "EVT0270"}
...
def read_role(path: Path, expected_role: str) -> tuple[list[dict[str, str]], dict[str, int]]:
    ...
    if len(rows) != 329 or statuses != {"CLOSED": 325, "SKIP_ZERO": 3, "ENTRY_REJECT": 1}:
...
def role_summary(path: Path, role: str) -> dict[str, object]:
    ...
    return {
        ...
        "degraded_cells": [
            {key: row[key] for key in ("event_id", "signed_flow", "direction", "net_base_usd")}
            for row in rows
...
def main() -> None:
    ...
    gates = {
        "trades_at_least_250": primary["base"]["trades"] >= 250,
    ...
    payload = {
        "schema_version": "event_aggflow_013_economic_analysis.v1",
        "hypothesis_id": "HYP-EVENT-AGGFLOW-EURUSD-TICK-013",
