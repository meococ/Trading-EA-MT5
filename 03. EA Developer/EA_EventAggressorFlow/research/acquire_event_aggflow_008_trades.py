#!/usr/bin/env python3
"""Inherit HYP006, retry EVT0081 once, and resume EVENTAGGFLOW008 under USD 1.00."""
...
HYPOTHESIS_ID = "HYP-EVENT-AGGFLOW-EURUSD-TICK-008"
ACQUISITION_ID = "EVENTAGGFLOW008-TRADES-DESIGN-SOURCE-001"
...
TOOL_REL = BASE_REL + "acquire_event_aggflow_008_trades.py"
TEST_REL = BASE_REL + "tests/test_acquire_event_aggflow_008_trades.py"
...
RECOVERY_PARENT_HYPOTHESIS_ID = "HYP-EVENT-AGGFLOW-EURUSD-TICK-006"
RECOVERY_PARENT_ACQUISITION_ID = "EVENTAGGFLOW006-TRADES-DESIGN-SOURCE-001"
...
PLAN_SHA256 = "B0888A8473FDD263885D10C43885998DF457FC920CED549E5C29BBB8A6E22381"
...
RECOVERY_REQUEST_ID = "EVT0081"
RECOVERY_ESTIMATED_USD = 0.004807770252
RECOVERY_BILLABLE_BYTES = 184368
RECOVERY_PARENT_COMPLETED_FILES = 80
RECOVERY_PARENT_COMPLETED_RECORDS = 37737
RECOVERY_PARENT_COMPLETED_BYTES = 645863
RECOVERY_WORST_CASE_AGGREGATE_USD = 0.8804778456659999
...
def validate_owner_authority(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or authority.get("worst_case_aggregate_with_one_manual_retry_usd")
        != RECOVERY_WORST_CASE_AGGREGATE_USD
        or authority.get("manual_retry_evt0081_authorized_once") is not True
...
def load_recovery_parent(
    ...
    if (
        ...
        or manifest.get("successor_paid_timeseries_calls") != 79
    ...
    if (
        ...
        or not math.isclose(
            total_usd + RECOVERY_ESTIMATED_USD,
            RECOVERY_WORST_CASE_AGGREGATE_USD,
            rel_tol=0,
            abs_tol=1e-12,
        )
        ...
        raise AcquisitionError("parent unresolved EVT0081 binding mismatch")
...
def inherit_parent_completed(
    *,
    ...
    manifest["manual_retry_evt0081_calls"] = 0
...
def acquisition_receipt(
    *,
    ...
    return {
        "schema_version": "event_aggflow_008_paid_acquisition_receipt.v1",
        ...
        "manual_retry_evt0081_calls": manifest.get(
            "manual_retry_evt0081_calls", 0
        ),
...
def execute(workspace: Path, workers: int) -> Path:
    ...
    with exclusive_campaign_lock(root):
        if plan_path.exists() or manifest_path.exists():
            ...
            total_usd, total_bytes = validate_live_quote(live, source_windows)
            plan = {
                "schema_version": "event_aggflow_008_detached_recovery_plan.v1",
                ...
                "worst_case_aggregate_usd": RECOVERY_WORST_CASE_AGGREGATE_USD,
                "manual_retry_evt0081_authorized_once": True,
            ...
            live_plan_sha256 = sha256_file(plan_path)
            manifest = {
                "schema_version": "event_aggflow_008_detached_recovery_manifest.v1",
        ...
        manual_retry_calls = manifest.get("manual_retry_evt0081_calls")
        if manual_retry_calls not in (0, 1):
            raise AcquisitionError("manual EVT0081 retry counter invalid")
        ...
        if (
            ...
            raise AcquisitionError("completed EVT0081 lacks exact manual retry provenance")
        if (
            isinstance(in_flight, dict)
            and in_flight.get("request_id") == RECOVERY_REQUEST_ID
            and manual_retry_calls != 1
        ):
            raise AcquisitionError("in-flight EVT0081 lacks exact manual retry provenance")
        if in_flight:
        ...
        for item in live:
            ...
            if request_id == RECOVERY_REQUEST_ID:
                if manifest.get("manual_retry_evt0081_calls") != 0:
                    raise AcquisitionError("manual EVT0081 retry already consumed")
                ...
                manifest["manual_retry_evt0081_calls"] = 1
...
def main() -> int:
    ...
    try:
        ...
        manifest = json.loads(manifest_path.read_text(encoding="ascii"))
        print(
            "EVENTAGGFLOW008_TRADES_ACQUIRE_OK "
        ...
        print(
            f"EVENTAGGFLOW008_TRADES_ACQUIRE_BLOCKED reason={exc}",
            file=sys.stderr,
        )
        return 2
