#!/usr/bin/env python3
"""Free DESIGN-only CME 6E trades quote for EVENTAGGFLOW001."""
...
HYPOTHESIS_ID = "HYP-EVENT-AGGFLOW-EURUSD-TICK-001"
...
TOOL_REL = BASE_REL + "quote_event_aggflow_001_trades.py"
TEST_REL = BASE_REL + "tests/test_quote_event_aggflow_001_trades.py"
...
PLAN_SHA256 = "0167C4F62B1020865771520AE9895AC302129BE524783B4BDFC2E1B0052E650F"
...
REVIEWED_REGISTRY_ROW_SHA256: str | None = "CC4DC6C43AEC9825A1F5ED9E5AFFDB0CEDFB9D9A523F5C4D0E8FD7A4404E6E41"
...
def load_api_key() -> str:
    key = os.environ.get("DATABENTO_API_KEY") or _read_user_environment(
        "DATABENTO_API_KEY"
    )
    if not key:
...
def quote_all(
    ...
    def one(window: dict[str, str]) -> dict[str, Any]:
        ...
        for attempt in range(1, 4):
            ...
            except Exception as exc:  # bounded metadata retry only
                last = exc
                if attempt < 3:
                    time_module.sleep((0.25, 1.0)[attempt - 1])
        raise QuoteError(
            f"metadata quote failed for {window['request_id']}: "
...
def execute(workspace: Path, workers: int) -> Path:
    ...
    if len({(row["start"], row["end"]) for row in windows}) != len(windows):
        raise QuoteError("duplicate quote windows")
    key = load_api_key()
    quotes = quote_all(lambda: make_client(key), windows, workers)
    if len(quotes) != EXPECTED_DESIGN_CLOCKS:
        raise QuoteError("metadata quote coverage mismatch")
    ...
    if not math.isfinite(total_usd) or total_usd < 0:
        raise QuoteError("aggregate metadata quote invalid")
    receipt = {
        "schema_version": "event_aggflow_001_trades_design_free_quote.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "quote_id": QUOTE_ID,
        "created_at_utc": datetime.now(timezone.utc)
        .isoformat()
        .replace("+00:00", "Z"),
        "status": "FREE_DESIGN_TRADES_METADATA_QUOTE_COMPLETE_NO_PAID_REQUEST",
        "dataset": DATASET,
        "schema": SCHEMA,
        "symbol": SYMBOL,
        "stype_in": STYPE_IN,
        ...
        "nonzero_billable_request_share": nonzero / len(quotes),
        "estimated_nonzero_cadence_per_week": nonzero / EXPECTED_ELAPSED_WEEKS,
        "quotes": quotes,
        "bindings": {
        ...
        "outcome_fields_used": [],
        "validation_source_quoted": False,
...
def main() -> int:
    ...
    try:
        ...
        print(
            "EVENTAGGFLOW001_TRADES_FREE_QUOTE_OK "
            ...
            f"bytes={receipt['estimated_total_billable_bytes']} paid=0"
        )
        print(f"RECEIPT {output}")
        return 0
    except QuoteError as exc:
        print(
            f"EVENTAGGFLOW001_TRADES_FREE_QUOTE_BLOCKED reason={exc}",
            file=sys.stderr,
        )
        return 2
