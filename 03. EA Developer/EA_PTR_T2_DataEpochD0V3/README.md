...
Minimal T2/P4 D0 data-epoch synchronization probe with corrected second-source
terminal/server series provenance for the frozen
...
- `OnInit` fails closed when `InpHypothesisId` is empty, generation is not `T2`,
  ...
  timeframe is not `M5`.
- `OnInit` also fails closed if M5/M1 series proof calls are invalid,
  M5 is not synchronized, terminal max bars is invalid, or `CopyTime` from
  the discovered M5 first epoch does not return exactly that one M5 timestamp.
- `OnTick` observes only the last completed M5 bar via `iTime(_Symbol, PERIOD_M5, 1)`.
- Journal markers are deterministic: `DATA_EPOCH_D0_SERIES_PROOF`,
  `DATA_EPOCH_D0_READY`,
