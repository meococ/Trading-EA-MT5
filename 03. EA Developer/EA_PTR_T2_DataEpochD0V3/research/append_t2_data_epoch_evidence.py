...
def parse_zulu_timestamp(value: Any, label: str) -> datetime:
    if not isinstance(value, str) or not value.endswith("Z"):
        raise EvidenceError(f"{label} must be a Z timestamp")
    try:
...
def validate_series_proof(raw: Any, symbol: str) -> None:
    ...
    if raw["copytime_from_epoch"] != raw["m5_first_epoch"] or raw["copytime_from_epoch"] <= 0:
        raise EvidenceError("series_proof CopyTime from epoch must match M5 first epoch")
    if raw["copytime_count"] != 1:
...
def validate_model_specific_journal(journal_text: str, symbol: str | None = None) -> None:
    if MODEL != 4:
        return
    errors = model4_mode_errors(
        journal_text,
        symbol=symbol,
        period=PERIOD,
        server=SERVER,
        label="Model 4 journal",
    )
    if errors:
...
def validate_manifest(
    ...
    actual_asof = dq_contract.get("availability_asof_utc") if isinstance(dq_contract, dict) else None
    expected_asof = expected_dq_contract["availability_asof_utc"]
    if isinstance(dq_contract, dict):
        dq_contract_without_asof = {key: value for key, value in dq_contract.items() if key != "availability_asof_utc"}
        expected_without_asof = {
            key: value for key, value in expected_dq_contract.items() if key != "availability_asof_utc"
        }
    else:
        dq_contract_without_asof = None
        expected_without_asof = expected_dq_contract
    if (
        dq_contract_without_asof != expected_without_asof
        or parse_zulu_timestamp(actual_asof, "run_manifest data-quality availability")
        != parse_zulu_timestamp(expected_asof, "epoch availability cutoff")
    ):
    ...
    validate_model_specific_journal(journal_text, symbol)
    parsed_range = journal_range(journal_text, symbol)
    if parsed_range is None:
        raise EvidenceError("journal lacks one exact-symbol history synchronization range")
    for key in ("actual_from", "actual_to", "exact_match_count", "distinct_range_count"):
        if dq_gate.get(key) != parsed_range[key]:
            raise EvidenceError(f"data_quality_gate {key} does not match journal")
    parsed_proof = journal_series_proof(journal_text, symbol, str(dq_gate.get("actual_from")))
    if parsed_proof is None:
        raise EvidenceError("journal D0 series proof is invalid or truncated-cache evidence")
    if (
        coverage_class != parsed_proof["coverage_class"]
        or dq_gate.get("series_proof") != parsed_proof["series_proof"]
    ):
        raise EvidenceError("data_quality_gate series proof/coverage class does not match journal")
    expected_basis = {
