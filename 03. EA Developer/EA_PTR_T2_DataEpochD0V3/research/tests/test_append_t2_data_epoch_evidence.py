...
def test_read_report_text_supports_native_mt5_utf16(tmp_path: Path) -> None:
    report = tmp_path / "report.html"
    report.write_text(
        f"<b>{sut.SERVER} (Build 6061)</b><table><tr><td>History Quality:</td><td><b>98%</b></td></tr></table>",
        encoding="utf-16",
    )

    text = sut.read_report_text(report)
    assert sut.report_history_quality(text) == 98.0
    assert sut.report_server_identity(text) == f"{sut.SERVER} (Build 6061)"


def test_model4_requires_explicit_real_tick_journal_readback() -> None:
    original_model = sut.MODEL
    try:
        sut.MODEL = 4
        exact = (
        ...
        sut.validate_model_specific_journal(exact, "XAUUSD")
        with pytest.raises(sut.EvidenceError, match="exact Tester execution mode"):
            sut.validate_model_specific_journal("Tester: real ticks cache warmed, no execution mode line\n")
        with pytest.raises(sut.EvidenceError, match="exact Tester execution mode"):
            sut.validate_model_specific_journal(
                "Tester: generating based on real ticks unavailable/fallback\n"
            )
        for wrong_line in (
            ...
            with pytest.raises(sut.EvidenceError, match="exact Tester execution mode"):
                sut.validate_model_specific_journal(wrong_line, "XAUUSD")
        with pytest.raises(sut.EvidenceError, match="contradictory generated-tick"):
            sut.validate_model_specific_journal(exact + generated, "XAUUSD")
    finally:
        sut.MODEL = original_model
...
def make_workspace(
    ...
    copytime_from_epoch: int | None = None,
    ...
    run_dir = workspace / "02. AlphaFactory/runs/EA_PTR_T2_DataEpochD0V3/TEST"
    ...
    mode_line = (
        f"CS\t0\t07:55:31.561\tTester\t{symbol},M5 ({sut.SERVER}): "
    ...
    journal.write_text(
        mode_line
        + f"{symbol}: history synchronized from 2010.01.01 to {sut.TO_DATE}\n"
        + (
            ...
            f"m1_server_first_epoch={first_epoch} m1_terminal_first_epoch={first_epoch} "
            "m5_bars=1000 terminal_maxbars=100000 "
            f"copytime_from_epoch={first_epoch if copytime_from_epoch is None else copytime_from_epoch} "
            f"copytime_count=1 copytime_result={copytime_result} "
            f"copytime_first_epoch={first_epoch if copytime_result == 1 else 0} "
    ...
    dq_gate = {
        ...
        "series_proof": {
            ...
            "copytime_from_epoch": first_epoch if copytime_from_epoch is None else copytime_from_epoch,
            "copytime_count": 1,
...
def test_append_rejects_datetime_zero_copytime_request(tmp_path: Path) -> None:
    workspace, manifest, summary, _ = make_workspace(tmp_path, copytime_from_epoch=0)
    with pytest.raises(sut.EvidenceError, match="M5 first epoch"):
        sut.build_row(workspace, "EURUSD", manifest, summary)


def test_append_reparses_journal_before_writing_row(tmp_path: Path) -> None:
    workspace, manifest, summary, _ = make_workspace(tmp_path)
    payload = json.loads(manifest.read_text(encoding="utf-8"))
    payload["data_quality_gate"]["actual_from"] = "2011.01.01"
    payload["data_quality_fingerprint_basis"]["actual_from"] = "2011.01.01"
    payload["data_quality_fingerprint"] = sut.text_sha256(
        sut.ps_compact_json(payload["data_quality_fingerprint_basis"])
    )
    write_json(manifest, payload)
    with pytest.raises(sut.EvidenceError, match="actual_from does not match journal"):
        sut.build_row(workspace, "EURUSD", manifest, summary)
