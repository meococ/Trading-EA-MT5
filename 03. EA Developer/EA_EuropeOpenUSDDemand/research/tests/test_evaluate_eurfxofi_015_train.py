...
SCRIPT = Path(__file__).resolve().parents[1] / "evaluate_eurfxofi_015_train.py"
SPEC = importlib.util.spec_from_file_location("eurfxofi015", SCRIPT)
...
def test_configure_sets_exact_hyp015_identity() -> None:
    ...
    assert foundation.DISPLAY_TAG == "HYP015"
    assert foundation.ARTIFACT_PREFIX == "EURFXOFI015"
    assert foundation.SCHEMA_PREFIX == "eurfxofi015"
