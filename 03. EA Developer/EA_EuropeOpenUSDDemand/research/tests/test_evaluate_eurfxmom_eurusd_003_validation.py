...
SOURCE = Path(__file__).resolve().parents[1] / "evaluate_eurfxmom_eurusd_003_validation.py"
SPEC = importlib.util.spec_from_file_location("eurfxmom003_validation", SOURCE)
assert SPEC is not None and SPEC.loader is not None
mom3 = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = mom3
SPEC.loader.exec_module(mom3)


def test_wrapper_sentinel_shape_is_fail_closed() -> None:
    sentinel = mom3.REVIEWED_REGISTRY_ROW_SHA256
...
def test_wrapper_normalized_hash_ignores_only_sentinel_value() -> None:
    ...
    base = mom3.normalized_evaluator_base_sha256(payload)
    lines = payload.splitlines(keepends=True)
    matches = [index for index, line in enumerate(lines) if mom3._SENTINEL_RE.match(line.rstrip(b"\n"))]
    assert len(matches) == 1
    index = matches[0]
    newline = b"\r\n" if lines[index].endswith(b"\r\n") else b"\n"
    lines[index] = b'REVIEWED_REGISTRY_ROW_SHA256: str | None = "' + b"A" * 64 + b'"' + newline
    armed = b"".join(lines)
    assert mom3.normalized_evaluator_base_sha256(armed) == base


def test_configure_binds_fresh_identity_without_changing_contract() -> None:
    module = mom3.configure()
    assert module.HYPOTHESIS_ID == "HYP-EURFXMOM-EURUSD-M1-003"
    assert module.ATTEMPT_ID == "EURFXMOM003-VALIDATION-001"
    assert module.PLAN_SHA256 == mom3.PLAN_SHA256
...
def test_foundation_and_plan_hashes_are_exact() -> None:
    root = mom3.workspace()
    assert mom3.sha256_file(root / mom3.FOUNDATION_REL) == mom3.FOUNDATION_SHA256
    assert mom3.sha256_file(root / mom3.PLAN_REL) == mom3.PLAN_SHA256
...
def test_configured_authority_rejects_disarmed_wrapper() -> None:
    if mom3.REVIEWED_REGISTRY_ROW_SHA256 is None:
        module = mom3.configure()
        with pytest.raises(module.ContractError, match="not armed"):
            module.validate_authority(mom3.workspace())
