# HYP-EURFXOFI-EURUSD-M1-013 â€” Outcome-blind TBBO aggressor classifier

## Why this is a fresh mechanism

- HYP012 was parked before source access because its copied HYP011 summary SHA was truncated. HYP013 carries the unchanged classifier with the exact 64-character predecessor hash; no source or outcome was exposed under HYP012.
...
- HYP011 read no EURUSD target or PnL. HYP013 changes only the source-side classifier using the pre-trade BBO already carried by each TBBO record.
- HYP011 source summary SHA256: `309D0B87A1CBEC06274D39B808DCF9325232463C834A5AAD636DB29F1B76B22B`.
...
2. For `side=N` with a valid, non-crossed pre-trade BBO:
   - trade price `>= ask` â†’ buy;
   - trade price `<= bid` â†’ sell;
   - otherwise price `> midpoint` â†’ buy, price `< midpoint` â†’ sell.
3. If price equals midpoint or the BBO is unavailable, use the sequential tick rule:
   - price above the previous trade â†’ buy;
   - price below the previous trade â†’ sell;
...
- Attempt ID: `EURFXOFI013-SOURCE-CLASSIFIER-001`; one local run after review/sentinel binding.
...
- PASS permits only a fresh TRAIN-only economics hypothesis using exact HYP013 feature/summary/manifest hashes. FAIL closes this classifier without threshold rescue.
