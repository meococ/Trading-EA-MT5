//+------------------------------------------------------------------+
//| EA_XBTMMTradeThroughV3.mq5                                       |
//| HYP-XBT-MM-TRADETHROUGH-003 - constant-notional virtual engine   |
//+------------------------------------------------------------------+
...
input string InpIndexFile="xbtmm\\pilot_20180101_20180101_index.csv";
input string InpOutputPrefix="xbtmm\\outputs\\XBTMM003_20180101";

const string HYPOTHESIS_ID="HYP-XBT-MM-TRADETHROUGH-003";
...
bool ProcessEventFile(const string utc_day,const datetime day_time,
                      ...
                      const long index_first_us,const long index_last_us)
  {
   ...
   if(schema!=1 || record_size!=58 || expected_records<=0 ||
     ...
     {
      PrintFormat("XBTMM_HEADER_FAIL day=%s schema=%d record_size=%d bytes=%I64u index_bytes=%I64d records=%I64d index_records=%I64d quotes=%I64d index_quotes=%I64d trades=%I64d index_trades=%I64d",
                  utc_day,schema,record_size,actual_bytes,index_bytes,
                  expected_records,index_records,expected_quotes,index_quotes,
                  expected_trades,index_trades);
      FileClose(handle);
      return false;
     }
   ...
   PrintFormat("XBTMM_DAY_SUMMARY hypothesis_id=%s day=%s file=%s event_sha256=%s records=%I64d expected=%I64d quotes=%I64d expected_quotes=%I64d trades=%I64d expected_trades=%I64d first_us=%I64d last_us=%I64d day_gate_pass=%s economic_use_forbidden=%s",
               HYPOTHESIS_ID,utc_day,event_file,event_sha256,local_records,
   ...
   while(!FileIsEnding(index_handle))
     {
      ...
      if(!ProcessEventFile(utc_day,day_time,event_file,event_sha256,event_bytes,
