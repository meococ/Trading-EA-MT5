...
$receipt = [ordered]@{
    ...
    note = 'Exactly one EURUSD M5 2018-2022 Model0 development falsification run for the frozen role-aware causal state machine. No optimization, validation, holdout or promotion authority.'
}

$json = $receipt | ConvertTo-Json -Depth 16
[System.IO.File]::WriteAllText($receiptPath, $json + "`n", [System.Text.UTF8Encoding]::new($false))
Write-Output ("ROLE_AWARE_RECEIPT_OK path={0} sha256={1} git_status_sha256={2}" -f `
    $receiptPath,
