...
bool BuildDecision(const RsfSnapshot &s,const SymbolProfile &profile,TradeDecision &d)
  {
   ...
   if(s.aird_confidence+1e-12<InpMinAirdConfidence) { g_reject_context++; return(false); }
   ...
   bool range_context=(s.vrc_regime==2 || s.vrc_regime==3) &&
                      (s.aird_regime==2 || s.p_range/100.0>=InpMinAirdStateProbability);
   bool bull_context=(s.vrc_regime>=4 && s.vrc_regime<=6 && s.vrc_direction>0.0) &&
                     (s.aird_regime==0 || s.p_bull/100.0>=InpMinAirdStateProbability);
   bool bear_context=(s.vrc_regime>=-1 && s.vrc_regime<=1 && s.vrc_direction<0.0) &&
                     (s.aird_regime==1 || s.p_bear/100.0>=InpMinAirdStateProbability);
   bool compression_origin=(s.vrc_previous_regime==7 || s.vrc_low_vol || s.mbb_release);
   ...
   if(InpAllowBreakoutMode && (profile.modes&RSF_MODE_BREAKOUT)!=0 && compression_origin)
     {
      if(s.s3_long && s.tb_structure_up && s.tb_displacement_up && qqe_long)
         d.signal=RSF_BREAKOUT_LONG;
      else if(s.s3_short && s.tb_structure_down && s.tb_displacement_down && qqe_short)
         d.signal=RSF_BREAKOUT_SHORT;
      if(d.signal!=RSF_SIGNAL_NONE) g_breakout_setups++;
     }

   if(d.signal==RSF_SIGNAL_NONE && InpAllowTrendMode && (profile.modes&RSF_MODE_TREND)!=0)
     {
      if(bull_context && s.s2_long && s.tb_bias>0 && (s.tb_structure_up||zone_long) && qqe_long)
         d.signal=RSF_TREND_LONG;
      else if(bear_context && s.s2_short && s.tb_bias<0 && (s.tb_structure_down||zone_short) && qqe_short)
         d.signal=RSF_TREND_SHORT;
      if(d.signal!=RSF_SIGNAL_NONE) g_trend_setups++;
     }

   if(d.signal==RSF_SIGNAL_NONE && InpAllowRangeMode && (profile.modes&RSF_MODE_RANGE)!=0 && range_context)
     {
      if(s.s1_long && s.tb_sweep_low && qqe_rising && MathMin(s.qqe_primary,s.qqe_secondary)<=-InpRangeQQEExtreme)
         d.signal=RSF_RANGE_LONG;
      else if(s.s1_short && s.tb_sweep_high && qqe_falling && MathMax(s.qqe_primary,s.qqe_secondary)>=InpRangeQQEExtreme)
         d.signal=RSF_RANGE_SHORT;
      if(d.signal!=RSF_SIGNAL_NONE) g_range_setups++;
     }

   if(d.signal==RSF_SIGNAL_NONE) { g_reject_timing++; return(false); }
   ...
   if(d.signal==RSF_RANGE_LONG || d.signal==RSF_RANGE_SHORT)
     {
   ...
   else if(d.signal==RSF_TREND_LONG || d.signal==RSF_TREND_SHORT)
     {
...
bool WriteRunMeta()
  {
   ...
   payload+="\"run_id\":\""+JsonEscape(g_run_id)+"\",\"ea_name\":\""+RSF_EA_NAME+"\",";
   ...
   payload+=StringFormat("\"reject_session\":%I64d,\"reject_context\":%I64d,\"reject_structure\":%I64d,\"reject_timing\":%I64d,",g_reject_session,g_reject_context,g_reject_structure,g_reject_timing);
...
int CreateQqeHandle()
  {
   return(iCustom(_Symbol,PERIOD_M5,"AlphaFactory\\QQE_MOD",
                  ...
                  InpQqePrimarySource,InpQqeSecondaryRsiLength,InpQqeSecondarySmoothing,InpQqeSecondaryFactor,
                  InpQqeSecondaryThreshold,InpQqeSecondarySource,InpQqeBollingerLength,InpQqeBollingerMultiplier));
  }
