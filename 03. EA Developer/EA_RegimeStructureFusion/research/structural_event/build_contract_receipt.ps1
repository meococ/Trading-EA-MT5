...
$receipt = [ordered]@{
    ...
    note = 'Exactly one EURUSD M5 2018-2022 Model0 development falsification run for the frozen closed structural-event then later retest/reclaim mechanism. No optimization, validation, holdout or promotion authority.'
}

$json = $receipt | ConvertTo-Json -Depth 16
[System.IO.File]::WriteAllText($receiptPath, $json + "`n", [System.Text.UTF8Encoding]::new($false))
Write-Output ("STRUCTURAL_EVENT_RECEIPT_OK path={0} sha256={1} git_status_sha256={2}" -f `
    $receiptPath,
