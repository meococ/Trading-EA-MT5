...
def main() -> int:
    ...
    event_x["event_structure_rise"] = events["structure_rise"].to_numpy(dtype=np.float32)
    event_x["event_displacement_rise"] = events["displacement_rise"].to_numpy(dtype=np.float32)
    event_x["event_sweep_rise"] = events["sweep_rise"].to_numpy(dtype=np.float32)
                    ...
                    fold_rows.append(
                        {
                            ...
                            "structure_trades": int(chosen_events.structure_rise.sum()),
                            "displacement_trades": int(chosen_events.displacement_rise.sum()),
                            "sweep_trades": int(chosen_events.sweep_rise.sum()),
                            **metrics(net, test_weeks),
                        }
                    )

    folds = pd.DataFrame(fold_rows)
    folds.to_csv(args.out_dir / "tb_event_acceptance_folds.csv", index=False)
    summaries: list[dict] = []
    for family in barrier.MODEL_FAMILIES:
        for target_atr in barrier.TARGET_ATR:
            ...
            summaries.append(
                {
                    ...
                    "selected_event_types": {
                        "structure": int(primary.structure_trades.sum()),
                        "displacement": int(primary.displacement_trades.sum()),
                        "sweep": int(primary.sweep_trades.sum()),
                    },
    ...
    result = {
        "schema_version": "rsf_tb_event_acceptance018_discovery.v1",
        ...
        "verdict": "DISCOVERY_SURVIVOR_FREEZE_REQUIRED" if any(row["survivor"] for row in ranked) else "KILL_NO_STABLE_TB_EVENT_EDGE",
    }
    (args.out_dir / "tb_event_acceptance018_results.json").write_text(
