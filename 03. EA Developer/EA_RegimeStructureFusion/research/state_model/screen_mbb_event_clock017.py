...
LONG_FLAGS = ("s1_long", "s2_long", "s3_long")
SHORT_FLAGS = ("s1_short", "s2_short", "s3_short")
...
def main() -> int:
    ...
    events = pd.DataFrame(
        {
            ...
            "s1_rise": (rises["s1_long"] | rises["s1_short"])[eligible].astype(int),
            "s2_rise": (rises["s2_long"] | rises["s2_short"])[eligible].astype(int),
            "s3_rise": (rises["s3_long"] | rises["s3_short"])[eligible].astype(int),
        }
    ...
    events.to_csv(args.out_dir / "mbb_event_clock.csv", index=False)
    ...
    result = {
        "schema_version": "rsf_mbb_event_clock017_stage0.v1",
        ...
        "by_year": by_year,
        "event_type_counts": {
            "s1_rise": int(events.s1_rise.sum()),
            "s2_rise": int(events.s2_rise.sum()),
            "s3_rise": int(events.s3_rise.sum()),
        },
        "gates": gates,
        "verdict": "PASS_STAGE0_AUTHORIZE_FROZEN_STAGE_B" if all(gates.values()) else "PARK_STAGE0_CADENCE_OR_BALANCE_FAIL",
    }
    (args.out_dir / "mbb_event_clock017_stage0.json").write_text(
