...
$hypothesisId = 'HYP-RSF-EURUSD-M5-LIQUIDITY-POOL-EXPORT-009'
...
if ([string]$latest.state -cne 'probe') {
    throw "Latest registry state must be probe, got '$($latest.state)'"
}
...
$packet = [ordered]@{
    ...
    run_role = 'control'
    purpose = 'One short-window Model1 engineering-only verification that TB v3 liquidity buffers 44-47 are exported through DRAW_NONE plots. Performance metrics have no authority.'
    ...
    execution_mode = 0
    fixed_delay_ms = 0
    overrides = 'InpEnableTelemetry=true;InpExpectedSymbol=EURUSD;InpHypothesisId=HYP-RSF-EURUSD-M5-LIQUIDITY-POOL-EXPORT-009;InpMagic=5867419;InpManualModeMask=6;InpManualSessionMask=6;InpProfileMode=1;InpResearchAutoMode=true;InpStructuralRequireLiveObjective=true;InpStructuralUseLiquidityPoolObjective=true;InpUseContextRouter=true;InpUseQqeTiming=true;InpUseStructuralEventSequence=true;InpUseTbStructure=true;InpVariantTag=LIQUIDITY_POOL_EXPORTED_V1'
