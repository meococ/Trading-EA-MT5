...
$receipt = [ordered]@{
    ...
    note = 'One EURUSD M5 January-2018 Model1 engineering-only verification of the TB v3 public liquidity-buffer export. Performance metrics have no authority.'
}

$json = $receipt | ConvertTo-Json -Depth 16
[System.IO.File]::WriteAllText($receiptPath, $json + "`n", [System.Text.UTF8Encoding]::new($false))
Write-Output ("LIQUIDITY_POOL_EXPORT_PROBE_RECEIPT_OK path={0} sha256={1} git_status_sha256={2}" -f `
    $receiptPath,
