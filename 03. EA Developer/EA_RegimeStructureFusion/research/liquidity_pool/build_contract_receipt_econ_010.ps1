...
$hypothesisId = 'HYP-RSF-EURUSD-M5-LIQUIDITY-POOL-ECON-010'
...
$receipt = [ordered]@{
    ...
    note = 'Exactly one EURUSD M5 2018-2022 Model0 economic falsification run for the frozen causal liquidity pool after the export probe passed. No optimization, validation, holdout or promotion authority.'
}

$json = $receipt | ConvertTo-Json -Depth 16
[System.IO.File]::WriteAllText($receiptPath, $json + "`n", [System.Text.UTF8Encoding]::new($false))
Write-Output ("LIQUIDITY_POOL_ECON_RECEIPT_OK path={0} sha256={1} git_status_sha256={2}" -f `
    $receiptPath,
