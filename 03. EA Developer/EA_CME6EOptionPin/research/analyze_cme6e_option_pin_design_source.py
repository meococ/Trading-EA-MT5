"""Build source-only DESIGN OI pins from frozen per-expiry statistics files."""
...
STATISTICS_RECEIPT = "phase_02_statistics_acquisition_receipt.json"
EVENT_RESULTS_FILE = "design_event_source_results.csv"
SURFACES_FILE = "design_oi_surfaces.csv"
PINS_FILE = "design_source_pins.csv"
ANALYSIS_FILE = "design_source_semantics_analysis.json"
RESULT_FILE = "design_source_semantics_receipt.json"
...
def surface_from_contracts(completed: pd.DataFrame) -> pd.DataFrame:
...
def analyze_event(
    ...
    missing = sorted(required.difference(statistics.columns))
    if missing:
        raise SourceAnalysisError(
            f"statistics {request['request_id']} missing columns: {missing}"
        )
    decision = pd.Timestamp(request["decision_utc"])
    max_reference = pd.Timestamp(request["max_oi_reference_utc"])
    ...
    surface = surface_from_contracts(completed)
    ...
    strict_temporal = bool(
        ...
        and (published["ts_ref"] <= max_reference).all()
    )
    source_valid = bool(
    ...
    result = {
        ...
        "max_oi_reference_utc": request["max_oi_reference_utc"],
        ...
        "unique_positive_pin": unique,
        "pin_strike": pin,
        "pin_total_oi": total_oi,
    }
...
def write_surfaces(path: Path, surfaces: list[pd.DataFrame]) -> None:
    temporary = path.with_suffix(path.suffix + ".tmp")
    combined = pd.concat(surfaces, ignore_index=True) if surfaces else pd.DataFrame()
    with temporary.open("w", encoding="ascii", newline="") as handle:
...
def execute(workspace: Path) -> Path:
    ...
    try:
        ...
        raise SourceAnalysisError("source root escaped workspace") from exc
    ...
    if not all(path.is_file() for path in required_paths):
        raise SourceAnalysisError("source campaign artifacts are incomplete")
    ...
    if (
        ...
        raise SourceAnalysisError("source campaign is not ready for semantics")
    ...
    if set(request_by_id) != set(payload_by_id):
        raise SourceAnalysisError("statistics payload/request identity mismatch")
    ...
    for request_id in sorted(request_by_id):
        ...
        try:
            ...
            raise SourceAnalysisError("statistics payload escaped workspace") from exc
        if not path.is_file() or sha256_file(path) != payload.get("raw_sha256"):
            raise SourceAnalysisError(f"statistics payload drift: {request_id}")
        ...
        if event_contracts.empty:
            raise SourceAnalysisError(f"event contract catalog empty: {request_id}")
    ...
    report = {
        "schema_version": "cme6e_option_pin_design_source_semantics.v1",
    ...
    result = {
        "schema_version": "cme6e_option_pin_design_source_semantics_receipt.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "campaign_id": CAMPAIGN_ID,
        "verdict": verdict,
        "source_valid_events": source_valid_count,
...
def main() -> int:
    ...
    try:
        ...
        print(f"CME6EOPTPIN_DESIGN_SOURCE_ANALYSIS_BLOCKED reason={exc}")
        return 2
