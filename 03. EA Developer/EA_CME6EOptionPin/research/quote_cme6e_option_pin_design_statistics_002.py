"""Quote frozen HYP002 source-only statistics windows without payload calls."""
...
SUMMARY_FILE = "design_statistics_quote_summary_pit.json"
INHERITED_DEFINITION_COST_USD = 4.65386341884732
INHERITED_DEFINITION_BILLED_SIZE = 2939439880
...
def load_requests(path: Path) -> list[dict[str, Any]]:
    ...
    for line_number, line in enumerate(path.read_text(encoding="ascii").splitlines(), 1):
        ...
        if (
            ...
            != "cme6e_option_pin_statistics_request.v2"
            or request.get("hypothesis_id") != HYPOTHESIS_ID
            ...
            or request.get("missing_oi_policy") != "UNKNOWN_EVENT_INVALID"
            or not request.get("request_id")
            or not request.get("event_id")
        ):
            raise QuoteError(f"invalid HYP002 statistics request at line {line_number}")
        requests.append(request)
    if not requests:
        raise QuoteError("HYP002 statistics request plan is empty")
    request_ids = [request["request_id"] for request in requests]
    if len(request_ids) != len(set(request_ids)):
        raise QuoteError("HYP002 statistics request IDs are not unique")
    return requests
...
def quote_all(requests: list[dict[str, Any]], api_key: str) -> list[dict[str, Any]]:
    ...
    def quote_one(request: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "schema_version": "cme6e_option_pin_statistics_quote.v2",
...
def execute(workspace: Path) -> Path:
    ...
    try:
        ...
        raise QuoteError("HYP002 source root escaped workspace") from exc
    discovery_path = root / DISCOVERY_RECEIPT
    requests_path = root / REQUESTS_FILE
    if not discovery_path.is_file() or not requests_path.is_file():
        raise QuoteError("HYP002 discovery artifacts are missing")
    discovery = json.loads(discovery_path.read_text(encoding="ascii"))
    if (
        discovery.get("hypothesis_id") != HYPOTHESIS_ID
        or discovery.get("campaign_id") != CAMPAIGN_ID
        ...
        raise QuoteError("HYP002 PIT source contract has not passed")
    requests = load_requests(requests_path)
    if len(requests) != int(discovery.get("statistics_request_count", -1)):
        raise QuoteError("HYP002 request count drifted from discovery receipt")
    ...
    cumulative = INHERITED_DEFINITION_COST_USD + statistics_cost
    summary = {
        "schema_version": "cme6e_option_pin_statistics_quote_summary.v2",
        ...
        "inherited_definition_cost_usd": INHERITED_DEFINITION_COST_USD,
        "inherited_definition_billed_size": INHERITED_DEFINITION_BILLED_SIZE,
        "inherited_definition_new_charge_usd": 0.0,
        "statistics_combined_estimated_usd": statistics_cost,
        "statistics_combined_billable_bytes": statistics_bytes,
        "cumulative_related_research_estimated_usd": cumulative,
