# HYP-ROUND-CASCADE-EURUSD-M5-008 — Execution-Source Guard Repair Plan
...
HYP007 one-shot attempt. It does **not** change any market rule, source
population, decision surface, reservation geometry, Stage-0 economic gate, dual
receipt binding, canonical manifest binding, or permission surface that would
rescue a market outcome.

The sole pre-outcome failure observed on HYP007 was:

- terminal status `ENGINEERING_INVALID_NO_MARKET_VERDICT`
- reason `forbidden outcome field: design_shards_opened`
- exact terminal SHA256
  `2135A63C5E30A826A119E6BAFB3276C09E4BDC0ABF5F9D5F3A70465BC163C771`

HYP007 already bound the dual raw + semantic DESIGN receipt and preserved the
HYP002 populations and arm-local twelve-complete-M5 classifier. After decoding
timestamp-only public DESIGN shards it emitted safe diagnostic counters named
`design_shards_opened` and `design_bytes_opened`. The strict outcome-blind
substring guard treats the token `open` as a prohibited price/OHLC concept, so
those diagnostic **names** were rejected even though their values were
non-economic nonnegative integers and no price, volume, direction, trade,
return, PnL or performance field was opened.

HYP008 therefore changes **only** the timestamp-diagnostic naming surface and
the exact safe-counter validation path:

1. bind the exact HYP007 terminal as parent
   (SHA256 `2135A63C5E30A826A119E6BAFB3276C09E4BDC0ABF5F9D5F3A70465BC163C771`);
2. rename safe diagnostic counters `design_shards_opened` → `design_shards_read`
   and `design_bytes_opened` → `design_bytes_read`;
3. keep `design_timestamp_rows_read` unchanged;
4. before bypassing the existing strict substring outcome guard, validate the
   three exact safe diagnostic keys
   `design_shards_read`, `design_timestamp_rows_read`, `design_bytes_read` as
   nonnegative plain integers (`type is int`, not `bool`, value `>= 0`);
5. **do not** broadly weaken the guard: actual or nested
   open/high/low/close/price/spread/volume/direction/trade/return/PnL/profit/
   performance/economic/outcome fields must still fail; arbitrary keys that
   merely contain those prohibited concepts must not gain an allowlist;
6. preserve every HYP007 market/source rule, exact HYP002 populations,
   time_utc-only projection, dual raw plus semantic receipt binding, canonical
   manifest binding, `NO_EXACT_ENTRY` priority, arm-local 12-complete-M5
   reservation state machine, Stage-0 gates, generic registry authority,
   one-shot durability, and all sealed permissions.

No post-hoc market rescue, threshold retune, source reselection, arm reorder,
Stage-0 loosening, receipt rebinding, or permission expansion is permitted.

## 1. Identity and exact failure radius

- Hypothesis: `HYP-ROUND-CASCADE-EURUSD-M5-008`.
- Parent: terminal `HYP-ROUND-CASCADE-EURUSD-M5-007` attempt
  `HYP007-EXEC-SOURCE-001`.
- Attempt: `HYP008-EXEC-SOURCE-001`, limit exactly one.
...
The exact HYP007 terminal path is
`03. EA Developer/EA_RoundNumberCascade/research/evidence/HYP-ROUND-CASCADE-EURUSD-M5-007_EXECUTION_SOURCE/HYP007-EXEC-SOURCE-001/attempt_terminal.json`,
SHA256 `2135A63C5E30A826A119E6BAFB3276C09E4BDC0ABF5F9D5F3A70465BC163C771`.
It binds only the started-artifact hash, status
`ENGINEERING_INVALID_NO_MARKET_VERDICT`, and the outcome-guard ContractError for
`design_shards_opened`; no outcome, trade or metric exists.

HYP008 preserves the HYP007 decision surface: a fresh, outcome-blind
...
## 2. Exact immutable source and public data (unchanged from HYP007)
...
public schema, but HYP008 may request and decode only `columns=['time_utc']`.
...
## 3. Frozen deterministic arm-local classifier (identical to HYP007)
...
## 4. Pre-outcome timestamp expectations, not count gates (identical to HYP007)
...
replay it and require the second canonical hash to equal the first. HYP008 must
...
## 5. Safe timestamp diagnostics and outcome-blind guard (sole intentional delta)
...
3. reject wrong types (float/bool/str/None), negative values, and nested
   non-integer payloads for those keys;
4. continue to reject the retired names `design_shards_opened` and
   `design_bytes_opened` via the existing forbidden-token path (`open`);
5. continue to reject real or nested open/high/low/close/price/spread/volume/
   direction/trade/return/PnL/profit/performance/economic/outcome fields and
   any arbitrary key that merely contains those prohibited concepts.

This is a naming + exact safe-counter validation repair only. It is not a
permission to open prices, outcomes or economic metrics.

## 6. PASS meaning and sealed permissions

`PASS_EXECUTION_SOURCE_MAY_DRAFT_HYP009_DESIGN_ECONOMICS` requires exact parent,
...
11. the three exact safe diagnostic keys are present as nonnegative plain
    integers and do not trip the outcome-blind guard.

Eligible and refractory counts are reported actuals, not Stage-0 thresholds.
**PASS authorizes only drafting a separately reviewed and frozen HYP009
...
## 7. Authority, durability and current implementation scope
...
of the exact HYP007 terminal before source ledger or DESIGN metadata access.

New authority surface (HYP008-local only):
...
- Review receipt path:
  `03. EA Developer/EA_RoundNumberCascade/research/HYP-ROUND-CASCADE-EURUSD-M5-008_EXECUTION_SOURCE_IMPLEMENTATION_REVIEW_RECEIPT.json`
- Review receipt schema:
  `round_cascade_008_execution_source_implementation_review.v1`
...
1. this plan;
2. `build_round_cascade_008_execution_source.py`;
3. `tests/test_build_round_cascade_008_execution_source.py`.
...
This is implementation repair of an outcome-guard false positive on safe
timestamp diagnostic names, not market rescue.
