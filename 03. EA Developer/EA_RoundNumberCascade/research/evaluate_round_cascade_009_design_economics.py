#!/usr/bin/env python3
"""Fail-closed DESIGN economics evaluator for HYP009.
...
burned into REVIEWED_RUN_PACKET_SHA256, exact hash-bound HYP008 eligible source,
exact HYP002 detail join, latest registry authority, and a fresh evidence root.

This is a fresh pre-outcome economics child enabled by outcome-blind HYP008
source classification. It is not a post-hoc market rescue of HYP005.
"""
...
PARENT_CANDIDATE = "HYP-ROUND-CASCADE-EURUSD-M5-008"
...
PLAN_REL = (
    ...
    "HYP-ROUND-CASCADE-EURUSD-M5-009_DESIGN_ECONOMICS_PLAN.md"
)
FROZEN_PLAN_SHA256 = "34465982698CFC0BF6117A6039D5FC8F783F979F0BEA504C66C499AE5B74422E"
EVALUATOR_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "evaluate_round_cascade_009_design_economics.py"
)
TEST_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/tests/"
    "test_evaluate_round_cascade_009_design_economics.py"
)
RUN_PACKET_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "HYP-ROUND-CASCADE-EURUSD-M5-009_DESIGN_ECONOMICS_RUN_PACKET.json"
)
REGISTRY_REL = "04. Memory/research/CANDIDATE_REGISTRY.jsonl"
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "HYP-ROUND-CASCADE-EURUSD-M5-009_DESIGN_ECONOMICS_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
...
HYP008_EVIDENCE_ROOT_REL = (
    ...
    "HYP008-EXEC-SOURCE-001"
)
ELIGIBLE_LEDGER_REL = f"{HYP008_EVIDENCE_ROOT_REL}/round_cascade_008_eligible_source_ledger.jsonl"
...
ELIGIBLE_LEDGER_SHA256 = "B84EF3925B5CC998A88D224BCF8B4A66D5A6076DFED87C4287325F369AAFF16B"
...
HYP008_STARTED_SHA256 = "59EC189F548BEE3174CD928BC4B719B0A41DF016B9BBF54271A75E3319096A0B"
HYP008_INELIGIBLE_LEDGER_SHA256 = "9C48A22BAEF82998038D9D24472A61863AD9DD8BBFF9BD9D52C5C0E66C2E6680"
HYP008_CLASSIFICATION_SHA256 = "97D0CB0FF8E471C961032206B11BC4B6E9ACB0DD13B99ECBED655C064D28F82D"
HYP008_ATTEMPT_ID = "HYP008-EXEC-SOURCE-001"
HYP008_PASS_STATUS = "PASS_EXECUTION_SOURCE_MAY_DRAFT_HYP009_DESIGN_ECONOMICS"
...
# Economics trade population = HYP008 eligible counts (not full HYP002).
EXPECTED_SIGNAL_COUNTS = {"TRUE_0050": 1220, "SHIFTED_0025": 1214}
ELIGIBLE_STATUS = "ELIGIBLE_EXACT_ENTRY_NONOVERLAP"
ELIGIBLE_KEYS = {
    "arm",
    "complete_m5_starts",
...
def load_verified_dsr(workspace_root: Path):
    ...
    namespace: dict[str, Any] = {"__builtins__": __builtins__, "__name__": "_hyp009_verified_dsr"}
...
def dsr_inputs_and_value(true_net: list[float], shifted_net: list[float], workspace_root: Path | None = None) -> dict[str, Any]:
    ...
    if len(true_net) != 1220 and EXPECTED_SIGNAL_COUNTS["TRUE_0050"] == 1220:
        raise EngineeringInvalid("TRUE DSR n_obs must be 1220 for HYP009")
...
def validate_run_packet(packet: dict[str, Any], packet_sha256: str, workspace_root: Path | None = None) -> None:
    ...
    required = {
        ...
        "hyp008_report_path": HYP008_REPORT_REL,
        "hyp008_report_sha256": HYP008_REPORT_SHA256,
        "hyp008_receipt_path": HYP008_RECEIPT_REL,
        "hyp008_receipt_sha256": HYP008_RECEIPT_SHA256,
...
def validate_latest_registry_authority(workspace_root: Path, packet_sha256: str, packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        if row.get("hypothesis_id") == HYPOTHESIS_ID:
            latest = row
    if latest is None:
        raise EngineeringInvalid("missing HYP009 registry row")
...
def validate_hyp008_chain_before_price(workspace_root: Path, packet: dict[str, Any]) -> dict[str, str]:
    """Bind HYP008 report/receipt/terminal/eligible ledger before DESIGN price access."""
    bindings = {
        "eligible_ledger": (packet["eligible_ledger_path"], ELIGIBLE_LEDGER_SHA256),
        "hyp008_report": (packet["hyp008_report_path"], HYP008_REPORT_SHA256),
        "hyp008_receipt": (packet["hyp008_receipt_path"], HYP008_RECEIPT_SHA256),
        "hyp008_terminal": (packet["hyp008_terminal_path"], HYP008_TERMINAL_SHA256),
    }
    out: dict[str, str] = {}
    ...
    report = load_json_file(
        resolve_workspace_file(workspace_root, packet["hyp008_report_path"]),
        HYP008_REPORT_SHA256,
        label="HYP008 report",
    )
    receipt = load_json_file(
        resolve_workspace_file(workspace_root, packet["hyp008_receipt_path"]),
        HYP008_RECEIPT_SHA256,
        label="HYP008 receipt",
    )
    terminal = load_json_file(
        resolve_workspace_file(workspace_root, packet["hyp008_terminal_path"]),
        HYP008_TERMINAL_SHA256,
        label="HYP008 terminal",
    )

    if report.get("hypothesis_id") != PARENT_CANDIDATE:
        raise EngineeringInvalid("HYP008 report hypothesis mismatch")
    if report.get("attempt_id") != HYP008_ATTEMPT_ID:
        raise EngineeringInvalid("HYP008 report attempt mismatch")
    if report.get("verdict") != HYP008_PASS_STATUS:
        raise EngineeringInvalid("HYP008 report verdict mismatch")
    if report.get("hyp009_drafting_authorized") is not True:
        raise EngineeringInvalid("HYP008 report does not authorize HYP009 drafting")
    actual_counts = report.get("actual_counts", {}).get(ELIGIBLE_STATUS, {})
    if actual_counts.get("TRUE_0050") != 1220 or actual_counts.get("SHIFTED_0025") != 1214:
        raise EngineeringInvalid("HYP008 report eligible counts mismatch")
    if report.get("classification_sha256") != HYP008_CLASSIFICATION_SHA256:
        raise EngineeringInvalid("HYP008 report classification hash mismatch")

    if receipt.get("hypothesis_id") != PARENT_CANDIDATE:
        raise EngineeringInvalid("HYP008 receipt hypothesis mismatch")
    if receipt.get("verdict") != HYP008_PASS_STATUS:
        raise EngineeringInvalid("HYP008 receipt verdict mismatch")
    if receipt.get("hyp009_drafting_authorized") is not True:
        raise EngineeringInvalid("HYP008 receipt does not authorize HYP009 drafting")
    expected_receipt_artifacts = {
        "attempt_started.json": HYP008_STARTED_SHA256,
        "round_cascade_008_eligible_source_ledger.jsonl": ELIGIBLE_LEDGER_SHA256,
        "round_cascade_008_execution_source_report.json": HYP008_REPORT_SHA256,
        "round_cascade_008_ineligible_source_ledger.jsonl": HYP008_INELIGIBLE_LEDGER_SHA256,
    }
    if receipt.get("artifact_sha256") != expected_receipt_artifacts:
        raise EngineeringInvalid("HYP008 receipt artifact hash-chain mismatch")

    if terminal.get("hypothesis_id") != PARENT_CANDIDATE:
        raise EngineeringInvalid("HYP008 terminal hypothesis mismatch")
    if terminal.get("attempt_id") != HYP008_ATTEMPT_ID:
        raise EngineeringInvalid("HYP008 terminal attempt mismatch")
    if terminal.get("status") != HYP008_PASS_STATUS:
        raise EngineeringInvalid("HYP008 terminal status mismatch")
    if terminal.get("hyp009_drafting_authorized") is not True:
        raise EngineeringInvalid("HYP008 terminal does not authorize HYP009 drafting")
    expected_terminal_artifacts = {
        "attempt_started.json": HYP008_STARTED_SHA256,
        "execution_source_receipt.json": HYP008_RECEIPT_SHA256,
        "round_cascade_008_eligible_source_ledger.jsonl": ELIGIBLE_LEDGER_SHA256,
        "round_cascade_008_execution_source_report.json": HYP008_REPORT_SHA256,
        "round_cascade_008_ineligible_source_ledger.jsonl": HYP008_INELIGIBLE_LEDGER_SHA256,
    }
    if terminal.get("artifact_sha256") != expected_terminal_artifacts:
        raise EngineeringInvalid("HYP008 terminal artifact hash-chain mismatch")
    if terminal.get("classification_sha256") != HYP008_CLASSIFICATION_SHA256:
        raise EngineeringInvalid("HYP008 terminal classification hash mismatch")
    ...
    for blob_name, blob in (("report", report), ("receipt", receipt), ("terminal", terminal)):
        ...
        if not isinstance(counters, dict):
            raise EngineeringInvalid(f"HYP008 {blob_name} source_only_counters missing")
        if (
            ...
            raise EngineeringInvalid(f"HYP008 {blob_name} sealed counters mismatch")
    return out
...
def execute_reviewed_design_economics(workspace_root: Path, run_packet_path: Path) -> int:
    ...
    # HYP008 chain + HYP002 detail join must complete before DESIGN price access.
    validate_hyp008_chain_before_price(workspace_root, packet)
