...
import evaluate_round_cascade_005_design_economics as sut
...
def test_import_is_inert_and_workspace_root_is_required() -> None:
    ...
    assert sut.PLAN_REL.endswith("HYP-ROUND-CASCADE-EURUSD-M5-005_DESIGN_ECONOMICS_PLAN_V2.md")
...
def test_missing_future_minute_does_not_retroactively_shift_entry() -> None:
    start = datetime(2019, 1, 2, 10, 0, tzinfo=UTC)
    rows = [m1(start + timedelta(minutes=i), open_=1.1000, high=1.1010, low=1.0995, close=1.1005) for i in range(65)]
    rows.pop(1)
    market = sut.build_observed_market_index(rows)
    mapped = sut.map_signal_to_market(signal(start), market)
    assert mapped.entry_time_utc == start
    assert mapped.exit_time_utc == start + timedelta(minutes=65)


def test_non_increasing_source_sequence_is_rejected_instead_of_sorted() -> None:
    start = datetime(2019, 1, 2, 10, 0, tzinfo=UTC)
    rows = [m1(start + timedelta(minutes=1)), m1(start)]
    with pytest.raises(sut.EngineeringInvalid, match="non-increasing M1 timestamp"):
        sut.build_observed_market_index(rows)
    with pytest.raises(sut.EngineeringInvalid, match="non-increasing M1 timestamp"):
        sut.build_observed_market_index([rows[1], rows[1]])


def test_entry_delay_boundary_and_weekend_gap_are_fail_closed() -> None:
    planned = datetime(2019, 1, 4, 22, 0, tzinfo=UTC)
    accepted_start = planned + timedelta(minutes=60)
    accepted = sut.build_observed_market_index(minute_window(accepted_start))
    mapped = sut.map_signal_to_market(signal(planned), accepted)
    assert mapped.entry_time_utc == accepted_start

    rejected_start = planned + timedelta(minutes=65)
    rejected = sut.build_observed_market_index(minute_window(rejected_start))
    with pytest.raises(sut.EngineeringInvalid, match="entry delay exceeds 60 minutes"):
        sut.map_signal_to_market(signal(planned), rejected)

    monday = datetime(2019, 1, 7, 0, 0, tzinfo=UTC)
    weekend = sut.build_observed_market_index(minute_window(monday))
    with pytest.raises(sut.EngineeringInvalid, match="entry delay exceeds 60 minutes"):
        sut.map_signal_to_market(signal(planned), weekend)
...
def test_partial_entry_bin_m1_stop_touch_is_not_skipped() -> None:
    entry = datetime(2019, 1, 2, 10, 0, tzinfo=UTC)
    rows = [m1(entry + timedelta(minutes=i), open_=1.1000, high=1.1005, low=1.0995, close=1.1000) for i in range(65)]
    rows.pop(1)
    rows[1]["low"] = 1.0989  # 10:02 is observed inside incomplete 10:00 bin.
    stopped = sut.simulate_signal(signal(entry, direction="LONG", atr20_pips=10.0), rows)
    assert stopped.exit_reason == "STOP"
    assert stopped.planned_entry_time_utc == entry
    assert stopped.entry_time_utc == entry
    assert stopped.exit_time_utc == entry + timedelta(minutes=3)
    assert stopped.gross_R == pytest.approx(-1.0)


def test_adverse_reopen_beyond_stop_fills_observed_open() -> None:
    entry = datetime(2019, 1, 2, 10, 0, tzinfo=UTC)
    entry_row = m1(entry, open_=1.1000, high=1.1000, low=1.1000, close=1.1000)
    after_gap = [
        m1(
            entry + timedelta(minutes=i),
            open_=1.0980 if i == 5 else 1.1000,
            high=1.1005,
            low=1.0975 if i == 5 else 1.0995,
            close=1.1000,
        )
        for i in range(5, 65)
    ]
    stopped = sut.simulate_signal(
        signal(entry, direction="LONG", atr20_pips=10.0),
        [entry_row, *after_gap],
    )
    assert stopped.exit_reason == "GAP_STOP"
    assert stopped.entry_bid == pytest.approx(1.1000)
    assert stopped.exit_bid == pytest.approx(1.0980)
    assert stopped.exit_time_utc == entry + timedelta(minutes=5)
    assert stopped.gross_R == pytest.approx(-2.0)
    assert stopped.gross_R < -1.0

    short_after_gap = [
        m1(
            entry + timedelta(minutes=i),
            open_=1.1020 if i == 5 else 1.1000,
            high=1.1025 if i == 5 else 1.1005,
            low=1.0995,
            close=1.1000,
        )
        for i in range(5, 65)
    ]
    short_stopped = sut.simulate_signal(
        signal(entry, direction="SHORT", atr20_pips=10.0),
        [entry_row, *short_after_gap],
    )
    assert short_stopped.exit_reason == "GAP_STOP"
    assert short_stopped.exit_bid == pytest.approx(1.1020)
    assert short_stopped.gross_R == pytest.approx(-2.0)


def test_time_exit_uses_close_of_twelfth_complete_observed_bar() -> None:
    planned = datetime(2019, 1, 2, 10, 2, tzinfo=UTC)
    entry = datetime(2019, 1, 2, 10, 5, tzinfo=UTC)
    timed = sut.simulate_signal(signal(planned, direction="LONG", atr20_pips=10.0), minute_window(entry, open_=1.1000, close=1.1015))
    assert timed.exit_reason == "TIME"
    assert timed.entry_time_utc == entry
    assert timed.exit_time_utc == entry + timedelta(minutes=60)
...
def test_fixed_population_counts_are_exact_and_no_drop_is_accepted() -> None:
    assert sut.EXPECTED_SIGNAL_COUNTS == {"TRUE_0050": 1229, "SHIFTED_0025": 1220}
    exemplar = trade(datetime(2019, 1, 2, 10, 0, tzinfo=UTC), 0.1)
    exact = {
        "TRUE_0050": [exemplar] * 1229,
        "SHIFTED_0025": [exemplar] * 1220,
    }
    sut.validate_trade_counts(exact)
    with pytest.raises(sut.EngineeringInvalid, match="SHIFTED_0025 trade count 1219 != 1220"):
        sut.validate_trade_counts({**exact, "SHIFTED_0025": exact["SHIFTED_0025"][:-1]})


def test_mapper_index_is_built_once_before_trade_simulation(monkeypatch: pytest.MonkeyPatch) -> None:
    start = datetime(2019, 1, 2, 10, 0, tzinfo=UTC)
    monkeypatch.setattr(sut, "EXPECTED_SIGNAL_COUNTS", {"TRUE_0050": 1, "SHIFTED_0025": 1})
    signals = {
        "TRUE_0050": [signal(start, arm="TRUE_0050")],
        "SHIFTED_0025": [signal(start, arm="SHIFTED_0025")],
    }
    rows = minute_window(start)
    m1_by_time = {sut.parse_utc(row["time_utc"]): row for row in rows}
    real_builder = sut.build_observed_market_index
    calls = 0

    def counted_builder(source_rows):
        nonlocal calls
        calls += 1
        return real_builder(source_rows)

    monkeypatch.setattr(sut, "build_observed_market_index", counted_builder)
    trades = sut.simulate_all(signals, m1_by_time)
    assert calls == 1
    assert {arm: len(rows) for arm, rows in trades.items()} == {"TRUE_0050": 1, "SHIFTED_0025": 1}
...
def test_packet_and_registry_tamper_are_fail_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    registry["validation"].update({
        ...
        "parent_hyp004_terminal_path": sut.PARENT_HYP004_TERMINAL_REL,
        "parent_hyp004_terminal_sha256": sut.PARENT_HYP004_TERMINAL_SHA256,
...
def minimal_packet(root: Path) -> dict:
    return {
        ...
        "parent_hyp004_started_path": sut.PARENT_HYP004_STARTED_REL,
        "parent_hyp004_started_sha256": sut.PARENT_HYP004_STARTED_SHA256,
        "parent_hyp004_terminal_path": sut.PARENT_HYP004_TERMINAL_REL,
        "parent_hyp004_terminal_sha256": sut.PARENT_HYP004_TERMINAL_SHA256,
...
def setup_synthetic_workspace(root: Path, monkeypatch: pytest.MonkeyPatch, *, bad_after_start: bool = False) -> Path:
    ...
    monkeypatch.setattr(sut, "PARENT_HYP004_STARTED_REL", "hyp004/attempt_started.json")
    monkeypatch.setattr(sut, "PARENT_HYP004_TERMINAL_REL", "hyp004/attempt_terminal.json")
    ...
    monkeypatch.setattr(sut, "EVIDENCE_ROOT_REL", "evidence/HYP005-DESIGN-ECON-001")
    ...
    parent_started_sha = write_json(root / "hyp004/attempt_started.json", {
        "hypothesis_id": sut.PARENT_CANDIDATE,
        "attempt_id": "HYP004-DESIGN-ECON-001",
        "status": "STARTED",
    })
    parent_terminal_sha = write_json(root / "hyp004/attempt_terminal.json", {
        "hypothesis_id": sut.PARENT_CANDIDATE,
        "attempt_id": "HYP004-DESIGN-ECON-001",
        "status": "ENGINEERING_INVALID_NO_MARKET_VERDICT",
        "reason": "missing exact M1 entry window",
        "artifact_sha256": {
            "attempt_started.json": parent_started_sha,
        },
    })
    monkeypatch.setattr(sut, "PARENT_HYP004_STARTED_SHA256", parent_started_sha)
    monkeypatch.setattr(sut, "PARENT_HYP004_TERMINAL_SHA256", parent_terminal_sha)
    ...
    packet.update({
        ...
        "parent_hyp004_started_path": sut.PARENT_HYP004_STARTED_REL,
        "parent_hyp004_started_sha256": parent_started_sha,
        "parent_hyp004_terminal_path": sut.PARENT_HYP004_TERMINAL_REL,
        "parent_hyp004_terminal_sha256": parent_terminal_sha,
        "design_manifest_path": sut.DESIGN_MANIFEST_REL,
        "design_manifest_sha256": manifest_sha,
        "design_receipt_path": sut.DESIGN_RECEIPT_REL,
        "design_receipt_sha256": sut.DESIGN_RECEIPT_SHA256,
...
def test_full_synthetic_execution_writes_artifact_hash_chain(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert all("entry_time_utc" in row for row in trade_rows)
    ...
    assert terminal["execution_evidence_class"] == sut.EXECUTION_EVIDENCE_CLASS
    assert terminal["promotion_evidence"] is False
...
def test_failure_after_start_writes_failure_terminal(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        sut,
        "simulate_mapped_signal",
        lambda *_args, **_kwargs: pytest.fail("trade simulation ran before all signals mapped"),
    )
    with pytest.raises(sut.EngineeringInvalid, match="right-censored observed M5 horizon"):
        sut.execute_reviewed_design_economics(tmp_path, packet_path)
    evidence_root = tmp_path / sut.EVIDENCE_ROOT_REL
    terminal = json.loads((evidence_root / "attempt_terminal.json").read_text(encoding="utf-8"))
    assert terminal["status"] == "ENGINEERING_INVALID_NO_MARKET_VERDICT"
    assert terminal["execution_evidence_class"] == sut.EXECUTION_EVIDENCE_CLASS
    assert terminal["promotion_evidence"] is False
    assert terminal["artifact_sha256"]["attempt_started.json"] == sut.sha256_file(evidence_root / "attempt_started.json")
    assert set(path.name for path in evidence_root.iterdir()) == {"attempt_started.json", "attempt_terminal.json"}
