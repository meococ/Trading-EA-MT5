...
import evaluate_round_cascade_011_design_economics as sut
...
def hyp010_sealed_permissions() -> dict:
    return {
...
def hyp010_source_only_counters() -> dict:
    return {
...
def test_import_is_inert_and_workspace_root_is_required() -> None:
    ...
    assert sut.HYPOTHESIS_ID == "HYP-ROUND-CASCADE-EURUSD-M5-011"
    assert sut.PARENT_CANDIDATE == "HYP-ROUND-CASCADE-EURUSD-M5-010"
    assert sut.ATTEMPT_ID == "HYP011-DESIGN-ECON-001"
    assert sut.EXPECTED_SIGNAL_COUNTS == {"TRUE_0050": 1218, "SHIFTED_0025": 1213}
    assert sut.ELAPSED_WEEKS == 260.5714285714
    assert sut.COST_TIERS_PIPS == [1.50, 2.25, 3.00]
    assert sut.RISK_FRACTION == 0.0025
    assert sut.PLAN_REL.endswith("HYP-ROUND-CASCADE-EURUSD-M5-011_DESIGN_ECONOMICS_PLAN.md")
...
def test_module_ast_parses_and_sentinel_is_disarmed() -> None:
    ...
    assert sut.REVIEWED_RUN_PACKET_SHA256 is None
    assert sut.ELIGIBLE_LEDGER_SHA256 == "C6E054433E9A0D12ACAB3E88063E266E31407F387A1A9E21F0A681A5F4A3A6F9"
    assert sut.HYP010_REPORT_SHA256 == "8A4CBFF9D4CF8ED9917D41AFFAB80BB3BB8FB1E8743838CB25BC4F4CD0AC8D6C"
    assert sut.HYP010_RECEIPT_SHA256 == "7DF404716561D3C041D6A4C5763C8061D73F0AEEA7A180AFBFE26F38D2F63E0C"
    assert sut.HYP010_TERMINAL_SHA256 == "489819A783B5D59FA0ED14D9CA6DFEA235E0CB64D95BCBDD80B85796D135D1AC"
...
def test_fixed_eligible_population_counts_and_dsr_n_obs() -> None:
    ...
    exact = {
        "TRUE_0050": [exemplar] * 1218,
        "SHIFTED_0025": [exemplar] * 1213,
    }
    sut.validate_trade_counts(exact)
    with pytest.raises(sut.EngineeringInvalid, match="TRUE_0050 trade count 1217 != 1218"):
        sut.validate_trade_counts({**exact, "TRUE_0050": exact["TRUE_0050"][:-1]})
    assert sut.cadence_per_elapsed_week(1218) == pytest.approx(1218 / 260.5714285714)
...
def test_hyp010_chain_binding_before_price(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(sut, "ELIGIBLE_LEDGER_REL", "hyp010/eligible.jsonl")
    monkeypatch.setattr(sut, "HYP010_REPORT_REL", "hyp010/report.json")
    monkeypatch.setattr(sut, "HYP010_RECEIPT_REL", "hyp010/receipt.json")
    monkeypatch.setattr(sut, "HYP010_TERMINAL_REL", "hyp010/terminal.json")

    eligible_sha = write_jsonl(tmp_path / "hyp010/eligible.jsonl", [])
    monkeypatch.setattr(sut, "ELIGIBLE_LEDGER_SHA256", eligible_sha)

    report = {
        "hypothesis_id": sut.PARENT_CANDIDATE,
        "attempt_id": sut.HYP010_ATTEMPT_ID,
        "verdict": sut.HYP010_PASS_STATUS,
        "hyp011_drafting_authorized": True,
        "classification_sha256": sut.HYP010_CLASSIFICATION_SHA256,
        "actual_counts": {
            sut.ELIGIBLE_STATUS: {"TRUE_0050": 1218, "SHIFTED_0025": 1213},
        },
        "source_only_counters": hyp010_source_only_counters(),
        "sealed_permissions": hyp010_sealed_permissions(),
    }
    report_sha = write_json(tmp_path / "hyp010/report.json", report)
    monkeypatch.setattr(sut, "HYP010_REPORT_SHA256", report_sha)
    ...
    receipt = {
        ...
        "classification_sha256": sut.HYP010_CLASSIFICATION_SHA256,
        "artifact_sha256": {
            "attempt_started.json": sut.HYP010_STARTED_SHA256,
            "round_cascade_010_eligible_source_ledger.jsonl": eligible_sha,
            "round_cascade_010_execution_source_report.json": report_sha,
            "round_cascade_010_ineligible_source_ledger.jsonl": sut.HYP010_INELIGIBLE_LEDGER_SHA256,
        },
        "source_only_counters": hyp010_source_only_counters(),
        "sealed_permissions": hyp010_sealed_permissions(),
    }
    receipt_sha = write_json(tmp_path / "hyp010/receipt.json", receipt)
    monkeypatch.setattr(sut, "HYP010_RECEIPT_SHA256", receipt_sha)

    terminal = {
        "hypothesis_id": sut.PARENT_CANDIDATE,
        "attempt_id": sut.HYP010_ATTEMPT_ID,
        "status": sut.HYP010_PASS_STATUS,
        "hyp011_drafting_authorized": True,
        "classification_sha256": sut.HYP010_CLASSIFICATION_SHA256,
    ...
    terminal_sha = write_json(tmp_path / "hyp010/terminal.json", terminal)
    monkeypatch.setattr(sut, "HYP010_TERMINAL_SHA256", terminal_sha)

    packet = {
        "eligible_ledger_path": sut.ELIGIBLE_LEDGER_REL,
        "hyp010_report_path": sut.HYP010_REPORT_REL,
        "hyp010_receipt_path": sut.HYP010_RECEIPT_REL,
        "hyp010_terminal_path": sut.HYP010_TERMINAL_REL,
    }
    out = sut.validate_hyp010_chain_before_price(tmp_path, packet)
    assert out["eligible_ledger_sha256"] == eligible_sha
    assert out["hyp010_terminal_sha256"] == terminal_sha

    # Tamper report verdict.
    report["verdict"] = "KILL"
    write_json(tmp_path / "hyp010/report.json", report)
    ...
    with pytest.raises(sut.EngineeringInvalid, match="SHA256 mismatch"):
        sut.validate_hyp010_chain_before_price(tmp_path, packet)
...
def test_registry_tamper_is_fail_closed(tmp_path: Path) -> None:
    registry = {
        ...
        "validation": {
            ...
            "hyp010_terminal_path": sut.HYP010_TERMINAL_REL,
            "hyp010_terminal_sha256": sut.HYP010_TERMINAL_SHA256,
            "eligible_ledger_path": sut.ELIGIBLE_LEDGER_REL,
...
def minimal_packet(root: Path) -> dict:
    return {
        ...
        "eligible_ledger_sha256": sut.ELIGIBLE_LEDGER_SHA256,
        "hyp010_report_path": sut.HYP010_REPORT_REL,
        "hyp010_report_sha256": sut.HYP010_REPORT_SHA256,
        "hyp010_receipt_path": sut.HYP010_RECEIPT_REL,
        "hyp010_receipt_sha256": sut.HYP010_RECEIPT_SHA256,
        "hyp010_terminal_path": sut.HYP010_TERMINAL_REL,
        "hyp010_terminal_sha256": sut.HYP010_TERMINAL_SHA256,
...
def setup_synthetic_workspace(root: Path, monkeypatch: pytest.MonkeyPatch, *, bad_after_start: bool = False) -> Path:
    ...
    monkeypatch.setattr(sut, "EVIDENCE_ROOT_REL", "evidence/HYP011-DESIGN-ECON-001")
    ...
    # For synthetic small population, report counts still assert frozen HYP010 constants.
    # That would fail join path only if validate_hyp010 is called with production counts.
    # Keep production constants in report; synthetic execution uses monkeypatched join counts.
    report_sha = write_json(root / "hyp010/report.json", report)
    monkeypatch.setattr(sut, "HYP010_REPORT_SHA256", report_sha)
    source_receipt = {
        "hypothesis_id": sut.PARENT_CANDIDATE,
        "attempt_id": sut.HYP010_ATTEMPT_ID,
    ...
    receipt_sha = write_json(root / "hyp010/receipt.json", source_receipt)
    monkeypatch.setattr(sut, "HYP010_RECEIPT_SHA256", receipt_sha)
    terminal = {
        "hypothesis_id": sut.PARENT_CANDIDATE,
        "attempt_id": sut.HYP010_ATTEMPT_ID,
    ...
    terminal_sha = write_json(root / "hyp010/terminal.json", terminal)
    monkeypatch.setattr(sut, "HYP010_TERMINAL_SHA256", terminal_sha)
    ...
    packet.update({
        ...
        "hyp010_report_path": sut.HYP010_REPORT_REL,
        "hyp010_report_sha256": report_sha,
        "hyp010_receipt_path": sut.HYP010_RECEIPT_REL,
        "hyp010_receipt_sha256": receipt_sha,
        "hyp010_terminal_path": sut.HYP010_TERMINAL_REL,
        "hyp010_terminal_sha256": terminal_sha,
        "design_manifest_path": sut.DESIGN_MANIFEST_REL,
        "design_manifest_sha256": manifest_sha,
        "design_receipt_path": sut.DESIGN_RECEIPT_REL,
        "design_receipt_sha256": sut.DESIGN_RECEIPT_SHA256,
