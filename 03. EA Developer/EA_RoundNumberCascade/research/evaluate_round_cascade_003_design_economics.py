#!/usr/bin/env python3
"""Fail-closed DESIGN economics evaluator for HYP003.
...
PLAN_REL = (
    ...
    "HYP-ROUND-CASCADE-EURUSD-M5-003_DESIGN_ECONOMICS_PLAN.md"
)
FROZEN_PLAN_SHA256 = "2FF4F2D62D9AC9C3DFBB4BEF339832FAF67D9691BE60CC9CAB80C43C448CB366"
EVALUATOR_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "evaluate_round_cascade_003_design_economics.py"
)
TEST_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/tests/"
    "test_evaluate_round_cascade_003_design_economics.py"
)
RUN_PACKET_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "HYP-ROUND-CASCADE-EURUSD-M5-003_DESIGN_ECONOMICS_RUN_PACKET.json"
)
...
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "HYP-ROUND-CASCADE-EURUSD-M5-003_DESIGN_ECONOMICS_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
...
def validate_signal_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if row["hypothesis_id"] != PARENT_CANDIDATE:
...
def load_verified_dsr(workspace_root: Path):
    ...
    namespace: dict[str, Any] = {"__builtins__": __builtins__, "__name__": "_hyp003_verified_dsr"}
...
def validate_latest_registry_authority(workspace_root: Path, packet_sha256: str, packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        if row.get("hypothesis_id") == HYPOTHESIS_ID:
            latest = row
    if latest is None:
        raise EngineeringInvalid("missing HYP003 registry row")
...
def validate_hyp002_artifacts(workspace_root: Path, packet: dict[str, Any]) -> dict[str, str]:
    ...
    if receipt.get("hypothesis_id") != PARENT_CANDIDATE or receipt.get("verdict") != "PASS_SOURCE_FEASIBILITY":
...
def validate_design_receipt(workspace_root: Path) -> dict[str, str]:
    ...
    if not {"design_manifest_sha256", "source_sha256", "research_validation_opened", "research_holdout_opened"} <= set(receipt):
        raise EngineeringInvalid("DESIGN receipt schema mismatch")
    if receipt["design_manifest_sha256"] != DESIGN_MANIFEST_SHA256:
        raise EngineeringInvalid("DESIGN receipt manifest SHA mismatch")
    if receipt["source_sha256"] != PUBLIC_M1_SOURCE_SHA256:
        raise EngineeringInvalid("DESIGN receipt source SHA mismatch")
    if receipt["research_validation_opened"] is not False or receipt["research_holdout_opened"] is not False:
        raise EngineeringInvalid("DESIGN receipt sealed split mismatch")
    return {"design_manifest_sha256": sha256_bytes(manifest_payload), "design_receipt_sha256": DESIGN_RECEIPT_SHA256}
...
def load_manifest_entries(workspace_root: Path, required_dates: set[str]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        for key in ("date", "relative_path", "sha256", "bytes", "rows"):
            if key not in row:
...
def load_required_m1_rows(workspace_root: Path, signals_by_arm: dict[str, list[dict[str, Any]]]) -> dict[datetime, dict[str, Any]]:
    ...
    for entry in load_manifest_entries(workspace_root, required_dates):
        ...
        if len(payload) != int(entry["bytes"]):
            raise EngineeringInvalid(f"DESIGN shard byte count mismatch: {rel_path}")
        daily_rows = read_m1_shard(payload, shard_path, label=rel_path)
        if len(daily_rows) != int(entry["rows"]):
            raise EngineeringInvalid(f"DESIGN shard row count mismatch: {rel_path}")
...
def read_m1_shard(payload: bytes, path: Path, *, label: str) -> list[dict[str, Any]]:
    ...
    if suffix == ".parquet":
        ...
        table = pq.read_table(io.BytesIO(payload), columns=sorted(M1_KEYS))
        return [validate_m1_row(row) for row in table.to_pylist()]
