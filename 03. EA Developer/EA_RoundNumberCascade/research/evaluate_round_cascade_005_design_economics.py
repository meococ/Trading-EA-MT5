#!/usr/bin/env python3
"""Fail-closed DESIGN economics evaluator for HYP005.
...
PLAN_REL = (
    ...
    "HYP-ROUND-CASCADE-EURUSD-M5-005_DESIGN_ECONOMICS_PLAN_V2.md"
)
FROZEN_PLAN_SHA256 = "55725CCCC23838F20DE5495E2876590A1EE0292A42EE4ED8A73F50163F27CA21"
EVALUATOR_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "evaluate_round_cascade_005_design_economics.py"
)
TEST_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/tests/"
    "test_evaluate_round_cascade_005_design_economics.py"
)
RUN_PACKET_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "HYP-ROUND-CASCADE-EURUSD-M5-005_DESIGN_ECONOMICS_RUN_PACKET.json"
)
...
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/"
    "HYP-ROUND-CASCADE-EURUSD-M5-005_DESIGN_ECONOMICS_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
PARENT_HYP004_EVIDENCE_ROOT_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/evidence/"
    "HYP-ROUND-CASCADE-EURUSD-M5-004_DESIGN_ECONOMICS/"
    "HYP004-DESIGN-ECON-001"
)
PARENT_HYP004_STARTED_REL = f"{PARENT_HYP004_EVIDENCE_ROOT_REL}/attempt_started.json"
PARENT_HYP004_TERMINAL_REL = f"{PARENT_HYP004_EVIDENCE_ROOT_REL}/attempt_terminal.json"
...
ATTEMPT_ID = "HYP005-DESIGN-ECON-001"
EVIDENCE_ROOT_REL = (
    "03. EA Developer/EA_RoundNumberCascade/research/evidence/"
    "HYP-ROUND-CASCADE-EURUSD-M5-005_DESIGN_ECONOMICS/"
    f"{ATTEMPT_ID}"
)
PARENT_HYP004_STARTED_SHA256 = "46ADCB275482CD496D8C6DE6B1803FD735D9EC3E128D93F87540C47FFE80527F"
PARENT_HYP004_TERMINAL_SHA256 = "899700298BA910A1FF4D29FD9B4B95144835BB10BF23FAFE537224A957F6AED5"
...
def map_signal_to_market(
    signal: dict[str, Any], market: ObservedMarketIndex
) -> MappedSignal:
    checked = validate_signal_row(_serialize_signal_times(signal))
    planned = checked["planned_entry_time_utc"]
    entry_candidate_index = bisect_left(market.aligned_entry_times, planned)
    if entry_candidate_index >= len(market.aligned_entry_times):
        raise EngineeringInvalid("no aligned observed M1 row at or after planned entry")
    entry_time = market.aligned_entry_times[entry_candidate_index]
    delay = entry_time - planned
    if delay < timedelta(0):
        raise EngineeringInvalid("negative observed M1 entry delay")
    if delay > timedelta(minutes=60):
        raise EngineeringInvalid("entry delay exceeds 60 minutes")

    complete_index = bisect_left(market.complete_m5_starts, entry_time)
...
def validate_run_packet(packet: dict[str, Any], packet_sha256: str, workspace_root: Path | None = None) -> None:
    ...
    required = {
        ...
        "parent_hyp004_started_path": PARENT_HYP004_STARTED_REL,
        "parent_hyp004_started_sha256": PARENT_HYP004_STARTED_SHA256,
        "parent_hyp004_terminal_path": PARENT_HYP004_TERMINAL_REL,
        "parent_hyp004_terminal_sha256": PARENT_HYP004_TERMINAL_SHA256,
...
def validate_latest_registry_authority(workspace_root: Path, packet_sha256: str, packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        if row.get("hypothesis_id") == HYPOTHESIS_ID:
            latest = row
    if latest is None:
        raise EngineeringInvalid("missing HYP005 registry row")
...
def validate_hyp004_parent_terminal(workspace_root: Path, packet: dict[str, Any]) -> dict[str, str]:
    bindings = {
        "parent_hyp004_started": (packet["parent_hyp004_started_path"], PARENT_HYP004_STARTED_SHA256),
        "parent_hyp004_terminal": (packet["parent_hyp004_terminal_path"], PARENT_HYP004_TERMINAL_SHA256),
    }
    out: dict[str, str] = {}
    for name, (rel_path, expected_sha) in bindings.items():
        out[f"{name}_sha256"] = sha256_bytes(read_verified_bytes_once(resolve_workspace_file(workspace_root, rel_path), expected_sha))
    started = load_json_file(
        resolve_workspace_file(workspace_root, packet["parent_hyp004_started_path"]),
        PARENT_HYP004_STARTED_SHA256,
        label="HYP004 started",
    )
    terminal = load_json_file(
        resolve_workspace_file(workspace_root, packet["parent_hyp004_terminal_path"]),
        PARENT_HYP004_TERMINAL_SHA256,
        label="HYP004 terminal",
    )
    if started.get("hypothesis_id") != PARENT_CANDIDATE or started.get("attempt_id") != "HYP004-DESIGN-ECON-001":
        raise EngineeringInvalid("HYP004 started identity mismatch")
    if terminal.get("hypothesis_id") != PARENT_CANDIDATE or terminal.get("attempt_id") != "HYP004-DESIGN-ECON-001":
        raise EngineeringInvalid("HYP004 terminal identity mismatch")
    if terminal.get("status") != "ENGINEERING_INVALID_NO_MARKET_VERDICT":
        raise EngineeringInvalid("HYP004 terminal is not engineering-invalid")
    reason = str(terminal.get("reason", ""))
    if reason != "missing exact M1 entry window":
        raise EngineeringInvalid("HYP004 terminal failure reason mismatch")
    artifact_hashes = terminal.get("artifact_sha256")
    if artifact_hashes != {"attempt_started.json": PARENT_HYP004_STARTED_SHA256}:
        raise EngineeringInvalid("HYP004 terminal artifact hash-chain mismatch")
    return out
...
def load_required_m1_rows(workspace_root: Path, signals_by_arm: dict[str, list[dict[str, Any]]]) -> dict[datetime, dict[str, Any]]:
    # HYP005 mapping permits gaps inside the 12-observed-bar horizon. The next
    # complete bar can therefore live in a later shard; preload the immutable
    # public DESIGN manifest exactly once instead of scanning per signal.
...
def simulate_all(signals_by_arm: dict[str, list[dict[str, Any]]], m1_by_time: dict[datetime, dict[str, Any]]) -> dict[str, list[TradeResult]]:
    ...
    # Complete the all-or-nothing source mapping gate before simulating a
    # single trade outcome or invoking any economics/DSR calculation.
    for arm, signals in signals_by_arm.items():
        for signal in signals:
            mapped_by_arm[arm].append(
                (signal, map_signal_to_market(signal, market))
            )
...
def execute_reviewed_design_economics(workspace_root: Path, run_packet_path: Path) -> int:
    ...
    validate_hyp004_parent_terminal(workspace_root, packet)
