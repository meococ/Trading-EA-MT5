//+------------------------------------------------------------------+
//| EA_M15LNYDualWindow.mq5 — London bias, dual PB windows           |
//| Symbol: USDJPY | Period: M15 | Magic: 880983                     |
//|                                                                   |
//| Hypothesis: HYP-LNY-DUAL-WINDOW-001                                |
//| LondonNY-class thick expectancy with STRUCTURAL cadence expand:  |
//| same London ATR bias (closed-bar), pullback entry in BOTH        |
//| late-London [12,15) AND NY [15,18). MaxPerDay=2. NOT S530        |
//| Mon/Wed day-mine. Not MaxKZ/RR/USBILL rescue. Risk 0.5% a priori.|
//| All Mon–Fri (S529-class day set); closed-bar[1] decisions.       |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15LNYDualWindow"
...
input group "=== London Trend (closed-bar) ==="
input int      InpLdnStartH     = 9;
input int      InpLdnMeasureH   = 12;
input double   InpTrendATR_Mult = 0.50;   // same family as S529 thick edge
input int      InpATR_Period    = 14;     // D1 ATR

input group "=== Dual Pullback Windows ==="
input int      InpWin1StartH    = 12;     // late London
input int      InpWin1EndH      = 15;
input int      InpWin2StartH    = 15;     // NY AM
input int      InpWin2EndH      = 18;
input int      InpPB_Lookback   = 3;
input double   InpPB_MinATR     = 0.15;
input double   InpPB_MaxATR     = 0.60;

input group "=== Days (NOT S530 skip) ==="
input bool     InpTradeMon      = true;
input bool     InpTradeTue      = true;
input bool     InpTradeWed      = true;
input bool     InpTradeThu      = true;
input bool     InpTradeFri      = true;   // S529-class; no Mon/Wed veto

input group "=== Risk (thick) ==="
input double   InpRiskPct       = 0.50;
input double   InpMaxLot        = 1.0;
input double   InpSL_ATR_Mult   = 0.50;
input double   InpRR_Ratio      = 2.00;
input int      InpMinSLPoints   = 50;
input int      InpMaxSLPoints   = 1500;
input int      InpMaxPerDay     = 2;      // structural: one per window max
...
double   g_londonDir = 0;     // +1/-1/0
bool     g_biasSet = false;
double   g_londonOpen = 0;
bool     g_usedWin1 = false;
bool     g_usedWin2 = false;
...
int OnInit()
{
   ...
   Print("[LNYD] HYP-LNY-DUAL-WINDOW-001 | dual [12,15)+[15,18) MaxPerDay=2");
...
void UpdateLondonBias(const int hour)
{
   // Capture London open from closed bar that opened at London start hour
   if(!g_biasSet && g_londonOpen <= 0.0 && hour == InpLdnStartH)
   {
      // bar[1] is the just-closed M15; if it opened in London start hour, use its open
      MqlDateTime odt;
      datetime ot = iTime(_Symbol, PERIOD_CURRENT, 1);
      if(ot > 0)
      {
         TimeToStruct(ot, odt);
         if(odt.hour == InpLdnStartH && odt.min == 0)
            g_londonOpen = iOpen(_Symbol, PERIOD_CURRENT, 1);
      }
   }

   // Fallback: scan today's bars for first bar at LdnStartH:00 once measure hour arrives
   if(!g_biasSet && g_londonOpen <= 0.0 && hour >= InpLdnMeasureH)
   {
      for(int i = 1; i < 96; i++)
      {
         ...
         MqlDateTime x;
         TimeToStruct(t, x);
         if(x.year * 1000 + x.day_of_year != g_lastTradeDay)
            break;
         if(x.hour == InpLdnStartH && x.min == 0)
         {
            g_londonOpen = iOpen(_Symbol, PERIOD_CURRENT, i);
            break;
         }
      }
   }

   if(g_biasSet || g_londonOpen <= 0.0)
      return;
   if(hour < InpLdnMeasureH)
      return;

   // Measure on first closed bar at/after measure hour
   double close1 = iClose(_Symbol, PERIOD_CURRENT, 1);
   double atr[];
   ArraySetAsSeries(atr, true);
   if(CopyBuffer(g_hATR_D1, 0, 1, 1, atr) < 1 || atr[0] <= 0.0)
      return;

   double move = close1 - g_londonOpen;
   double thr = atr[0] * InpTrendATR_Mult;
   if(move > thr)
      g_londonDir = 1;
   else if(move < -thr)
      g_londonDir = -1;
...
bool PullbackValid(const double atr, double &pbExtreme)
{
   pbExtreme = 0;
   double close1 = iClose(_Symbol, PERIOD_CURRENT, 1);
   ...
   if(depth < atr * InpPB_MinATR || depth > atr * InpPB_MaxATR)
      return false;

   if(g_londonDir > 0 && close1 > open1)
   {
      pbExtreme = recentLow;
      return true;
   }
...
void OnTick()
{
   ...
   if(CountPositions() > 0)
   {
      if(dt.hour >= InpFlatHour || dt.day_of_week == 0 || dt.day_of_week == 6)
         CloseAll("flat");
      return;
   }
   ...
   UpdateLondonBias(dt.hour);

   if(!g_biasSet || g_londonDir == 0.0)
      return;
   ...
   double pbExtreme = 0;
   if(!PullbackValid(atr[0], pbExtreme))
      return;

   bool isBuy = (g_londonDir > 0);
   ...
   double tp = (isBuy ? entry + slDist * InpRR_Ratio : entry - slDist * InpRR_Ratio);
   ...
   string tag = (win == 1 ? "LNYD|w1" : "LNYD|w2");
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, tag, res))
      return;
   if(IsSuccessfulRetcode(res.retcode))
   {
      g_tradesToday++;
      if(win == 1) g_usedWin1 = true;
      if(win == 2) g_usedWin2 = true;
   }
}
