//+------------------------------------------------------------------+
//| EA_OnlineRidge_EUR_H1_V12ML.mq5                                 |
//| HYP-ORLS-EURUSD-H1-001: causal delayed-label online ridge       |
//+------------------------------------------------------------------+
#property strict
...
input string InpHypothesisId="HYP-ORLS-EURUSD-H1-001";
...
const string EA_NAME="EA_OnlineRidge_EUR_H1_V12ML";
const string EXPECTED_HYPOTHESIS="HYP-ORLS-EURUSD-H1-001";
...
bool LoadRawFeatures(const MqlTick &q,double &bar_open,double &atr14,double &spread_points,double &spread_return,double &baseline_score,double &raw[])
  {
   MqlRates r[];ArraySetAsSeries(r,true);const int need=25;if(CopyRates(_Symbol,PERIOD_H1,0,need,r)!=need){g_skip_missing++;return(false);}
...
bool SubmitEntry(const ModelSignal &s)
  {
   ...
   PrintFormat("ORLS001_ENTRY decision=%I64d direction=%s volume=%.2f entry=%.5f sl=%.5f score=%.9f hurdle=%.9f baseline=%.9f atr14=%.6f spread_points=%.1f risk_cash=%.2f margin_usage_pct=%.4f",(long)s.decision_time,(s.direction>0?"LONG":"SHORT"),volume,g_entry_price,sl,s.score,s.hurdle,s.baseline_score,s.atr14,spread,g_entry_risk_cash,g_entry_margin_pct);return(true);
  }
