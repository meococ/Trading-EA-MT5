//+------------------------------------------------------------------+
//| EA_SchaffTrendCycle.mq5                                         |
//| HYP-STC-EURUSD-M15-001: classic STC trend-cycle continuation    |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Untuned EURUSD M15 Schaff Trend Cycle continuation baseline"

input group "--- Frozen research authority ---"
input bool   InpResearchAutoMode=false;
input bool   InpEnableTelemetry=false;
input string InpHypothesisId="HYP-STC-EURUSD-M15-001";
input string InpVariantTag="STC_CLASSIC_TREND_CYCLE_V1";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5603701;
...
const string EA_NAME="EA_SchaffTrendCycle";
const string EXPECTED_HYPOTHESIS="HYP-STC-EURUSD-M15-001";
const string EXPECTED_VARIANT="STC_CLASSIC_TREND_CYCLE_V1";
...
bool PreloadIndicatorState()
  {
   long synchronized=0;
   if(!SeriesInfoInteger(_Symbol,PERIOD_M15,SERIES_SYNCHRONIZED,synchronized) ||
      synchronized!=1)
      return(false);
   ...
   if(copied!=preload_count || rates[0].time!=STC_PRELOAD_FIRST ||
      rates[copied-1].time!=last_closed)
      return(false);
   ResetIndicatorState();
   ...
   return(g_indicator_bar_time==last_closed && IsUsable(g_previous_stc) &&
...
bool ClosePositionTicket(const ulong ticket,const string reason)
  {
   ...
   if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
      PrintFormat("STC001_CLOSE_REJECT ticket=%I64u retcode=%u error=%d reason=%s",
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   request.deviation=InpDeviationPoints;
   request.type_filling=ResolveFilling();
   request.comment="STC001";
   if(!OrderCheck(request,check) || check.retcode!=0)
     {
      PrintFormat("STC001_ORDER_CHECK_REJECT decision=%I64d retcode=%u comment=%s",
                  (long)signal.decision_time,check.retcode,check.comment);
      return(false);
     }
   if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
      PrintFormat("STC001_ORDER_SEND_REJECT decision=%I64d retcode=%u error=%d",
                  (long)signal.decision_time,result.retcode,GetLastError());
      return(false);
     }
   g_entries_accepted++;
   g_daily_entries++;
   PrintFormat("STC001_ENTRY decision=%I64d availability=%I64d direction=%d volume=%.2f entry=%.5f sl=%.5f tp=%.5f prev_stc=%.8f stc=%.8f macd=%.8f atr=%.8f",
...
void ExecuteSignal(const SignalDecision &signal,const datetime now)
  {
   PrintFormat("STC001_SIGNAL decision=%I64d availability=%I64d direction=%d prev_stc=%.8f stc=%.8f macd=%.8f atr=%.8f",
               (long)signal.decision_time,(long)signal.availability_time,
...
bool ValidateFrozenInputs()
  {
   return(_Symbol==EXPECTED_SYMBOL && _Period==PERIOD_M15 &&
          ...
          InpMagic==5603701 && MathAbs(InpRiskPercent-0.25)<1e-12 &&
...
int OnInit()
  {
   if(!ValidateFrozenInputs())
     {
      Print("STC001_FATAL reason=FROZEN_INPUT_OR_SYMBOL_CONTRACT");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(!CurrentM15Open(g_last_bar_open))
     {
      Print("STC001_FATAL reason=INITIAL_M15_CLOCK");
      return(INIT_FAILED);
     }
   if(!PreloadIndicatorState())
     {
      Print("STC001_FATAL reason=INDICATOR_PRELOAD");
      return(INIT_FAILED);
     }
   const double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   if(!IsUsable(equity) || equity<=0.0)
     {
      Print("STC001_FATAL reason=INITIAL_EQUITY");
      return(INIT_FAILED);
     }
   g_daily_day_key=DateKey(TimeCurrent());
   g_daily_start_equity=equity;
   g_peak_equity=equity;
   PrintFormat("STC001_INIT hypothesis=%s variant=%s symbol=%s period=%d",
...
void OnDeinit(const int reason)
  {
   PrintFormat("STC001_SUMMARY hypothesis=%s closed_bars=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d runtime_failed=%s reason=%d",
...
void OnTick()
  {
   ...
   if(current_open<g_last_bar_open)
     {
      g_runtime_failed=true;
      Print("STC001_FATAL reason=BAR_CLOCK_REGRESSION");
      return;
     }
