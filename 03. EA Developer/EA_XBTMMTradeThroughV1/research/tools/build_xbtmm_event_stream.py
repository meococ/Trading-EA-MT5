...
def build(quote_path: Path, trade_path: Path, out_path: Path, manifest_path: Path, symbol: str) -> dict:
            ...
            if take_trade:
                ...
                output.write(
                    RECORD.pack(
                        ...
                        current_quote["bid"],
                        current_quote["ask"],
                        current_quote["bid_size"],
                        current_quote["ask_size"],
                        event["price"],
                        event["size"],
                        event["side"],
                    )
    ...
    source_gate_pass = within_day and boundary_coverage and invalid_quote_ratio <= MAX_INVALID_QUOTE_RATIO

    payload = {
        "schema_version": "xbtmm_event_stream_manifest.v2",
        "symbol": symbol,
        "utc_day": utc_day,
