...
EA_SOURCE = PACKAGE / "EA_XBTMMTradeThroughV1.mq5"
PREREG = PACKAGE / "research" / "HYP-XBT-MM-TRADETHROUGH-001_FROZEN_DESIGN_PREREG.md"
...
def test_mql5_freezes_strict_trade_through_and_latency_boundaries() -> None:
    ...
    assert "time_us>=state.bid_order.live_us" in source
    assert "time_us>=state.ask_order.live_us" in source
...
def test_quote_expiry_precedes_every_trade_match_and_does_not_consume_qvr() -> None:
    ...
    assert "order.expiry_us=g_last_quote_us+MAX_QUOTE_AGE_US;" in source
...
def test_current_preflight_and_prereg_keep_future_splits_sealed() -> None:
    manifest_path = PACKAGE / "research" / "preflight" / "HYP-XBT-MM-TRADETHROUGH-001" / "20180101_event_manifest.json"
