...
HYPOTHESIS_ID = "HYP-IRSK-XAUUSD-M5-001"
ATTEMPT_ID = "IRSK001-SOURCE-001"
...
PREREG_PATH = RESEARCH_DIR / "HYP-IRSK-XAUUSD-M5-001_FROZEN_SOURCE_PREREG.md"
REVIEW_PATH = RESEARCH_DIR / "HYP-IRSK-XAUUSD-M5-001_PRE_SOURCE_REVIEW.md"
TEST_PATH = RESEARCH_DIR / "tests/test_analyze_irsk_source.py"
...
def claim_attempt() -> None:
    ...
    write_exclusive(START_PATH, json_bytes({
        "schema_version": "irsk_source_attempt_started.v1",
...
def measure_session(group: pd.DataFrame) -> dict | None:
    ...
    realized_skewness = third_moment / (second_moment ** 1.5)
    session_return = math.log(float(closes[-1]) / float(closes[0]))
    if not math.isfinite(realized_skewness) or not math.isfinite(session_return):
        return None
    return {
        ...
        "return_mean": mean,
        "return_second_moment": second_moment,
        "return_third_moment": third_moment,
    }


def select_direction(measure: dict) -> str | None:
    if measure["session_return"] < 0.0 and measure["realized_skewness"] > 0.0:
        return "LONG"
    if measure["session_return"] > 0.0 and measure["realized_skewness"] < 0.0:
        return "SHORT"
    return None
...
def extract_events(frame: pd.DataFrame) -> tuple[list[dict], dict]:
    ...
    for day, group in intraday.groupby("utc_date", sort=True):
        ...
        raw.append({
            ...
            "direction": direction,
            "realized_skewness": measure["realized_skewness"],
            "session_return": measure["session_return"],
            "return_mean": measure["return_mean"],
            "return_second_moment": measure["return_second_moment"],
            "return_third_moment": measure["return_third_moment"],
...
def analyze(frame: pd.DataFrame, validation: dict) -> tuple[dict, list[dict]]:
    ...
    return {
        "schema_version": "irsk_source_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "outcomes_opened": False,
        "economics_evaluated": False,
        "parameters": {"session_utc": "00:00-15:55", "shape": "population_realized_skewness",
                       "event": "strict_price_skewness_divergence_fade"},
...
def execute() -> dict:
    ...
    try:
        ...
        write_exclusive(RECEIPT_PATH, json_bytes({
            "schema_version": "irsk_source_attempt_receipt.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
        ...
        write_exclusive(TERMINAL_PATH, json_bytes({
            "schema_version": "irsk_source_attempt_terminal.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
            "status": "COMPLETE",
