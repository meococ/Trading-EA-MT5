...
BOUNDED_DIFF_PROOF = (
    PACKAGE / "research" / f"{HYPOTHESIS_ID}_V10_V11_TELEMETRY_DIFF_PROOF.md"
)
COMPACT_TELEMETRY_TEST = (
    PACKAGE / "research" / "tests" / "test_stbs024_compact_telemetry_contract.py"
)
PARENT_PACKAGE = ROOT / "03. EA Developer" / "EA_SupertrendBurstScalperTradeV10"
PARENT_FAILURE_PACKET = (
    PARENT_PACKAGE / "research" / "HYP-STBS-XAUUSD-M15-023_FAILURE_PACKET.json"
)
PARENT_POST_FAILURE_REVIEW = (
    PARENT_PACKAGE / "research" / "HYP-STBS-XAUUSD-M15-023_POST_FAILURE_REVIEW.md"
)
...
RESERVED_POST_PACKET_REVIEW_STATUS_LINE = (
    '?? "03. EA Developer/EA_SupertrendBurstScalperTradeV11/research/'
    'HYP-STBS-XAUUSD-M15-024_POST_PACKET_REVIEW.md"'
)
PACKET_ATTEMPT_ROOT = (
    PACKAGE / "research" / "evidence" / HYPOTHESIS_ID / "STBS024-PACKET-BUILD-001"
)
...
PACKET_ATTEMPT_ID = "STBS024-PACKET-BUILD-001"
MT5_ATTEMPT_ID = "STBS024-MODEL0-TRAIN-001"
PROBE_FALSE_FIELDS = (
...
EXPECTED_SOURCE_SHA256 = "7CC7A9D7C30216A1669D84AEEA867E32EA15F2E9E8C195D171BD574A4D2EB0BC"
EXPECTED_PREREG_SHA256 = "092AFD8CFEEF4640635DC684FEA6B6946E0AFB7D4E9CF0823DD5B6531A5D6D97"
EXPECTED_CONTRACT_SHA256 = "4A2BAB503A148C78A85348B70340DDB0CDBBF31CD5941472BE0836BDE00A9578"
EXPECTED_COST_SHA256 = "7167F5066D90B9CA7BA935F5B5BDC87E243F348E21958ABE283599E20C552676"
EXPECTED_GITIGNORE_SHA256 = "E25255A715C77C9F04212B9510E21E8C7AEDE716E97700E13B4C8E29DC76503C"
EXPECTED_RESERVED_POST_PACKET_REVIEW_SHA256 = (
    "A492ED6A74E26B3E606A78E3361C87C97575271A09AA32CF0AB1A0586BFD4EC5"
)
EXPECTED_PARENT_TERMINAL_ROW_SHA256 = (
    "2CA6AD8D565F0670E18DFBD58EA17CEE93372B18D1F06C27216288E49D6FA6B3"
)
EXPECTED_PARENT_TERMINAL_VERDICT = (
    "KILL_ENGINEERING_JOURNAL_DELTA_TRUNCATED_UNTYPED_POST_MT5_NO_ECONOMIC_VERDICT"
)
EXPECTED_PARENT_FAILURE_PACKET_SHA256 = (
    "7A70CAAC8421B158D26CAD074BC81EBAABFD457B11717845FF04DA524AA319A8"
)
EXPECTED_PARENT_POST_FAILURE_REVIEW_SHA256 = (
    "5906FE93BAE9FA6C700F57D1DC490F0C4FA319276E6D2DED9AA8667A6A7F6FC5"
)
...
EXPECTED_JOURNAL_BUDGET_ADDENDUM_SHA256 = (
    "C0AD425D1F368A41FE740E3EC22D1CF823F088005848BF14789C6EFFDD3AEF21"
)
EXPECTED_BOUNDED_DIFF_PROOF_SHA256 = (
    "0B95D49C9D2234ED790E6E09F8AF800C809B499B5366CB451CDD26EDC7732F34"
)
EXPECTED_COMPACT_TELEMETRY_TEST_SHA256 = (
    "C6CB1D4CE000DF09B8478E1A64FA30542E5D38E5AF84C834A6834FD2EE5A4CB0"
)
OVERRIDES = (
    "InpAuditOnly=false;InpEnableTelemetry=true;"
    "InpHypothesisId=HYP-STBS-XAUUSD-M15-024;InpMagic=5604124;"
...
def latest_registry_identity(registry_raw: bytes | None = None) -> tuple[str, str, dict]:
    ...
    if row.get("hypothesis_id") != HYPOTHESIS_ID or row.get("state") != "probe":
        raise RuntimeError("latest registry row is not the frozen HYP024 packet-only probe authority")
    if (
        row.get("source_hash") != EXPECTED_SOURCE_SHA256
        or row.get("prereg_sha256") != EXPECTED_PREREG_SHA256
        or row.get("parent_candidate") != "HYP-STBS-XAUUSD-M15-023"
    ):
        raise RuntimeError("HYP024 authority source/prereg binding is invalid")
    if row.get("verdict") != "FROZEN_HYP024_PACKET_BUILDER_SEMANTIC_AUTHORITY":
        raise RuntimeError("latest HYP024 authority has the wrong packet-builder verdict")
    metrics = row.get("metrics")
    validation = row.get("validation")
    if not isinstance(metrics, dict) or not isinstance(validation, dict):
        raise RuntimeError("HYP024 packet-builder authority metrics/validation are absent")
    if row.get("run_ids") != [] or any(
        metrics.get(name) != expected for name, expected in PROBE_ZERO_METRICS.items()
    ):
        raise RuntimeError("HYP024 packet-builder authority counters are not pristine")
    if (
        ...
        or validation.get("parent_hyp023_terminal_row_sha256") != EXPECTED_PARENT_TERMINAL_ROW_SHA256
        or validation.get("parent_hyp023_terminal_verdict") != EXPECTED_PARENT_TERMINAL_VERDICT
        or validation.get("parent_hyp023_failure_packet_path") != repo_relative(PARENT_FAILURE_PACKET)
        or validation.get("parent_hyp023_failure_packet_sha256") != EXPECTED_PARENT_FAILURE_PACKET_SHA256
        or validation.get("parent_hyp023_post_failure_review_path") != repo_relative(PARENT_POST_FAILURE_REVIEW)
        or validation.get("parent_hyp023_post_failure_review_sha256") != EXPECTED_PARENT_POST_FAILURE_REVIEW_SHA256
        ...
        raise RuntimeError(
            "HYP024 packet-builder authority is not exact packet-only authority"
        )
    parent_raw = None
    parent_row = None
    for candidate_raw in reversed(rows[:-1]):
        candidate = strict_json_loads(candidate_raw)
        if candidate.get("hypothesis_id") == "HYP-STBS-XAUUSD-M15-023":
            parent_raw = candidate_raw
            parent_row = candidate
            break
    if parent_raw is None or parent_row is None:
        raise RuntimeError("terminal HYP023 parent row is absent from the same registry snapshot")
    ...
    if (
        ...
        raise RuntimeError("terminal HYP023 parent row is not the exact frozen engineering kill")
...
def main() -> None:
    ...
    try:
        ...
        "parent_hypothesis_id": "HYP-STBS-XAUUSD-M15-023",
