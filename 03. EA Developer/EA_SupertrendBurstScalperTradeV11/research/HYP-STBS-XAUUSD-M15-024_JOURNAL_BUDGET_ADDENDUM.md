# HYP024 journal-budget engineering addendum

## Informing failure

HYP023 is terminally killed only for truncated journal evidence. Its sealed failure packet and post-failure review remain unchanged. This addendum records the later full-source-log measurement used to scope HYP024; it does not reopen HYP023 outcomes.
...
Removing the per-volume print alone would still exceed the former combined one-MiB raw cap. Raising the cap alone would retain more than 17 MiB of spam and also fail. HYP024 therefore couples two engineering-only changes:
...
2. Freeze an explicit combined raw journal cap of `4,194,304` bytes through task packet, receipt, AlphaFactory normalized manifest and HYP024 runner. Other hypotheses that omit the field retain the one-MiB default.
