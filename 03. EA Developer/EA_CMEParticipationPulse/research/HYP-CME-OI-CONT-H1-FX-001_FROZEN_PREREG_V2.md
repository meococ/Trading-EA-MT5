# HYP-CME-OI-CONT-H1-FX-001 — Frozen pre-outcome preregistration V2

State: `FROZEN_PROBE_PENDING`  
Frozen at: 2026-07-16 before any price outcome was evaluated for this mechanism.

Pre-outcome operational correction: after the source profile passed and before
any MT5 price bar was loaded, the spot anchor and same-day exit were moved from
21:00 to 20:00 UTC so the exact H1 bar exists across Friday/DST close regimes.
The year-concentration cap is 45% in the four-year train and 70% in the
two-year internal validation; a 45% cap is mathematically impossible in a
two-year split. No economic direction, threshold, symbol, cost or outcome was
...
4. Compute the selected MT5 spot symbol's prior public move from the H1 open at
   20:00 UTC on `t_prev` to the H1 open at 20:00 UTC on `t`. Zero move skips.
...
6. Initial stop is `1.5 * ATR(14)` from fully closed H1 bars at entry. Exit at
   the stop or the 20:00 UTC H1 open on the same UTC date, whichever occurs
...
On every date where the candidate has at least one positive-OI eligible symbol,
ignore CME OI and select the symbol with the largest absolute prior public spot
move measured in standard pips across the same three-symbol universe. Trade
that move's direction with identical entry, stop, exit, risk and cost handling.
Thus candidate and control have matched dates and maximum trade count. This is
the price-only continuation control.
...
- no single calendar year may contribute more than 45% of positive x1 R in
  train or 70% in internal validation.
