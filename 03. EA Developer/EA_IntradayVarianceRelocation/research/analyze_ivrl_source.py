...
HYPOTHESIS_ID = "HYP-IVRL-XAUUSD-M5-001"
ATTEMPT_ID = "IVRL001-SOURCE-001"
...
PREREG_PATH = RESEARCH_DIR / "HYP-IVRL-XAUUSD-M5-001_FROZEN_SOURCE_PREREG.md"
REVIEW_PATH = RESEARCH_DIR / "HYP-IVRL-XAUUSD-M5-001_PRE_SOURCE_REVIEW.md"
TEST_PATH = RESEARCH_DIR / "tests/test_analyze_ivrl_source.py"
...
def claim_attempt() -> None:
    ...
    write_exclusive(START_PATH, json_bytes({
        "schema_version": "ivrl_source_attempt_started.v1",
...
def measure_session(group: pd.DataFrame) -> dict | None:
    ...
    early_returns = returns[:95]
    late_returns = returns[95:]
    if len(early_returns) != 95 or len(late_returns) != 96:
        return None
    early_mean_squared_return = float(np.mean(early_returns ** 2))
    late_mean_squared_return = float(np.mean(late_returns ** 2))
    late_session_return = math.log(float(closes[-1]) / float(closes[95]))
    if not all(math.isfinite(value) for value in (
        early_mean_squared_return, late_mean_squared_return, late_session_return
    )):
        return None
    return {
        "decision_index": int(ordered.index[-1]),
        "early_mean_squared_return": early_mean_squared_return,
        "late_mean_squared_return": late_mean_squared_return,
        "late_session_return": late_session_return,
        "early_return_count": 95,
        "late_return_count": 96,
    }


def select_direction(measure: dict) -> str | None:
    if measure["late_mean_squared_return"] <= measure["early_mean_squared_return"]:
        return None
    if measure["late_session_return"] > 0.0:
        return "LONG"
    if measure["late_session_return"] < 0.0:
        return "SHORT"
    return None
...
def extract_events(frame: pd.DataFrame) -> tuple[list[dict], dict]:
    ...
    for day, group in intraday.groupby("utc_date", sort=True):
        ...
        direction = select_direction(measure)
        ...
        raw.append({
            ...
            "direction": direction,
            "early_mean_squared_return": measure["early_mean_squared_return"],
            "late_mean_squared_return": measure["late_mean_squared_return"],
            "late_session_return": measure["late_session_return"],
            "early_return_count": measure["early_return_count"],
            "late_return_count": measure["late_return_count"],
...
def analyze(frame: pd.DataFrame, validation: dict) -> tuple[dict, list[dict]]:
    ...
    return {
        "schema_version": "ivrl_source_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
        "outcomes_opened": False,
        "economics_evaluated": False,
        "parameters": {
            "session_utc": "00:00-15:55",
            "early_return_destinations_utc": "00:05-07:55",
            "late_return_destinations_utc": "08:00-15:55",
            "early_return_count": 95,
            "late_return_count": 96,
            "variance_measure": "mean_squared_log_return_about_zero",
            "event": "strict_late_variance_relocation_with_late_return_continuation",
        },
...
def execute() -> dict:
    ...
    try:
        ...
        write_exclusive(RECEIPT_PATH, json_bytes({
            "schema_version": "ivrl_source_attempt_receipt.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
        ...
        write_exclusive(TERMINAL_PATH, json_bytes({
            "schema_version": "ivrl_source_attempt_terminal.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
            "status": "COMPLETE",
