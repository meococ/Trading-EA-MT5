//+------------------------------------------------------------------+
//| EA_IntradayVarianceRelocation.mq5                            |
//| HYP-IVRL-XAUUSD-M5-002: intraday variance relocation        |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Untuned XAUUSD M5 intraday variance relocation"
...
input string InpHypothesisId="HYP-IVRL-XAUUSD-M5-002";
input string InpVariantTag="INTRADAY_VARIANCE_RELOCATION";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5604411;
...
const string EA_NAME="EA_IntradayVarianceRelocation";
const string EXPECTED_HYPOTHESIS="HYP-IVRL-XAUUSD-M5-002";
const string EXPECTED_VARIANT="INTRADAY_VARIANCE_RELOCATION";
const string EXPECTED_SYMBOL="XAUUSD";
const int SESSION_ROWS=192;
const int EARLY_RETURN_COUNT=95;
const int LATE_RETURN_COUNT=96;
...
struct SessionSummary
  {
   ...
   double early_mean_squared_return;
   double late_mean_squared_return;
   double late_session_return;
  };
...
void AppendCompleteSession(SessionSummary &sessions[],
                           ...
                           const double &closes[])
  {
      ...
      if(i>0)
        {
         ...
         if(!IsFinite(value))
            return;
         if(i<=EARLY_RETURN_COUNT)
            early_squared_sum+=value*value;
         else
            late_squared_sum+=value*value;
        }
     }
   const double early_mean_squared_return=early_squared_sum/(double)EARLY_RETURN_COUNT;
   const double late_mean_squared_return=late_squared_sum/(double)LATE_RETURN_COUNT;
   const double late_session_return=MathLog(closes[SESSION_ROWS-1]/closes[EARLY_RETURN_COUNT]);
   if(!IsFinite(early_mean_squared_return) || !IsFinite(late_mean_squared_return) ||
      !IsFinite(late_session_return))
      return;
   ...
   sessions[size].early_mean_squared_return=early_mean_squared_return;
   sessions[size].late_mean_squared_return=late_mean_squared_return;
   sessions[size].late_session_return=late_session_return;
  }
...
bool BuildSignal(const datetime availability_time,SignalDecision &signal)
  {
   ...
   g_complete_sessions++;
   if(today.late_mean_squared_return<=today.early_mean_squared_return ||
      today.late_session_return==0.0)
      return(false);
   const int direction=(today.late_session_return>0.0 ? 1 : -1);
   ...
   signal.early_mean_squared_return=today.early_mean_squared_return;
   signal.late_mean_squared_return=today.late_mean_squared_return;
   signal.late_session_return=today.late_session_return;
   signal.atr=atr;
   signal.structural_stop=stop;
   g_raw_signals++;
   if(direction>0) g_long_signals++; else g_short_signals++;
   PrintFormat("IVRL002_SIGNAL decision=%I64d availability=%I64d utc_date=%d direction=%s early_msr=%.12f late_msr=%.12f late_return=%.10f early_count=95 late_count=96 atr=%.5f stop=%.5f",
               (long)decision_time,(long)availability_time,key,
               (direction>0 ? "LONG" : "SHORT"),today.early_mean_squared_return,
               today.late_mean_squared_return,today.late_session_return,atr,stop);
   return(true);
  }
...
bool CloseOwned(const string reason)
  {
   ...
   if(!g_trade.PositionClose(ticket,InpDeviationPoints))
     {
      g_close_rejects++;
      PrintFormat("IVRL002_CLOSE_REJECT reason=%s ticket=%I64u retcode=%u",reason,ticket,g_trade.ResultRetcode());
      return(false);
     }
   ...
   if(retcode!=TRADE_RETCODE_DONE && retcode!=TRADE_RETCODE_DONE_PARTIAL)
     {
      g_close_rejects++;
      PrintFormat("IVRL002_CLOSE_REJECT reason=%s ticket=%I64u retcode=%u",reason,ticket,retcode);
      return(false);
     }
   g_closes++;
   g_entry_time=0;
   PrintFormat("IVRL002_CLOSE reason=%s ticket=%I64u retcode=%u",reason,ticket,retcode);
   return(true);
  }
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   if(!sent || (retcode!=TRADE_RETCODE_DONE && retcode!=TRADE_RETCODE_DONE_PARTIAL))
     {
      g_entry_rejects++;
      PrintFormat("IVRL002_ENTRY_REJECT direction=%s volume=%.2f entry=%.5f sl=%.5f tp=%.5f retcode=%u",
                  (signal.direction>0 ? "LONG" : "SHORT"),volume,entry,sl,tp,retcode);
      return(false);
     }
   g_entries++;
   g_entry_time=signal.availability_time;
   PrintFormat("IVRL002_ENTRY direction=%s volume=%.2f entry=%.5f sl=%.5f tp=%.5f risk_pct=%.3f retcode=%u",
...
int OnInit()
  {
   if(!InpResearchAutoMode || InpEnableTelemetry || _Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_M5 ||
      ...
      InpMagic!=5604411 || MathAbs(InpRiskPercent-0.10)>1e-12 ||
   ...
   if(!CurrentBarOpen(g_last_bar_open) || g_last_bar_open<=0)
      return(INIT_FAILED);
   PrintFormat("IVRL002_INIT ea=%s hypothesis=%s variant=%s symbol=%s timeframe=M5",
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("IVRL002_SUMMARY reason=%d runtime_failed=%s closed=%I64d sessions=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d close_attempts=%I64d close_rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d risk_lock_skips=%I64d",
