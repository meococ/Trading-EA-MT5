...
//+------------------------------------------------------------------+
//| EA_IntradaySerialDependenceSwitch.mq5                               |
//| HYP-ISDS-XAUUSD-M5-001: intraday serial-dependence switch  |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Untuned XAUUSD M5 intraday serial-dependence switch"
...
input string InpHypothesisId="HYP-ISDS-XAUUSD-M5-001";
input string InpVariantTag="INTRADAY_SERIAL_DEPENDENCE_SWITCH";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5604404;
...
const string EA_NAME="EA_IntradaySerialDependenceSwitch";
const string EXPECTED_HYPOTHESIS="HYP-ISDS-XAUUSD-M5-001";
const string EXPECTED_VARIANT="INTRADAY_SERIAL_DEPENDENCE_SWITCH";
...
void AppendCompleteSession(SessionSummary &sessions[],
                           ...
                           const double &closes[])
  {
   ...
   double returns[];
   ArrayResize(returns,SESSION_ROWS-1);
   ...
   for(int i=0;i<pairs;i++)
     {
      const double dx=returns[i]-mean_x;
      const double dy=returns[i+1]-mean_y;
      covariance+=dx*dy;
      variance_x+=dx*dx;
      variance_y+=dy*dy;
     }
   const double denominator=MathSqrt(variance_x*variance_y);
   if(!IsFinite(denominator) || denominator<=0.0)
      return;
   const double rho=covariance/denominator;
   const double recent_return=MathLog(closes[SESSION_ROWS-1]/closes[SESSION_ROWS-7]);
   if(!IsFinite(rho) || !IsFinite(recent_return))
      return;
   ...
   sessions[size].rho=rho;
   sessions[size].recent_return=recent_return;
   sessions[size].recent_start_close=closes[SESSION_ROWS-7];
   sessions[size].end_close=closes[SESSION_ROWS-1];
  }
...
bool BuildSignal(const datetime availability_time,SignalDecision &signal)
  {
   ...
   g_complete_sessions++;
   if(today.rho==0.0 || today.recent_return==0.0)
      return(false);
   const int direction=(((today.rho>0.0 && today.recent_return>0.0) ||
                        (today.rho<0.0 && today.recent_return<0.0)) ? 1 : -1);
   ...
   signal.rho=today.rho;
   signal.recent_return=today.recent_return;
   signal.atr=atr;
   signal.structural_stop=stop;
   g_raw_signals++;
   if(direction>0) g_long_signals++; else g_short_signals++;
   PrintFormat("ISDS001_SIGNAL decision=%I64d availability=%I64d utc_date=%d direction=%s regime=%s rho=%.10f recent30=%.10f atr=%.5f stop=%.5f",
               (long)decision_time,(long)availability_time,key,
               (direction>0 ? "LONG" : "SHORT"),(today.rho>0.0 ? "PERSISTENT" : "ANTI_PERSISTENT"),
               today.rho,today.recent_return,atr,stop);
   return(true);
  }
...
bool CloseOwned(const string reason)
  {
   ...
   if(!g_trade.PositionClose(ticket,InpDeviationPoints))
     {
      PrintFormat("ISDS001_CLOSE_REJECT reason=%s ticket=%I64u retcode=%u",reason,ticket,g_trade.ResultRetcode());
      return(false);
     }
   ...
   g_closes++;
   g_entry_time=0;
   PrintFormat("ISDS001_CLOSE reason=%s ticket=%I64u retcode=%u",reason,ticket,retcode);
   return(true);
  }
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   if(!sent || (retcode!=TRADE_RETCODE_DONE && retcode!=TRADE_RETCODE_DONE_PARTIAL))
     {
      g_entry_rejects++;
      PrintFormat("ISDS001_ENTRY_REJECT direction=%s volume=%.2f entry=%.5f sl=%.5f tp=%.5f retcode=%u",
                  (signal.direction>0 ? "LONG" : "SHORT"),volume,entry,sl,tp,retcode);
      return(false);
     }
   g_entries++;
   g_entry_time=signal.availability_time;
   PrintFormat("ISDS001_ENTRY direction=%s volume=%.2f entry=%.5f sl=%.5f tp=%.5f risk_pct=%.3f retcode=%u",
...
int OnInit()
  {
   if(!InpResearchAutoMode || InpEnableTelemetry || _Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_M5 ||
      ...
      InpMagic!=5604404 || MathAbs(InpRiskPercent-0.10)>1e-12 ||
   ...
   if(!CurrentBarOpen(g_last_bar_open) || g_last_bar_open<=0)
      return(INIT_FAILED);
   PrintFormat("ISDS001_INIT ea=%s hypothesis=%s variant=%s symbol=%s timeframe=M5",
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("ISDS001_SUMMARY reason=%d runtime_failed=%s closed=%I64d sessions=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d",
