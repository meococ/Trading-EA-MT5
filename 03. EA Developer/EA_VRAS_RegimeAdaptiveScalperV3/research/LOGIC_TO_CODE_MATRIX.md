# Logic-to-code matrix - HYP-VRAS-EURUSD-M5-003

HYP-003 keeps the exact HYP-001 signal/risk surface. The only behavioral
revision is the mechanical `OrderCheck()` API correction documented in the
HYP-001 invalid-engineering readout. It also makes the tester identity guard
consistent with its own ID/magic after HYP-002 failed at OnInit; no threshold
or filter was changed.
