#!/usr/bin/env python3
"""One-shot TRAIN proxy evaluator for HYP-LOFIX-USDJPY-M1-002."""
...
HYPOTHESIS_ID = "HYP-LOFIX-USDJPY-M1-002"
ATTEMPT_ID = "LOFIX002-TRAIN-ECON-001"
PLAN_REL = (
    "03. EA Developer/EA_LondonFixHalfHourMomentum/research/"
    "HYP-LOFIX-USDJPY-M1-002_TRAIN_ECONOMIC_PROBE_PLAN_V2.md"
)
PLAN_SHA256 = "0E552F0BCAF792710EAA8E15C59640C3168F03B6160454DDB0F9862939BECC87"
EVALUATOR_REL = (
    "03. EA Developer/EA_LondonFixHalfHourMomentum/research/"
...
TEST_REL = (
    ...
    "test_evaluate_lofix_002_train.py"
)
...
REVIEWED_REGISTRY_ROW_SHA256: str | None = "1D49E3AE947051DAE3CA0754B162F1E02E95AFF52DFE66CFDCE479144E82865B"
...
def compute_dsr(
    ...
    arms = {
        ...
        "lofix002_primary": _arm_moments(current_primary),
        "lofix002_reverse": _arm_moments(current_reverse),
    }
    sharpe_values = [float(item["sr"]) for item in arms.values()]
    variance = float(np.var(sharpe_values, ddof=1))
    primary = arms["lofix002_primary"]
    value = float(
        module.dsr(
            float(primary["sr"]),
            int(primary["n"]),
...
def trade_rows(trades: pd.DataFrame) -> bytes:
    ...
    for row in trades.itertuples(index=False):
        ...
        payload = {
            "trade_id": f"LOFIX002-{local_date}",
...
def execute(workspace: Path) -> Path:
    ...
    started = {
        "schema_version": "lofix_002_attempt_started.v1",
    ...
    terminal = {
        "schema_version": "lofix_002_train_economic_terminal.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "attempt_id": ATTEMPT_ID,
...
def main() -> int:
    ...
    print(
        f"LOFIX002_RESULT verdict={terminal['verdict']} "
