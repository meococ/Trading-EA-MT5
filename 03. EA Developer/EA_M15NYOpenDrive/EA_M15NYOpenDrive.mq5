//+------------------------------------------------------------------+
//| EA_M15NYOpenDrive.mq5 — NY opening-range breakout (M15)          |
//| Symbol: USDJPY | Period: M15 | Magic: 880942                     |
//|                                                                   |
//| Hypothesis: HYP-NY-OPEN-DRIVE-M15-001                              |
//| Independent NY first-hour auction ORB — NOT a LondonORB window   |
//| retune. Differs from Spark/HourOpen/PDH/ITSM/SB/USBILL/Keltner.  |
//|                                                                   |
//| Build Hi/Lo on closed M15 bars in NY first-hour [15,16); lock;   |
//| enter on closed bar[1] break with body + D1 EMA50 in [16,20).    |
//| Mon–Thu a priori; weekend flat; risk 0.5%. No day mining.         |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15NYOpenDrive"
...
input group "=== NY ORB ==="
input int      InpOrbStart      = 15;     // Inclusive hour (server) NY open
input int      InpOrbEnd        = 16;     // Exclusive hour — lock here
...
input int      InpTradeStart    = 16;     // Breakout window start
input int      InpTradeEnd      = 20;     // Breakout window end (exclusive)
...
int OnInit()
{
   ...
   PrintFormat("[NYOD] HYP-NY-OPEN-DRIVE-M15-001 | ORB[%d,%d) trade[%d,%d) TP=%.2fR risk=%.2f%%",
               InpOrbStart, InpOrbEnd, InpTradeStart, InpTradeEnd, InpTP_Ratio, InpRiskPct);
...
//+------------------------------------------------------------------+
// Closed-bar only: decision from bar[1]; ORB built from prior closed bars.
...
void OnTick()
{
   ...
   UpdateOrbFromClosedBar(dt.hour, h1, l1, atr[0]);
   ...
   if(IsSuccessfulRetcode(res.retcode))
   {
      ...
      g_brokeToday = true;
      g_holdBars = 0;
      PrintFormat("[NYOD] %s lot=%.2f @ %.5f ORB[%.5f,%.5f]",
                  isBuy ? "BUY" : "SELL", lot, res.price, orbHi, orbLo);
   }
}
