...
PACKAGE = ROOT / "03. EA Developer" / "EA_SchaffTrendCycleV3"
SOURCE = PACKAGE / "EA_SchaffTrendCycleV3.mq5"
CONTRACT = PACKAGE / "ALPHAFACTORY_EA_CONTRACT.json"
PREREG = PACKAGE / "research" / "HYP-STC-EURUSD-M15-003_FROZEN_PREREG.md"
ORIGIN = PACKAGE / "research" / "HYP-STC-EURUSD-M15-003_PRELOAD_ORIGIN_PROOF.json"
COMPILE_LOG = PACKAGE / "EA_SchaffTrendCycleV3.log"
EX5 = PACKAGE / "EA_SchaffTrendCycleV3.ex5"
...
class Stc003ContractTests(unittest.TestCase):
    def test_identity_and_single_indicator_mechanism(self):
        text = source()
        self.assertIn('EXPECTED_HYPOTHESIS="HYP-STC-EURUSD-M15-003"', text)
        self.assertIn('EXPECTED_VARIANT="STC_CLASSIC_TREND_CYCLE_V3_TESTER_CLOSED_PRELOAD"', text)
    ...
    def test_closed_bar_and_exact_next_clock(self):
        ...
        self.assertIn("const datetime STC_PRELOAD_LAST=D'2015.12.31 19:45';", text)
        self.assertIn("const int      STC_PRELOAD_BARS=24775;", text)
        self.assertNotIn("SERIES_SYNCHRONIZED", text)
        ...
        for stage in ("last_closed", "bars", "copy", "endpoints", "advance", "final_state"):
    ...
    def test_contract_is_frozen_model0_baseline(self):
        ...
        self.assertEqual(profile["hypothesis_id_default"], "HYP-STC-EURUSD-M15-003")
        self.assertEqual(contract["inputs"]["InpMagic"], 5603703)

    def test_tester_closed_origin_is_exact_and_monotonic(self):
        proof = json.loads(ORIGIN.read_text(encoding="utf-8"))
        closed = proof["tester_visible_closed_preload"]
        self.assertEqual(closed["count"], 24775)
        self.assertEqual(closed["first_source_epoch"], 1420189200)
        self.assertEqual(closed["last_source_epoch"], 1451591100)
        self.assertTrue(closed["strictly_increasing"])
        self.assertEqual(closed["duplicate_count"], 0)
        self.assertEqual(
            closed["timestamp_sequence_sha256"],
            "C9006D5C5E0ED8BE72C63BA4F1C0FB1B12AD9DF518F0D7BE68639305EAD114FF",
        )
        native = proof["native_range_observation"]
        self.assertEqual(native["count"], 24776)
        self.assertEqual(native["last_source_epoch"], 1451592000)
