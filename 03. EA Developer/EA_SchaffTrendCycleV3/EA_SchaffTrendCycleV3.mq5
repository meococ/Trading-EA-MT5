//+------------------------------------------------------------------+
//| EA_SchaffTrendCycleV3.mq5                                         |
//| HYP-STC-EURUSD-M15-003: classic STC trend-cycle continuation    |
//+------------------------------------------------------------------+
#property strict
#property version   "3.00"
#property description "Untuned EURUSD M15 Schaff Trend Cycle continuation baseline; tester-closed preload revision"

input group "--- Frozen research authority ---"
input bool   InpResearchAutoMode=false;
input bool   InpEnableTelemetry=false;
input string InpHypothesisId="HYP-STC-EURUSD-M15-003";
input string InpVariantTag="STC_CLASSIC_TREND_CYCLE_V3_TESTER_CLOSED_PRELOAD";

input group "--- Frozen execution and risk ---"
input long   InpMagic=5603703;
...
const string EA_NAME="EA_SchaffTrendCycleV3";
const string EXPECTED_HYPOTHESIS="HYP-STC-EURUSD-M15-003";
const string EXPECTED_VARIANT="STC_CLASSIC_TREND_CYCLE_V3_TESTER_CLOSED_PRELOAD";
...
const datetime STC_PRELOAD_LAST=D'2015.12.31 19:45';
const int      STC_PRELOAD_BARS=24775;
...
bool PreloadIndicatorState()
  {
   ...
   if(last_closed!=STC_PRELOAD_LAST)
     {
      PrintFormat("STC003_PRELOAD_FAIL stage=last_closed actual=%I64d expected=%I64d error=%d",
   ...
   if(preload_count!=STC_PRELOAD_BARS || preload_count<STC_MIN_BARS)
     {
      PrintFormat("STC003_PRELOAD_FAIL stage=bars actual=%d expected=%d error=%d",
   ...
   if(copied!=preload_count)
     {
      PrintFormat("STC003_PRELOAD_FAIL stage=copy actual=%d expected=%d error=%d",
                  copied,preload_count,copy_error);
      return(false);
     }
   if(rates[0].time!=STC_PRELOAD_FIRST || rates[copied-1].time!=last_closed)
     {
      PrintFormat("STC003_PRELOAD_FAIL stage=endpoints first=%I64d expected_first=%I64d last=%I64d expected_last=%I64d",
   ...
   for(int i=0;i<copied;i++)
     {
      if(!AdvanceIndicatorState(rates[i]))
        {
         PrintFormat("STC003_PRELOAD_FAIL stage=advance index=%d time=%I64d prior=%I64d error=%d",
   ...
   if(!ready)
      PrintFormat("STC003_PRELOAD_FAIL stage=final_state bar=%I64d expected=%I64d prev_stc=%.8f stc=%.8f macd=%.8f atr=%.8f",
...
bool ClosePositionTicket(const ulong ticket,const string reason)
  {
   ...
   if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
      PrintFormat("STC003_CLOSE_REJECT ticket=%I64u retcode=%u error=%d reason=%s",
...
bool SubmitEntry(const SignalDecision &signal)
  {
   ...
   request.deviation=InpDeviationPoints;
   request.type_filling=ResolveFilling();
   request.comment="STC003";
   if(!OrderCheck(request,check) || check.retcode!=0)
     {
      PrintFormat("STC003_ORDER_CHECK_REJECT decision=%I64d retcode=%u comment=%s",
                  (long)signal.decision_time,check.retcode,check.comment);
      return(false);
     }
   if(!OrderSend(request,result) || !AcceptedRetcode(result.retcode))
     {
      PrintFormat("STC003_ORDER_SEND_REJECT decision=%I64d retcode=%u error=%d",
                  (long)signal.decision_time,result.retcode,GetLastError());
      return(false);
     }
   g_entries_accepted++;
   g_daily_entries++;
   PrintFormat("STC003_ENTRY decision=%I64d availability=%I64d direction=%d volume=%.2f entry=%.5f sl=%.5f tp=%.5f prev_stc=%.8f stc=%.8f macd=%.8f atr=%.8f",
...
void ExecuteSignal(const SignalDecision &signal,const datetime now)
  {
   PrintFormat("STC003_SIGNAL decision=%I64d availability=%I64d direction=%d prev_stc=%.8f stc=%.8f macd=%.8f atr=%.8f",
               (long)signal.decision_time,(long)signal.availability_time,
...
bool ValidateFrozenInputs()
  {
   return(_Symbol==EXPECTED_SYMBOL && _Period==PERIOD_M15 &&
          ...
          InpMagic==5603703 && MathAbs(InpRiskPercent-0.25)<1e-12 &&
...
int OnInit()
  {
   if(!ValidateFrozenInputs())
     {
      Print("STC003_FATAL reason=FROZEN_INPUT_OR_SYMBOL_CONTRACT");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(!CurrentM15Open(g_last_bar_open))
     {
      Print("STC003_FATAL reason=INITIAL_M15_CLOCK");
      return(INIT_FAILED);
     }
   if(!PreloadIndicatorState())
     {
      Print("STC003_FATAL reason=INDICATOR_PRELOAD");
      return(INIT_FAILED);
     }
   const double equity=AccountInfoDouble(ACCOUNT_EQUITY);
   if(!IsUsable(equity) || equity<=0.0)
     {
      Print("STC003_FATAL reason=INITIAL_EQUITY");
      return(INIT_FAILED);
     }
   g_daily_day_key=DateKey(TimeCurrent());
   g_daily_start_equity=equity;
   g_peak_equity=equity;
   PrintFormat("STC003_INIT hypothesis=%s variant=%s symbol=%s period=%d",
...
void OnDeinit(const int reason)
  {
   PrintFormat("STC003_SUMMARY hypothesis=%s closed_bars=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d runtime_failed=%s reason=%d",
...
void OnTick()
  {
   ...
   if(current_open<g_last_bar_open)
     {
      g_runtime_failed=true;
      Print("STC003_FATAL reason=BAR_CLOCK_REGRESSION");
      return;
     }
