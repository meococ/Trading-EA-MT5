//+------------------------------------------------------------------+
//| EA_ChopDonchian.mq5 — Choppiness Index + Donchian Channel        |
//| Symbol: USDJPY+  |  Period: M15                                   |
//|                                                                   |
//| HYPOTHESIS TEST: Regime filter IS the edge, signal doesn't matter |
//| If ChopRegime works with EMA crossover (S630 PF 1.26), does it   |
//| also work with the simplest possible signal — Donchian breakout?  |
//| This tests whether the alpha comes from the CI regime filter      |
//| or from the specific signal generator.                            |
//|                                                                   |
//| Novelty: Type #89 — CI regime filter + Donchian channel breakout  |
//| SIGNALS ON BAR[1] ONLY — no repaint.                              |
//| Max | 2026-04-13 | v1.0                                          |
//+------------------------------------------------------------------+
#property copyright "Max — EA_ChopDonchian v1.0"
#property version   "1.00"
#property strict

input group "=== General ==="
input ulong    InpMagic         = 222001;
input int      InpDeviation     = 30;

input group "=== Choppiness Index ==="
input int      InpChopPeriod    = 14;
input double   InpChopHigh      = 50.0;          // CI < this = trending

input group "=== Donchian Channel ==="
input int      InpDonchianLen   = 20;            // Lookback for high/low channel
...
int GetSignal()
{
   ...
   double hh = -999999, ll = 999999;
   for(int i = 2; i < 2 + InpDonchianLen; i++)  // bar[2] to bar[N+1] — PRIOR channel
   {
      double h = iHigh(_Symbol, PERIOD_CURRENT, i);
      double l = iLow(_Symbol, PERIOD_CURRENT, i);
      if(h > hh) hh = h;
   ...
   // Breakout: bar[1] closed above previous Donchian high
   if(close1 > hh && close1 > trend[0]) return +1;
   // Breakdown: bar[1] closed below previous Donchian low
   if(close1 < ll && close1 < trend[0]) return -1;
   return 0;
}
...
void OnTick()
{
   ...
   req.comment=StringFormat("CD|CI=%.1f",ComputeChoppiness(1));
   req.type_filling=ORDER_FILLING_FOK;
   if(!OrderSend(req,res)){req.type_filling=ORDER_FILLING_IOC;if(!OrderSend(req,res))return;}
   if(res.retcode==TRADE_RETCODE_DONE||res.retcode==TRADE_RETCODE_PLACED)
   { g_tradesToday++; PrintFormat("[CHOPDON] %s %.2f @ %.2f CI=%.1f",isBuy?"BUY":"SELL",lot,res.price,ComputeChoppiness(1)); }
}
