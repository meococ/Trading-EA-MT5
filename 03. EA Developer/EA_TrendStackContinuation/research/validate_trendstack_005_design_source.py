...
PENDING_TREE_SCHEMA = "trendstack_005_pending_tree.v1"
DATE_SET_PREFIX = b"trendstack_002_design_date_set.v1\n"
PROJECTION_SCHEMA = "trendstack_005_design_projection_row.v1"
...
def validate_design_source(
    ...
    try:
        ...
        manifest = _canonical_rows(base_payloads["design_h1_manifest.jsonl"])
        ...
        source_receipt = _canonical_object(base_payloads["design_h1_source_receipt.json"])
        ...
        for day, request in zip(dates, requests):
            if set(request) != _REQUEST_FIELDS:
                raise ValueError
            if (
                request["schema_version"] != "trendstack_005_design_request.v1"
                or request["date"] != day
                or request["parent_opportunity_id"] != day
                or request["request_id"] != "HYP005::" + day + "::H1_SOURCE"
        ...
        for day in dates:
            expected_directories.add("raw_h1/DESIGN/" + day)
            expected_files.add("raw_h1/DESIGN/" + day + "/1201_1800.parquet")
        ...
        pending_tree_payloads = {
            ...
            if relative != "design_h1_source_receipt.json"
        }
        for index, day in enumerate(dates):
            relative = "raw_h1/DESIGN/" + day + "/1201_1800.parquet"
            ...
            if set(trace_item) != _TRACE_FIELDS:
                raise ValueError
            if (
                trace_item.get("schema_version") != "trendstack_005_design_source_trace.v1"
        ...
        bindings = {
            ...
            "h1_manifest_sha256": _digest(base_payloads["design_h1_manifest.jsonl"]),
        ...
        if set(request_receipt) != {"design_date_set_sha256", "request_count", "request_plan_sha256", "schema_version"}:
            raise ValueError
        if (
            request_receipt.get("schema_version") != "trendstack_005_design_request_receipt.v1"
        ...
        if set(projection_receipt) != {
            ...
            raise ValueError
        if (
            projection_receipt.get("schema_version") != "trendstack_005_design_projection_receipt.v1"
        ...
        if set(reconciliation) != {
            ...
            raise ValueError
        if (
            reconciliation.get("schema_version") != "trendstack_005_design_source_reconciliation.v1"
            ...
            or reconciliation.get("h1_rows") != total_rows
        ):
            raise ValueError
        if (
            source_receipt.get("schema_version") != "trendstack_005_design_source_receipt.v1"
            ...
            or source_receipt.get("h1_rows") != authority.expected_total_rows
            ...
            raise ValueError
        source_receipt_sha256 = _digest(base_payloads["design_h1_source_receipt.json"])
