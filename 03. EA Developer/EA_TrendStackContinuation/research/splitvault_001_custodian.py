...
def parquet_footer_sha256(path: Path | str) -> str:
    try:
        return _footer_digest(_stable_read(Path(path)))
    except Exception as exc:
        raise InvalidCustody(PUBLIC_ERROR) from exc
...
def _validate_authority(authority: CustodyAuthority) -> None:
    ...
    if (
        ...
        or not authority.source_attempt_id.startswith("HYP003-SOURCE-ATTEMPT-")
        or len(authority.source_attempt_id) != len("HYP003-SOURCE-ATTEMPT-") + 16
...
def _footer_digest(payload: bytes) -> str:
...
def run_custody(
    ...
    try:
        ...
        if (
            ...
            or _footer_digest(source_payload) != authority.source_footer_sha256
        ...
        private_receipt = {
            ...
            "schema_version": "splitvault_001_private_custody_receipt.v1",
