...
PENDING_TREE_SCHEMA = "trendstack_004_pending_tree.v1"
DATE_SET_PREFIX = b"trendstack_002_design_date_set.v1\n"
PROJECTION_SCHEMA = "trendstack_004_design_projection_row.v1"
    ...
    try:
        ...
        for day, request in zip(dates, requests):
            if set(request) != _REQUEST_FIELDS:
                raise ValueError
            if (
                request["schema_version"] != "trendstack_004_design_request.v1"
                or request["date"] != day
                or request["parent_opportunity_id"] != day
                or request["request_id"] != "HYP004::" + day + "::M1_SOURCE"
        ...
        for index, day in enumerate(dates):
            ...
            if set(trace_item) != _TRACE_FIELDS:
                raise ValueError
            if (
                trace_item.get("schema_version") != "trendstack_004_design_source_trace.v1"
        ...
        if set(request_receipt) != {"design_date_set_sha256", "request_count", "request_plan_sha256", "schema_version"}:
            raise ValueError
        if (
            request_receipt.get("schema_version") != "trendstack_004_design_request_receipt.v1"
        ...
        if set(projection_receipt) != {
            ...
            raise ValueError
        if (
            projection_receipt.get("schema_version") != "trendstack_004_design_projection_receipt.v1"
        ...
        if set(reconciliation) != {
            ...
            raise ValueError
        if (
            reconciliation.get("schema_version") != "trendstack_004_design_source_reconciliation.v1"
            ...
            raise ValueError
        if (
            source_receipt.get("schema_version") != "trendstack_004_design_source_receipt.v1"
