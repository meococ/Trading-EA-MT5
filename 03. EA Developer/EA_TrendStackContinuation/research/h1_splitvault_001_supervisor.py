...
RUN_PACKET_SCHEMA = "trendstack_005_splitvault_source_run_packet.v1"
COLLECTION_ID = "DATASET-FIVEPERCENT-EURUSD-H1-SPLITVAULT-002"
HYPOTHESIS_ID = "HYP-TRENDSTACK-EURUSD-H1-005"
RUN_PACKET_VERDICT = "FROZEN_OUTCOME_BLIND_SOURCE_RUN_AUTHORIZED"
RUN_PACKET_FILENAME = "HYP-TRENDSTACK-EURUSD-H1-005_SOURCE_RUN_PACKET.json"
...
COLLECTION_PLAN_FILENAME = "DATASET-FIVEPERCENT-EURUSD-H1-SPLITVAULT-002_PLAN.md"
HYPOTHESIS_PLAN_SHA256 = "2C82C993A390A56AC33C9A7956F8B3D6B70C426AD5947B36BAE48FBC90CD2F07"
HYPOTHESIS_PLAN_FILENAME = "HYP-TRENDSTACK-EURUSD-H1-005_PROBE_PLAN.md"
...
SOURCE_ATTEMPT_PREFIX = "HYP005-SOURCE-ATTEMPT-"
AUTHORITY_SENTINEL_LINE = b"REVIEWED_RUN_PACKET_SHA256: str | None = " + b"None"
ATTEMPT_EVIDENCE_PARENT = "HYP-TRENDSTACK-EURUSD-H1-005_SOURCE_ATTEMPTS"
...
_PENDING_TREE_SCHEMA = "trendstack_005_pending_tree.v1"
_PENDING_BASE_FILES = {
...
def _create_attempt_started(
    ...
    marker = {
        ...
        "schema_version": "trendstack_005_source_attempt_started.v1",
...
def _create_attempt_terminal(
    ...
    terminal = {
        **evidence,
        ...
        "packet_sha256": packet_sha256,
        "runtime_supervisor_sha256": runtime_supervisor_sha256,
        "schema_version": "trendstack_005_source_attempt_terminal.v1",
        "source_attempt_id": packet["source_attempt_id"],
        "supervisor_review_base_sha256": packet["supervisor_review_base_sha256"],
        "verdict": verdict,
    }
...
def _validate_registry_authority(payload: bytes, packet: dict[str, object]) -> None:
    ...
    if (
        ...
        != "03. EA Developer/EA_TrendStackContinuation/research/HYP-TRENDSTACK-EURUSD-H1-005_PROBE_PLAN.md"
...
def _bind_pending_output(output_root: Path | str, result: dict[str, object]) -> dict[str, object]:
        ...
        with os.scandir(current) as entries:
            for entry in entries:
                ...
                if stat.S_ISDIR(info.st_mode):
                    ...
                    raise ValueError
    if _directory_identity(output) != root_identity or "design_h1_manifest.jsonl" not in files:
        raise ValueError
    manifest = _canonical_rows(files["design_h1_manifest.jsonl"])
    shard_paths: set[str] = set()
    expected_directories = {".", "raw_h1", "raw_h1/DESIGN"}
    for item in manifest:
        relative = item.get("relative_path")
        day = item.get("date")
        if type(relative) is not str or type(day) is not str or relative != f"raw_h1/DESIGN/{day}/1201_1800.parquet":
            raise ValueError
    ...
    if set(files) != _PENDING_BASE_FILES | shard_paths or set(directories) != expected_directories:
        raise ValueError
    receipt_payload = files["design_h1_source_receipt.json"]
    ...
    entries = [
        ...
        for relative in sorted(set(files) - {"design_h1_source_receipt.json"})
    ]
...
def _production_expected_paths(source_attempt_id: str) -> dict[str, Path]:
    ...
    paths = {
        ...
        "source_path": workspace / "02. AlphaFactory" / "data" / "fivepercent" / "EURUSD" / "EURUSD_H1_2015_now.parquet",
        ...
        "custodian_tool_path": research / "h1_splitvault_001_custodian.py",
        "supervisor_tool_path": research / "h1_splitvault_001_supervisor.py",
        "design_builder_tool_path": research / "build_trendstack_005_design_source.py",
        "validator_tool_path": research / "validate_trendstack_005_design_source.py",
        "custodian_test_path": package / "tests" / "test_h1_splitvault_001_custodian.py",
        "supervisor_test_path": package / "tests" / "test_h1_splitvault_001_supervisor.py",
        "design_builder_test_path": package / "tests" / "test_build_trendstack_005_design_source.py",
        "validator_test_path": package / "tests" / "test_validate_trendstack_005_design_source.py",
        "splitvault_output_root": workspace / "02. AlphaFactory" / "data" / "fivepercent" / "EURUSD" / "h1_splitvault_001",
        "design_source_output_root": workspace / "02. AlphaFactory" / "data" / "fivepercent" / "EURUSD" / "trendstack_005_design_h1",
    }
