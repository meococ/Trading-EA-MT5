...
TOOL_PATH = ROOT / "research" / "splitvault_002_supervisor.py"
CUSTODIAN_PATH = ROOT / "research" / "splitvault_002_custodian.py"
...
VALIDATOR_TEST_PATH = ROOT / "tests" / "test_validate_trendstack_004_design_source.py"
COLLECTION_PLAN_NAME = "DATASET-FIVEPERCENT-EURUSD-M1-SPLITVAULT-002_PLAN.md"
HYPOTHESIS_PLAN_NAME = "HYP-TRENDSTACK-EURUSD-H1-004_PROBE_PLAN.md"
...
PARENT_EVIDENCE_ROOT = (
    ...
    / "HYP003-SOURCE-ATTEMPT-1323330E76ED3671"
)
...
def load_tool():
    spec = importlib.util.spec_from_file_location("splitvault_002_supervisor_test", TOOL_PATH)
...
def packet_fixture(tmp_path: Path, *, copied_tools: bool = False):
    ...
    evidence_parent = tmp_path / "evidence" / "HYP-TRENDSTACK-EURUSD-H1-004_SOURCE_ATTEMPTS"
    evidence_parent.mkdir(parents=True, exist_ok=True)
    files = {
        "collection_plan": bound_file(tmp_path, COLLECTION_PLAN_NAME, b"collection 002\n"),
        "hypothesis_plan": bound_file(tmp_path, HYPOTHESIS_PLAN_NAME, b"hypothesis 004\n"),
        ...
        "custodian_test": bound_file(tmp_path, "test_custodian.py", (ROOT / "tests" / "test_splitvault_002_custodian.py").read_bytes()),
        "supervisor_test": bound_file(tmp_path, "test_supervisor.py", (ROOT / "tests" / "test_splitvault_002_supervisor.py").read_bytes()),
        "design_builder_test": bound_file(tmp_path, "test_builder.py", (ROOT / "tests" / "test_build_trendstack_004_design_source.py").read_bytes()),
...
def test_hyp004_packet_requires_every_exact_authority_field(tmp_path: Path, missing: str) -> None:
    tool, _, packet, packet_path, frozen = packet_fixture(tmp_path)
    packet.pop(missing)
    packet_path.write_bytes(canonical(packet) + b"\n")
    with pytest.raises(tool.InvalidSupervisor, match="^INVALID_SUPERVISOR$"):
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "HYP004-SOURCE-ATTEMPT-0123456789ABCDE",
        "HYP004-SOURCE-ATTEMPT-0123456789ABCDEG",
        "HYP003-SOURCE-ATTEMPT-0123456789ABCDEF",
        "hyp003-source-attempt-0123456789abcdef",
        "HYP004-SOURCE-ATTEMPT-../123456789ABC",
        "HYP004-SOURCE-ATTEMPT-0123456789ABC\nEF",
        123,
    ],
)
...
@pytest.mark.parametrize("attack", ["wrong_index", "wrong_row_hash", "changed_row", "later_hyp004"])
def test_hyp004_registry_row_authority_fails_before_marker_and_source(tmp_path: Path, attack: str) -> None:
    tool, files, packet, packet_path, _ = packet_fixture(tmp_path)
...
def test_parent_parked_row_and_failure_evidence_are_exact_before_marker_or_source(
    ...
    if attack == "parent_row_index":
        ...
        later = canonical({"hypothesis_id": "HYP-TRENDSTACK-EURUSD-H1-003", "state": "parked"}) + b"\n"
        files["registry"].write_bytes(files["registry"].read_bytes() + later)
        packet["registry_sha256"] = sha256(files["registry"].read_bytes())
...
def test_parent_footer_digest_is_explicitly_rejected_before_marker_or_source(tmp_path: Path) -> None:
    tool, files, packet, packet_path, _ = packet_fixture(tmp_path)
...
def test_attempt_paths_are_deterministic_and_alternate_id_cannot_reuse_them(tmp_path: Path) -> None:
    ...
    packet["source_attempt_id"] = "HYP004-SOURCE-ATTEMPT-FEDCBA9876543210"
    packet_path.write_bytes(canonical(packet) + b"\n")
    review_base = TOOL_PATH.read_bytes()
    files["supervisor_tool"].write_bytes(armed_supervisor(review_base, sha256(packet_path.read_bytes())))
    frozen = tool.FrozenBindings.from_packet_for_testing(packet)
...
def test_production_paths_pin_hyp004_parent_failure_and_corrected_footer() -> None:
    tool = load_tool()
    paths = tool._production_expected_paths(SOURCE_ATTEMPT_ID)
    assert tool.COLLECTION_PLAN_SHA256 == "F4321C66548B26E867A6CDF0B4B02B3E6B5E1CCA352AC5FB022B3FCA6C320382"
    assert tool.HYPOTHESIS_PLAN_SHA256 == "2C82C993A390A56AC33C9A7956F8B3D6B70C426AD5947B36BAE48FBC90CD2F07"
    ...
    assert tool.SOURCE_FOOTER_LENGTH == 10121
    assert tool.SOURCE_FOOTER_START == 104955716
    assert tool.SOURCE_FOOTER_SHA256 == "691BE204EBC508FD61C925972F91482854AED46625EF7B05F330B7FDFBC9970F"
    ...
    assert paths["custody_stage_path"].name == ".splitvault_002.attempt-" + SOURCE_ATTEMPT_ID
    assert paths["design_stage_path"].name == ".trendstack_004_design_m1.attempt-" + SOURCE_ATTEMPT_ID
    output_text = "\n".join(
        str(paths[key])
        for key in ("splitvault_output_root", "design_source_output_root", "custody_stage_path", "design_stage_path")
    ).lower()
    assert "splitvault_001" not in output_text and "trendstack_003_design_m1" not in output_text
...
def test_supervisor_rejects_bare_or_mismatched_pending_design_result(tmp_path: Path, attack: str) -> None:
    ...
    if attack == "bare":
        ...
        pending["source_attempt_id"] = "HYP004-SOURCE-ATTEMPT-FEDCBA9876543210"
    elif attack == "path":
...
def test_source_has_no_network_subprocess_or_dynamic_shell_surface() -> None:
    ...
    for forbidden in (
        ...
        ".trendstack_003_design_m1.attempt-",
    ):
...
def test_verified_builder_executes_inside_child_boundary_on_public_bytes_only(tmp_path: Path) -> None:
    tool = load_tool()
    fixtures = load_path(ROOT / "tests" / "test_build_trendstack_004_design_source.py", "sealed_builder_fixtures")
    day = "2016-01-04"
