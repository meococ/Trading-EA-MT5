...
SUPERVISOR_PATH = ROOT / "research" / "h1_splitvault_001_supervisor.py"
...
def hour_payload(tmp_path: Path, day: str) -> bytes:
    ...
    pq.write_table(pa.Table.from_pylist(rows, schema=source_schema()), path, row_group_size=7)
...
def built_fixture(tmp_path: Path, dates: list[str] | None = None):
    ...
    payloads = {day: hour_payload(tmp_path, day) for day in dates}
    ...
    source_receipt = json.loads((output / "design_h1_source_receipt.json").read_bytes())
...
def test_independent_validator_is_only_component_that_emits_source_ready(tmp_path: Path) -> None:
    ...
    assert result["source_receipt_sha256"] == sha256((output / "design_h1_source_receipt.json").read_bytes())
    ...
    assert json.loads((output / "design_h1_source_receipt.json").read_bytes())["schema_version"] == (
        "trendstack_005_design_source_receipt.v1"
    )
    assert b"HYP004::" not in (output / "design_request_plan.jsonl").read_bytes()
...
def test_actual_supervisor_pending_binding_is_accepted_by_actual_validator(tmp_path: Path) -> None:
    ...
    receipt_payload = (output / "design_h1_source_receipt.json").read_bytes()
    builder_result = {
...
def test_validator_rejects_tree_tamper_extra_missing_alias_and_mixed_projection(tmp_path: Path, attack: str) -> None:
    ...
    if attack == "tamper":
        ...
        target = output / "design_h1_source_receipt.json"
...
def test_validator_detects_identity_swap_after_inventory(tmp_path: Path) -> None:
    validator, output, authority = built_fixture(tmp_path)
    target = output / "design_h1_manifest.jsonl"
