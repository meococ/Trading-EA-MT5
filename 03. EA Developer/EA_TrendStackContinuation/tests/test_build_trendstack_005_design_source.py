...
TOOL_PATH = ROOT / "research" / "build_trendstack_005_design_source.py"
...
def write_mixed_projection_fixture(tmp_path: Path, design_dates: list[str]):
    tool = load(TOOL_PATH, "trendstack_005_builder_test")
...
def hour_rows(day: str, *, missing: set[int] = frozenset(), duplicate: int | None = None, bad_last: bool = False):
    start = datetime.fromisoformat(day + "T12:00:00")
    rows = []
    for index in range(7):
        if index in missing:
            continue
        stamp = start + timedelta(hours=index)
        if bad_last and index == 6:
...
def contract(tool, dates: list[str], date_hash: str, stage: Path):
    return tool.DesignSourceContract(
        ...
        expected_total_rows=len(dates) * 7,
...
def test_exact_design_source_build_writes_7_row_daily_shards_and_false_outcomes(tmp_path: Path) -> None:
    dates = ["2016-01-04", "2016-01-05"]
    tool, _, projection, date_hash = write_mixed_projection_fixture(tmp_path, dates)
    payloads = {day: payload_for(hour_rows(day), tmp_path, day + ".parquet") for day in dates}
    ...
    assert receipt["request_count"] == 2 and receipt["h1_rows"] == 14
    ...
    assert tool.PARENT_HYPOTHESIS_ID == "HYP-TRENDSTACK-EURUSD-H1-002"
    requests = (tmp_path / "output" / "design_request_plan.jsonl").read_bytes()
    assert b"HYP005::" in requests and b"HYP004::" not in requests
    assert receipt["schema_version"] == "trendstack_005_design_source_receipt.v1"
    for key in (
    ...
    for day in dates:
        shard = tmp_path / "output" / "raw_h1" / "DESIGN" / day / "1200_1800.parquet"
        assert pq.ParquetFile(shard).metadata.num_rows == 7
...
@pytest.mark.parametrize(
    "field,value",
    [
        ("source_attempt_id", "HYP005-SOURCE-ATTEMPT-FEDCBA9876543210"),
...
def test_design_contract_rejects_mismatched_attempt_provenance(tmp_path: Path, field: str, value: object) -> None:
    ...
    payload = payload_for(hour_rows(day), tmp_path, "day.parquet")
...
def test_gap_duplicate_and_outside_window_fail_before_receipt(tmp_path: Path, attack: str) -> None:
    ...
    assert not (output / "design_h1_source_receipt.json").exists()
...
def test_output_collision_and_crash_are_quarantined_without_success(tmp_path: Path) -> None:
    ...
    assert not (failed / "design_h1_source_receipt.json").exists()
...
def test_late_output_collision_preserves_attempt_outside_attacker_root(tmp_path: Path) -> None:
    ...
    assert len(attempts) == 1 and (attempts[0] / "design_h1_source_receipt.json").exists()
...
def test_late_output_reparse_preserves_attempt_outside_target(tmp_path: Path) -> None:
    ...
    assert not (attacker / "design_h1_source_receipt.json").exists()
...
def test_builder_source_has_no_raw_monolith_parent_quarantine_network_or_economics_surface() -> None:
    ...
    for forbidden in ("eurusd_h1_2015_now", "trendstack_002_design_h1", "failure_manifest", "profit_factor", "net_r", "mfe", "mae"):
        assert forbidden not in lowered
    assert "hyp004::" not in lowered
