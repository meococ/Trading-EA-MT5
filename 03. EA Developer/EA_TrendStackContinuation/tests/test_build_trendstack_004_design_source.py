...
TOOL_PATH = ROOT / "research" / "build_trendstack_004_design_source.py"
...
def write_mixed_projection_fixture(tmp_path: Path, design_dates: list[str]):
    tool = load(TOOL_PATH, "trendstack_004_builder_test")
...
def test_exact_design_source_build_writes_360_row_daily_shards_and_false_outcomes(tmp_path: Path) -> None:
    ...
    assert b"HYP004::" in requests and b"HYP003::" not in requests
    assert receipt["schema_version"] == "trendstack_004_design_source_receipt.v1"
    for key in (
...
@pytest.mark.parametrize(
    "field,value",
    [
        ("source_attempt_id", "HYP004-SOURCE-ATTEMPT-FEDCBA9876543210"),
