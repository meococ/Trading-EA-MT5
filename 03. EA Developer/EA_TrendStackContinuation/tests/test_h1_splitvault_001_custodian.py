...
TOOL_PATH = ROOT / "research" / "h1_splitvault_001_custodian.py"
SOURCE_ATTEMPT_ID = "HYP005-SOURCE-ATTEMPT-0123456789ABCDEF"


def load_tool():
    spec = importlib.util.spec_from_file_location("h1_splitvault_001_custodian_test", TOOL_PATH)
...
def write_fixture(tmp_path: Path, rows: list[dict[str, object]] | None = None):
    ...
    if rows is None:
        rows = [
            row("2015-12-31T23:00:00", 1.01),
            row("2016-01-04T12:00:00", 1.11),
            row("2016-01-04T13:00:00", 1.12),
            row("2021-01-04T12:00:00", 9.91),
            row("2023-01-03T12:00:00", 8.81),
        ]
...
def test_calendar_only_split_is_exact_once_and_public_surface_is_design_only(tmp_path: Path) -> None:
    ...
    assert receipt["source_rows"] == authority.source_rows
    ...
    assert private_receipt_value["source_rows"] == authority.source_rows
...
def test_capability_denies_parent_private_future_and_path_escape(tmp_path: Path) -> None:
    ...
    for value in ("../sealed/HOLDOUT/2023-01-03/h1.parquet", "2021-01-04", "2023-01-03", "C:/escape"):
...
def test_duplicate_bad_geometry_and_clock_drift_fail_closed(tmp_path: Path) -> None:
    cases = [
        [row("2016-01-04T12:00:00", 1.1), row("2016-01-04T12:00:00", 1.2)],
        [{**row("2016-01-04T12:00:00", 1.1), "low": 2.0}],
        [{**row("2016-01-04T12:00:00", 1.1), "time_server": datetime(2016, 1, 4, 13, 0)}],
        [row("2016-01-04T12:30:00", 1.1)],
    ]
...
def test_runtime_type_confusion_and_wrong_footer_are_rejected(tmp_path: Path) -> None:
    ...
    bad = authority.__class__(**{**authority.__dict__, "source_rows": authority.source_rows + 1})
    with pytest.raises(tool.InvalidCustody, match="^INVALID_CUSTODY$"):
        run_custody(tool, source, manifest, plan, clock, tmp_path / "rows", bad)
