...
TOOL_PATH = ROOT / "research" / "h1_splitvault_001_supervisor.py"
CUSTODIAN_PATH = ROOT / "research" / "h1_splitvault_001_custodian.py"
BUILDER_PATH = ROOT / "research" / "build_trendstack_005_design_source.py"
VALIDATOR_PATH = ROOT / "research" / "validate_trendstack_005_design_source.py"
VALIDATOR_TEST_PATH = ROOT / "tests" / "test_validate_trendstack_005_design_source.py"
COLLECTION_PLAN_NAME = "DATASET-FIVEPERCENT-EURUSD-H1-SPLITVAULT-001_PLAN.md"
HYPOTHESIS_PLAN_NAME = "HYP-TRENDSTACK-EURUSD-H1-005_PROBE_PLAN.md"
REGISTRY_NAME = "CANDIDATE_REGISTRY.jsonl"
REGISTRY_ROW_INDEX = 279
REGISTRY_ROW_SHA256 = "C020BC354829FB7E9273D424F960F3C41ABB3912E6B9F7C09B9C4E9F86F8ABD1"
PARENT_REGISTRY_ROW_INDEX = 277
PARENT_REGISTRY_ROW_SHA256 = "E01166D8952722B69BAF6C9AC72B95D374BDC365F5189A390AE9D77900211ED9"
PARENT_FAILURE_MANIFEST_SHA256 = "51E87178ABA98F9C848D7EAA0501197F44C9356E9B023FD4AB52A414354C0461"
PARENT_TERMINAL_SHA256 = "A6B912C84175FC45B871B7683DD2BC2F05591D99F93299E6E9636E42A70A8B3D"
ACTIVE_REGISTRY_PATH = ROOT.parents[1] / "04. Memory" / "research" / REGISTRY_NAME
PARENT_EVIDENCE_ROOT = (
    ROOT
    / "research"
    / "evidence"
    / "HYP-TRENDSTACK-EURUSD-H1-004_SOURCE_ATTEMPTS"
    / "HYP004-SOURCE-ATTEMPT-2C82C993A390A56A"
)
SOURCE_ATTEMPT_ID = "HYP005-SOURCE-ATTEMPT-0123456789ABCDEF"
AUTHORITY_SENTINEL = b"REVIEWED_RUN_PACKET_SHA256: str | None = None"


def load_tool():
    spec = importlib.util.spec_from_file_location("h1_splitvault_001_supervisor_test", TOOL_PATH)
...
def packet_fixture(tmp_path: Path, *, copied_tools: bool = False):
    ...
    evidence_parent = tmp_path / "evidence" / "HYP-TRENDSTACK-EURUSD-H1-005_SOURCE_ATTEMPTS"
    evidence_parent.mkdir(parents=True, exist_ok=True)
    files = {
        "collection_plan": bound_file(tmp_path, COLLECTION_PLAN_NAME, b"collection 002\n"),
        "hypothesis_plan": bound_file(tmp_path, HYPOTHESIS_PLAN_NAME, b"hypothesis 005\n"),
        "registry": bound_file(tmp_path, REGISTRY_NAME, frozen_registry_payload()),
        "source": bound_file(
            tmp_path,
            "source.parquet",
            b"PAR1synthetic-body" + b"footer" + (6).to_bytes(4, "little") + b"PAR1",
        ),
        ...
        "parent_failure_manifest": bound_file(
            tmp_path, "failure_manifest.json", (PARENT_EVIDENCE_ROOT / "failure_manifest.json").read_bytes()
        ),
        "parent_attempt_terminal": bound_file(
            tmp_path, "attempt_terminal.json", (PARENT_EVIDENCE_ROOT / "attempt_terminal.json").read_bytes()
        ),
        ...
        "custodian_test": bound_file(tmp_path, "test_custodian.py", (ROOT / "tests" / "test_h1_splitvault_001_custodian.py").read_bytes()),
        "supervisor_test": bound_file(tmp_path, "test_supervisor.py", (ROOT / "tests" / "test_h1_splitvault_001_supervisor.py").read_bytes()),
        "design_builder_test": bound_file(tmp_path, "test_builder.py", (ROOT / "tests" / "test_build_trendstack_005_design_source.py").read_bytes()),
    ...
    packet = {
        ...
        "parent_failure_manifest_path": str(files["parent_failure_manifest"].resolve()),
        "parent_failure_manifest_sha256": sha256(files["parent_failure_manifest"].read_bytes()),
        "parent_attempt_terminal_path": str(files["parent_attempt_terminal"].resolve()),
        "parent_attempt_terminal_sha256": sha256(files["parent_attempt_terminal"].read_bytes()),
        ...
        "source_footer_length": 6,
        "source_footer_start": files["source"].stat().st_size - 8 - 6,
        "source_footer_sha256": sha256(files["source"].read_bytes()[-14:]),
...
def test_hyp005_packet_requires_every_exact_authority_field(tmp_path: Path, missing: str) -> None:
    tool, _, packet, packet_path, frozen = packet_fixture(tmp_path)
    packet.pop(missing)
    packet_path.write_bytes(canonical(packet) + b"\n")
    with pytest.raises(tool.InvalidSupervisor, match="^INVALID_SUPERVISOR$"):
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "HYP005-SOURCE-ATTEMPT-0123456789ABCDE",
        "HYP005-SOURCE-ATTEMPT-0123456789ABCDEG",
        "HYP004-SOURCE-ATTEMPT-0123456789ABCDEF",
        "hyp004-source-attempt-0123456789abcdef",
        "HYP005-SOURCE-ATTEMPT-../123456789ABC",
        "HYP005-SOURCE-ATTEMPT-0123456789ABC\nEF",
        123,
    ],
)
...
def test_frozen_plan_and_registry_drift_fail_before_source_open(tmp_path: Path, attack: str) -> None:
    ...
    if attack == "collection_hash":
        packet["collection_plan_sha256"] = "A" * 64
    elif attack == "collection_path":
        wrong = bound_file(tmp_path, "wrong_collection.md", files["collection_plan"].read_bytes())
        packet["collection_plan_path"] = str(wrong.resolve())
        packet["collection_plan_sha256"] = sha256(wrong.read_bytes())
    elif attack == "hypothesis_hash":
        packet["hypothesis_plan_sha256"] = "C" * 64
    elif attack == "hypothesis_path":
        wrong = bound_file(tmp_path, "wrong_hypothesis.md", files["hypothesis_plan"].read_bytes())
        packet["hypothesis_plan_path"] = str(wrong.resolve())
        packet["hypothesis_plan_sha256"] = sha256(wrong.read_bytes())
...
@pytest.mark.parametrize("attack", ["wrong_index", "wrong_row_hash", "changed_row", "later_hyp005"])
def test_hyp005_registry_row_authority_fails_before_marker_and_source(tmp_path: Path, attack: str) -> None:
    tool, files, packet, packet_path, _ = packet_fixture(tmp_path)
...
@pytest.mark.parametrize(
    "attack",
    ["parent_row_index", "parent_row_hash", "later_parent_row", "failure_manifest", "terminal"],
)
def test_parent_parked_row_and_failure_evidence_are_exact_before_marker_or_source(
    tmp_path: Path, attack: str
) -> None:
    tool, files, packet, packet_path, _ = packet_fixture(tmp_path)
    if attack == "parent_row_index":
        packet["parent_registry_row_index"] = PARENT_REGISTRY_ROW_INDEX - 1
    elif attack == "parent_row_hash":
        packet["parent_registry_row_sha256"] = "B" * 64
    elif attack == "later_parent_row":
        later = canonical({"hypothesis_id": "HYP-TRENDSTACK-EURUSD-H1-004", "state": "parked"}) + b"\n"
        files["registry"].write_bytes(files["registry"].read_bytes() + later)
        packet["registry_sha256"] = sha256(files["registry"].read_bytes())
    elif attack == "failure_manifest":
        files["parent_failure_manifest"].write_bytes(files["parent_failure_manifest"].read_bytes() + b" ")
        packet["parent_failure_manifest_sha256"] = sha256(files["parent_failure_manifest"].read_bytes())
    else:
        files["parent_attempt_terminal"].write_bytes(files["parent_attempt_terminal"].read_bytes() + b" ")
        packet["parent_attempt_terminal_sha256"] = sha256(files["parent_attempt_terminal"].read_bytes())
    frozen = refreeze_packet(tool, files, packet, packet_path)
    source = Path(str(packet["source_path"])).absolute()
...
def test_wrong_h1_footer_digest_is_rejected_before_marker_or_source(tmp_path: Path) -> None:
    tool, files, packet, packet_path, _ = packet_fixture(tmp_path)
    packet["source_footer_sha256"] = "92E8403266EF971ED2F4C05523ECB6C10AE5B5723F0F7504E09694663A779727"
    frozen = refreeze_packet(tool, files, packet, packet_path)
    with pytest.raises(tool.InvalidSupervisor, match="^INVALID_SUPERVISOR$"):
        tool._supervise_reviewed(packet_path, sha256(packet_path.read_bytes()), frozen)
    assert not Path(str(packet["attempt_evidence_root"])).exists()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("parent_failure_manifest_path", "parent_failure_manifest"),
        ("parent_attempt_terminal_path", "parent_attempt_terminal"),
...
def test_attempt_paths_are_deterministic_and_alternate_id_cannot_reuse_them(tmp_path: Path) -> None:
    ...
    packet["source_attempt_id"] = "HYP005-SOURCE-ATTEMPT-FEDCBA9876543210"
    packet_path.write_bytes(canonical(packet) + b"\n")
    review_base = TOOL_PATH.read_bytes()
    files["supervisor_tool"].write_bytes(armed_supervisor(review_base, sha256(packet_path.read_bytes())))
    frozen = tool.FrozenBindings.from_packet_for_testing(packet)
...
def test_production_paths_pin_hyp005_parent_failure_and_h1_footer() -> None:
    tool = load_tool()
    paths = tool._production_expected_paths(SOURCE_ATTEMPT_ID)
    assert tool.COLLECTION_PLAN_SHA256 == "270D46B4A96B40196F5F359290D199DAE5A2D9D0D174491C349A1404CA088769"
    assert tool.HYPOTHESIS_PLAN_SHA256 == "53A1A54B9AD3CD0A1AFFF46C4804F99FCF1CD6E091CAE984AAACBEA4210F1B33"
    assert tool.REGISTRY_ROW_INDEX == REGISTRY_ROW_INDEX
    assert tool.REGISTRY_ROW_SHA256 == REGISTRY_ROW_SHA256
    assert tool.PARENT_REGISTRY_ROW_INDEX == PARENT_REGISTRY_ROW_INDEX
    assert tool.PARENT_REGISTRY_ROW_SHA256 == PARENT_REGISTRY_ROW_SHA256
    assert tool.PARENT_FAILURE_MANIFEST_SHA256 == PARENT_FAILURE_MANIFEST_SHA256
    assert tool.PARENT_TERMINAL_SHA256 == PARENT_TERMINAL_SHA256
    assert tool.SOURCE_FOOTER_LENGTH == 5392
    assert tool.SOURCE_FOOTER_START == 2776497
    assert tool.SOURCE_FOOTER_SHA256 == "01C090CF494A45AC99603E8A4BBE3447884253DEF3828964AE5555086FF91E3B"
    assert tool.SOURCE_SHA256 == "71860016AF1BD1B17353B043AFF799233A787E9DF3F587913FCD2F5328BB1E08"
    assert tool.SOURCE_BYTES == 2781897
    assert tool.SOURCE_ROWS == 71785
    assert tool.EXPECTED_ROWS_PER_DAY == 7
    assert tool.EXPECTED_TOTAL_ROWS == 9079
    assert paths["collection_plan_path"] == ROOT / "research" / COLLECTION_PLAN_NAME
    assert paths["hypothesis_plan_path"] == ROOT / "research" / HYPOTHESIS_PLAN_NAME
    assert paths["registry_path"] == ROOT.parents[1] / "04. Memory" / "research" / REGISTRY_NAME
    assert paths["parent_failure_manifest_path"] == PARENT_EVIDENCE_ROOT / "failure_manifest.json"
    assert paths["parent_attempt_terminal_path"] == PARENT_EVIDENCE_ROOT / "attempt_terminal.json"
    assert paths["attempt_evidence_root"].name == SOURCE_ATTEMPT_ID
    assert paths["custody_stage_path"].name == ".h1_splitvault_001.attempt-" + SOURCE_ATTEMPT_ID
    assert paths["design_stage_path"].name == ".trendstack_005_design_h1.attempt-" + SOURCE_ATTEMPT_ID
    output_text = "\n".join(
        str(paths[key])
        for key in ("splitvault_output_root", "design_source_output_root", "custody_stage_path", "design_stage_path")
    ).lower()
    assert "h1_splitvault_001" in output_text and "trendstack_005_design_h1" in output_text
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda p: {**p, "economics_authorized": True},
        lambda p: {**p, "mt5_authorized": True},
        lambda p: {**p, "validation_authorized": True},
        lambda p: {**p, "holdout_authorized": True},
        lambda p: {**p, "promotion_authorized": True},
        lambda p: {**p, "paper_authorized": True},
        lambda p: {**p, "live_authorized": True},
        lambda p: {**p, "deploy_authorized": True},
...
def test_start_marker_precedes_stages_and_success_terminal_reconciles(tmp_path: Path) -> None:
    ...
    assert started["collection_plan_sha256"] == packet["collection_plan_sha256"]
    assert started["hypothesis_plan_sha256"] == packet["hypothesis_plan_sha256"]
    assert started["registry_row_index"] == REGISTRY_ROW_INDEX
    assert started["registry_row_sha256"] == REGISTRY_ROW_SHA256
    assert started["parent_registry_row_index"] == PARENT_REGISTRY_ROW_INDEX
    assert started["parent_registry_row_sha256"] == PARENT_REGISTRY_ROW_SHA256
    assert started["parent_failure_manifest_sha256"] == PARENT_FAILURE_MANIFEST_SHA256
    assert started["parent_attempt_terminal_sha256"] == PARENT_TERMINAL_SHA256
    assert started["source_footer_length"] == packet["source_footer_length"]
    assert started["source_footer_start"] == packet["source_footer_start"]
    assert started["source_rows"] == packet["source_rows"]
...
def test_supervisor_rejects_bare_or_mismatched_pending_design_result(tmp_path: Path, attack: str) -> None:
    ...
    if attack == "bare":
        ...
        pending["source_attempt_id"] = "HYP005-SOURCE-ATTEMPT-FEDCBA9876543210"
    elif attack == "path":
...
def test_source_has_no_network_subprocess_or_dynamic_shell_surface() -> None:
    ...
    for forbidden in (
        "hyp-trendstack-eurusd-h1-003_source_run_packet",
        ".splitvault_001.attempt-",
        ".trendstack_003_design_h1.attempt-",
    ):
...
def test_verified_builder_executes_inside_child_boundary_on_public_bytes_only(tmp_path: Path) -> None:
    tool = load_tool()
    fixtures = load_path(ROOT / "tests" / "test_build_trendstack_005_design_source.py", "sealed_builder_fixtures")
    day = "2016-01-04"
    builder, _, projection, date_hash = fixtures.write_mixed_projection_fixture(tmp_path, [day])
    payload = fixtures.payload_for(fixtures.hour_rows(day), tmp_path, "day.parquet")
    ...
    assert (output / "raw_h1" / "DESIGN" / day / "1200_1800.parquet").is_file()
