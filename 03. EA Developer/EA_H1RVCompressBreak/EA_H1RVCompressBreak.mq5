//+------------------------------------------------------------------+
//| EA_H1RVCompressBreak.mq5 — H1 RV-compress → Donchian break RR=2.5|
//| Symbol: USDJPY | Period: H1 | Magic: 880994                       |
//| Hypothesis: HYP-H1-RV-COMPRESS-BREAK-001                          |
//| Closed-bar[1] only. Mon–Thu; weekend flat; risk 0.5%.             |
//| Not VolExp M15 expand; not Keltner; not NR7; not LowVol fade.     |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_H1RVCompressBreak"
...
input group "=== Compress + Break ==="
input int      InpShortRV       = 6;      // short range-RV window
input int      InpLongRV        = 48;     // long range-RV window
input double   InpCompressMax   = 0.55;   // short/long RV ratio max
input int      InpDonchian      = 20;
input double   InpMinBodyATR    = 0.40;   // break bar body quality
...
int OnInit()
{
   if(Period() != PERIOD_H1)
   {
      Print("[RVCB] FAIL — chart must be H1");
      return INIT_FAILED;
   }
   ...
   Print("[RVCB] HYP-H1-RV-COMPRESS-BREAK-001 RR=", InpTP_Ratio,
         " compress<=", InpCompressMax);
...
double RangeRV(const int startShift, const int bars)
{
   double sum = 0.0;
...
bool DetectCompressBreak(bool &isBuy)
{
   isBuy = false;
   // Compression measured on bars ending at shift 2 (fully closed before signal bar)
   double shortRV = RangeRV(2, InpShortRV);
   double longRV = RangeRV(2, InpLongRV);
   if(shortRV <= 0.0 || longRV <= 0.0)
      return false;
   if((shortRV / longRV) > InpCompressMax)
      return false;
...
void OnTick()
{
   ...
   MqlTradeResult res = {};
   if(!SendDealWithRetry(isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL, lot, sl, tp, 0, "RVCB|brk", res))
      return;
