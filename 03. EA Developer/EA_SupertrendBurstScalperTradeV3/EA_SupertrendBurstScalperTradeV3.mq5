...
#property description "H1 Supertrend flip with durable execution and lifecycle-v3 evidence."

input string InpHypothesisId        = "HYP-STBS-XAUUSD-M15-013";
input string InpVariantTag          = "STBS_H1_FLIP_M15_BURST_TRADE_FSM_V3_TELEMETRY";
input bool   InpAuditOnly           = false;
input bool   InpEnableTelemetry     = true;
input long   InpMagic               = 5604113;
...
const string EA_NAME              = "EA_SupertrendBurstScalperTradeV3";
...
bool WriteRunMeta()
{
   ...
   const string payload=StringFormat(
      ...
      "\"geometry_ready\":%I64d,\"entries\":%I64d,\"entry_rejects\":%I64d,"
      "\"closes\":%I64d,\"lifecycle_rows\":%I64d,\"runtime_failed\":%s}}",
      ...
      g_geometry_ready_events,g_entries_submitted,g_entry_rejects,
      g_closes_submitted,g_lifecycle_rows,g_runtime_failed ? "true" : "false");
...
bool PositionDealVolumes(const ulong position_id,double &open_volume,double &close_volume)
{
   open_volume=0.0;
   close_volume=0.0;
...
bool LogLifecycleDeal(const ulong deal)
{
   ...
   if(!PositionDealVolumes(position_id,open_volume,close_volume))
      return false;
...
int OnInit()
{
   if(!MQLInfoInteger(MQL_TESTER) || MQLInfoInteger(MQL_OPTIMIZATION) ||
      _Symbol!="XAUUSD" || _Period!=PERIOD_M15 ||
       InpHypothesisId!="HYP-STBS-XAUUSD-M15-013" ||
       InpVariantTag!="STBS_H1_FLIP_M15_BURST_TRADE_FSM_V3_TELEMETRY" ||
       !InpEnableTelemetry || InpMagic!=5604113 ||
   ...
   if(!OpenTelemetry() || !RecoverTelemetryPositionContext())
   {
      g_runtime_failed=true;
