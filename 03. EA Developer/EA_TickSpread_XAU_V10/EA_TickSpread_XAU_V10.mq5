//+------------------------------------------------------------------+
//| EA_TickSpread_XAU_V10.mq5                                       |
//| HYP-TSDR-XAUUSD-TICK-001: tick spread dislocation and refill    |
...
input string InpHypothesisId="HYP-TSDR-XAUUSD-TICK-001";
...
input long   InpMagic=5605601;

const string EA_NAME="EA_TickSpread_XAU_V10";
const string EXPECTED_HYPOTHESIS="HYP-TSDR-XAUUSD-TICK-001";
...
bool EmitSeriesProof()
  {
   long sync=0,first=0,terminal_first=0,server_first=0,bars=0;
   if(!SeriesInfoInteger(_Symbol,PERIOD_M1,SERIES_SYNCHRONIZED,sync) ||
      !SeriesInfoInteger(_Symbol,PERIOD_M1,SERIES_FIRSTDATE,first) ||
      !SeriesInfoInteger(_Symbol,PERIOD_M1,SERIES_TERMINAL_FIRSTDATE,terminal_first) ||
      !SeriesInfoInteger(_Symbol,PERIOD_M1,SERIES_SERVER_FIRSTDATE,server_first) ||
      !SeriesInfoInteger(_Symbol,PERIOD_M1,SERIES_BARS_COUNT,bars))return(false);
   const long maxbars=TerminalInfoInteger(TERMINAL_MAXBARS);
   datetime a[];ArraySetAsSeries(a,false);ResetLastError();
   const int n=CopyTime(_Symbol,PERIOD_M1,(datetime)first,1,a);
   const int err=GetLastError();
   const long copied=(n==1?(long)a[0]:0);
   PrintFormat("DATA_EPOCH_D0_SERIES_PROOF symbol=%s m1_synchronized=%I64d m1_first_epoch=%I64d m1_terminal_first_epoch=%I64d m1_server_first_epoch=%I64d m1_bars=%I64d terminal_maxbars=%I64d copytime_result=%d copytime_first_epoch=%I64d copytime_last_error=%d",_Symbol,sync,first,terminal_first,server_first,bars,maxbars,n,copied,err);
   return(sync==1&&first>0&&terminal_first>0&&server_first>0&&bars>0&&maxbars>0&&n==1&&copied==first&&err==0);
  }
...
bool InputsAreFrozen()
  {
   return(InpResearchAutoMode&&InpEnableTelemetry&&InpHypothesisId==EXPECTED_HYPOTHESIS&&
          ...
          MathAbs(InpLongZeroRunMaxPct-40.0)<1e-12&&InpHistogramMaxPoints==HIST_CAP&&InpMagic==5605601);
  }
...
int OnInit()
  {
   ...
   PrintFormat("TSDR001_INIT ea=%s hypothesis=%s variant=%s symbol=%s timeframe=M1 required_broker_days=%d preflight_only=true",EA_NAME,InpHypothesisId,InpVariantTag,_Symbol,InpRequiredBrokerDays);
...
void OnDeinit(const int reason)
  {
   if(!g_finalized)
      PrintFormat("TSDR001_PREFLIGHT_INCOMPLETE reason=%d broker_days=%d required_days=%d total_ticks=%I64d distribution_ticks=%I64d",reason,g_distinct_days,InpRequiredBrokerDays,g_total_ticks,g_distribution_ticks);
   PrintFormat("TSDR001_SUMMARY reason=%d finalized=%s result=%s runtime_failed=%s broker_days=%d total_ticks=%I64d valid_bid_ask=%I64d invalid_quotes=%I64d non_monotonic=%I64d same_time_ticks=%I64d distribution_ticks=%I64d hist_overflow=%I64d",reason,(g_finalized?"true":"false"),(g_finalized?(g_passed?"PASS":"DATA_FRONTIER_BLOCKED"):"INCOMPLETE"),(g_runtime_failed?"true":"false"),g_distinct_days,g_total_ticks,g_valid_quotes,g_invalid_quotes,g_non_monotonic,g_same_time,g_distribution_ticks,g_hist_overflow);
  }
