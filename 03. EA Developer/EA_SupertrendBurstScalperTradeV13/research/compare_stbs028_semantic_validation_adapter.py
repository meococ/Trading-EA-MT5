...
HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-028"
PARENT_ID = "HYP-STBS-XAUUSD-M15-027"
TARGET_ID = "HYP-STBS-XAUUSD-M15-026"
ATTEMPT_ID = "STBS028-COMPARATOR-001"
...
PARENT_ATTEMPT_ROOT = ROOT / "02. AlphaFactory/runtime/comparator_attempts/HYP-STBS-XAUUSD-M15-027/STBS027-COMPARATOR-001"

PREREG = Path(__file__).with_name("HYP-STBS-XAUUSD-M15-028_SEMANTIC_VALIDATION_ADAPTER_RECOVERY_PREREG.md")
TEST = Path(__file__).with_name("tests") / "test_stbs028_semantic_validation_adapter.py"
PRE_REVIEW = Path(__file__).with_name("HYP-STBS-XAUUSD-M15-028_INDEPENDENT_PRE_COMPARATOR_REVIEW.md")
PARENT_FAILURE = Path(__file__).with_name("HYP-STBS-XAUUSD-M15-027_COST_RUNMETA_ADAPTER_FAILURE.md")
PARENT_REVIEW = Path(__file__).with_name("HYP-STBS-XAUUSD-M15-027_INDEPENDENT_POST_FAILURE_REVIEW.md")
PARENT_TASK = Path(__file__).with_name("HYP-STBS-XAUUSD-M15-027_SEALED_EXISTING_RUN_RECOVERY_PREREG.md")
...
PARENT_TERMINAL_ROW_SHA = "020AA793BB63BA4003F3555F627ADA3C07BB61DCAECAA95BE5E839343885D68E"
PARENT_TERMINAL_VERDICT = "KILL_VERIFIED_COST_RUNMETA_SEMANTIC_VALIDATION_ADAPTER_FIELD_MISMATCH_BEFORE_UNIFIED_NO_ECONOMIC_VERDICT"
PARENT_START_SHA = "293C6125CA13EF47F69CB9460B8AEF41144553454A387B734FC51A4BC5EF4324"
PARENT_TERMINAL_SHA = "A94C59ABE091962944F66B6ED0C0E0EFA0709D4AB6C9ACBA450543CF6F46348F"
PARENT_FAILURE_SHA = "695EFCFCEE1F2073F9C5EC1DB0FB220435FBBB939F12892F2AC205E756CA7F04"
PARENT_REVIEW_SHA = "12644767F4259EB5775F194E6B1B9D7897C2B1B8AD7ABF5F5264244B89EB69F0"
PARENT_TASK_SHA = "2F929D2B5A0E9D6CA392CBD4E4FC520286019746422128273557ADBEE7CBA70C"
...
PARENT_AUTHORITY_ROW_SHA = "F3DB9F70D36A2016FB91F20046BE0B892BBD5C4B174E08090B5E5A44924B23DF"
PARENT_NR_ARTIFACT_SHA = "FFD938A910792867BCDD7D1E642DBCB1A011CB327897E368F234B34CD1B471D0"
PARENT_COST_ARTIFACT_SHA = "FDCC562F6C034DEE1F6F9785074F236B241B29ECE0F2C1FCD5148D7709C67A2C"
PARENT_DERIVED_RUN_MANIFEST_SHA = "7574C68AAB54875AD0BFC7E104AB080CF24793405ADC2B255E5731F4FB902982"
PARENT_DERIVED_COST_MANIFEST_SHA = "A58C135B9D7CAD02798B592E536D102CB287F32FD281F0BF715114A0B1D75A38"
...
def claim() -> tuple[Path, str]:
    ...
    raw = canonical(
        {
            "schema_version": "stbs028_comparator_attempt_started.v1",
...
def validate_authority_row(row: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise RuntimeError("HYP028 root authority mismatch")
    expected = {
        ...
        "parent_prereg_sha256": PARENT_TASK_SHA,
        "parent_authority_row_sha256": PARENT_AUTHORITY_ROW_SHA,
        "parent_nonrepaint_artifact_sha256": PARENT_NR_ARTIFACT_SHA,
        "parent_verified_cost_artifact_sha256": PARENT_COST_ARTIFACT_SHA,
        "parent_derived_run_manifest_sha256": PARENT_DERIVED_RUN_MANIFEST_SHA,
        "parent_derived_cost_manifest_sha256": PARENT_DERIVED_COST_MANIFEST_SHA,
...
def authority() -> tuple[dict[str, Any], str, str, bytes, bytes, bytes]:
    ...
    if not parent_rows:
        raise RuntimeError("terminal HYP027 row absent")
    ...
    if (
        ...
        raise RuntimeError("terminal HYP027 row mismatch")
    if not own_rows:
        raise RuntimeError("HYP028 authority absent")
...
def validate_cost(cost: dict[str, Any], report_path: Path) -> None:
    ...
    if "semantic" in runmeta:
        raise RuntimeError("verified cost RunMeta legacy/dual semantic field is forbidden")
    semantic = runmeta.get("semantic_validation")
    expected_semantic = {
    ...
    if semantic != expected_semantic:
        raise RuntimeError("verified cost RunMeta semantic_validation mismatch")
...
def execute() -> dict[str, Any]:
    ...
    try:
        ...
        bind("parent_prereg.md", PARENT_TASK, PARENT_TASK_SHA)
        bind("parent_nonrepaint_audit.json", PARENT_ATTEMPT_ROOT / "analysis/nonrepaint_audit.json", PARENT_NR_ARTIFACT_SHA)
        bind("parent_verified_cost_artifact.json", PARENT_ATTEMPT_ROOT / "analysis/verified_cost_artifact.json", PARENT_COST_ARTIFACT_SHA)
        bind("parent_derived_run_manifest.json", PARENT_ATTEMPT_ROOT / "sealed_run/run_manifest.json", PARENT_DERIVED_RUN_MANIFEST_SHA)
        bind("parent_derived_cost_manifest.json", PARENT_ATTEMPT_ROOT / "controls/derived_cost_manifest.json", PARENT_DERIVED_COST_MANIFEST_SHA)
        ...
        result = {
            "schema_version": "stbs028_semantic_validation_adapter_result.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "target_hypothesis_id": TARGET_ID,
            "target_run_id": RUN_ID,
            "verdict": verdict,
        ...
        receipt = {
            "schema_version": "stbs028_comparator_receipt.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
            "created_at_utc": now(),
        ...
        terminal = {
            "schema_version": "stbs028_comparator_attempt_terminal.v1",
            "hypothesis_id": HYPOTHESIS_ID,
            "attempt_id": ATTEMPT_ID,
            "status": "COMPLETE",
...
def main() -> int:
    ...
    if not args.execute:
        print("STBS028 comparator package loaded; --execute requires the sole frozen authority")
        return 0
