...
#property description "H1 Supertrend flip with broker-aware margin safety and reconciled lifecycle evidence."

input string InpHypothesisId        = "HYP-STBS-XAUUSD-M15-014";
input string InpVariantTag          = "STBS_H1_FLIP_M15_BURST_TRADE_FSM_V4_MARGIN_SAFE";
input bool   InpAuditOnly           = false;
input bool   InpEnableTelemetry     = false;
input long   InpMagic               = 5604114;
...
input double InpStopoutHeadroomFactor       = 1.25;
...
const string EA_NAME              = "EA_SupertrendBurstScalperTradeV4";
...
MarginSafetyResult EvaluateMarginCandidate(EntryPlan &plan,const double volume)
{
   ...
   if(so_mode==ACCOUNT_STOPOUT_MODE_PERCENT)
   {
      threshold=MathMax(InpMinProjectedMarginLevelPct,
                        MathMax(so_call,so_stop)*InpStopoutHeadroomFactor);
      safe=check.margin_level>=threshold;
   }
   else
   {
      threshold=MathMax(so_call,so_stop)*InpStopoutHeadroomFactor;
...
void ConsumeFlipEvent(const MqlRates &bar,const int prior_state,const datetime next_time)
{
   ...
   PrintFormat("STBS_SIGNAL|source=%s|decision=%s|source_epoch=%I64d|decision_epoch=%I64d|direction=%s|exact_next=true|atr_ready=%s|geometry_ready=%s|margin_ready=%s|atr=%.8f|entry=%.8f|sl=%.8f|tp=%.8f|volume=%.8f|projected_level=%.8f|audit=%s",
               TimeToString(bar.time,TIME_DATE|TIME_SECONDS),
               TimeToString(next_time,TIME_DATE|TIME_SECONDS),
               (long)ServerToUtc(bar.time),(long)ServerToUtc(next_time),
...
int OnInit()
{
   if(!MQLInfoInteger(MQL_TESTER) || MQLInfoInteger(MQL_OPTIMIZATION) ||
      _Symbol!="XAUUSD" || _Period!=PERIOD_M15 ||
       InpHypothesisId!="HYP-STBS-XAUUSD-M15-014" ||
       InpVariantTag!="STBS_H1_FLIP_M15_BURST_TRADE_FSM_V4_MARGIN_SAFE" ||
       !InpAuditOnly || InpEnableTelemetry || InpMagic!=5604114 ||
