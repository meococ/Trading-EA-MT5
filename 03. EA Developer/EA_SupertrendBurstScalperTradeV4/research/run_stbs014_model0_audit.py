#!/usr/bin/env python3
"""One-shot, zero-trade MT5 correctness audit for HYP-STBS-XAUUSD-M15-014."""
...
HYPOTHESIS_ID = "HYP-STBS-XAUUSD-M15-014"
ATTEMPT_ID = "STBS014-MODEL0-AUDIT-001"
EA_NAME = "EA_SupertrendBurstScalperTradeV4"
MAGIC = 5604114
OVERRIDES = (
    "InpAuditOnly=true;"
    f"InpHypothesisId={HYPOTHESIS_ID};InpMagic={MAGIC};"
...
COST_MANIFEST = ROOT / "03. EA Developer" / EA_NAME / "research" / f"{HYPOTHESIS_ID}_COLLECTION_ONLY_COST_MANIFEST.json"
...
EXPECTED_PREREG_SHA = "033A6247186BF3C2459F6A8420B22C4E3FD350574B0F42FDFC10F7BB2E51076D"
...
def read_registry_authority() -> tuple[dict, str, str]:
    ...
    if latest is None or latest_raw is None:
        raise RuntimeError("HYP014 authority row is missing")
...
def ensure_clean_stable_sidecars() -> None:
    names = {
        f"XAUUSD_LifecycleTrades_{HYPOTHESIS_ID}_{MAGIC}.csv",
        f"XAUUSD_RunMeta_{HYPOTHESIS_ID}_{MAGIC}.json",
    }
...
def validate_journal_text(text: str) -> dict:
    ...
    expected_summary = {
        "hypothesis": HYPOTHESIS_ID, "raw": "690", "executable": "683", "gaps": "7",
...
def execute() -> dict:
    ...
    try:
        ...
        cmd = [
            ...
            "-Spread", "current", "-ContractReceipt", str(RECEIPT),
