#!/usr/bin/env python3
"""Inert source/cadence probe for HYP-ARUC-EURUSD-M15-003.
...
HYPOTHESIS_ID = "HYP-ARUC-EURUSD-M15-003"
PARENT_HYPOTHESIS_ID = "HYP-ARUC-EURUSD-M15-002"
EA_NAME = "EA_ActivityResponseContinuation"
FAMILY = "activity-response-underreaction-continuation"
ATTEMPT_ID = "ARUC003-SOURCE-ATTEMPT-001"

PLAN_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "HYP-ARUC-EURUSD-M15-003_SOURCE_FEASIBILITY_PLAN.md"
)
PLAN_SHA256 = "FB5275217AEF154AF7A254190B937D85F1BCDB220647283B4F13FF590BBEEE16"
BUILDER_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "build_aruc_003_source.py"
)
TEST_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/tests/"
    "test_build_aruc_003_source.py"
)
EVIDENCE_ROOT_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "evidence/HYP-ARUC-EURUSD-M15-003_SOURCE_FEASIBILITY_ATTEMPTS/"
    f"{ATTEMPT_ID}"
)
REVIEW_RECEIPT_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "HYP-ARUC-EURUSD-M15-003_SOURCE_IMPLEMENTATION_REVIEW_RECEIPT.json"
)
REVIEW_RECEIPT_SCHEMA = "aruc_003_source_implementation_review_receipt.v1"
...
def _prepare_activity_indexes(
    ...
    date_index = types.MappingProxyType({day: index for index, day in enumerate(dates)})
    bars_by_date_slot = types.MappingProxyType(_bar_index(bars))
    return dates, date_index, bars_by_date_slot
...
def _activity_ratio_indexed(
    ...
    *,
    ...
    index = date_index.get(current_date) if type(current_date) is date else None
    if index is None or index < lookback:
        return None
    values: list[int] = []
    for prior_date in dates[index - lookback : index]:
        prior = bars_by_date_slot.get((prior_date, slot))
...
def activity_ratio_for(
    ...
    *,
    ...
    dates, date_index, bars_by_date_slot = _prepare_activity_indexes(bars, business_dates)
    return _activity_ratio_indexed(
        current,
        dates,
        date_index,
        bars_by_date_slot,
...
def _shifted_tick_features_indexed(
    current: Mapping[str, object],
    dates: tuple[date, ...],
    date_index: Mapping[date, int],
    bars_by_date_slot: Mapping[tuple[date, tuple[int, int]], Mapping[str, object]],
    *,
    ...
    current_date = current.get("date")
    slot = current.get("slot")
    current_index = date_index.get(current_date) if type(current_date) is date else None
    if current_index is None:
        return None
    source_index = current_index - shift_dates
    if source_index < lookback:
        return None
    source = bars_by_date_slot.get((dates[source_index], slot))
    if source is None:
        return None
...
def shifted_tick_features(
    ...
    *,
    ...
    dates, date_index, bars_by_date_slot = _prepare_activity_indexes(bars, business_dates)
    return _shifted_tick_features_indexed(
        current,
        dates,
        date_index,
        bars_by_date_slot,
...
def build_arm_ledgers(
    ...
    for arm in arms:
        ...
        for signal in ordered:
            ...
            signal_id = f"ARUC003-{arm}-{sha256_bytes(identity)[:16]}"
...
def _scan_source(m1_rows: Sequence[Mapping[str, object]], h1_rows: Sequence[Mapping[str, object]], business_dates: Sequence[date]) -> dict[str, object]:
    ...
    formation_scheduled = max(0, len(dates) - 20) * 32
    ...
    for bar in bars:
        ...
        shifted = _shifted_tick_features_indexed(
            bar,
            dates,
            date_index,
            bars_by_date_slot,
    ...
    report = {
        "schema_version": "aruc_003_source_feasibility_report.v1",
