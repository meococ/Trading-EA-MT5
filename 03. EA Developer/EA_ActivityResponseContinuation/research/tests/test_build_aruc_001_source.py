...
SOURCE = Path(__file__).resolve().parents[1] / "build_aruc_001_source.py"
SPEC = importlib.util.spec_from_file_location("build_aruc_001_source", SOURCE)
...
def registry_fixture() -> tuple[bytes, dict[str, object]]:
    validation = {
        "source_feasibility_only": True,
        "source_feasibility_run_authorized": True,
        "source_feasibility_attempt_id": "ARUC001-SOURCE-ATTEMPT-001",
    ...
    row = {
        "record_type": "hypothesis_state", "schema_version": "alphafactory_candidate_registry.v1",
        "hypothesis_id": sut.HYPOTHESIS_ID, "ea_name": sut.EA_NAME, "feature_family": sut.FAMILY,
        "state": "probe", "model": None, "source_path": None, "source_hash": None, "run_ids": [],
        "prereg_path": sut.PLAN_REL, "prereg_sha256": sut.PLAN_SHA256, "validation": validation,
    }
    payload = canonical(row) + b"\n"
    packet = {"registry_row_index": 1, "registry_row_sha256": sha(payload)}
    return payload, packet


def registry_v2_fixture(*, with_noncanonical_history: bool = True) -> tuple[bytes, dict[str, object]]:
    selected_payload, _ = registry_fixture()
    selected = json.loads(selected_payload)
    selected["validation"].update(
        {
            "registry_validator_path": "04. Memory/research/validate_candidate_registry.py",
            "registry_validator_sha256": "B04B379E11F556A0CF3E6C3264768176310FF01CF360CC3B92464C51A2996DD0",
            "registry_schema_path": "04. Memory/research/CANDIDATE_REGISTRY.schema.json",
            "registry_schema_sha256": "96C80D3C46A105A9754CA1325F3DD6C160D92A9D5800ECBC402DE0F40C612F5C",
        }
    )
    selected_payload = canonical(selected) + b"\n"
    history = b'{ "hypothesis_id": "HYP-HISTORY-001" }\n' if with_noncanonical_history else b""
    payload = history + selected_payload
    packet = {
        "registry_row_index": 2 if with_noncanonical_history else 1,
        "registry_row_sha256": sha(selected_payload),
    }
    return payload, packet
...
def test_registry_exact_validation_whitelist_is_accepted() -> None:
    registry, packet = registry_fixture()

    parsed = sut.validate_registry_authority(registry, packet)
...
def test_registry_false_authorities_reject_false_aliases(alias: object) -> None:
    ...
    packet["registry_row_sha256"] = sha(mutated)
    with pytest.raises(sut.ContractError, match="authority|sealed"):
        sut.validate_registry_authority(mutated, packet)


def test_registry_true_authority_rejects_numeric_alias() -> None:
    registry, packet = registry_fixture()
    row = json.loads(registry)
    row["validation"]["source_feasibility_run_authorized"] = 1
    mutated = canonical(row) + b"\n"
    packet["registry_row_sha256"] = sha(mutated)
...
def test_registry_v2_noncanonical_history_passes_only_with_clean_canonical_validation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    registry, packet = registry_v2_fixture()
...
def test_registry_parser_rejects_duplicate_malformed_nonfinite_and_blank_rows(
    ...
    registry, _ = registry_v2_fixture(with_noncanonical_history=False)
...
def test_registry_selected_raw_line_hash_drift_fails() -> None:
    ...
    with pytest.raises(sut.ContractError, match="registry|binding|canonical"):
        sut.validate_registry_authority(b"".join(lines), packet)
...
def test_registry_snapshot_validator_or_schema_hash_drift_fails(
    ...
    registry, _ = registry_v2_fixture()
    validator = fake_registry_validator()
    schema = b'{"type":"object"}\n'
...
def run_packet_fixture(*, v1: bool = False) -> tuple[bytes, bytes, bytes]:
    builder = b"REVIEWED_RUN_PACKET_SHA256: str | None = None\n"
    tests = b"synthetic tests\n"
    packet = {
        ...
        "builder_path": sut.BUILDER_REL,
        "builder_base_sha256": sut.reviewed_base_source_sha256(builder),
        "test_path": sut.TEST_REL,
        "test_sha256": sha(tests),
        "registry_path": sut.REGISTRY_REL,
        "registry_row_index": 301,
        ...
        "m1_manifest_path": sut.M1_MANIFEST_REL,
        "m1_manifest_sha256": sut.M1_MANIFEST_SHA256,
        "m1_receipt_path": sut.M1_RECEIPT_REL,
        "m1_receipt_sha256": sut.M1_RECEIPT_SHA256,
        "m1_source_sha256": sut.M1_SOURCE_SHA256,
        "h1_manifest_path": sut.H1_MANIFEST_REL,
        "h1_manifest_sha256": sut.H1_MANIFEST_SHA256,
        "h1_receipt_path": sut.H1_RECEIPT_REL,
        "h1_receipt_sha256": sut.H1_RECEIPT_SHA256,
        "h1_price_side": "BID",
        "permissions": dict(sut.PACKET_PERMISSIONS),
    }
    if v1:
        packet["schema_version"] = "aruc_001_source_feasibility_run_packet.v1"
        packet["plan_path"] = (
            "03. EA Developer/EA_ActivityResponseContinuation/research/"
            "HYP-ARUC-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN.md"
        )
        packet["plan_sha256"] = "BAA96FF852611996ACC1074BF9955C9F570D4593C86CF66753579C3B3480F1B4"
        for field in (
            "registry_validator_path", "registry_validator_sha256",
            "registry_schema_path", "registry_schema_sha256",
        ):
            del packet[field]
    return canonical(packet) + b"\n", builder, tests


def test_v2_packet_hash_binds_registry_validator_and_schema() -> None:
    packet, builder, tests = run_packet_fixture()

    parsed = sut.validate_run_packet(
        packet,
        reviewed_sha256=sha(packet),
        builder_payload=builder,
        test_payload=tests,
    )

    assert parsed["registry_validator_sha256"] == sut.REGISTRY_VALIDATOR_SHA256
    assert parsed["registry_schema_sha256"] == sut.REGISTRY_SCHEMA_SHA256


def test_v1_packet_cannot_authorize_v2_source() -> None:
    packet, builder, tests = run_packet_fixture(v1=True)
    with pytest.raises(sut.ContractError, match="packet"):
        sut.validate_run_packet(
            packet,
            reviewed_sha256=sha(packet),
            builder_payload=builder,
            test_payload=tests,
...
def test_default_disarm_blocks_before_any_real_source_access(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(sut.ContractError, match="disarmed|run switch|packet"):
        sut.execute_probe(workspace_root=tmp_path, packet_path=tmp_path / "packet.json", run_switch=False)
    assert calls == []
