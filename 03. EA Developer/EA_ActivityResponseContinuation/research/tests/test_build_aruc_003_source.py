...
SOURCE = Path(__file__).resolve().parents[1] / "build_aruc_003_source.py"
SPEC = importlib.util.spec_from_file_location("build_aruc_003_source", SOURCE)
...
def synthetic_complete_bars(dates: tuple[date, ...]) -> list[dict[str, object]]:
    rng = random.Random(3003)
    slots = ((7, 0), (7, 15), (14, 30), (14, 45))
    bars: list[dict[str, object]] = []
    for day_index, day in enumerate(dates):
        for slot_index, (hour, minute) in enumerate(slots):
            at = utc(day.year, day.month, day.day, hour, minute)
            direction = 1 if (day_index + slot_index) % 2 == 0 else -1
            baseline = rng.randint(80, 120)
            sum_tv = baseline if day_index < 20 else baseline + 130
            first_volume = sum_tv - 14
            volumes = (first_volume,) + (1,) * 14
            signs = (direction,) * 15
            open_price = 1.1000 + rng.randint(-20, 20) * 0.00001
            close = open_price + direction * 0.00025
            bars.append(
                {
                    "time_utc": at,
...
def test_precomputed_indexes_preserve_legacy_three_arm_ledgers_and_stage0() -> None:
    dates = business_days(date(2016, 1, 4), 65)
    bars = synthetic_complete_bars(dates)
    legacy_dates = tuple(dates)

    def legacy_index() -> dict[tuple[date, tuple[int, int]], dict[str, object]]:
        return {(bar["date"], bar["slot"]): bar for bar in bars}

    def legacy_activity(current: dict[str, object]) -> float | None:
        try:
            current_index = legacy_dates.index(current["date"])
        except ValueError:
            return None
        if current_index < 20:
            return None
        by_slot = legacy_index()
        values: list[int] = []
        for prior_date in legacy_dates[current_index - 20 : current_index]:
            prior = by_slot.get((prior_date, current["slot"]))
            if prior is None:
                return None
            values.append(int(prior["sum_tv"]))
        denominator = float(median(values))
        return None if denominator <= 0 else int(current["sum_tv"]) / denominator

    def legacy_shifted(current: dict[str, object]) -> dict[str, object] | None:
        try:
            current_index = legacy_dates.index(current["date"])
        except ValueError:
            return None
        source_index = current_index - 5
        if source_index < 20:
            return None
        source = legacy_index().get((legacy_dates[source_index], current["slot"]))
        if source is None:
            return None
        activity = legacy_activity(source)
        if activity is None:
            return None
        volumes = source["tick_volumes"]
        signs = current["price_signs"]
        return {
            "source_date": legacy_dates[source_index],
            "q": sum(int(sign) * int(volume) for sign, volume in zip(signs, volumes)),
            "sum_tv": sum(int(value) for value in volumes),
            "activity": activity,
        }

    indexed_dates, date_index, bars_by_date_slot = sut._prepare_activity_indexes(bars, dates)

    def features(*, indexed: bool) -> list[dict[str, object]]:
        result: list[dict[str, object]] = []
        for bar in bars:
            if indexed:
                activity = sut._activity_ratio_indexed(
                    bar, indexed_dates, date_index, bars_by_date_slot, lookback=20
                )
                shifted = sut._shifted_tick_features_indexed(
                    bar,
                    indexed_dates,
                    date_index,
                    bars_by_date_slot,
                ...
                activity = legacy_activity(bar)
                shifted = legacy_shifted(bar)
            candidate = {
                **bar,
                "atr20": 0.001,
                "r": (float(bar["close"]) - float(bar["open"])) / 0.001,
            }
    ...
    legacy_features = features(indexed=False)
    indexed_features = features(indexed=True)
    assert indexed_features == legacy_features

    def outputs(candidates: list[dict[str, object]]) -> tuple[dict[str, list[dict[str, object]]], dict[str, object]]:
        signals = {
            arm: sut.select_daily_signals(candidates, arm=arm)
            for arm in ("PRIMARY", "PRICE_ONLY", "SHIFTED_TICKS")
        }
        ledgers = sut.build_arm_ledgers(
            signals,
            [bar["time_utc"] for bar in bars],
        )
        primary = signals["PRIMARY"]
        gate = sut.evaluate_stage0_gates(
            primary,
            elapsed_weeks=13.0,
            formation_complete=sum(
                date_index[bar["date"]] >= 20 and sut._decision_slot(bar["time_utc"])
                for bar in bars
            ),
            formation_scheduled=(len(dates) - 20) * 32,
            horizon_records=[row["horizon"] for row in ledgers["PRIMARY"]],
        )
        return ledgers, gate

    legacy_ledgers, legacy_gate = outputs(legacy_features)
    indexed_ledgers, indexed_gate = outputs(indexed_features)

    assert all(indexed_ledgers[arm] for arm in indexed_ledgers)
    assert indexed_ledgers == legacy_ledgers
    assert indexed_gate == legacy_gate


def test_scan_source_builds_activity_index_once_and_never_calls_sequence_index(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    dates = business_days(date(2016, 1, 4), 40)
    bars = synthetic_complete_bars(dates)

    class NoIndexDates(list[date]):
        def index(self, *args: object, **kwargs: object) -> int:
            raise AssertionError("scan must not call Sequence.index")

    calls = 0
    real_bar_index = sut._bar_index

    def counted_bar_index(values: object):
        nonlocal calls
        calls += 1
        return real_bar_index(values)

    monkeypatch.setattr(sut, "build_complete_m15", lambda _rows: (bars, {"observed_bins": len(bars), "complete_bins": len(bars), "incomplete_bins": 0}))
    monkeypatch.setattr(sut, "wilder_atr20_by_close", lambda _rows: [(bars[0]["availability_utc"], 0.001)])
    monkeypatch.setattr(sut, "_bar_index", counted_bar_index)

    report = sut._scan_source([], [], NoIndexDates(dates))

    assert calls == 1
    assert report["source_contract"]["m15_quality"]["complete_bins"] == len(bars)


@pytest.mark.parametrize("malformed", ["duplicate_bar", "duplicate_date"])
def test_precomputed_indexes_reject_duplicate_inputs(malformed: str) -> None:
    dates = business_days(date(2020, 1, 6), 2)
    bars = synthetic_complete_bars(dates)
    if malformed == "duplicate_bar":
        bars.append(dict(bars[0]))
    else:
        dates = dates + (dates[-1],)

    with pytest.raises(sut.ContractError, match="duplicate|business-date"):
        sut._prepare_activity_indexes(bars, dates)
...
def test_parent_002_row_or_source_cannot_authorize_child_003(case: str) -> None:
    registry, reviewed_sha, builder, tests = registry_fixture(
        hypothesis_id="HYP-ARUC-EURUSD-M15-002" if case == "row" else None
    )
    if case == "source":
        builder = (
            b"# HYP-ARUC-EURUSD-M15-002\n"
