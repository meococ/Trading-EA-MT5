#!/usr/bin/env python3
"""Inert source/cadence probe for HYP-ARUC-EURUSD-M15-001.

Importing and default CLI execution cannot read real DESIGN data. A later real
read requires an explicit switch, a reviewed packet whose SHA replaces the
``REVIEWED_RUN_PACKET_SHA256`` sentinel, and an exact latest registry authority
row. The computational surface is intentionally usable with synthetic rows.
"""
...
HYPOTHESIS_ID = "HYP-ARUC-EURUSD-M15-001"
EA_NAME = "EA_ActivityResponseContinuation"
FAMILY = "activity-response-underreaction-continuation"
ATTEMPT_ID = "ARUC001-SOURCE-ATTEMPT-001"

PLAN_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "HYP-ARUC-EURUSD-M15-001_SOURCE_FEASIBILITY_PLAN_V2.md"
)
PLAN_SHA256 = "8835CC6D8B25C154BE288E2EF9F6CE631AD481653BF324FD040098D5E602E5EC"
BUILDER_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "build_aruc_001_source.py"
)
TEST_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/tests/"
    "test_build_aruc_001_source.py"
)
PACKET_REL = (
    "03. EA Developer/EA_ActivityResponseContinuation/research/"
    "HYP-ARUC-EURUSD-M15-001_SOURCE_RUN_PACKET_V2.json"
)
...
_SENTINEL_RE = re.compile(
    rb'^REVIEWED_RUN_PACKET_SHA256: str \| None = (?:None|"[A-F0-9]{64}")\r?$'
)
...
INTENDED_TRUE_VALIDATION_FIELDS = {
    ...
    "source_feasibility_run_authorized",
}
...
def reviewed_base_source_sha256(payload: bytes) -> str:
    ...
    if len(matches) != 1:
        raise ContractError("builder must contain exactly one reviewed packet sentinel")
    index = matches[0]
    newline = b"\r\n" if lines[index].endswith(b"\r\n") else b"\n"
    lines[index] = b"REVIEWED_RUN_PACKET_SHA256: str | None = None" + newline
...
def build_arm_ledgers(
    ...
    for arm in arms:
        ...
        for signal in ordered:
            ...
            signal_id = f"ARUC001-{arm}-{sha256_bytes(identity)[:16]}"
...
def validate_registry_authority(registry_payload: bytes, packet: Mapping[str, object]) -> dict[str, object]:
    """Require the packet-selected row to be the exact latest row for this ID."""

    try:
        rows, raw_rows = parse_registry_jsonl(registry_payload)
        index = packet.get("registry_row_index")
        expected_sha = packet.get("registry_row_sha256")
        if (
            type(index) is not int
            or isinstance(index, bool)
            or not 1 <= index <= len(rows)
            or not _valid_sha(expected_sha)
            or sha256_bytes(raw_rows[index - 1]) != expected_sha
        ):
            raise ValueError("row binding")
        row = rows[index - 1]
        if canonical_json(row) + b"\n" != raw_rows[index - 1]:
            raise ContractError("selected registry row is not canonical")
        latest = [position for position, item in enumerate(rows, start=1) if item.get("hypothesis_id") == HYPOTHESIS_ID]
...
PACKET_PERMISSIONS = {
    "source_feasibility_read": True,
    "outcome_prices": False,
    "economics": False,
    "validation": False,
    "holdout": False,
    "private_or_sealed": False,
    "network": False,
    "paid": False,
    "mql5_or_mt5": False,
    "promotion_or_live": False,
...
def validate_run_packet(
    ...
    *,
    ...
    if not _valid_sha(reviewed_sha256) or sha256_bytes(payload) != reviewed_sha256:
        raise ContractError("reviewed packet SHA mismatch")
    packet = parse_canonical_object(payload, label="run packet")
    fixed = {
        "schema_version": "aruc_001_source_feasibility_run_packet.v2",
        ...
        "builder_base_sha256": reviewed_base_source_sha256(builder_payload),
        "test_path": TEST_REL,
        ...
        "m1_manifest_path": M1_MANIFEST_REL,
        "m1_manifest_sha256": M1_MANIFEST_SHA256,
        "m1_receipt_path": M1_RECEIPT_REL,
        "m1_receipt_sha256": M1_RECEIPT_SHA256,
        "m1_source_sha256": M1_SOURCE_SHA256,
        "h1_manifest_path": H1_MANIFEST_REL,
        "h1_manifest_sha256": H1_MANIFEST_SHA256,
        "h1_receipt_path": H1_RECEIPT_REL,
        "h1_receipt_sha256": H1_RECEIPT_SHA256,
        "h1_price_side": "BID",
        "permissions": PACKET_PERMISSIONS,
    }
    if set(packet) != PACKET_FIELDS or any(packet.get(key) != value for key, value in fixed.items()):
        raise ContractError("run packet contract mismatch")
    if type(packet.get("registry_row_index")) is not int or isinstance(packet["registry_row_index"], bool) or not _valid_sha(packet.get("registry_row_sha256")):
        raise ContractError("run packet registry binding mismatch")
    permissions = packet["permissions"]
    if type(permissions) is not dict or permissions.get("source_feasibility_read") is not True:
        raise ContractError("run packet source authority mismatch")
    if any(permissions.get(key) is not False for key in PACKET_PERMISSIONS if key != "source_feasibility_read"):
        raise ContractError("run packet false authority aliases are invalid")
    return packet
...
def _scan_source(m1_rows: Sequence[Mapping[str, object]], h1_rows: Sequence[Mapping[str, object]], business_dates: Sequence[date]) -> dict[str, object]:
    ...
    report = {
        "schema_version": "aruc_001_source_feasibility_report.v1",
        "hypothesis_id": HYPOTHESIS_ID,
        "ea_name": EA_NAME,
        "feature_family": FAMILY,
...
def execute_probe(*, workspace_root: Path, packet_path: Path, run_switch: bool) -> dict[str, object]:
    ...
    if REVIEWED_RUN_PACKET_SHA256 is None:
        raise ContractError("source probe is disarmed; reviewed packet sentinel is absent")
    if not _valid_sha(REVIEWED_RUN_PACKET_SHA256):
        raise ContractError("reviewed packet sentinel is invalid")

    workspace = Path(workspace_root).absolute()
    expected_packet_path = (workspace / PACKET_REL).absolute()
    if Path(packet_path).absolute() != expected_packet_path:
        raise ContractError("packet path is not canonical")
    packet_payload = stable_read_regular(expected_packet_path, workspace)
    ...
    packet = validate_run_packet(
        packet_payload,
        reviewed_sha256=REVIEWED_RUN_PACKET_SHA256,
    ...
    validate_registry_authority(registry_payload, packet)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--packet", type=Path, default=Path(PACKET_REL))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    packet = args.packet if args.packet.is_absolute() else args.workspace_root / args.packet
