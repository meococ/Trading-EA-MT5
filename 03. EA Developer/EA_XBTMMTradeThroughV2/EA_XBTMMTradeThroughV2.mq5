//+------------------------------------------------------------------+
//| EA_XBTMMTradeThroughV2.mq5                                       |
//| HYP-XBT-MM-TRADETHROUGH-002 - latency/FIFO virtual simulator     |
//+------------------------------------------------------------------+
...
input string InpOutputPrefix="xbtmm\\outputs\\XBTMM002_20180101";

const string HYPOTHESIS_ID="HYP-XBT-MM-TRADETHROUGH-002";
...
bool RunSimulation()
  {
   ...
   // The terminal liquidation changes realized XBT NAV.  Capture that final
   // state in drawdown accounting instead of leaving the pre-liquidation mark
   // as the last risk observation.
   UpdateEquity(g_candidate,MidPrice());
   UpdateEquity(g_null,MidPrice());
   if(g_fill_handle!=INVALID_HANDLE)
     {
...
void PrintEngineSummary(const EngineState &state)
  {
   ...
   PrintFormat("XBTMM_ENGINE_SUMMARY hypothesis_id=%s engine=%s maker_fills=%I64d forced_flattens=%I64d closed_fragments=%I64d inventory=%I64d fifo_lots=%d realized_xbt=%.12f taker_fees_xbt=%.12f gross_profit_xbt=%.12f gross_loss_xbt=%.12f pf=%.8f max_dd_xbt_pct=%.8f collateral_usd_dd_pct=%.8f quote_actions=%I64d max_actions_hour=%I64d action_interval_violations=%I64d pending_latency_violations=%I64d funding_live_after_blackout=%I64d max_age_matched_after_expiry=%I64d fifo_violations=%I64d hard_cap_violations=%I64d pending_fill_races=%I64d touch_ignored=%I64d exact_ignored=%I64d quote_expiries=%I64d engineering_gate_pass=%s economic_use_forbidden=%s",
               ...
               state.pending_action_fill_races,state.touch_ignored,state.exact_ignored,
