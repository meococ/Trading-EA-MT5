...
#property description "Build-only dual-engine LOMX M5 engineering scaffold"
...
const string EA_NAME="EA_LOMX_MultiAssetMomentum";
...
const double MARGIN_STOPOUT_SAFETY_FACTOR=1.20;
const double MARGIN_FREE_EQUITY_FLOOR=0.10;
...
bool MarginSafeVolume(const int direction,const double entry,
                      ...
                      double &volume)
  {
   ...
   if(equity<=0.0 || used_margin<0.0 || stopout_level<0.0)
     {
      g_reject_margin_stopout++;
   ...
   if(mode==ACCOUNT_STOPOUT_MODE_MONEY)
     {
      // Money-mode brokers stop positions when free margin reaches the
      // currency threshold. Preserve both a 20% threshold buffer and 10%
      // of current equity; risk sizing may only move volume downward.
      double required_free=MathMax(stopout_level*MARGIN_STOPOUT_SAFETY_FACTOR,
   ...
   else
     {
      ...
      double required_level=MathMax(MARGIN_LEVEL_FLOOR_PCT,
                                    stopout_level*MARGIN_STOPOUT_SAFETY_FACTOR);
      required_level=MathMax(required_level,stopout_level+20.0);
...
void OnTick()
  {
   ...
   int minute_of_day=MinuteOfDay(utc_parts);
   if(utc_parts.day_of_week==0 || utc_parts.day_of_week==6 ||
      minute_of_day<InpTradeStartMinutesUtc ||
      minute_of_day>=InpTradeEndMinutesUtc)
      return;
   ...
   MqlRates rates[];
   double atr=0.0;
   if(!LoadClosedBars(rates) || !ClosedAtr(atr))
     {
      g_reject_history++;
   ...
   double asian_high=0.0;
   double asian_low=0.0;
   if(!LoadExactAsianRange(utc_now,asian_high,asian_low))
     {
