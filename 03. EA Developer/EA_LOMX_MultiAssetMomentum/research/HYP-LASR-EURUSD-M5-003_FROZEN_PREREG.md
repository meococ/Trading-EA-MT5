# HYP-LASR-EURUSD-M5-003 - Frozen Account-Compatible Successor Preregistration
...
- Parent: `HYP-LASR-EURUSD-M5-002` (pre-EA invalid account contract, no outcome)
- Source SHA-256:
...
- Static non-repaint result: `PASS`, SHA-256:
  `7F5B1BD5FC7ED91729F62E406795516EAE41CF8C27FBF5815C8A5384B717DFEF`
- Cost-source manifest: `RESEARCH_PROXY`, SHA-256:
  `662E0A95CDA8635193B86454D950BE71B95BE1F671E4AB4128F8F6A7844A9AFC`

`HYP-LASR-EURUSD-M5-001` synchronized the requested history but a `$10,000`
tester account was stopped after 159 M5 bars. A subsequent read-only probe under
`HYP-LASR-EURUSD-M5-002` established the actual contract before execution:
FivePercent uses money-mode margin call `$92,000` and stop-out `$90,000` for this
account class. ID `002` was parked with zero MT5 launches and zero outcome reads.

The only successor change is an outcome-independent capital-safety invariant:

- use `OrderCalcMargin` before submission;
- use a `$100,000` tester deposit matching the FivePercent account class;
- in money mode, preserve free margin above the larger of margin call and
  stop-out plus `max(20% of remaining headroom, 1% of equity)`;
- in percent mode, preserve margin level above `max(120%, 1.20 * declared,
  declared + 20pp)`, where `declared=max(margin call, stop-out)`;
...
```text
InpATRPeriod=14;InpAsianEndMinutesUtc=360;InpAsianStartMinutesUtc=0;InpDailyFlattenMinutesUtc=1200;InpDeviationPoints=20;InpEnableTelemetry=true;InpEngineMode=0;InpFridayFlattenMinutesUtc=1200;InpHypothesisId=HYP-LASR-EURUSD-M5-003;InpLotConsistencyLookbackFills=10;InpLotConsistencyMaxFactor=1.50;InpLotConsistencyMinFactor=0.50;InpLotConsistencyMinFills=10;InpMagic=5603104;InpMaxAccountDrawdownPct=8.0;InpMaxDailyLossPct=3.5;InpMaxHoldBars=96;InpMaxSpreadToRisk=0.15;InpMaxTradesPerDay=3;InpResearchAutoMode=true;InpRiskPercent=0.25;InpSweepEpsilonMult=0.30;InpSweepMinTp2R=1.50;InpSweepScaleOutFraction=0.50;InpSweepStopAtrMult=0.20;InpTradeEndMinutesUtc=960;InpTradeStartMinutesUtc=420;InpVariantTag=LASR_EUR_SWEEP_ACCOUNTSAFE_MODEL0;InpVolumeLookback=20;InpVolumeThreshold=1.50
```
...
- Economic trials consumed before this run: `0` for this identity; one invalid
  EURUSD tester run and one pre-EA account-contract park exist upstream.
...
- The `001`/`002` findings permit only the documented margin/account-contract
  repair and no numeric signal tuning.
