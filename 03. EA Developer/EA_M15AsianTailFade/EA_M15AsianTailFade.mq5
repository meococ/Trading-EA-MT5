//+------------------------------------------------------------------+
//| EA_M15AsianTailFade.mq5 — Asian early-move fade (USDJPY port)    |
//| Symbol: USDJPY | Period: M15 | Magic: 880973                     |
//|                                                                   |
//| Hypothesis: HYP-ASIAN-TAIL-FADE-USDJPY-001                        |
//| Port of EA_AsianTailFade: fade early-Asia directional move in    |
//| late-Asia window before London. ATR thresholds (not gold pts).   |
//| Independent of Spark Asian breakout / ASR reclaim / SB.          |
//| Closed-bar[1]. Mon–Thu a priori. Weekend flat.                   |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15AsianTailFade"
...
input group "=== General ==="
input ulong    InpMagic         = 880973;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== Asia Early Window (measure) ==="
input int      InpAsiaEarlyStart = 0;
input int      InpAsiaEarlyEnd   = 3;

input group "=== Fade Window ==="
input int      InpFadeStart     = 4;
input int      InpFadeEnd       = 8;
input int      InpForceCloseH   = 9;

input group "=== Displacement ==="
input int      InpATRPeriod     = 14;
input double   InpMinMoveATR    = 0.60;
input double   InpMaxMoveATR    = 4.00;
input double   InpMaxReversion  = 0.60;  // skip if already reverted this far
...
input double   InpSL_ATRMult    = 1.50;
input int      InpMinSLPoints   = 50;
input int      InpMaxSLPoints   = 1200;
input double   InpTP_Revert     = 0.50;  // TP = fraction of early move
input double   InpTP_RatioMin   = 1.0;   // floor TP vs SL
...
double   g_asiaEarlyOpen = 0;
double   g_asiaEarlyClose = 0;
double   g_earlyMove = 0;
bool     g_earlyMeasured = false;
...
int OnInit()
{
   ...
   Print("[ATFJ] HYP-ASIAN-TAIL-FADE-USDJPY-001");
...
void OnTick()
{
   ...
   {
      if(g_asiaEarlyOpen > 0.0 && g_asiaEarlyClose > 0.0)
      {
   ...
   if(CountPositions() > 0)
   {
      if(dt.hour >= InpForceCloseH || dt.day_of_week == 5 ||
   ...
   if(!g_earlyMeasured)
      return;
   if(dt.hour < InpFadeStart || dt.hour >= InpFadeEnd)
      return;
   ...
   double moveATR = moveAbs / atr[0];
   if(moveATR < InpMinMoveATR || moveATR > InpMaxMoveATR)
      return;

   double c1 = iClose(_Symbol, PERIOD_CURRENT, 1);
   double rev = 0.0;
   if(g_earlyMove > 0.0)
      rev = (g_asiaEarlyClose - c1) / g_earlyMove;
   else if(g_earlyMove < 0.0)
      rev = (c1 - g_asiaEarlyClose) / (-g_earlyMove);
   if(rev > InpMaxReversion)
      return;

   bool isBuy = (g_earlyMove < 0.0);
   double slDist = atr[0] * InpSL_ATRMult;
   ...
   if(tpDist < slDist * InpTP_RatioMin)
      tpDist = slDist * InpTP_RatioMin;
