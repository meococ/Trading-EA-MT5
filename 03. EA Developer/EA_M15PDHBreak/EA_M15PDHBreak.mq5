//+------------------------------------------------------------------+
//| EA_M15PDHBreak.mq5 — Previous-day high/low continuation breakout |
//| Symbol: USDJPY | Period: M15 | Magic: 880941                     |
//|                                                                   |
//| Hypothesis: HYP-PDH-BREAK-M15-001                                  |
//| Independent of London ORB / Spark / HourOpen / InsideBar / ITSM / |
//| SB / USBILL / Keltner. Opposite of LiqSweep/PDLevel *fade* kills  |
//| (S159–S161, S249–S251): those faded PDH/PDL; this continues break.|
//|                                                                   |
//| PDH/PDL from closed D1 shift>=1 only. Enter on closed M15 bar[1]  |
//| break with body + D1 EMA50. Mon–Thu; weekend flat; risk 0.5%.     |
//+------------------------------------------------------------------+
#property copyright "SonicR / EA_M15PDHBreak"
...
input group "=== General ==="
input ulong    InpMagic         = 880941;
input int      InpDeviation     = 30;
input bool     InpKillSwitch    = false;

input group "=== PDH/PDL Break ==="
...
int OnInit()
{
   ...
   PrintFormat("[PDHB] HYP-PDH-BREAK-M15-001 | trade[%d,%d) TP=%.2fR risk=%.2f%%",
               InpTradeStart, InpTradeEnd, InpTP_Ratio, InpRiskPct);
...
//+------------------------------------------------------------------+
// Closed-bar only: PDH/PDL from D1 shift>=1; decision from M15 bar[1].
int GetSignal(const int hour, double &pdh, double &pdl)
{
   pdh = 0.0;
   pdl = 0.0;
   if(g_brokeToday)
   ...
   // Previous completed D1 bar only (shift 1) — non-repaint.
...
void OnTick()
{
   ...
   int signal = GetSignal(dt.hour, pdh, pdl);
   if(signal == 0)
      return;
   ...
   // Invalidation = reclaim of broken PD level.
   double slRaw = (isBuy ? pdh - atr[0] * InpSL_BufferATR
                         : pdl + atr[0] * InpSL_BufferATR);
   ...
   if(IsSuccessfulRetcode(res.retcode))
   {
      ...
      g_brokeToday = true;
      g_holdBars = 0;
      PrintFormat("[PDHB] %s lot=%.2f @ %.5f PDH=%.5f PDL=%.5f",
                  isBuy ? "BUY" : "SELL", lot, res.price, pdh, pdl);
   }
}
