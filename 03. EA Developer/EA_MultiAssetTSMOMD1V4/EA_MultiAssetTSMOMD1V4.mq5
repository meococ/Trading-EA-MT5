//+------------------------------------------------------------------+
//| EA_MultiAssetTSMOMD1V4.mq5                                      |
//| HYP-MULTI-TSMOM-D1-004 - calendar-365 multi-asset TSMOM         |
//+------------------------------------------------------------------+
...
input ulong  InpMagic=260812006;
input int    InpDeviationPoints=20;

const string HYPOTHESIS_ID="HYP-MULTI-TSMOM-D1-004";
const string EXPECTED_SYMBOL="AFD_EURUSD_DUKA_TSMOM_V4";
...
string g_symbols[ASSET_COUNT]={"AFD_EURUSD_DUKA_TSMOM_V4",
                               "AFD_GBPUSD_DUKA_TSMOM_V4",
                               "AFD_AUDUSD_DUKA_TSMOM_V4",
                               "AFD_NZDUSD_DUKA_TSMOM_V4",
                               "AFD_USDJPY_DUKA_TSMOM_V4",
                               "AFD_USDCAD_DUKA_TSMOM_V4",
                               "AFD_USDCHF_DUKA_TSMOM_V4",
                               "AFD_XAUUSD_DUKA_TSMOM_V4",
                               "AFD_BTCUSD_DUKA_TSMOM_V4"};
   ...
   if(transition_ok && TargetsReached())
     {
      ...
      PrintFormat("MTS004_REBALANCE_COMPLETE decision=%I64d execution=%I64d latency_sec=%I64d usd_factor=%.8f",
...
void EmitFinancingExposure(const datetime now,const bool force,const string reason)
  {
   ...
   PrintFormat("MTS004_FINANCE_EXPOSURE epoch=%I64d day=%d reason=%s fx_usd=%.2f xau_usd=%.2f btc_usd=%.2f",
...
int OnInit()
  {
   if(_Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_H1 || !InpResearchAutoMode ||
      InpMagic!=260812006 || InpDeviationPoints!=20 ||
      AccountInfoString(ACCOUNT_CURRENCY)!="USD")
     {
      PrintFormat("MTS004_IDENTITY_FAIL symbol=%s period=%d currency=%s",
   ...
   for(int i=0;i<ASSET_COUNT;i++)
     {
      ...
      if(!SymbolSelect(g_symbols[i],true) || GetLastError()!=0)
        {
         PrintFormat("MTS004_SYMBOL_SELECT_FAIL symbol=%s error=%d",
   ...
   PrintFormat("MTS004_READY hypothesis_id=%s primary=%s lookback_calendar_days=365 vol_returns=60 btc_active=%I64d",
               HYPOTHESIS_ID,_Symbol,(long)BTC_ACTIVE_FROM);
...
void OnDeinit(const int reason)
  {
   ...
   for(int i=0;i<ASSET_COUNT;i++)
      PrintFormat("MTS004_SYMBOL_SOURCE symbol=%s missing_weeks=%I64d long=%I64d short=%I64d",
                  g_symbols[i],g_missing_weeks[i],g_long_signals[i],g_short_signals[i]);
   PrintFormat("MTS004_SOURCE_SUMMARY hypothesis_id=%s attempts=%I64d source_valid=%I64d weight_valid=%I64d y2018=%I64d y2019=%I64d y2020=%I64d y2021=%I64d reason=%d",
   ...
   PrintFormat("MTS004_ECON_TELEMETRY ticks=%I64d deal_profit=%.2f deal_swap=%.2f deal_commission=%.2f native_net=%.2f",
