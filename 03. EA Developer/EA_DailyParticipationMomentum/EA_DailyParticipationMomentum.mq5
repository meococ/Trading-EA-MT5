//+------------------------------------------------------------------+
//| EA_DailyParticipationMomentum.mq5                               |
//| HYP-DPMO-XAUUSD-M5-001: daily activity participation momentum  |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Untuned XAUUSD M5 daily participation momentum"
...
input string InpHypothesisId="HYP-DPMO-XAUUSD-M5-001";
input string InpVariantTag="DAILY_ACTIVITY_MEDIAN20_MOMENTUM";
...
const string EA_NAME="EA_DailyParticipationMomentum";
const string EXPECTED_HYPOTHESIS="HYP-DPMO-XAUUSD-M5-001";
const string EXPECTED_VARIANT="DAILY_ACTIVITY_MEDIAN20_MOMENTUM";
...
bool LoadClosedRates(MqlRates &rates[])
  {
   ...
   return(copied>=SESSION_ROWS*(ACTIVITY_LOOKBACK+1));
  }
...
void AppendCompleteSession(SessionSummary &sessions[],
                           ...
                           const double start_close,
                           const double end_close)
  {
   if(date_key<=0 || broken || count!=SESSION_ROWS || activity<=0.0 ||
      !IsFinite(start_close) || !IsFinite(end_close) || start_close<=0.0 || end_close<=0.0)
      return;
   ...
   sessions[size].activity=activity;
   sessions[size].start_close=start_close;
   sessions[size].end_close=end_close;
  }
   ...
   for(int i=0;i<total;i++)
     {
      ...
      if(key!=current_key)
        {
         AppendCompleteSession(sessions,current_key,decision_server,count,broken,
                               first_utc,last_utc,activity,start_close,end_close);
         current_key=key;
         count=0;
         broken=false;
         first_utc=0;
      ...
      activity+=(double)rates[i].tick_volume;
      end_close=rates[i].close;
     }
   AppendCompleteSession(sessions,current_key,decision_server,count,broken,
                         first_utc,last_utc,activity,start_close,end_close);
   return(ArraySize(sessions)>ACTIVITY_LOOKBACK);
  }

double PriorMedian20(const SessionSummary &sessions[],const int current_index)
  {
   double values[];
   ArrayResize(values,ACTIVITY_LOOKBACK);
   for(int i=0;i<ACTIVITY_LOOKBACK;i++)
      values[i]=sessions[current_index-ACTIVITY_LOOKBACK+i].activity;
   ArraySort(values);
   return((values[ACTIVITY_LOOKBACK/2-1]+values[ACTIVITY_LOOKBACK/2])/2.0);
  }
...
bool BuildSignal(const datetime availability_time,SignalDecision &signal)
  {
   ...
   g_complete_sessions++;
   const double median=PriorMedian20(sessions,current);
   if(!IsFinite(median) || median<=0.0 || !(today.activity>median) || today.end_close==today.start_close)
      return(false);
   const int direction=(today.end_close>today.start_close ? 1 : -1);
   ...
   signal.activity=today.activity;
   signal.median20=median;
   signal.session_return=MathLog(today.end_close/today.start_close);
   signal.atr=atr;
   signal.structural_stop=stop;
   g_raw_signals++;
   if(direction>0) g_long_signals++; else g_short_signals++;
   PrintFormat("DPMO001_SIGNAL decision=%I64d availability=%I64d utc_date=%d direction=%s activity=%.0f median20=%.1f return=%.10f atr=%.5f stop=%.5f",
               (long)decision_time,(long)availability_time,key,
               (direction>0 ? "LONG" : "SHORT"),today.activity,median,
               signal.session_return,atr,stop);
   return(true);
  }
...
bool CloseOwned(const string reason)
  {
   ...
   if(!g_trade.PositionClose(ticket,InpDeviationPoints))
     {
      PrintFormat("DPMO001_CLOSE_REJECT reason=%s ticket=%I64u retcode=%u",reason,ticket,g_trade.ResultRetcode());
      return(false);
     }
   ...
   g_closes++;
   g_entry_time=0;
   PrintFormat("DPMO001_CLOSE reason=%s ticket=%I64u retcode=%u",reason,ticket,retcode);
   return(true);
  }
...
bool SubmitEntry(const SignalDecision &signal)
  {
   if(!signal.fired || g_daily_locked || g_drawdown_locked || AnySymbolExposure())
      return(false);
   MqlTick tick;
   ...
   if(!sent || (retcode!=TRADE_RETCODE_DONE && retcode!=TRADE_RETCODE_DONE_PARTIAL))
     {
      g_entry_rejects++;
      PrintFormat("DPMO001_ENTRY_REJECT direction=%s volume=%.2f entry=%.5f sl=%.5f tp=%.5f retcode=%u",
                  (signal.direction>0 ? "LONG" : "SHORT"),volume,entry,sl,tp,retcode);
      return(false);
     }
   g_entries++;
   g_entry_time=signal.availability_time;
   PrintFormat("DPMO001_ENTRY direction=%s volume=%.2f entry=%.5f sl=%.5f tp=%.5f risk_pct=%.3f retcode=%u",
...
int OnInit()
  {
   if(!InpResearchAutoMode || InpEnableTelemetry || _Symbol!=EXPECTED_SYMBOL || _Period!=PERIOD_M5 ||
      ...
      InpMagic!=5604401 || MathAbs(InpRiskPercent-0.10)>1e-12 ||
   ...
   if(!CurrentBarOpen(g_last_bar_open) || g_last_bar_open<=0)
      return(INIT_FAILED);
   PrintFormat("DPMO001_INIT ea=%s hypothesis=%s variant=%s symbol=%s timeframe=M5",
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("DPMO001_SUMMARY reason=%d runtime_failed=%s closed=%I64d sessions=%I64d raw=%I64d long=%I64d short=%I64d entries=%I64d rejects=%I64d closes=%I64d clock_rejects=%I64d invalid=%I64d",
               reason,(g_runtime_failed ? "true" : "false"),g_closed_bars,
               g_complete_sessions,g_raw_signals,g_long_signals,g_short_signals,
               g_entries,g_entry_rejects,g_closes,g_clock_rejects,g_invalid_inputs);
  }
